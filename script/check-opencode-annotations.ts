...
/**
 * Verifies that every Altru Coder-specific change in shared upstream-owned source files
 * is annotated with a altrucoder_change marker.
 *
 * Usage:
 ...
 *   - contains a altrucoder_change marker comment           (inline annotation)
 *   - falls inside a altrucoder_change start/end block      (block annotation)
 ...
 *
 * Exempt paths (no markers needed — entirely Altru Coder-specific):
 *   - packages/opencode/src/altrucoder/**
 *   - packages/opencode/test/altrucoder/**
 *   - Any path containing "altrucoder" in directory or filename
 *   - Any path with a directory starting with "altru-coder-" (e.g. altru-coder-sessions/)
 *   - script/upstream/**
 *   - Altru Coder-specific annotation checker support files
 */
...
function isSource(file: string) {
  ...
  if (ext) return false
  return content(file).startsWith("#!") // altrucoder_change
}

// Parses the unified=0 diff for `file` against `base` and returns:
//   - added: every added line number on HEAD
//   - revert: true when the file's diff removes any altrucoder_change marker.
//     In that case the changes are reverting Altru Coder modifications back to the
...
//     Altru Coder construct (e.g. `unixSkip(` → `unix(`) often live in different
...
// Matches the start of a altrucoder_change marker in JS, JSX, YAML, TOML, and shell comments.
...
function coveredLines(text: string): { lines: string[]; covered: Set<number> } {
  ...
  // Whole-file annotation: first non-shebang non-empty line is a altrucoder_change - new file marker.
...
for (const file of files) {
  ...
  if (revert) continue // altrucoder_change - file is reverting Altru Coder modifications back to upstream

  const text = content(file) // altrucoder_change
...
if (violations.length === 0) {
  console.log("All shared upstream changes are annotated with altrucoder_change markers.")
  process.exit(0)
}

console.error(
  [
    "Unannotated Altru Coder changes found in shared upstream files:",
    "",
    ...violations,
    "",
    "Every Altru Coder-specific change in shared upstream source files must be annotated.",
    "",
    "Checked paths:",
    ...SCOPES.map((scope) => `  - ${scope}/**`),
    "",
    "Inline (single line):",
    "  const url = Flag.ALTRU_CODER_MODELS_URL || 'https://models.dev' // altrucoder_change",
    "",
    ...
    "  - packages/opencode/src/altrucoder/**",
    "  - packages/opencode/test/altrucoder/**",
    "  - Any path containing 'altrucoder' in the directory or filename",
    "  - Any directory starting with 'altru-coder-' (e.g. altru-coder-sessions/)",
    "  - script/upstream/**",
    "  - Altru Coder-specific annotation checker support files",
    "",
