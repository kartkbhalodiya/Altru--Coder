 ...
 *
 * Enforcement scope (Altru Coder-owned paths only, to avoid upstream-sync churn):
 *   - Any top-level markdown file (TESTING.md, AGENTS.md, README.md, …)
 *   - Any path segment containing "altrucoder" or starting with "altru-coder-"
...
function skip(file: string) {
  ...
  if (parts.some((p) => p.includes("altrucoder") || p.startsWith("altru-coder-"))) return false
