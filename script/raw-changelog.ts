...
const repo = process.env.GH_REPO ?? "Altru-Coder/altrucoder"
...
const order = ["Core", "TUI", "SDK", "Extensions"] as const // altrucoder_change
const sections = {
  core: "Core",
  tui: "TUI",
  sdk: "SDK",
...
function section(areas: Set<string>) {
  const priority = ["core", "tui", "sdk", "plugin", "extensions/zed", "extensions/vscode", "github"] // altrucoder_change
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/opencode packages/sdk packages/plugin sdks/vscode packages/extensions github`.text() // altrucoder_change
