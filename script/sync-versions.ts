...
// Sync every Altru Coder version string across the monorepo to a single target.
//
...
// `.altru-coder/command/upstream-manual-merge.md`). It's also handy mid-merge to
// rebase our version bumps onto any new Altru Coder main releases.
//
// Usage:
...
//   - the five Altru-Coder download URLs inside that toml
//
// Intentionally NOT touched:
//   - `packages/altru-coder-jetbrains/**` — the JetBrains plugin has its own release
...
for await (const rel of glob.scan({ cwd: root, onlyFiles: true })) {
  ...
  if (rel.startsWith("packages/altru-coder-jetbrains/")) continue
...
if (await Bun.file(zed).exists()) {
  ...
  const next = text
    ...
    .replace(
      /https:\/\/github\.com\/Altru-Coder\/altrucoder\/releases\/download\/v[^/]+\//g,
      `https://github.com/Altru-Coder/altrucoder/releases/download/v${target}/`,
    )
  if (next !== text) {
