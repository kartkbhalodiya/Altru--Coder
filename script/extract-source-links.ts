...
const OUTPUT = path.join(ROOT, "packages/altru-coder-docs/source-links.md")

const check = process.argv.includes("--check")

const DIRS = [
  path.join(ROOT, "packages/altru-coder-vscode/src"),
  path.join(ROOT, "packages/altru-coder-vscode/webview-ui"),
...
const EXCLUDE_PATTERNS = [
  ...
  /^https?:\/\/altru\.internal/,
  /^https?:\/\/dev\.altru-coder\.ai/,
  ...
  /^https?:\/\/api\.altru-coder\.ai\/api\//,
  /^https?:\/\/ingest\.altru-codersessions\.ai/,
  /^https?:\/\/api\.openai\.com/,
  /^https?:\/\/api\.github\.com/,
  /^https?:\/\/api\.githubcopilot\.com/,
  /^https?:\/\/[^/]+\.openai\.azure\.com\/openai/, // altrucoder_change
...
function shouldSkipFile(filepath: string): boolean {
  ...
  if (parts.includes("i18n") && path.basename(filepath) !== "en.ts") return true // altrucoder_change
...
if (check) {
  const committed = await Bun.file(OUTPUT)
    ...
    .catch(() => "")
  if (committed === output) {
    console.log("packages/altru-coder-docs/source-links.md is up to date.")
    process.exit(0)
  }
  console.error(
    [
      "ERROR: packages/altru-coder-docs/source-links.md is out of date.",
      "",
...
console.log(`Wrote ${sorted.length} unique URLs to packages/altru-coder-docs/source-links.md`)
