 ...
 * Used to clean build artifacts from Altru Coder-specific directories after checking
...
/**
 * Check if the "ours" version of a conflicted file contains altrucoder_change markers.
 ...
 */
export async function oursHasAltruCoderChanges(file: string): Promise<boolean> {
  const result = await $`git show :2:${file}`.quiet().nothrow()
  if (result.exitCode !== 0) return false
  return result.stdout.toString().includes("altrucoder_change")
}
...
export async function trainRerere(grep: string): Promise<number> {
  ...
  const dir = join(tmpdir(), `altru-coder-rerere-train-${randomUUID()}`)
