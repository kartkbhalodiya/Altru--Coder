 ...
 * Pass currentContent (our version of the file) to detect altrucoder_change markers
...
export function getRecommendation(
  ...
  // Check if file should be skipped entirely (doesn't exist in Altru Coder, shouldn't be added)
  if (shouldSkipFile(path, skipFiles)) {
    return {
      recommendation: "skip",
      reason: "File should be skipped (does not exist in Altru Coder fork)",
    }
  }
  ...
  if (keepOurs.some((pattern) => path.includes(pattern) || path === pattern)) {
    return {
      ...
      reason: "File is Altru Coder-specific and should not be overwritten",
    }
  }

  // Altru Coder directories should always keep ours
  if (
    matches(
      path,
      defaultConfig.altruDirectories.map((dir) => `${dir}/**`),
    )
  ) {
    return {
      recommendation: "keep-ours",
      reason: "File is in a Altru Coder-specific directory",
    }
  }
  ...
  if (shouldTakeTheirsTransform(path)) {
    // If our version has altrucoder_change markers, flag for manual review
    if (currentContent?.includes("altrucoder_change")) {
      return {
        recommendation: "manual",
        reason: "File has altrucoder_change markers — auto-transform skipped, needs manual review",
      }
    }
    return {
      recommendation: "take-theirs-transform",
      reason: "Branding-only file: take upstream and apply Altru Coder branding transforms",
    }
  }

  switch (type) {
    case "i18n":
      // i18n files that have altrucoder_change markers need manual review
      if (currentContent?.includes("altrucoder_change")) {
        return {
          recommendation: "manual",
          reason: "i18n file has altrucoder_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "i18n-transform",
        reason: "i18n file: take upstream translations and apply Altru Coder branding",
      }
    case "script":
      if (currentContent?.includes("altrucoder_change")) {
        return {
          recommendation: "manual",
          reason: "Script file has altrucoder_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
    ...
    case "extension":
      if (currentContent?.includes("altrucoder_change")) {
        return {
          recommendation: "manual",
          reason: "Extension file has altrucoder_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "extension-transform",
        reason: "Extension file: take upstream and apply Altru Coder branding",
      }
    case "web":
      if (currentContent?.includes("altrucoder_change")) {
        return {
          recommendation: "manual",
          reason: "Web/docs file has altrucoder_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "web-transform",
        reason: "Web/docs file: take upstream and apply Altru Coder branding",
      }
    case "markdown":
      return {
        recommendation: "keep-ours",
        reason: "Markdown files are typically Altru Coder-specific documentation",
      }
    case "package":
      if (currentContent?.includes("altrucoder_change")) {
        return {
          recommendation: "manual",
          reason: "package.json has altrucoder_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "package-transform",
        reason: "Package.json: take upstream, transform names, inject Altru Coder deps, preserve version",
      }
    case "code":
      return {
        recommendation: "manual",
        reason: "Code files need manual review for altrucoder_change markers",
      }
    case "config":
      return {
        recommendation: "manual",
        reason: "Config files may have Altru Coder-specific settings",
      }
    default:
      return {
        recommendation: "manual",
...
export async function analyzeConflicts(
  ...
  for (const path of files) {
    const type = classifyFile(path)
    // Read current file content (our version) to detect altrucoder_change markers
...
export function generateMarkdownReport(report: ConflictReport): string {
  ...
  for (const rec of order) {
    ...
    const titleMap: Record<ConflictFile["recommendation"], string> = {
      ...
      "i18n-transform": "i18n Transform (Auto-Apply Altru Coder Branding)",
      "take-theirs-transform": "Take Upstream + Altru Coder Branding (Auto)",
      ...
      "keep-ours": "Keep Altru Coder Version (Ours)",
