...
test("recommends keep ours for Altru Coder directories", () => {
  expect(getRecommendation("packages/altru-coder-vscode/.prettierignore", [], []).recommendation).toBe("keep-ours")
  expect(getRecommendation("packages/altru-coder-i18n/tsconfig.json", [], []).recommendation).toBe("keep-ours")
})
