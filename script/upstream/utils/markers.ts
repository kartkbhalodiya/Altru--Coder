#!/usr/bin/env bun
/**
 * Shared altrucoder_change marker helpers used by both the marker fixer and the
...
export const standalone = [
  /^\s*\/\/\s*altrucoder_change\b.*$/,
  /^\s*#\s*altrucoder_change\b.*$/,
  /^\s*\{?\s*\/\*\s*altrucoder_change\b.*\*\/\}?\s*$/,
]
export const start = /\baltrucoder_change\s+start\b/
export const end = /\baltrucoder_change\s+end\b/
export const freshmark = /\baltrucoder_change\s*-\s*new\s*file\b/
...
function strip(file: string, line: string): { line: string | null; mark?: string } {
  ...
  if (style(file) === "hash") return comment(line, [/^#\s*altrucoder_change\b/])
  return comment(line, [/^\{\/\*\s*altrucoder_change\b/, /^\/\*\s*altrucoder_change\b/, /^\/\/\s*altrucoder_change\b/])
}
...
function note(mode: Style) {
  if (mode === "hash") return " # altrucoder_change"
  if (mode === "jsx") return " {/* altrucoder_change */}"
  if (mode === "block") return " /* altrucoder_change */"
  return " // altrucoder_change"
}
...
export async function changed(base: Text, head: Text, opts?: { ignoreWhitespace?: boolean }): Promise<Diff> {
  const dir = await mkdtemp(path.join(tmpdir(), "altru-coder-markers-"))
