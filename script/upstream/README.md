...
Scripts for automating the merge of upstream opencode changes into Altru Coder.
...
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/altru-opencode-v1.1.44
```
...
| `fix-altrucoder-markers.ts` | Rebuild `altrucoder_change` markers for one file against the last merged upstream |
...
| Script | Description |
|---|---|
| `transforms/package-names.ts` | Transform opencode package names to altru-coder |
| `transforms/preserve-versions.ts` | Preserve Altru Coder's package versions |
| `transforms/keep-ours.ts` | Keep Altru Coder's version of specific files |
| `transforms/skip-files.ts` | Skip/remove files that shouldn't exist in Altru Coder |
| `transforms/transform-i18n.ts` | Transform i18n files with Altru Coder branding |
| `transforms/transform-take-theirs.ts` | Take upstream + apply Altru Coder branding for branding-only files |
| `transforms/transform-package-json.ts` | Enhanced package.json with Altru Coder dependency injection |
...
4. **Create branches**
   ...
   - `<author>/altru-opencode-<version>` - Merge target branch
   - `<author>/opencode-<version>` - Transformed upstream branch

5. **Apply ALL transformations to upstream branch (PRE-MERGE)**:
   - Remove files that should not exist in Altru Coder (`skipFiles`)
   - Transform package names (opencode-ai -> @altru-coder/cli)
   - Preserve Altru Coder's versions
   - Transform i18n files with Altru Coder branding
   - Transform branding-only files (UI components, configs)
   - Transform package.json files (names, deps, Altru Coder injections)
   - Transform script files (GitHub API references)
   - Transform extension files (Zed, etc.)
   - Transform web/docs files
   - Reset Altru Coder-specific files

6. **Merge** transformed upstream into Altru Coder branch
   - Since all branding transforms are applied pre-merge, conflicts should be minimal
   - Remaining conflicts are files with actual code differences (altrucoder_change markers)

7. **Auto-resolve** any remaining conflicts
   - Skip files that shouldn't exist in Altru Coder
   - Keep Altru Coder's version of specific files
...
```typescript
{
  ...
  packageMappings: [
    { from: "opencode-ai", to: "@altru-coder/cli" },
    { from: "@opencode-ai/cli", to: "@altru-coder/cli" },
    // ...
  ],

  // Files to always keep Altru Coder's version (never take upstream)
  keepOurs: [
    "README.md",
    "CONTRIBUTING.md",
    "AGENTS.md",
  ...
  // Files to take upstream + apply Altru Coder branding transforms
  takeTheirsAndTransform: [
    "packages/ui/src/**/*.tsx",
    // ...
  ],

  // Altru Coder-specific directories (preserved)
  altruDirectories: [
    "packages/opencode/src/altrucoder",
...
**Key insight**: By applying all branding transforms to the upstream branch BEFORE merging, we eliminate most conflicts that would otherwise occur due to branding differences (OpenCode -> Altru Coder).

### Transform Order (Pre-Merge)

The following transforms are applied to the opencode branch before merging:

1. **Skip files** - Remove upstream-only packages/files that should not exist in Altru Coder
2. **Package names** - `opencode-ai` -> `@altru-coder/cli`, etc.
3. **Versions** - Preserve Altru Coder's version numbers
4. **i18n files** - OpenCode -> Altru Coder in user-visible strings
5. **Branding files** - UI components, configs with branding only
6. **package.json** - Names, dependencies, Altru Coder injections
...
|---|---|---|
| i18n files | `i18n-transform` | Take upstream, apply Altru Coder branding |
| UI components | `take-theirs-transform` | Take upstream, apply branding (no logic changes) |
| package.json | `package-transform` | Take upstream, transform names, inject Altru Coder deps |
...
| README/docs | `keep-ours` | Keep Altru Coder's version |
| GitHub workflows | `keep-ours` | Keep Altru Coder's version (manual review) |
| Code with markers | `manual` | Has `altrucoder_change` markers, needs review |
...
- Altru Coder had `Altru Coder` branding
- Git saw these as conflicting changes

Now:

- We transform upstream to `Altru Coder` branding BEFORE merge
- Both branches have the same branding
- Git sees no conflict for branding-only files

The only remaining conflicts are files with **actual code differences** - files with `altrucoder_change` markers that contain Altru Coder-specific logic.
...
| `.worktrees/opencode-merge/altru-main` | The Altru Coder base branch snapshot used for the merge |
...
The command finds the newest upstream tag already merged into `HEAD`, reads that upstream version of the file, applies the same branding transforms used by upstream merge automation, strips existing `altrucoder_change` markers from the current file, and adds fresh markers around the remaining lines that differ from upstream.
...
- Altru Coder-only paths: anything under `packages/altru-*/`, any `**/altrucoder/**` subdir, `script/upstream/`.
- Non-code assets: SVG, PNG, fonts, archives, lock files, etc. (see `SKIP_EXTENSIONS` / `SKIP_FILENAMES` in the script).
- Files covered by the merge config's `keepOurs` or `skipFiles` lists in `utils/config.ts` — these are intentionally preserved or removed in Altru Coder and must not be bulk-reset.
...
| `markers-only` | Stripping `altrucoder_change` markers makes local match upstream | reset |
...
| `upstream-missing` | File does not exist upstream (altru-coder-only, intentional) | skipped |
| `local-missing` | File tracked but missing locally (deleted in Altru Coder) | skipped |
...
# Create PR: catrielmuller/altru-opencode-v1.1.44 -> main

# Second merge: v1.1.50 based on the previous PR (without waiting for approval)
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/altru-opencode-v1.1.44

# Create PR: catrielmuller/altru-opencode-v1.1.50 -> catrielmuller/altru-opencode-v1.1.44
# OR: catrielmuller/altru-opencode-v1.1.50 -> main (once first PR is merged)
```
...
bun run analyze.ts --version v1.1.50 --base-branch catrielmuller/altru-opencode-v1.1.44

# 2. Run the merge
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/altru-opencode-v1.1.44

# 3. Create PR from catrielmuller/altru-opencode-v1.1.50
#    - Target: catrielmuller/altru-opencode-v1.1.44 (if first PR not merged yet)
...
2. Look for `altrucoder_change` markers to identify Altru Coder-specific code
3. Resolve conflicts, keeping Altru Coder-specific changes
4. Stage and commit:
   ```bash
   git add -A
