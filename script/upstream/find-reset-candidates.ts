 ...
 *   - markers-only        : only diff is altrucoder_change markers wrapping
 ...
 *   - upstream-missing    : file does not exist upstream (altru-coder-only, skipped)
...
const ALTRU_CODER_ONLY_PATHSPECS = [
  ":(exclude,glob)packages/altru-*/**",
  ":(exclude,glob)**/altrucoder/**",
...
async function candidates(commit: string, scope: string | undefined, top: string): Promise<CandidateSet> {
  const pathspecs = [scope ?? ".", ...ALTRU_CODER_ONLY_PATHSPECS]
...
/**
 * Files the upstream merge config marks as "keep ours" (Altru Coder-specific preserved
 * versions) or "skip" (upstream-only, removed in Altru Coder) should never be touched
 * by the bulk resetter. They show up in `git diff` against raw upstream but
 * resetting them would undo deliberate Altru Coder decisions.
 */
