#!/usr/bin/env bun
/**
 * Keep Altru Coder's version of specific files during merge
 *
 * This script handles files that should always keep Altru Coder's version
...
export function shouldKeepOurs(filePath: string, patterns: string[]): boolean {
  const dirs = defaultConfig.altruDirectories.map((dir) => `${dir}/**`)
  return matches(filePath, [...patterns, ...dirs]) || patterns.some((pattern) => filePath.includes(pattern))
}

/**
 * Keep Altru Coder's version of conflicted files
 */
...
/**
 * Reset specific files to Altru Coder's version (even if not conflicted)
 */
export async function resetToOurs(files: string[], options: KeepOursOptions = {}): Promise<KeepOursResult[]> {
  ...
  for (const file of files) {
    if (options.dryRun) {
      ...
      try {
        // Get the file from HEAD (Altru Coder's version)
...
if (import.meta.main) {
  ...
  console.log()
  success(`Kept Altru Coder's version for ${kept.length} files`)
