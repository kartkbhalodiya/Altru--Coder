#!/usr/bin/env bun
/**
 * Transform files by taking upstream version and applying Altru Coder branding
 *
 * This script handles files that have only branding differences (no logic changes).
 * It takes the upstream version and applies Altru Coder branding transforms.
 *
 * Use this for:
 * - UI components with OpenCode -> Altru Coder branding
 * - Config files with predictable patterns
 * - Files without altrucoder_change logic blocks
 */
...
export function applyBrandingTransforms(content: string, verbose = false): { result: string; replacements: number } {
  ...
  for (const line of lines) {
    // Skip lines with altrucoder_change marker (already customized)
    if (line.includes("// altrucoder_change")) {
