#!/usr/bin/env bun
/**
 * Transform i18n translation files with Altru Coder branding
 *
 * This script handles i18n files by:
 * 1. Taking upstream's version as the base (to get new translation keys)
 * 2. Applying intelligent string replacements for Altru Coder branding
 * 3. Preserving lines marked with `// altrucoder_change`
 *
 * String replacement rules:
 * - opencode.ai -> altru-coder.ai (domain)
 * - app.opencode.ai -> app.altru-coder.ai (app domain)
 * - OpenCode -> Altru Coder (product name in user-visible text)
 * - opencode upgrade -> altru-coder upgrade (CLI commands)
 * - npx opencode -> npx altru-coder (CLI invocation)
 * - anomalyco/opencode -> Altru-Coder/altrucoder (GitHub repo)
 *
 * Preserved (not replaced):
 * - opencode.json (actual config filename)
 * - .opencode/ (actual directory name)
 * - Lines with `// altrucoder_change`
 */
...
const I18N_REPLACEMENTS: StringReplacement[] = [
  // GitHub repo references
  {
    ...
    replacement: "github.com/Altru-Coder/altrucoder",
    description: "GitHub URL",
  },
  {
    pattern: /anomalyco\/opencode/g,
    replacement: "Altru-Coder/altrucoder",
  ...
  // Domain replacements (specific first)
  {
    ...
    replacement: "app.altru-coder.ai",
    description: "App domain",
  },
  {
    pattern: /opencode\.ai(?!\/zen)/g,
    replacement: "altru-coder.ai",
...
/**
 * Check if a line should be preserved (has altrucoder_change marker)
 */
function shouldPreserveLine(line: string): boolean {
  return line.includes("// altrucoder_change")
}

/**
 * Apply string replacements to content, preserving altrucoder_change lines
 */
export function transformI18nContent(
  ...
  for (const line of lines) {
    // Skip lines marked with altrucoder_change
 ...
 * Takes upstream version (theirs) and applies Altru Coder branding
 */
export async function transformConflictedI18n(
  ...
  for (const file of files) {
    ...
    if (!options.dryRun && (await oursHasAltruCoderChanges(file))) {
    ...
    // Then apply Altru Coder branding transformations
    ...
    if (options.dryRun) {
      ...
      success(`Transformed ${file}: took upstream + ${result.replacements} Altru Coder branding replacements`)
    }
  }
