 ...
 *
 * Automates the process of merging upstream opencode changes into Altru Coder.
 *
 * Usage:
...
function logWorktrees(refs: worktree.RefInfo, input: worktree.RefInput, baseName: string): void {
  ...
  logger.info(`  altru-coder-main:  ${refs.main} (${baseName}, ${input.base.slice(0, 8)})`)
  ...
  logger.info(`  - Altru Coder base main: ${refs.main}`)
...
async function main() {
  ...
  logger.header("Altru Coder Upstream Merge Tool")
  ...
  if (keepOursCount > 0) {
    conflictReport.recommendations.push(`${keepOursCount} files will keep Altru Coder's version`)
  }
  if (codemodCount > 0) {
  ...
  const altruVersion = await version.getCurrentAltruCoderVersion()
  ...
  // Create Altru Coder merge branch
  const altruBranch = `${author}/altru-opencode-${targetVersion.tag}`
  const altruBackup = await git.backupAndDeleteBranch(altruBranch)
  if (altruBackup) {
    logger.info(`Backed up existing branch to: ${altruBackup}`)
  }
  await git.createBranch(altruBranch)

  if (options.push) {
    await git.push(config.originRemote, altruBranch, true)
  }
  logger.info(`Created Altru Coder branch: ${altruBranch}`)
  ...
  // This reduces conflicts by transforming upstream code to Altru Coder conventions BEFORE merging
  logger.step(6, 8, "Applying transformations to opencode branch (pre-merge)...")

  logger.info("Removing files skipped in Altru Coder...")
  ...
  // 6a. Transform package names (opencode-ai -> @altru-coder/cli)
  ...
  // 6b. Preserve Altru Coder versions
  logger.info("Preserving Altru Coder versions...")
  const versionResults = await preserveAllVersions({
    dryRun: false,
    verbose: options.verbose,
    targetVersion: altruVersion,
  })
  logger.success(`Preserved versions in ${versionResults.length} files`)

  // 6c. Transform i18n files (OpenCode -> Altru Coder branding)
  ...
  if (i18nPreCount > 0) {
    logger.success(`Transformed ${i18nPreCount} i18n files with Altru Coder branding`)
  }
  ...
  if (brandingCount > 0) {
    logger.success(`Transformed ${brandingCount} files with Altru Coder branding`)
  }

  // 6f. Transform package.json files (names, deps, Altru Coder injections)
  ...
  // 6j. Reset keep-ours files to Altru Coder's version
  logger.info("Resetting Altru Coder-specific files...")
  const keepOursResults = await resetToOurs(config.keepOurs, { dryRun: false, verbose: options.verbose })
  logger.success(`Reset ${keepOursResults.length} files to Altru Coder's version`)

  // Clean untracked build artifacts from Altru Coder-specific directories.
  ...
  logger.info("Cleaning Altru Coder-specific directory artifacts...")
  await git.cleanDirectories(config.altruDirectories)

  // Commit all transformations
  await git.stageAll()
  await git.commit(`refactor: altru-coder compat for ${targetVersion.tag}`)
  logger.success("Committed pre-merge transformations")

  // Step 7: Merge into Altru Coder branch
  logger.step(7, 8, "Merging into Altru Coder branch...")

  await git.checkout(altruBranch)
  ...
  if (!mergeResult.success) {
    ...
    // These are likely files with altrucoder_change markers or actual logic differences.

    // Step 7a: Skip files that shouldn't exist in Altru Coder
    logger.info("Removing files that shouldn't exist in Altru Coder...")
    ...
    logger.info("Keeping Altru Coder-specific files...")
    ...
    if (autoResolved.length > 0) {
      logger.success(`Auto-resolved ${autoResolved.length} conflicts (kept Altru Coder's version)`)
    }
    ...
    // Files with altrucoder_change markers are flagged for manual resolution instead.
    ...
    if (conflictedFiles.length > 0) {
      ...
      // altrucoder_change markers, plus JSON/YAML/TOML key merges and other
      ...
      if (i18nFlagged.length > 0) {
        logger.warn(`${i18nFlagged.length} i18n file(s) have altrucoder_change markers — flagged for manual resolution`)
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (takeFlagged.length > 0) {
          logger.warn(
            `${takeFlagged.length} branding file(s) have altrucoder_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (pkgFlagged.length > 0) {
          logger.warn(
            `${pkgFlagged.length} package.json file(s) have altrucoder_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (scriptFlagged.length > 0) {
          logger.warn(
            `${scriptFlagged.length} script file(s) have altrucoder_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (extFlagged.length > 0) {
          logger.warn(
            `${extFlagged.length} extension file(s) have altrucoder_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (webFlagged.length > 0) {
          logger.warn(
            `${webFlagged.length} web/docs file(s) have altrucoder_change markers — flagged for manual resolution`,
          )
    ...
    // Combine git-reported conflicts with files flagged due to altrucoder_change markers
    const allManual = [...new Set([...remaining, ...flaggedFiles])]
    if (allManual.length > 0) {
      if (flaggedFiles.length > 0) {
        logger.warn(`${flaggedFiles.length} file(s) were flagged because they contain altrucoder_change markers:`)
        logger.list(flaggedFiles)
        logger.info("  These files have intentional Altru Coder-specific changes. Keep our version or merge carefully.")
        logger.info("")
      }
      ...
      logger.info("")
      logger.info("These conflicts contain altrucoder_change markers or actual code differences.")
      ...
      conflictReport.mergeBranch = altruBranch
      const reportPath = `upstream-merge-report-${targetVersion.version}.md`
      await report.saveReport(conflictReport, reportPath)
      logger.success(`Report saved to ${reportPath}`)
      ...
      logger.info(`  3. git push ${config.originRemote} ${altruBranch}`)
      logger.info("  4. Create PR from " + altruBranch + " to " + config.baseBranch)
      logger.info("")
  ...
  if (options.push) {
    await git.push(config.originRemote, altruBranch)
    logger.success(`Pushed ${altruBranch} to ${config.originRemote}`)
  }
  ...
  logger.info(`Altru Coder branch: ${altruBranch}`)
  ...
  if (remainingConflicts.length > 0) {
    ...
    logger.info("  2. Create PR from " + altruBranch + " to " + config.baseBranch)
  }
