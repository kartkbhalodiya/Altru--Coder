 ...
 *   bun run script/upstream/analyze.ts --version v1.1.49 --base-branch catrielmuller/altru-opencode-v1.1.44
 */
...
async function main() {
  ...
  if (keepOursFiles.length > 0) {
    conflictReport.recommendations.push(`${keepOursFiles.length} files will keep Altru Coder's version`)
  }
