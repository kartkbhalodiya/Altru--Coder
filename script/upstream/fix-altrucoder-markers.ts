#!/usr/bin/env bun
/**
 * Rebuild altrucoder_change markers for one file by comparing it with the last
 * merged upstream version.
 *
 * Usage:
 *   bun run script/upstream/fix-altrucoder-markers.ts packages/opencode/src/file.ts
 *   bun run script/upstream/fix-altrucoder-markers.ts packages/opencode/src/file.ts --dry-run
 */
...
function usage() {
  console.log(`Usage: bun run script/upstream/fix-altrucoder-markers.ts <repo-relative-file> [--dry-run]

Rebuilds altrucoder_change markers by:
  ...
  4. Removing existing altrucoder_change markers and adding fresh markers around remaining changed lines.
...
async function main() {
  ...
  header("Fix altrucoder_change markers")
  ...
  if (base === null && annotates(file)) warn(`${file} does not exist upstream; marked as a new Altru Coder file`)
  ...
  if (next === current) {
    success(`${file} already has normalized altrucoder_change markers`)
    return
  }
