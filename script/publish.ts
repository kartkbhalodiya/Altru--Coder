...
// altrucoder_change start - consume changesets on the publish runner so changelog
...
{
  await $`bun install`
  const paths = ["packages/altru-coder-vscode/CHANGELOG.md", "packages/opencode/CHANGELOG.md"]
...
if (Script.release) {
  // altrucoder_change start - commit, tag, and push with rebase + retry to handle
  ...
  // altrucoder_change start - mark prerelease GitHub releases accordingly
  ...
  const changelogPath = fileURLToPath(new URL("../packages/altru-coder-vscode/CHANGELOG.md", import.meta.url))
...
// altrucoder_change start
console.log("\n=== vscode ===\n")
await import(`../packages/altru-coder-vscode/script/publish.ts`)
// altrucoder_change end

const dir = fileURLToPath(new URL("..", import.meta.url))
process.chdir(dir)

// altrucoder_change start - extract latest changelog section for release notes
