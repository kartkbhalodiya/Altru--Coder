...
const reg = process.env.REGISTRY ?? "ghcr.io/altru-org" // altrucoder_change
...
const setup = async () => {
  ...
  if (list.includes("altru-coder")) {
    await $`docker buildx use altru-coder`
    return
  }
  await $`docker buildx create --name altru-coder --use`
