...
import { AltruCoderClient } from "./gen/sdk.gen.js"
export { type Config as AltruCoderClientConfig, AltruCoderClient }
...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ...
    ["x-altru-coder-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-altru-coder-directory")
  next.headers.delete("x-altru-coder-workspace")
  return next
}

export function createAltruCoderClient(
  config?: Config & { directory?: string; experimental_workspaceID?: string; experimentalApi?: boolean },
) {
  if (!config?.fetch) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-altru-coder-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-altru-coder-workspace": config.experimental_workspaceID,
    }
  }
  ...
  return new AltruCoderClient({ client })
}
