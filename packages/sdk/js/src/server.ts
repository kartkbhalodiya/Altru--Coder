...
// altrucoder_change start - Merge existing ALTRU_CODER_CONFIG_CONTENT with new config
// This preserves AltruCoder-injected modes when spawning nested CLI instances
...
function parseExistingConfig(): Config | undefined {
  const content = process.env.ALTRU_CODER_CONFIG_CONTENT
...
export async function createAltruCoderServer(options?: ServerOptions) {
  ...
  const proc = launch(`altru-coder`, args, {
    // altrucoder_change
    env: {
      ...process.env,
      ALTRU_CODER_CONFIG_CONTENT: buildConfigEnv(options.config), // altrucoder_change
    },
  })
  let clear = () => {}
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    proc.stdout?.on("data", (chunk) => {
      ...
      for (const line of lines) {
        ...
        if (line.startsWith("altru-coder server listening")) {
          // altrucoder_change end
...
export function createAltruCoderTui(options?: TuiOptions) {
  const args = []
