// altrucoder_change start - Tests for ALTRU_CODER_CONFIG_CONTENT merging
...
describe("buildConfigEnv", () => {
  ...
  test("preserves existing ALTRU_CODER_CONFIG_CONTENT when spawning with new config", () => {
    // Simulate AltruCoder having injected modes via ALTRU_CODER_CONFIG_CONTENT
    process.env.ALTRU_CODER_CONFIG_CONTENT = JSON.stringify({
      agent: {
        translate: { mode: "primary", prompt: "You are a translator" },
      },
      instructions: [".altru-coder/rules/main.md"],
    })
    ...
    expect(parsed.instructions).toContain(".altru-coder/rules/main.md")
  ...
  test("handles invalid JSON in existing ALTRU_CODER_CONFIG_CONTENT gracefully", () => {
    process.env.ALTRU_CODER_CONFIG_CONTENT = "invalid json {"
