...
// altrucoder_change: marked-shiki highlighted code blocks synchronously during
...
// altrucoder_change start: import types for double-dollar math extension
import type { MarkedExtension, TokenizerAndRendererExtension } from "marked"
// altrucoder_change end
import { bundledLanguages, type BundledLanguage } from "shiki"
import { parseFilePath } from "../file-path" // altrucoder_change
import { createSimpleContext } from "./helper"
import { getSharedHighlighter, registerCustomTheme, ThemeRegistrationResolved } from "@pierre/diffs"

registerCustomTheme("Altru Coder", () => {
...
// altrucoder_change start: double-dollar-only math rules for marked.
const BLOCK = /^\$\$\n((?:\\[^]|[^\\])+?)\n\$\$(?:\n|$)/
const INLINE = /^\$\$(?!\$)((?:\\.|[^\\\n])*?(?:\\.|[^\\\n$]))\$\$/
// altrucoder_change end
...
function renderMathInText(text: string): string {
  ...
  // altrucoder_change: removed single-dollar inline math ($...$) rendering.
...
// altrucoder_change: parseFilePath imported from ../file-path

// altrucoder_change start: highlight cache for deferred highlighting
...
export async function deferredHighlight(
  ...
  const highlighter = await getSharedHighlighter({ themes: ["Altru Coder"], langs: [] })
      ...
      setTimeout(() => {
        ...
        try {
          ...
          const highlight = () => {
            ...
            const html = highlighter.codeToHtml(code, { lang: language, theme: "Altru Coder", tabindex: false })
...
export const { use: useMarked, provider: MarkedProvider } = createSimpleContext({
  ...
  init: (props: { nativeParser?: NativeMarkdownParser }) => {
    // altrucoder_change start: two-pass parser — first pass skips Shiki highlighting
    ...
    const parser = marked.use(
      ...
      // altrucoder_change start: enable only double-dollar math.
      // Single $ is far more common as a currency symbol in agent responses
      ...
      // altrucoder_change end
      // altrucoder_change: markedShiki removed — the custom `code` renderer
