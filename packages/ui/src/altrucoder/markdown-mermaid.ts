...
function flowchart(source: string) {
  ...
  return line ? /^(?:graph|flowchart)\b/i.test(line.trim()) : false
}
...
function repair(source: string) {
  ...
  return source.replace(/[^\r\n]+/g, (line) => {
    ...
    return line.replace(
      /(^|[^\w])([A-Za-z_][\w-]*)\[(?![\[("'`])([^\]\r\n]+)\]/g,
      (_match, prefix: string, id: string, text: string) => `${prefix}${id}[${quote(text)}]`,
    )
  })
}
...
function save(url: string, filename: string) {
  const event = new CustomEvent("altru-coder:save-image", {
    bubbles: true,
    cancelable: true,
...
async function svg(renderer: Mermaid, source: string, cfg: ReturnType<typeof config>) {
  return enqueue(async () => {
    const draw = async (value: string) => {
      ...
      await renderer.parse(value)
      return renderer.render(`markdown-mermaid-${fnv1a(value)}-${cache.id++}`, value)
    }

    try {
      return await draw(source)
    } catch (err) {
      const fixed = repair(source)
      if (fixed === source) throw err
      return draw(fixed)
    }
  })
}
