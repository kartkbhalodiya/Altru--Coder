...
import { tryFastRender } from "../altrucoder/markdown-fast-path" // altrucoder_change
import { hasMermaid, preserveMermaid, renderMermaid, type MermaidLabels } from "../altrucoder/markdown-mermaid" // altrucoder_change
...
export function Markdown(
  ...
  // altrucoder_change start: generation counter prevents stale deferredHighlight
  ...
  // altrucoder_change start: Mermaid diagram rendering
  const mermaidState = { gen: 0, signal: { aborted: false } }
  // altrucoder_change end

  // altrucoder_change start: rAF-coalesced morphdom render.
  ...
  createEffect(() => {
    ...
    if (!content) {
      // altrucoder_change start: cancel any in-flight coalesced render so a
    ...
    // altrucoder_change start: queue the latest content for a single rAF tick.
    ...
    pendingFrame = requestAnimationFrame(() => {
      ...
      // altrucoder_change start: morphdom guard for highlighted blocks (issue #6221)
      ...
      morphdom(container, temp, {
        ...
        onBeforeElUpdated: (fromEl, toEl) => {
          ...
          // altrucoder_change start: preserve rendered Mermaid diagrams across
          ...
          if (
            ...
            // Source changed during streaming — fall through so morphdom replaces // altrucoder_change
      ...
      kickMermaid(container, local.streaming ?? false, mermaid) // altrucoder_change
      kickHighlight(container, nextLabels)
    })
    // altrucoder_change end
  })

  // altrucoder_change start: progressive Shiki highlighting (issue #6221, PR #7102).
  ...
  onCleanup(() => {
    // altrucoder_change: cancel any in-flight deferredHighlight pass so its
    ...
    mermaidState.gen++
    // altrucoder_change end
    // altrucoder_change: cancel any queued rAF parse so it doesn't touch the
