...
import type { SnapshotFileDiff, VcsFileDiff } from "@altru-coder/sdk/v2"
...
// altrucoder_change start - expose patch text extraction without building FileDiffMetadata on the UI thread
...
export type ViewDiff = {
  file: string
  patch: string
  before: string // altrucoder_change
  after: string // altrucoder_change
  additions: number
  deletions: number
  status?: "added" | "deleted" | "modified"
...
export function normalize(diff: ReviewDiff): ViewDiff {
  const next = contents(diff) // altrucoder_change
  return {
    file: diff.file, // altrucoder_change
    patch: next.patch, // altrucoder_change
    before: next.before, // altrucoder_change
    after: next.after, // altrucoder_change
