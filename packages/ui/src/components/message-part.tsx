...
export interface MessageProps {
  ...
  queued?: boolean // altrucoder_change
      ...
      <Match when={props.message.role === "user" && props.message}>
        {(userMessage) => (
          <UserMessageDisplay
            ...
            queued={props.queued} // altrucoder_change
          />
        )}
      </Match>
            ...
            {(file) => {
              ...
              return (
                <div
                  ...
                  data-queued={props.queued ? "" : undefined} // altrucoder_change
...
PART_MAPPING["text"] = function TextPartDisplay(props) {
  ...
  const isLastTextPart = createMemo(() => {
    const last = (data.store.part?.[props.message.id] ?? [])
      .filter((item): item is TextPart => item?.type === "text" && !!item.text?.trim() && !item.synthetic) // altrucoder_change
      .at(-1)
    return last?.id === part().id
  })
  const showCopy = createMemo(() => {
    if (part().synthetic) return false // altrucoder_change
  ...
  return (
    <Show when={text() && showSyntheticPart() /* altrucoder_change */}>
