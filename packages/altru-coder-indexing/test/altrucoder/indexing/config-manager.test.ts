...
describe("CodeIndexConfigManager", () => {
  ...
  test("configures Altru Coder with hosted auth options and explicit model metadata", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "altru-coder",
        openAiKey: undefined,
        altruApiKey: "altru-coder-token",
        altruBaseUrl: "https://example.test/api/gateway/",
    ...
    expect(cfg.isFeatureConfigured).toBe(true)
    expect(cfg.getConfig().altruOptions).toEqual({
  ...
  test("requires Altru Coder model metadata from Cloud config", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "altru-coder",
        openAiKey: undefined,
  ...
  test("uses configured dimension for Altru Coder models outside the fallback catalog", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "altru-coder",
        openAiKey: undefined,
  ...
  describe("loadConfiguration restart checks", () => {
    ...
    test("requires restart when Altru Coder auth changes", () => {
      const cfg = new CodeIndexConfigManager(
        createInput({
          embedderProvider: "altru-coder",
          openAiKey: undefined,
          altruApiKey: "old-token",
          modelId: "mistralai/mistral-embed-2312",
          modelDimension: 1024,
        }),
      )
