...
describe("FileWatcher", () => {
  ...
  test("processFile skips files matched by .altrucoderignore during incremental updates", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "file-watcher-test-"))
    const cacheDir = path.join(root, ".cache")
    const file = path.join(root, "secret.ts")

    await mkdir(cacheDir, { recursive: true })
    await writeFile(path.join(root, ".altrucoderignore"), "secret.ts\n")
    ...
    expect(result.reason).toBe("File is ignored by .gitignore or .altrucoderignore")
  })
})
