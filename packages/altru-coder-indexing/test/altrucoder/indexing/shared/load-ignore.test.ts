...
describe("loadIgnore", () => {
  ...
  test("loads root .altrucoderignore in addition to root .gitignore", async () => {
    await writeFile(path.join(root, ".gitignore"), "dist/\n")
    await writeFile(path.join(root, ".altrucoderignore"), "secret/\n")
  ...
  test("preserves existing .gitignore-only behavior when .altrucoderignore is absent", async () => {
  ...
  test("ignores the ignore files themselves", async () => {
    ...
    expect(ig.ignores(".altrucoderignore")).toBe(true)
  })
})
