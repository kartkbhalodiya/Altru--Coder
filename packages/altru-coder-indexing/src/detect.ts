export const INDEXING_PLUGIN_NAMES = ["altru-coder-indexing", "@altru-coder/altru-coder-indexing"] as const
...
function normalizePath(value: string): string {
  ...
  const scoped = parts.findIndex((part, i) => part === "@altru-coder" && parts[i + 1] === "altru-coder-indexing")
  if (scoped >= 0) return "@altru-coder/altru-coder-indexing"

  const workspace = parts.findIndex((part, i) => part === "packages" && parts[i + 1] === "altru-coder-indexing")
  if (workspace >= 0) return "@altru-coder/altru-coder-indexing"
