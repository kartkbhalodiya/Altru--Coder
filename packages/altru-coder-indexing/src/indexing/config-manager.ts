...
export class CodeIndexConfigManager {
  ...
  private altruOptions?: { apiKey: string; baseUrl?: string; organizationId?: string }
  ...
  private applyInput(input: IndexingConfigInput): void {
    ...
    this.altruOptions = input.altruApiKey
      ? { apiKey: input.altruApiKey, baseUrl: input.altruBaseUrl, organizationId: input.altruOrganizationId }
      : undefined
  ...
  private captureSnapshot(): PreviousConfigSnapshot {
    return {
      ...
      altruApiKey: this.altruOptions?.apiKey ?? "",
      altruBaseUrl: this.altruOptions?.baseUrl ?? "",
      altruOrganizationId: this.altruOptions?.organizationId ?? "",
  ...
  public isConfigured(): boolean {
    ...
    if (provider === "altru-coder") return !!(this.altruOptions?.apiKey && this.modelId && this.currentModelDimension && hasStore)
  ...
  doesConfigChangeRequireRestart(prev: PreviousConfigSnapshot): boolean {
    ...
    // Auth changes
    if ((prev.altruApiKey ?? "") !== (this.altruOptions?.apiKey ?? "")) return true
    if ((prev.altruBaseUrl ?? "") !== (this.altruOptions?.baseUrl ?? "")) return true
    if ((prev.altruOrganizationId ?? "") !== (this.altruOptions?.organizationId ?? "")) return true
  ...
  public getConfig(): CodeIndexConfig {
    return {
      ...
      modelId: this.modelId,
      modelDimension: this.modelDimension,
      altruOptions: this.altruOptions,
