...
import { AltruCoderEmbedder } from "./embedders/altru-coder"
...
export class CodeIndexServiceFactory {
  ...
  public createEmbedder(): IEmbedder {
    ...
    if (provider === "altru-coder") {
      if (!config.altruOptions?.apiKey) throw new Error("Altru Coder API key is required for embedding.")
      if (!config.modelId) throw new Error("Altru Coder embedding model is required.")
      return new AltruCoderEmbedder({
        apiKey: config.altruOptions.apiKey,
        baseUrl: config.altruOptions.baseUrl,
        organizationId: config.altruOptions.organizationId,
