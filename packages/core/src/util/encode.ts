export function base64Encode(value: string) {
  const normalized = value.replace(/\\/g, "/") // altrucoder_change - normalize Windows paths
  const bytes = new TextEncoder().encode(normalized) // altrucoder_change - encode as UTF-8 bytes
