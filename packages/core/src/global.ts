...
import { markNoIndex } from "./altrucoder/spotlight" // altrucoder_change
import { Flag } from "./flag/flag"

const app = "altru-coder" // altrucoder_change
...
// which breaks every `altru-coder` invocation at startup (including the SDK
...
const paths = {
  get home() {
    return (process.env.ALTRU_CODER_TEST_HOME ?? os.homedir()).trim() // altrucoder_change — defensive trim, see above
  },
  data,
...
// altrucoder_change start - keep generated Altru Coder data out of macOS Spotlight
...
export function make(input: Partial<Interface> = {}): Interface {
  return {
    ...
    data: Path.data,
    cache: Path.cache,
    config: Flag.ALTRU_CODER_CONFIG_DIR ?? Path.config,
    state: Path.state,
    tmp: Path.tmp,
    bin: Path.bin,
    log: Path.log,
