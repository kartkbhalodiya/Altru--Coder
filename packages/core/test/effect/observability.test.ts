...
const opencodeClient = process.env.ALTRU_CODER_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opencodeClient === undefined) delete process.env.ALTRU_CODER_CLIENT
  else process.env.ALTRU_CODER_CLIENT = opencodeClient
})
