...
import ai.altrucoder.backend.cli.AltruCoderCliDataParser
...
/**
 * Pure unit tests for [AltruCoderCliDataParser].
 *
 ...
 */
class AltruCoderCliDataParserTest {
    ...
    fun `extractEventType - parses type from JSON data`() {
        val result = AltruCoderCliDataParser.extractEventType(
    ...
    fun `extractEventType - returns unknown for missing type`() {
        assertEquals("unknown", AltruCoderCliDataParser.extractEventType("""{"data":"something"}"""))
    }

    @Test
    fun `extractEventType - returns unknown for empty string`() {
        assertEquals("unknown", AltruCoderCliDataParser.extractEventType(""))
    }
    ...
    fun `parseChatEvent - message updated with GlobalEvent wrapper`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("message.updated", data)
    ...
    fun `parseChatEvent - message part delta`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("message.part.delta", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - message part updated`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("message.part.updated", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - turn open`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("session.turn.open", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - turn close`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("session.turn.close", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session error`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("session.error", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - message removed`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("message.removed", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - unknown type returns null`() {
        val data = globalEvent("""
            ...
            "properties": { "sessionID": "ses_1" }
        """)
        assertNull(AltruCoderCliDataParser.parseChatEvent("some.unknown.event", data))
    }

    @Test
    fun `parseChatEvent - malformed JSON returns null`() {
        assertNull(AltruCoderCliDataParser.parseChatEvent("message.updated", "not json"))
    }

    @Test
    fun `parseChatEvent - missing properties returns null`() {
        assertNull(AltruCoderCliDataParser.parseChatEvent("message.updated", """{"payload":{"type":"x"}}"""))
    }
    ...
    fun `parseChatEvent - missing sessionID returns null`() {
        ...
        """)
        assertNull(AltruCoderCliDataParser.parseChatEvent("message.updated", data))
    }
    ...
    fun `parseSessionStatus - valid status event`() {
        ...
        val result = AltruCoderCliDataParser.parseSessionStatus(data)
        assertNotNull(result)
    ...
    fun `parseSessionStatus - missing sessionID returns null`() {
        ...
        assertNull(AltruCoderCliDataParser.parseSessionStatus(data))
    }
    ...
    fun `parseSession - full session response`() {
        ...
        val result = AltruCoderCliDataParser.parseSession(raw)
    ...
    fun `parseMessages - empty array`() {
        assertEquals(emptyList(), AltruCoderCliDataParser.parseMessages("[]"))
    }
    ...
    fun `parseMessages - user and assistant messages`() {
        ...
        val result = AltruCoderCliDataParser.parseMessages(raw)
    ...
    fun `parseMessages - step finish part with tokens`() {
        ...
        val part = AltruCoderCliDataParser.parseMessages(raw)[0].parts[0]
    ...
    fun `parseMessages - malformed JSON returns empty`() {
        assertEquals(emptyList(), AltruCoderCliDataParser.parseMessages("not json"))
    }
    ...
    fun `parseModelState - parses favorites`() {
        val result = AltruCoderCliDataParser.parseModelState(
            """{"favorite":[{"providerID":"altru-coder","modelID":"auto"},{"providerID":"openai","modelID":"gpt"}]}""",
        )

        assertEquals(listOf("altru-coder/auto", "openai/gpt"), result.favorite.map { "${it.providerID}/${it.modelID}" })
    }
    ...
    fun `parseModelState - parses model selections and variants`() {
        val result = AltruCoderCliDataParser.parseModelState(
            """{"model":{"code":{"providerID":"altru-coder","modelID":"auto"}},"variant":{"altru-coder/auto":"medium"}}""",
        )

        assertEquals("altru-coder", result.model["code"]?.providerID)
        assertEquals("auto", result.model["code"]?.modelID)
        assertEquals("medium", result.variant["altru-coder/auto"])
    }

    @Test
    fun `parseModelState - drops malformed favorites`() {
        val result = AltruCoderCliDataParser.parseModelState(
            """{"favorite":[{"providerID":"altru-coder"},false,{"providerID":"openai","modelID":"gpt"}]}""",
        )
    ...
    fun `parseModelState - malformed inputs return empty favorites`() {
        for (raw in listOf("", "not-json", "[]", "42", "null")) {
            assertTrue(AltruCoderCliDataParser.parseModelState(raw).favorite.isEmpty(), raw)
        }
    }
    ...
    fun `parseModelState - drops malformed model selections and variants`() {
        val result = AltruCoderCliDataParser.parseModelState(
            """{"model":{"bad":false,"ok":{"providerID":"altru-coder","modelID":"auto"}},"variant":{"":"low","altru-coder/auto":false,"openai/gpt":"high"}}""",
        )
    ...
    fun `buildModelStateJson - preserves unrelated keys and replaces favorites`() {
        val raw = """{"model":{"code":{"providerID":"altru-coder","modelID":"auto"}},"recent":[{"providerID":"old","modelID":"recent"}],"variant":{"altru-coder/auto":"fast"},"extra":true,"favorite":[]}"""
        val result = AltruCoderCliDataParser.buildModelStateJson(raw, listOf(ModelSelectionDto("anthropic", "claude")))
        ...
        assertEquals(listOf("anthropic/claude"), AltruCoderCliDataParser.parseModelState(result).favorite.map { "${it.providerID}/${it.modelID}" })
    }

    @Test
    fun `buildModelStateJson - writes model selections and variants`() {
        val raw = """{"recent":[],"extra":true}"""
        val result = AltruCoderCliDataParser.buildModelStateJson(
            raw,
            ModelStateDto(
                model = mapOf("code" to ModelSelectionDto("altru-coder", "auto")),
                variant = mapOf("altru-coder/auto" to "medium"),
                recent = listOf(ModelSelectionDto("anthropic", "claude")),
            ),
        )

        val state = AltruCoderCliDataParser.parseModelState(result)
    ...
    fun `buildPromptJson - text only`() {
        ...
        val result = AltruCoderCliDataParser.buildPromptJson(prompt)
    ...
    fun `buildSummarizeJson - writes provider and model`() {
        val result = AltruCoderCliDataParser.buildSummarizeJson(ModelSelectionDto("anthropic", "claude-4"))
    ...
    fun `buildConfigPartial - model only`() {
        val result = AltruCoderCliDataParser.buildConfigPartial(ConfigUpdateDto(model = "anthropic/claude-4"))
        assertEquals("""{"model":"anthropic/claude-4"}""", result)
    }

    @Test
    fun `buildConfigPartial - agent and temperature`() {
        val result = AltruCoderCliDataParser.buildConfigPartial(
    ...
    fun `buildConfigPartial - empty update`() {
        val result = AltruCoderCliDataParser.buildConfigPartial(ConfigUpdateDto())
        assertEquals("{}", result)
    }

    @Test
    fun `buildConfigPartial - temperature without agent defaults to ask`() {
        val result = AltruCoderCliDataParser.buildConfigPartial(ConfigUpdateDto(temperature = 0.5))
    ...
    fun `parseChatEvent - permission asked`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("permission.asked", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - permission replied`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("permission.replied", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - question asked`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("question.asked", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - question replied`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("question.replied", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - question rejected`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("question.rejected", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session status idle`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("session.status", data) as ChatEventDto.SessionStatusChanged
    ...
    fun `parseChatEvent - message part removed`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("message.part.removed", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - todo updated`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("todo.updated", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session idle`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("session.idle", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session compacted`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("session.compacted", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session updated`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("session.updated", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session diff`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("session.diff", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - part updated with callID`() {
        ...
        val result = AltruCoderCliDataParser.parseChatEvent("message.part.updated", data) as ChatEventDto.PartUpdated
    ...
    fun `buildPermissionReplyJson - once reply`() {
        val result = AltruCoderCliDataParser.buildPermissionReplyJson(PermissionReplyDto(reply = "once"))
        assertEquals("""{"reply":"once"}""", result)
    }

    @Test
    fun `buildPermissionReplyJson - always reply with message`() {
        val result = AltruCoderCliDataParser.buildPermissionReplyJson(PermissionReplyDto(reply = "always", message = "approved"))
    ...
    fun `buildPermissionAlwaysRulesJson - approved list`() {
        val result = AltruCoderCliDataParser.buildPermissionAlwaysRulesJson(
    ...
    fun `buildQuestionReplyJson - single question single answer`() {
        val result = AltruCoderCliDataParser.buildQuestionReplyJson(QuestionReplyDto(answers = listOf(listOf("A"))))
        assertEquals("""{"answers":[["A"]]}""", result)
    }

    @Test
    fun `buildQuestionReplyJson - multiple questions`() {
        val result = AltruCoderCliDataParser.buildQuestionReplyJson(
    ...
    fun `parsePermissionRequests - parses list`() {
        ...
        val result = AltruCoderCliDataParser.parsePermissionRequests(raw)
        assertEquals(1, result.size)
    ...
    fun `parsePermissionRequests - empty list`() {
        assertEquals(emptyList(), AltruCoderCliDataParser.parsePermissionRequests("[]"))
    }
    ...
    fun `parseQuestionRequests - parses list`() {
        ...
        val result = AltruCoderCliDataParser.parseQuestionRequests(raw)
        assertEquals(1, result.size)
