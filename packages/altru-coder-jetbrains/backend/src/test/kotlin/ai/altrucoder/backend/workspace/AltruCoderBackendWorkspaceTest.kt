...
import ai.altrucoder.backend.workspace.AltruCoderBackendWorkspace
...
class AltruCoderBackendWorkspaceTest {
    ...
    private fun setup(): AltruCoderBackendAppService =
        AltruCoderBackendAppService.create(scope, FakeCliServer(mock), log)

    private suspend fun ready(app: AltruCoderBackendAppService): AltruCoderBackendWorkspace {
        app.connect()
        withTimeout(10_000) {
            app.appState.first { it is AltruCoderAppState.Ready }
        }
    ...
    fun `get creates workspace on demand after Ready`() = runBlocking {
        ...
        withTimeout(15_000) {
            ws.state.first { it is AltruCoderWorkspaceState.Ready }
        }

        val state = ws.state.value as AltruCoderWorkspaceState.Ready
    ...
    fun `same directory returns same workspace instance`() = runBlocking {
        val app = setup()
        app.connect()
        withTimeout(10_000) { app.appState.first { it is AltruCoderAppState.Ready } }
    ...
    fun `workspaces stopped on app disconnect`() = runBlocking {
        ...
        assertIs<AltruCoderWorkspaceState.Pending>(ws.state.value)
    ...
    fun `workspace reaches Ready after creation`() = runBlocking {
        ...
        assertIs<AltruCoderWorkspaceState.Ready>(ws.state.value)
    }
    ...
    fun `providers failure retries then transitions to Error`() = runBlocking {
        ...
        withTimeout(15_000) {
            ws.state.first { it is AltruCoderWorkspaceState.Error }
        }

        val err = ws.state.value as AltruCoderWorkspaceState.Error
    ...
    fun `concurrent load calls on workspace produce valid final state`() = runBlocking {
        ...
        withTimeout(15_000) {
            while (true) {
                ...
                if (state is AltruCoderWorkspaceState.Ready) {
                    delay(300)
                    if (ws.state.value is AltruCoderWorkspaceState.Ready) break
                }
                delay(100)
            }
        }
        ...
        withTimeout(15_000) {
            ...
            while (true) {
                ...
                if (state is AltruCoderAppState.Ready) {
                    delay(300)
                    if (app.appState.value is AltruCoderAppState.Ready) break
                }
                delay(100)
            }
        }
