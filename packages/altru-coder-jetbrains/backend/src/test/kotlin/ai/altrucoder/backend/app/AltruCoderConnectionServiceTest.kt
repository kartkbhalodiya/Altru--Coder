...
import ai.altrucoder.backend.app.ConnectionState
import ai.altrucoder.backend.app.AltruCoderConnectionService
...
class AltruCoderConnectionServiceTest {
    ...
    fun `connect transitions to Connected`() = runBlocking {
        ...
        val svc = AltruCoderConnectionService(
          scope,
          fake,
    ...
    fun `connect provides API client`() = runBlocking {
        val svc = AltruCoderConnectionService(scope, fake, {}, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    fun `init error transitions to Error state`() = runBlocking {
        ...
        val svc = AltruCoderConnectionService(scope, failing, {}, log)
        svc.connect()
    ...
    // extractType tests moved to AltruCoderCliDataParserTest
