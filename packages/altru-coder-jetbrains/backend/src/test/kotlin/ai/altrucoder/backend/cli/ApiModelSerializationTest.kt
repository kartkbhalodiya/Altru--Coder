...
import ai.altrucoder.jetbrains.api.infrastructure.Serializer
import ai.altrucoder.jetbrains.api.model.Config
import ai.altrucoder.jetbrains.api.model.GlobalHealth200Response
import ai.altrucoder.jetbrains.api.model.AltruCoderNotifications200ResponseInner
import ai.altrucoder.jetbrains.api.model.AltruCoderNotifications200ResponseInnerAction
import ai.altrucoder.jetbrains.api.model.AltruCoderProfile200Response
import ai.altrucoder.jetbrains.api.model.AltruCoderProfile200ResponseBalance
import ai.altrucoder.jetbrains.api.model.AltruCoderProfile200ResponseProfile
import ai.altrucoder.jetbrains.api.model.AltruCoderProfile200ResponseProfileOrganizationsInner
...
class ApiModelSerializationTest {
    ...
    @Test
    fun `AltruCoderNotifications200ResponseInner with action`() {
        val src = """{
            "id": "notif-1",
        ...
        val obj = json.decodeFromString<AltruCoderNotifications200ResponseInner>(src)
    ...
    @Test
    fun `AltruCoderNotifications200ResponseInner without action`() {
    ...
    @Test
    fun `AltruCoderNotifications200ResponseInner with showIn and suggestModelId`() {
        val src = """{
            "id": "n3",
            "title": "Try new model",
    ...
    fun `empty notifications array`() {
        val list = json.decodeFromString<List<AltruCoderNotifications200ResponseInner>>("[]")
        assertTrue(list.isEmpty())
    }

    @Test
    fun `AltruCoderProfile200Response with balance`() {
        val src = """{
            "profile": {"email": "user@test.com", "name": "User"},
            "balance": {"balance": 42.5},
            "currentOrgId": "org-1"
        }"""
        val obj = json.decodeFromString<AltruCoderProfile200Response>(src)
    ...
    @Test
    fun `AltruCoderProfile200Response with null balance`() {
        val src = """{
    ...
    @Test
    fun `AltruCoderProfile200Response with organizations`() {
        val src = """{
            "profile": {
                "email": "user@test.com",
                "organizations": [
