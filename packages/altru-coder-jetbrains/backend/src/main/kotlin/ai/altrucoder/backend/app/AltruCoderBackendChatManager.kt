 ...
 *
 * All JSON parsing is delegated to [AltruCoderCliDataParser].
 */
class AltruCoderBackendChatManager(
        ...
        watcher = cs.launch {
            sse.collect { event ->
                if (event.type in CHAT_EVENTS) {
                    val parsed = AltruCoderCliDataParser.parseChatEvent(event.type, event.data)
                    if (parsed != null) {
    ...
    fun prompt(id: String, dir: String, prompt: PromptDto) {
        ...
        val body = AltruCoderCliDataParser.buildPromptJson(prompt)
    ...
    fun compact(id: String, dir: String, model: ModelSelectionDto) {
        ...
        val http = requireClient()
        val url = requireBase()
        val body = AltruCoderCliDataParser.buildSummarizeJson(model)
        val request = Request.Builder()
    ...
    fun messages(id: String, dir: String): List<MessageWithPartsDto> {
        ...
        return http.newCall(request).execute().use { response ->
            ...
            val parsed = AltruCoderCliDataParser.parseMessages(raw)
    ...
    fun updateConfig(dir: String, update: ConfigUpdateDto) {
        ...
        val partial = AltruCoderCliDataParser.buildConfigPartial(update)
    ...
    fun replyPermission(requestId: String, dir: String, reply: PermissionReplyDto) {
        ...
        val body = AltruCoderCliDataParser.buildPermissionReplyJson(reply)
    ...
    fun savePermissionRules(requestId: String, dir: String, rules: PermissionAlwaysRulesDto) {
        ...
        val body = AltruCoderCliDataParser.buildPermissionAlwaysRulesJson(rules)
    ...
    fun replyQuestion(requestId: String, dir: String, answers: QuestionReplyDto) {
        ...
        val body = AltruCoderCliDataParser.buildQuestionReplyJson(answers)
    ...
    fun pendingPermissions(dir: String): List<PermissionRequestDto> {
        ...
        val parsed = AltruCoderCliDataParser.parsePermissionRequests(raw)
    ...
    fun pendingQuestions(dir: String): List<QuestionRequestDto> {
        ...
        val parsed = AltruCoderCliDataParser.parseQuestionRequests(raw)
