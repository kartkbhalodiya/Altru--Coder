...
class AltruCoderBackendModelStateManager(
    private val log: AltruCoderLog,
) {
    companion object {
        private val DEFAULT_DIR = Path.of(System.getProperty("user.home"), ".local", "state", "altru-coder")
    }
    ...
    suspend fun state(): ModelStateDto = mutex.withLock {
        AltruCoderCliDataParser.parseModelState(read().orEmpty())
    }
    ...
    suspend fun favorite(update: ModelFavoriteUpdateDto): ModelStateDto = mutex.withLock {
        ...
        val state = AltruCoderCliDataParser.parseModelState(raw.orEmpty())
        ...
        write(AltruCoderCliDataParser.buildModelStateJson(raw, updated))
        updated
    }
