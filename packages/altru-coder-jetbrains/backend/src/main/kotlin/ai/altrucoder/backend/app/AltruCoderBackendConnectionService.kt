 ...
 *
 * Concurrency is handled by the owning [AltruCoderBackendAppService] — `connect`,
 * `restart`, and `reinstall` are called under its mutex. Internal reconnect
 * attempts delegate back to the owner via [onReconnect].
 *
 * Not a service — owned and instantiated by [AltruCoderBackendAppService].
 */
class AltruCoderConnectionService(
  private val cs: CoroutineScope,
  private val server: CliServer,
  private val onReconnect: () -> Unit,
  private val log: AltruCoderLog = AltruCoderLog.create(AltruCoderConnectionService::class.java),
) {
     ...
     *
     * Called under [AltruCoderBackendAppService]'s mutex — no internal guard needed.
     */
    suspend fun connect() {
        open()
    }
     ...
     *
     * Called under [AltruCoderBackendAppService]'s mutex.
     */
    suspend fun restart() {
    ...
    private suspend fun open() {
        ...
        val ac = AltruCoderBackendHttpClients.api(password)
        val hc = AltruCoderBackendHttpClients.health(password)
        apiClient = ac
        healthClient = hc
    ...
    private val listener = object : EventSourceListener() {
        ...
        override fun onEvent(src: EventSource, id: String?, type: String?, data: String) {
            lastEvent.set(System.currentTimeMillis())
            val kind = type ?: AltruCoderCliDataParser.extractEventType(data)
     ...
     * goes through [AltruCoderBackendAppService]'s mutex for a full restart.
     */
    ...
    private fun close() {
        api = null
        apiClient?.let { AltruCoderBackendHttpClients.shutdown(it) }
        apiClient = null
        healthClient?.let { AltruCoderBackendHttpClients.shutdown(it) }
        healthClient = null
    }
    ...
    fun dispose() {
        ...
        log.info("AltruCoderConnectionService disposed")
    }
}
