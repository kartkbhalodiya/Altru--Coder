...
openApiGenerate {
    ...
    packageName.set("ai.altrucoder.jetbrains.api")
    apiPackage.set("ai.altrucoder.jetbrains.api.client")
    modelPackage.set("ai.altrucoder.jetbrains.api.model")
...
val localCli by tasks.registering(PrepareLocalCliTask::class) {
    ...
    exe.set(platform.map { if (it.startsWith("windows")) "altru-coder.exe" else "altru-coder" })
}
