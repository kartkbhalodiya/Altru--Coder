...
/**
 * Logging interface for the Altru Coder JetBrains plugin.
 *
 ...
 * system property), output is additionally written to a `altru-coder-dev.log` file inside the sandbox log directory.
 ...
 * Usage:
 * ```kotlin
 * private val log = AltruCoderLog.create(this::class.java)
 *
 ...
 * ```
 *
 * The log level for the sandbox file can be controlled via the `altru-coder.dev.log.level` system property
 * (DEBUG, INFO, WARN, ERROR, OFF). Defaults to INFO.
 */
interface AltruCoderLog {
    ...
    companion object {
        ...
        fun create(cls: Class<*>): AltruCoderLog {
...
internal class IntellijLog(cls: Class<*>) : AltruCoderLog {
...
internal class FileLog(cls: Class<*>) : AltruCoderLog {
    ...
    companion object {
        ...
        private val root: java.util.logging.Logger by lazy {
            val logger = java.util.logging.Logger.getLogger("ai.altrucoder")
        ...
        private val handler: FileHandler by lazy {
            ...
            val path = dir.resolve("altru-coder-dev.log")
            val h = FileHandler(path.toString(), true)
            h.formatter = AltruCoderFormatter()
            h
        }
        ...
        private fun resolveLogDir(): Path {
            ...
            while (current.parent != null) {
                ...
                if (name.startsWith("log_run")) {
                    side = if (name.lowercase().contains("frontend")) "altru-coder-frontend" else "altru-coder-backend"
                }
                if (name == "altru-coder.jetbrains" && side != null) {
        ...
        private fun resolveLevel(): Level {
            val prop = System.getProperty("altru-coder.dev.log.level") ?: return Level.INFO
...
internal class AltruCoderFormatter : Formatter() {
    ...
    override fun format(record: LogRecord): String {
        ...
        val category = record.sourceClassName ?: record.loggerName ?: "altru-coder.dev"
...
internal class CompositeLog(vararg val delegates: AltruCoderLog) : AltruCoderLog {
    override val isDebugEnabled: Boolean
