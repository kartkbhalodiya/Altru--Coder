 ...
 * or worktree). Each call routes to a [AltruCoderBackendWorkspace]
 * via the workspace manager.
 */
@Rpc
interface AltruCoderWorkspaceRpcApi : RemoteApi<Unit> {
    companion object {
        suspend fun getInstance(): AltruCoderWorkspaceRpcApi {
            return RemoteApiProviderService.resolve(remoteApiDescriptor<AltruCoderWorkspaceRpcApi>())
        }
    }
    ...
    suspend fun state(directory: String): Flow<AltruCoderWorkspaceStateDto>
