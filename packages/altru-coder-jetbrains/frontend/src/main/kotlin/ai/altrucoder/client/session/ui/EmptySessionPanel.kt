...
import ai.altrucoder.client.ui.md.MdView
...
class EmptySessionPanel(
    ...
    private val recentTitle = JBLabel(AltruCoderBundle.message("session.empty.recent")).apply {
    ...
    private val md = MdView.html().apply {
        ...
        opaque = false
        foreground = UIUtil.getContextHelpForeground()
        set(AltruCoderBundle.message("session.empty.welcome"))
    }
    ...
    private fun createContent(): BorderLayoutPanel {
        val logo = JBLabel(
            IconLoader.getIcon("/icons/altru-content.svg", EmptySessionPanel::class.java),
        ).apply {
    ...
    private fun title(session: SessionDto) =
        session.title.takeIf { it.isNotBlank() } ?: AltruCoderBundle.message("session.tab.untitled")
    ...
    private fun time(session: SessionDto, now: Long = System.currentTimeMillis()): String {
        ...
        if (diff < MINUTE) return AltruCoderBundle.message("session.empty.time.moments")
        if (diff < HOUR) return AltruCoderBundle.message("session.empty.time.minutes", (diff / MINUTE).coerceAtLeast(1))
        if (diff < DAY) return AltruCoderBundle.message("session.empty.time.hours", (diff / HOUR).coerceAtLeast(1))
        return AltruCoderBundle.message("session.empty.time.days", (diff / DAY).coerceAtLeast(1))
    }
