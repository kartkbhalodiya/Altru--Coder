...
private data class ContextData(
    ...
    fun tip(): String {
        val lines = mutableListOf(AltruCoderBundle.message("session.header.context.used", num(used), num(limit)))
        if (output > 0) lines.add(AltruCoderBundle.message("session.header.context.reserved", num(output)))
        if (available > 0) lines.add(AltruCoderBundle.message("session.header.context.available", num(available)))
