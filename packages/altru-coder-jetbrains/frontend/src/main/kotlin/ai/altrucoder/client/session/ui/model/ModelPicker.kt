...
class ModelPicker : UiStyle.Pickers.Label() {
    ...
    init {
        isEnabled = false
        text = " "
        toolTipText = AltruCoderBundle.message("model.picker.tooltip")
    ...
    private fun showPopup() {
        ...
        val list = JBList(model).apply {
            ...
            isFocusable = false
            emptyText.text = AltruCoderBundle.message("model.picker.no.matches")
        ...
        val search = SearchTextField(false).apply {
            textEditor.emptyText.text = AltruCoderBundle.message("model.picker.search")
        }
...
internal object ModelText {
    private val small = setOf("altru-coder-auto/small", "auto-small")
    ...
    fun small(item: ModelPicker.Item): Boolean = item.provider == "altru-coder" && item.id in small

    fun providerSort(id: String): Int = if (id == "altru-coder") 0 else 1

    fun freeBg(): JBColor = JBColor.namedColor("Altru Coder.ModelPicker.freeBadgeBackground", JBColor(0x95D6AC, 0x7FCA99))
}
