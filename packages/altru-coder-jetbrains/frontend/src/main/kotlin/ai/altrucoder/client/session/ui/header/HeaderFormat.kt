...
import ai.altrucoder.client.session.model.ContextUsage
...
internal fun contextTip(value: ContextUsage?): String? {
    val ctx = value ?: return null
    val pct = ctx.percentage
    if (pct != null) return AltruCoderBundle.message("session.header.context.tooltip.percent", num(ctx.tokens), pct)
    if (ctx.tokens > 0) return AltruCoderBundle.message("session.header.context.tooltip.tokens", num(ctx.tokens))
    return null
}

internal fun todo(done: Int, total: Int): String? {
    if (total <= 0) return null
    if (done >= total) return AltruCoderBundle.message("session.header.todos.done", total)
    return AltruCoderBundle.message("session.header.todos.progress", done, total)
}
