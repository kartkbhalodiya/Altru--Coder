package ai.altrucoder.client

import ai.altrucoder.client.actions.NewSessionAction
import ai.altrucoder.client.app.AltruCoderWorkspaceService
import ai.altrucoder.client.app.Workspace
import ai.altrucoder.client.session.SessionSidePanelManager
...
/**
 * Creates the Altru Coder tool window and delegates session content management.
 *
 ...
 * completes.
 */
class AltruCoderToolWindowFactory : ToolWindowFactory, DumbAware {

    companion object {
        private val LOG = AltruCoderLog.create(AltruCoderToolWindowFactory::class.java)
    }

    override fun createToolWindowContent(project: Project, toolWindow: ToolWindow) {
        try {
            val workspaces = service<AltruCoderWorkspaceService>()
            ...
            LOG.error("Failed to create Altru Coder tool window content", e)
        }
    }
    ...
    private fun setup(
        ...
        try {
            ...
            ActionManager.getInstance().getAction("Altru Coder.Settings")?.let { settings ->
                toolWindow.setTitleActions(listOf(NewSessionAction(), settings))
            }
        } catch (e: Exception) {
            LOG.error("Failed to set up Altru Coder tool window content", e)
        }
    }
}
