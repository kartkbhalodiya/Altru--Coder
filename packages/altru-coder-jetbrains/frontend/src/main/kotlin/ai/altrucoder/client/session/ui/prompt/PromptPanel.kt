...
import ai.altrucoder.client.actions.SendPromptAction
import ai.altrucoder.client.actions.StopSessionAction
...
class PromptPanel(
    ...
    companion object {
        private val LOG = AltruCoderLog.create(PromptPanel::class.java)
    ...
    private val reset = UiStyle.Buttons.HoverIcon().apply {
        ...
        toolTipText = AltruCoderBundle.message("model.picker.reset")
        accessibleContext.accessibleName = AltruCoderBundle.message("model.picker.reset")
        isVisible = false
    ...
    private fun tooltip(): String {
        val id = if (busy) StopSessionAction.ID else SendPromptAction.ID
        val text = if (busy) {
            AltruCoderBundle.message("prompt.button.stop")
        } else {
            AltruCoderBundle.message("prompt.button.send")
        }
        ...
        return XmlStringUtil.wrapInHtml(
            XmlStringUtil.escapeString(tip) + "<br>" +
                XmlStringUtil.escapeString(AltruCoderBundle.message("prompt.button.send.tooltip.stop", stop))
        )
    }
    ...
    private fun placeholder(): String {
        ...
        if (send.isNotEmpty() && line.isNotEmpty()) {
            return AltruCoderBundle.message("prompt.placeholder.with.shortcuts", send, line)
        }
        if (send.isNotEmpty()) return AltruCoderBundle.message("prompt.placeholder.with.send", send)
        if (line.isNotEmpty()) return AltruCoderBundle.message("prompt.placeholder.with.newline", line)
        return AltruCoderBundle.message("prompt.placeholder")
    }
