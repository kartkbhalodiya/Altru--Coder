...
private fun title(tool: Tool) = when (tool.name) {
    "read" -> AltruCoderBundle.message("session.part.tool.read")
    "bash" -> AltruCoderBundle.message("session.part.tool.shell")
    else -> toolTitle(tool)
}
...
private fun stateText(tool: Tool) = when (tool.state) {
    ToolExecState.PENDING -> AltruCoderBundle.message("session.part.tool.pending")
    ToolExecState.RUNNING -> AltruCoderBundle.message("session.part.tool.running")
    ToolExecState.COMPLETED -> ""
    ToolExecState.ERROR -> AltruCoderBundle.message("session.part.tool.error")
}
...
private class Preview {
    ...
    fun build(): String {
        ...
        text.append(AltruCoderBundle.message("session.part.tool.truncated"))
