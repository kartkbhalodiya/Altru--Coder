...
import ai.altrucoder.client.session.ui.ConnectionPanel
import ai.altrucoder.client.session.ui.EmptySessionPanel
import ai.altrucoder.client.session.ui.ReasoningPicker
import ai.altrucoder.client.session.ui.mode.ModePicker
import ai.altrucoder.client.session.ui.model.ModelPicker
import ai.altrucoder.client.session.ui.PermissionPanel
import ai.altrucoder.client.session.ui.prompt.PromptPanel
import ai.altrucoder.client.session.ui.QuestionPanel
import ai.altrucoder.client.session.ui.SessionRootPanel
import ai.altrucoder.client.session.ui.SessionMessageListPanel
import ai.altrucoder.client.session.ui.header.SessionHeaderPanel
import ai.altrucoder.client.session.ui.SessionStyle
import ai.altrucoder.client.session.ui.SessionStyleTarget
import ai.altrucoder.client.session.update.EVENT_FLUSH_MS
import ai.altrucoder.client.session.update.SessionController
import ai.altrucoder.client.session.update.SessionControllerEvent
...
class SessionUi private constructor(
    project: Project,
    workspace: Workspace,
    sessions: AltruCoderSessionService,
    app: AltruCoderAppService,
    cs: CoroutineScope,
    id: String?,
    displayMs: Long,
    ...
    companion object {
        private val LOG = AltruCoderLog.create(SessionUi::class.java)
    }
    ...
    private val flushMs =
        Registry.intValue("altru-coder.session.flushMs", EVENT_FLUSH_MS.toInt())
            .takeIf { it > 0 }
            ?.toLong()
            ?: EVENT_FLUSH_MS
    ...
    private val controller = SessionController(
        ...
        flushMs = flushMs,
        condense = Registry.`is`("altru-coder.session.condense", true),
        displayMs = displayMs,
        session = session,
        open = open,
    ...
    private fun buildUi() {
        ...
        progressBody = JPanel(BorderLayout()).apply {
            isOpaque = false
            loadingLabel = JBLabel(AltruCoderBundle.message("session.empty.loading"))
            add(Centerizer(
                loadingLabel,
