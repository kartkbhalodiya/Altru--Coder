package ai.altrucoder.client.session.ui.header

import ai.altrucoder.client.plugin.AltruCoderBundle
import ai.altrucoder.client.session.model.SessionHeaderSnapshot
...
import ai.altrucoder.rpc.dto.TokensDto
...
class SessionHeaderPanel(
    ...
    companion object {
        ...
        internal const val EXPANDED_KEY = "altru-coder.session.header.expanded"
    }
    ...
    private val compact = UiStyle.Buttons.HoverIcon().apply {
        icon = COMPRESS_ICON
        toolTipText = AltruCoderBundle.message("session.header.compact.description")
        accessibleContext.accessibleName = AltruCoderBundle.message("session.header.compact")
        addActionListener { controller.compact() }
    }
    private val expand = UiStyle.Buttons.HoverIcon().apply {
        icon = CHEVRON_ICON
        toolTipText = AltruCoderBundle.message("session.header.expand")
        accessibleContext.accessibleName = AltruCoderBundle.message("session.header.expand")
    ...
    private val tokenTitle = JBLabel(AltruCoderBundle.message("session.header.tokens"))
    ...
    private fun setTokens(value: TokensDto?) {
        ...
        tokenTitle.text = AltruCoderBundle.message("session.header.tokens")
        tokens.toolTipText = AltruCoderBundle.message("session.header.tokens.description")
        tokenTitle.isVisible = total > 0
        set(input, if (sent > 0) num(sent) else null)
        set(output, if (received > 0) num(received) else null)
        set(cacheWrite, if (write > 0) AltruCoderBundle.message("session.header.cache.write", num(write)) else null)
        set(cacheRead, if (read > 0) AltruCoderBundle.message("session.header.cache.read", num(read)) else null)
    ...
    private fun setExpand(expanded: Boolean) {
        ...
        expand.toolTipText = AltruCoderBundle.message(key)
        expand.accessibleContext.accessibleName = AltruCoderBundle.message(key)
    }
