...
import ai.altrucoder.client.session.model.AgentItem
import ai.altrucoder.client.session.model.ModelLimitItem
import ai.altrucoder.client.session.model.ModelItem
...
import ai.altrucoder.client.session.model.PermissionRequestState
...
import ai.altrucoder.client.session.model.ToolCallRef
...
class SessionController(
  parent: Disposable,
  id: String?,
  private val sessions: AltruCoderSessionService,
  private val workspace: Workspace,
  private val app: AltruCoderAppService,
  ...
  private val loaded: (Boolean) -> Unit = {},
    ...
    companion object {
        private val LOG = AltruCoderLog.create(SessionController::class.java)
    ...
    fun prompt(text: String) {
        ...
        cs.launch {
            try {
                ...
                edt {
                    val msg = e.message ?: AltruCoderBundle.message("session.error.prompt")
                    updateModel {
    ...
    fun compact() {
        ...
        cs.launch {
            try {
                ...
                edt {
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: AltruCoderBundle.message("session.error.compact")))
                    }
                }
            }
        }
    ...
    fun retryConnection() {
        ...
        if (model.app.status != AltruCoderAppStatusDto.READY || model.app.status == AltruCoderAppStatusDto.ERROR) {
            app.retryAsync()
            return
        }
        ...
        if (model.workspace.status == AltruCoderWorkspaceStatusDto.ERROR) {
            workspace.reload()
        }
    }
    ...
    init {
        ...
        cs.launch {
            app.state.collect { state ->
                if (state.status == AltruCoderAppStatusDto.READY) app.fetchVersionAsync()
        ...
        cs.launch {
            workspace.state.collect { state ->
                fire(SessionControllerEvent.WorkspaceChanged) {
                    ...
                    if (state.status != AltruCoderWorkspaceStatusDto.READY) return@fire
                    ...
                    model.models = state.providers?.let { providers ->
                        providers.providers
                            .filter { it.id == ALTRU_CODER_PROVIDER || it.id in providers.connected }
                            .flatMap { provider ->
                ...
                if (state.status == AltruCoderWorkspaceStatusDto.READY) {
                    fire(SessionControllerEvent.WorkspaceReady)
                    edt {
    ...
    private fun seedStatus(dto: SessionStatusDto) {
        ...
        val state = when (dto.type) {
            "busy" -> SessionState.Busy(AltruCoderBundle.message("session.status.considering"))
    ...
    private fun handle(event: ChatEventDto) {
        ...
        when (event) {
            ...
            is ChatEventDto.TurnOpen -> {
                partType = null
                tool = null
                model.setState(SessionState.Busy(AltruCoderBundle.message("session.status.considering")))
            }
            ...
            is ChatEventDto.Error -> {
                partType = null
                tool = null
                val msg = event.error?.message ?: event.error?.type ?: AltruCoderBundle.message("session.error.unknown")
                ...
                val state = when (event.status.type) {
                    ...
                    "busy" -> {
                        ...
                        if (current is SessionState.Idle || current is SessionState.Error)
                            SessionState.Busy(AltruCoderBundle.message("session.status.considering"))
    ...
    private fun configModel(agent: String): String? {
        if (model.app.status != AltruCoderAppStatusDto.READY) return null
        val cfg = model.app.config
    ...
    private fun status(): String = when (partType) {
        "reasoning" -> AltruCoderBundle.message("session.status.thinking")
        "text" -> AltruCoderBundle.message("session.status.writing")
        "tool" -> when (tool) {
            "task" -> AltruCoderBundle.message("session.status.delegating")
            "todowrite", "todoread" -> AltruCoderBundle.message("session.status.planning")
            "read" -> AltruCoderBundle.message("session.status.gathering")
            "glob", "grep", "list" -> AltruCoderBundle.message("session.status.searching.codebase")
            "webfetch", "websearch", "codesearch" -> AltruCoderBundle.message("session.status.searching.web")
            "edit", "write" -> AltruCoderBundle.message("session.status.editing")
            "bash" -> AltruCoderBundle.message("session.status.commands")
            else -> AltruCoderBundle.message("session.status.considering")
        }
        else -> AltruCoderBundle.message("session.status.considering")
    }
    ...
    private fun resolveConnectionState(): SessionControllerEvent.ConnectionChanged {
        ...
        if (app.status == AltruCoderAppStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                AltruCoderBundle.message("session.connection.error.app"),
                app.errors.toErrorText() ?: app.error,
            )
        }

        if (workspace.status == AltruCoderWorkspaceStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                AltruCoderBundle.message("session.connection.error.workspace"),
                workspace.errors.toErrorText() ?: workspace.error,
                "workspace",
            )
        }

        if (app.status == AltruCoderAppStatusDto.READY && workspace.status == AltruCoderWorkspaceStatusDto.READY && app.warnings.isNotEmpty()) {
        ...
        if (app.status == AltruCoderAppStatusDto.READY && workspace.status == AltruCoderWorkspaceStatusDto.READY) {
...
private fun summary(count: Int): String {
    val base = AltruCoderBundle.message("session.connection.warning.config")
...
private const val ALTRU_CODER_PROVIDER = "altru-coder"
private const val ALTRU_CODER_AUTO_MODEL = "altru-coder-auto/free"
...
private fun resolveModelSelection(
    ...
    fallback: ModelSelectionDto? = ModelSelectionDto(ALTRU_CODER_PROVIDER, ALTRU_CODER_AUTO_MODEL),
): ModelSelectionDto? {
...
private fun valid(providers: ProvidersDto?, item: ModelSelectionDto?): ModelSelectionDto? {
    ...
    if (item.providerID != ALTRU_CODER_PROVIDER && item.providerID !in providers.connected) return null
