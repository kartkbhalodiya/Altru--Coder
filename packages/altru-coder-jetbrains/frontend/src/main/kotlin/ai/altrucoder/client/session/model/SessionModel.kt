package ai.altrucoder.client.session.model

import ai.altrucoder.rpc.dto.DiffFileDto
 ...
 *
 * **EDT-only access** — no synchronization. [ai.altrucoder.client.session.update.SessionController] guarantees all
...
class SessionModel {
    ...
    var app: AltruCoderAppStateDto = AltruCoderAppStateDto(AltruCoderAppStatusDto.DISCONNECTED)
    var version: String? = null

    var workspace: AltruCoderWorkspaceStateDto = AltruCoderWorkspaceStateDto(AltruCoderWorkspaceStatusDto.PENDING)
    ...
    fun isReady(): Boolean = app.status == AltruCoderAppStatusDto.READY && workspace.status == AltruCoderWorkspaceStatusDto.READY
...
private fun ai.altrucoder.rpc.dto.PartTimeDto.durationMs(): Long? {
