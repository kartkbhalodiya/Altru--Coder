...
class SessionHeaderControllerTest : SessionControllerTestBase() {
    ...
    fun `test session updated refreshes model metadata`() {
        ...
        assertTrue(modelEvents.any { it is ai.altrucoder.client.session.model.SessionModelEvent.SessionUpdated })
    }
    ...
    fun `test compact calls RPC with selected model when eligible`() {
        ...
        assertEquals("altru-coder", call.third.providerID)
    ...
    fun `test compact is blocked without model`() {
        appRpc.state.value = ai.altrucoder.rpc.dto.AltruCoderAppStateDto(ai.altrucoder.rpc.dto.AltruCoderAppStatusDto.READY, config = ai.altrucoder.rpc.dto.ConfigDto(model = null))
    ...
    fun `test opened session metadata is seeded through constructor`() {
        rpc.history.add(ai.altrucoder.rpc.dto.MessageWithPartsDto(msg("msg1", "ses_test", "assistant"), emptyList()))
        val c = controller(
            id = "ses_test",
