...
import ai.altrucoder.client.session.model.Message
...
class TurnViewTest : BasePlatformTestCase() {
    ...
    fun `test upsertPart adds a new TextView for Text content`() {
        val mv = MessageView(msg("a1", "assistant"))
        val text = ai.altrucoder.client.session.model.Text("p1")
    ...
    fun `test upsertPart updates existing part rather than adding duplicate`() {
        val mv = MessageView(msg("a1", "assistant"))
        val t1 = ai.altrucoder.client.session.model.Text("p1").also { it.content.append("v1") }
        mv.upsertPart(t1)

        val t2 = ai.altrucoder.client.session.model.Text("p1").also { it.content.append("v2") }
        mv.upsertPart(t2)
    ...
    fun `test removePart removes the renderer`() {
        val mv = MessageView(msg("a1", "assistant"))
        mv.upsertPart(ai.altrucoder.client.session.model.Text("p1").also { it.content.append("x") })
        mv.removePart("p1")
    ...
    fun `test appendDelta reaches TextView`() {
        val mv = MessageView(msg("a1", "assistant"))
        mv.upsertPart(ai.altrucoder.client.session.model.Text("p1").also { it.content.append("hello ") })
    ...
    fun `test MessageView pre-populates parts from Message on creation`() {
        ...
        val text = ai.altrucoder.client.session.model.Text("p1").also { it.content.append("preloaded") }
