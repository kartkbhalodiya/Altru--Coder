package ai.altrucoder.client.session.ui.model
...
class ModelPickerTest : BasePlatformTestCase() {
    ...
    fun `test section order starts with favorites and recommended`() {
        val rows = modelPickerRows(listOf(
            ...
            item("auto", "Altru Coder Auto", "altru-coder", "Altru Coder", 0.0),
        ...
        assertEquals("altru-coder/auto", rows[1].item.key)
    ...
    fun `test altru-coder provider group is first and other providers keep source order`() {
        val rows = modelPickerRows(listOf(
            item("gpt", "GPT", "openai", "OpenAI"),
            item("auto", "Auto", "altru-coder", "Altru Coder"),
            item("claude", "Claude", "anthropic", "Anthropic"),
        ), emptyList(), "")

        assertEquals(listOf("Altru Coder", "OpenAI", "Anthropic"), rows.indices.mapNotNull { modelPickerSectionTitle(rows, it) })
    }
    ...
    fun `test display parts sanitize free suffix`() {
        val parts = ModelText.parts(item("auto", "Auto Free (free)", "altru-coder", "Altru Coder"))
    ...
    fun `test renderer shows empty favorite star only for selected row`() {
        val row = ModelPickerRow(item("auto", "Auto", "altru-coder", "Altru Coder"), "Altru Coder", false)
    ...
    fun `test renderer keeps favorite star visible`() {
        val row = ModelPickerRow(item("auto", "Auto", "altru-coder", "Altru Coder"), "Altru Coder", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { setOf("altru-coder/auto") })
        val list = JBList(model)
    ...
    fun `test renderer shows free badge for free model`() {
        val row = ModelPickerRow(ModelPicker.Item("auto", "Auto", "altru-coder", "Altru Coder", free = true), "Altru Coder", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { emptySet() })
        val list = JBList(model)
