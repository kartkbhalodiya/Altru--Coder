 ...
 * reconnect. JetBrains now does the same in [ai.altrucoder.client.session.update.SessionController.recoverPending].
 */
class SessionRecoveryTest : SessionControllerTestBase() {
    ...
    // ------ Status seeding from AltruCoderSessionService.statuses ------
    ...
    fun `test busy status is seeded from statuses map`() {
        ...
        assertSession(
            """
            [code] [altru-coder/gpt-5] [busy] [considering next steps]
            """,
            m, show = false,
        )
    }
    ...
    fun `test retry status is seeded with message attempt and next`() {
        ...
        assertSession(
            """
            [code] [altru-coder/gpt-5] [retry] [Rate limited]
            """,
            m, show = false,
        )
    ...
    fun `test offline status is seeded with message and requestId`() {
        ...
        assertSession(
            """
            [code] [altru-coder/gpt-5] [offline] [No network]
            """,
            m, show = false,
        )
    ...
    fun `test pending permission overrides a seeded busy status`() {
        ...
        assertSession(
            ...
            [code] [altru-coder/gpt-5] [awaiting-permission]
            """,
            m, show = false,
        )
    }
    ...
    fun `test pending question overrides a seeded retry status`() {
        ...
        assertSession(
            ...
            [code] [altru-coder/gpt-5] [awaiting-question]
            """,
            m, show = false,
        )
    }
