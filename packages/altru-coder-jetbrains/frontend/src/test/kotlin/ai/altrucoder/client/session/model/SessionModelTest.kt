...
class SessionModelTest : UsefulTestCase() {
    ...
    fun `test initial app and workspace state`() {
        assertEquals(AltruCoderAppStatusDto.DISCONNECTED, model.app.status)
        assertEquals(AltruCoderWorkspaceStatusDto.PENDING, model.workspace.status)
        assertFalse(model.isReady())
        assertEquals(SessionState.Idle, model.state)
    }

    fun `test isReady requires app and workspace readiness`() {
        model.app = AltruCoderAppStateDto(AltruCoderAppStatusDto.READY)
        assertFalse(model.isReady())

        model.workspace = AltruCoderWorkspaceStateDto(AltruCoderWorkspaceStatusDto.READY)
