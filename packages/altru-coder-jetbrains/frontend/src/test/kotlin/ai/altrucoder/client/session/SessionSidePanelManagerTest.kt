...
class SessionSidePanelManagerTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        workspaces = AltruCoderWorkspaceService(scope, FakeWorkspaceRpcApi().also {
    ...
    fun `test inactive sessions keep queued style updates`() {
        ...
        val style = ai.altrucoder.client.session.ui.SessionStyle.create(family = "Courier New", size = 24)
    ...
    private fun JPanel.controller(): ai.altrucoder.client.session.update.SessionController {
        val field = SessionUi::class.java.getDeclaredField("controller")
        field.isAccessible = true
        return field.get(this) as ai.altrucoder.client.session.update.SessionController
    }
