...
import ai.altrucoder.client.session.model.ToolExecState
import ai.altrucoder.client.session.model.ToolKind
import ai.altrucoder.client.session.ui.SessionStyle
import ai.altrucoder.client.session.update.SessionControllerTestBase
...
import ai.altrucoder.rpc.dto.PartTimeDto
import ai.altrucoder.rpc.dto.ProviderDto
import ai.altrucoder.rpc.dto.TodoDto
...
class SessionHeaderPanelTest : SessionControllerTestBase() {
    ...
    fun `test starts hidden for empty header`() {
        appRpc.state.value = ai.altrucoder.rpc.dto.AltruCoderAppStateDto(ai.altrucoder.rpc.dto.AltruCoderAppStatusDto.READY)
    ...
    private fun promptedHeader(): ai.altrucoder.client.session.update.SessionController {
        appRpc.state.value = ai.altrucoder.rpc.dto.AltruCoderAppStateDto(
            ai.altrucoder.rpc.dto.AltruCoderAppStatusDto.READY,
            config = ai.altrucoder.rpc.dto.ConfigDto(model = "altru-coder/gpt-5"),
        )
        projectRpc.state.value = workspaceReady(
            providers = listOf(
                ProviderDto(
                    ...
                    models = mapOf(
                        "gpt-5" to ModelDto(
                            id = "gpt-5",
                            name = "GPT-5",
                            limit = ai.altrucoder.rpc.dto.ModelLimitDto(context = 2_000_000, output = 200_000),
                        ),
                    ),
                ),
            ),
