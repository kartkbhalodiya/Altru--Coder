...
import ai.altrucoder.client.session.views.TextView
...
class SessionUiUpdateTest : BasePlatformTestCase() {
    ...
    fun `test tool state transitions are reflected in ToolView`() {
        ...
        val tv = panel.findMessage("a1")!!.part("t1") as ai.altrucoder.client.session.views.ToolView
    ...
    fun `test compaction part appears in panel`() {
        ...
        assertTrue(mv.part("cp1") is ai.altrucoder.client.session.views.CompactionView)
    }
    ...
    fun `test unknown part type falls back to GenericView`() {
        ...
        val gv = mv.part("g1")
        assertNotNull(gv)
        assertTrue(gv is ai.altrucoder.client.session.views.GenericView)
        assertTrue((gv as ai.altrucoder.client.session.views.GenericView).labelText().contains("snapshot"))
    }
