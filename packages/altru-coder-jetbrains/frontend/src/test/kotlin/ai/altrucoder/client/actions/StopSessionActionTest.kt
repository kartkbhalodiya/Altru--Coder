...
import ai.altrucoder.client.session.ui.prompt.SendPromptContext
...
class StopSessionActionTest : BasePlatformTestCase() {
    fun `test action invokes prompt context`() {
        ...
        assertEquals("Stop the current Altru Coder session", action.templatePresentation.description)
    }
