...
description: Use when creating, modifying, or reviewing Kotlin/Swing UI code for the Altru Coder JetBrains plugin. Applies to dialogs, settings pages, tool windows, forms, panels, component layout, sizing, spacing, colors, borders, icons, lists, trees, popups, notifications, and IntelliJ platform UI components.
---

# JetBrains UI Style

Use this skill whenever creating, modifying, or reviewing UI code in `packages/altru-coder-jetbrains`.

## Scope

This skill covers Kotlin/Swing UI code for the Altru Coder JetBrains plugin, including dialogs, settings pages, tool windows, forms, panels, component layout, sizing, spacing, colors, borders, icons, lists, trees, popups, notifications, and IntelliJ Platform UI components.
...
```kotlin
panel {
    row(AltruCoderBundle.message("settings.name")) {
        textField()
            .align(AlignX.FILL)
            .resizableColumn()
    }
...
```kotlin
private val gap = JBValue.UIInteger("AltruCoderComponent.gap", 16)
...
```kotlin
private val gap = JBValue.UIInteger("AltruCoderComponent.gap", 16).get()
```
...
```kotlin
panel {
    group(AltruCoderBundle.message("settings.section")) {
        row(AltruCoderBundle.message("settings.name")) {
            textField()
                .align(AlignX.FILL)
                .resizableColumn()
        ...
        indent {
            row { checkBox(AltruCoderBundle.message("settings.enabled")) }
        }
    }
}
```
...
## Altru Coder Session Styling

For session/chat UI under `packages/altru-coder-jetbrains/frontend/src/main/kotlin/ai/altrucoder/client/session/`, keep general UI chrome separate from transcript styling.

- Use `ai.altrucoder.client.ui.UiStyle` for general UI helpers: `Colors`, `Borders`, `Insets`, `Gap`, `Space`, `Size`, and `Buttons`.
- Use `ai.altrucoder.client.session.ui.SessionStyle` for session transcript/content styling that follows editor settings or live transcript configuration.
...
```kotlin
class AltruCoderPanel : JPanel() {
    override fun updateUI() {
        super.updateUI()
        border = JBUI.Borders.customLine(
            JBColor.namedColor("AltruCoderPanel.borderColor", JBColor.border()),
            1,
        )
    }
}
...
Only define plugin-owned theme keys when a platform key is not enough. Themeable insets can use `JBUI.insets("AltruCoderPanel.insets", JBUI.insets(4, 8))`; themeable colors should use `JBColor.namedColor("AltruCoderPanel.borderColor", fallback)`.
...
{
  "ui": {
    "AltruCoderPanel.insets": "6,12,6,12",
    "AltruCoderPanel.gap": 20,
    "AltruCoderPanel.borderColor": "#FF0000",
    "AltruCoderPanel.arc": 10
  }
}
```
...
```xml
<themeMetadataProvider path="/META-INF/Altru Coder.themeMetadata.json"/>
```
...
```json
{
  ...
  "fixed": false,
  "ui": [
    { "key": "AltruCoderPanel.borderColor", "description": "Panel border color" },
    { "key": "AltruCoderPanel.gap", "description": "Gap between panel items" }
  ]
}
```
...
textField()
    .validationOnInput {
        if (it.text.isBlank()) error(AltruCoderBundle.message("settings.name.required")) else null
    }
```
...
override fun doValidate(): ValidationInfo? {
    if (field.text.isBlank()) return ValidationInfo(AltruCoderBundle.message("settings.name.required"), field)
    return null
}
```
...
- Show with `Notification("Altru Coder", "message", NotificationType.INFORMATION).notify(project)`.
...
- Tool-window-bound notifications use `displayType="TOOL_WINDOW" toolWindowId="Altru Coder"`.
