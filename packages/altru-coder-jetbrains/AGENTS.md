# AGENTS.md — Altru Coder JetBrains Plugin

## Skills

- Use `packages/altru-coder-jetbrains/.altru-coder/skills/jetbrains-ui-style/SKILL.md` when creating, modifying, or reviewing Kotlin/Swing UI code for this plugin. It covers IntelliJ UI conventions, platform components, theme-aware colors/fonts, spacing, manual Swing vs Kotlin UI DSL, session transcript styling, and retained Swing component-state patterns for hover/expand/update flows.

## Architecture (Split Mode)

- **Split-mode plugin** with three Gradle modules: `shared/`, `frontend/`, `backend/`. The module descriptors are `altru-coder.jetbrains.shared.xml`, `altru-coder.jetbrains.frontend.xml`, `altru-coder.jetbrains.backend.xml` — these must stay in sync with `plugin.xml`'s `<content>` block.
...
- Kotlin source goes under `{module}/src/main/kotlin/ai/altrucoder/jetbrains/`. Package name is `ai.altrucoder.jetbrains` (matches `group` in root `build.gradle.kts`).
...
- Non-light services that need XML registration go in `altru-coder.jetbrains.backend.xml` under `<extensions defaultExtensionNs="com.intellij"><applicationService>` (or `<projectService>`).
...
- The plugin spawns `altru-coder serve --port 0` (OS assigns random port) and reads stdout for `listening on http://...:(\d+)` to discover the port.
- A random 32-byte hex password is passed via `ALTRU_CODER_SERVER_PASSWORD` env var for Basic Auth.
- Key env vars: `ALTRU_CODER_CLIENT=jetbrains`, `ALTRU_CODER_PLATFORM=jetbrains`, `ALTRU_CODER_APP_NAME=altru-coder`, `ALTRU_CODER_ENABLE_QUESTION_TOOL=true`.
- This is the same protocol used by the VS Code extension (`packages/altru-coder-vscode/src/services/cli-backend/server-manager.ts`).

## Build

- **Full build**: `bun run build` from `packages/altru-coder-jetbrains/` (builds CLI + Gradle plugin).
- **Gradle only**: `./gradlew buildPlugin` from `packages/altru-coder-jetbrains/` (requires CLI binaries already present).
- **Via Turbo**: `bun turbo build --filter=@altru-coder/altru-coder-jetbrains` from repo root.
- **Run in sandbox**: `./gradlew runIde` — launches sandboxed IntelliJ with the plugin. Does NOT build CLI binaries.

## Files That Must Change Together

- `plugin.xml` `<content>` entries ↔ module XML descriptors (`altru-coder.jetbrains.{shared,frontend,backend}.xml`)
...
`frontend/src/main/kotlin/ai/altrucoder/client/session/`.
...
- After history load, `recoverPending()` seeds state in this priority order: (1) pending permission, (2) pending question, (3) current session status (`busy`/`retry`/`offline` from `AltruCoderSessionService.statuses`), (4) `Idle`.
...
**Do not use Kotlin Compose or `intellij.platform.compose` in this plugin.** The JetBrains modular template uses Compose for its demo tool window, but Altru Coder should use standard Swing with IntelliJ Platform components only. Keep all plugin UI in the existing Swing-based stack.
...
- Register declaratively in module XML via `com.intellij.toolWindow` extension point (already done in `altru-coder.jetbrains.frontend.xml`).
...
- Declare in module XML: `<notificationGroup id="Altru Coder" displayType="BALLOON"/>`.
- Show: `Notification("Altru Coder", "message", NotificationType.INFORMATION).notify(project)`.
...
- Tool-window-bound: `displayType="TOOL_WINDOW" toolWindowId="Altru Coder"`.
