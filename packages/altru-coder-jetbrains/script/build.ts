...
/**
 * Build the Altru Coder JetBrains plugin.
 *
 * Usage:
 ...
 * 2. Copies them into backend/build/generated/cli/cli/{os}/altru[.exe]
 *    so they end up inside the backend jar at /cli/{os}/altru.
...
const platforms = [
  { os: "darwin-arm64", exe: "altru-coder" },
  { os: "darwin-x64", exe: "altru-coder" },
  { os: "linux-arm64", exe: "altru-coder" },
  { os: "linux-x64", exe: "altru-coder" },
  { os: "windows-x64", exe: "altru-coder.exe" },
  { os: "windows-arm64", exe: "altru-coder.exe" },
] as const
...
function distBinPath(os: string, exe: string): string {
  return join(distDir, `@altru-coder/cli-${os}`, "bin", exe)
}
