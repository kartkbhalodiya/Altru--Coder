    ...
    fun run() {
        ...
        if (!resolved.exists() || resolved.listFiles()?.isEmpty() != false) {
            throw GradleException(
                ...
                "Run 'bun run build' from packages/altru-coder-jetbrains/ to build CLI and plugin together."
            )
        }
        if (production.get()) {
            val missing = platforms.get().filter { platform ->
                val d = File(resolved, platform)
                val exe = if (platform.startsWith("windows")) "altru-coder.exe" else "altru-coder"
                !File(d, exe).exists()
            }
