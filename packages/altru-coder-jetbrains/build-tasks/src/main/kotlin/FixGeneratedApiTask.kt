...
abstract class FixGeneratedApiTask : DefaultTask() {
    ...
    private fun fixEmptyWrappers(root: File) {
        val models = File(root, "ai/altrucoder/jetbrains/api/model")
    ...
    private fun fixJsonElementQueryParams(root: File) {
        val api = File(root, "ai/altrucoder/jetbrains/api/client/DefaultApi.kt")
        if (!api.isFile) return

        val models = File(root, "ai/altrucoder/jetbrains/api/model")
        val missing = mutableSetOf<String>()
        val text = Regex("""import ai\.altrucoder\.jetbrains\.api\.model\.(\w+Parameter)\n""")
