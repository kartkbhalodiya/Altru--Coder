# Feature Parity Plan — Altru Coder VS Code Extension (Rebuild)

## Overview

This extension is a **ground-up rebuild** of the [old Altru Coder extension](https://github.com/Altru-Coder/altrucoder-legacy) using Altru Coder CLI as the backend. Rather than migrating the old extension's codebase, we started fresh with a Solid.js webview, a CLI server manager, and a message-based protocol between extension host and webview. This new extension lives in the [altrucoder monorepo](https://github.com/Altru-Coder/altrucoder/tree/main/packages/altru-coder-vscode).

This document tracks remaining work needed for feature parity with the old extension. Each feature links to its detailed parity requirement doc. Features sourced from the [GitHub project board](https://github.com/orgs/Altru-Coder/projects/25/views/1) include issue links.
...
| [Cloud Task Support](non-agent-features/cloud-task-support.md) | Upload local sessions to cloud, real-time sync, conflict resolution | Altru Coder cloud API + CLI; extension provides UI | P2 |
...
Open issues from the [GitHub project board](https://github.com/orgs/Altru-Coder/projects/25/views/1) not covered by the feature docs above. Each item has its own detailed doc.
...
| [Dedicated Output Channel](infrastructure/dedicated-output-channel.md) | General "Altru Coder" output channel and centralized logging utility | P2 |
...
- [ ] **Review and tighten CSP** — The current policy in [`AltruCoderProvider._getHtmlForWebview()`](../src/AltruCoderProvider.ts:829) has several areas to audit:
  - `style-src 'unsafe-inline'` is broadly permissive — investigate whether nonce-based style loading is feasible now that altru-coder-ui styles are bundled
...
- [ ] **Validate `openExternal` URLs** — The [`openExternal` handler](../src/AltruCoderProvider.ts:186) passes any URL from the webview directly to `vscode.env.openExternal()` with no allowlist or scheme check. Restrict to `https:` (and possibly `vscode:`) schemes, or allowlist specific hosts
...
- [ ] **Bundle size audit** — With altru-coder-ui and its transitive dependencies (shiki, marked, katex, dompurify, etc.) now bundled, measure `dist/webview.js` size and verify the total `.vsix` package size is acceptable
...
- [ ] **Dedicated output channel** — All logging currently goes to `console.log` mixed with other extensions ([details](infrastructure/dedicated-output-channel.md)). Create a dedicated "Altru Coder" output channel before production
- [ ] **Remove or guard verbose logging** — Many `console.log` calls with emojis and debug detail exist in [`AltruCoderProvider.ts`](../src/AltruCoderProvider.ts). Gate behind a debug flag or move to the output channel at appropriate log levels
...
### altru-coder-ui Shared Library

- **altru-coder-ui shared library**: The webview now heavily uses `@altru-coder/altru-coder-ui` for UI components. A `DataBridge` component in App.tsx adapts the session store to altru-coder-ui's `DataProvider` expected shape, enabling shared components like `<AltruCoderMessage>` to work with the extension's data model.
...
- Permissions flow through CLI's ask/reply model, not extension-side approval queues. Permissions are rendered through altru-coder-ui's DataProvider pattern, not a standalone PermissionDialog.
