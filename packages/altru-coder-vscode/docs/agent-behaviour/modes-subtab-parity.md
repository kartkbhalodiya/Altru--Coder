...
| Custom Instructions (per-mode) | `<TextArea>` (10 rows) + link to `.altrucoder/rules-{slug}/rules.md` | Not available as separate field (merged into system prompt) |
...
| Override System Prompt | Advanced disclosure with link to `.altrucoder/system-prompt-{slug}` | Not available |
...
| Link to global rules file | Link to `.altrucoder/rules/rules.md` | Not available |

### Organization Features

| Feature | Legacy | New |
|---|---|---|
| Share modes banner | `AltruCoderShareModesBanner` component | Not available |
