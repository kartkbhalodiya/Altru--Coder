...
/**
 * Force all solid-js imports (from altru-coder-ui and the webview) to resolve to
...
const solidDedupePlugin = {
  name: "solid-dedupe",
  setup(build) {
    // Resolve these bare specifiers to the altru-coder-vscode-local copy
    ...
    build.onLoad({ filter: /sprite\.svg$/ }, (args) => {
      ...
      return {
        contents: `
          ...
          const inject = () => {
            if (!document.getElementById("altru-coder-sprite")) {
              const el = document.createElement("div");
              el.id = "altru-coder-sprite";
