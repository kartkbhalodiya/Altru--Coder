// AltruCoderClaw active-conversation view — message list + composer.
// Mirrors cloud/apps/web/src/app/(app)/claw/altru-chat/components/MessageArea.tsx
...
import { createAutoScroll } from "@altru-coder/altru-coder-ui/hooks"
...
export function MessageArea() {
  ...
  //
  // Uses @altru-coder/altru-coder-ui's createAutoScroll (the same one MessageList.tsx
  ...
  const sendDisabledReason = createMemo(() => {
    ...
    const st = botDisplay().state
    if (st === "unknown") return t("altruClaw.chat.waitingBotStatus")
    return t("altruClaw.chat.botOffline")
  })

  // Render the typing banner with friendly names. Bot members come in as
  // `bot:altru-coder-claw:{sandboxId}` — the bot is the only non-self member in
  // 1:1 chats, so we resolve bot ids to the user-configured `botName`
  // (falls back to the default "AltruCoderClaw" label) and render any human
  // collaborators by their raw memberId.
  const typingNames = createMemo(() => {
    const activeId = claw.activeConversationId()
    if (!activeId) return []
    const assistant = claw.assistantName() ?? t("altruClaw.message.bot")
  ...
  return (
    <Show
      ...
      fallback={
        <div class="altru-coder-claw-empty-area">
          <p>{t("altruClaw.conversations.selectOne")}</p>
        </div>
      }
    >
      <div class="altru-coder-claw-area">
        {/* Messages */}
        <div class="altru-coder-claw-area-messages-wrap">
          <div
            class="altru-coder-claw-area-messages"
            ref={(el) => {
              scrollEl = el
              auto.scrollRef(el)
            }}
          ...
          >
            <div ref={auto.contentRef}>
              <Show when={claw.messages().length === 0}>
                <div class="altru-coder-claw-empty">
                  {claw.assistantName()
                    ? t("altruClaw.chat.emptyWithBot").replace("{bot}", claw.assistantName()!)
                    : t("altruClaw.chat.empty")}
                </div>
              </Show>
          ...
          <Show when={showScrollButton()}>
            <button
              type="button"
              class="altru-coder-claw-scrollbtn"
        ...
        <Show when={typingNames().length > 0}>
          <div class="altru-coder-claw-typing">
            <Spinner />
            <span>
              {typingNames().length === 1
                ? t("altruClaw.typing.one").replace("{name}", typingNames()[0])
                : t("altruClaw.typing.many").replace("{count}", String(typingNames().length))}
            </span>
          </div>
        </Show>
        ...
        <Show when={replyingTo()}>
          {(r) => (
            <div class="altru-coder-claw-reply-preview">
              <span class="altru-coder-claw-reply-preview-label">{t("altruClaw.message.replyTo")}</span>
              <span class="altru-coder-claw-reply-preview-text">{replyText(r())}</span>
              <button
                type="button"
                class="altru-coder-claw-iconbtn-sm"
                onClick={() => setReplyingTo(null)}
                aria-label={t("altruClaw.message.cancelReply")}
              >
                ×
              </button>
            </div>
        ...
        <div class="altru-coder-claw-input-wrap">
          <textarea
            ref={input}
            class="altru-coder-claw-input"
            placeholder={sendDisabledReason() ?? t("altruClaw.chat.placeholder")}
            disabled={!canSend()}
            value={text()}
            onInput={onInput}
            onKeyDown={onKeyDown}
            rows={1}
            aria-label={t("altruClaw.chat.placeholder")}
          />
          <Button variant="primary" disabled={!canSend() || !text().trim()} onClick={submit}>
            {t("altruClaw.chat.send")}
          </Button>
        </div>
      </div>
    </Show>
