...
export function ClawProvider(props: { children: JSX.Element }) {
  ...
  const applyState = (s: AltruCoderClawState) => {
    setPhase(s.phase)
    setLocale(s.locale)
    if (s.phase === "ready") {
      setStatus(s.status)
  ...
  const handleConversationMessage = (msg: AltruCoderClawOutMessage, active: string | null): boolean => {
    switch (msg.type) {
      case "altru-coder-claw.messages":
        if (msg.conversationId === active) {
          setMessages(msg.messages)
          setHasMoreMessages(msg.hasMore)
        }
        return true
      case "altru-coder-claw.messageOptimistic":
        if (msg.conversationId === active) {
          setMessages((prev) => (prev.some((m) => m.id === msg.message.id) ? prev : [...prev, msg.message]))
        }
        return true
      case "altru-coder-claw.messageReplaced":
        if (msg.conversationId === active) {
  ...
  const handler = (event: MessageEvent) => {
    const msg = event.data as AltruCoderClawOutMessage
    if (msg?.type === "fontSizeChanged") {
      applyFontSize(msg.fontSize)
      return
    }
    if (!msg?.type?.startsWith("altru-coder-claw.")) return
    const active = activeConversationId()
    if (handleConversationMessage(msg, active)) return

    switch (msg.type) {
      case "altru-coder-claw.state":
        applyState(msg.state)
        return
      case "altru-coder-claw.status":
        setStatus(msg.data)
        return
      case "altru-coder-claw.locale":
        setLocale(msg.locale)
        return
      case "altru-coder-claw.error":
        showToast({ title: msg.error, variant: "error", duration: 5000 })
        return
      case "altru-coder-claw.conversations":
        setConversations(msg.conversations)
        setHasMoreConversations(msg.hasMore)
        return
      case "altru-coder-claw.activeConversation":
        setActiveConversationId(msg.conversationId)
        setTypingMembers([])
        return
      case "altru-coder-claw.botStatus":
        setBotStatus(msg.status)
        return
      case "altru-coder-claw.conversationStatus":
  ...
  onMount(() => {
    vscode.postMessage({ type: "altru-coder-claw.ready" })
  })
  ...
  const ctx: ClawCtx = {
    ...
    locale,
    error,
    retry: () => vscode.postMessage({ type: "altru-coder-claw.ready" }),
    ...
    editMessage: (conversationId, messageId, content) =>
      vscode.postMessage({ type: "altru-coder-claw.editMessage", conversationId, messageId, content }),
    deleteMessage: (conversationId, messageId) =>
      vscode.postMessage({ type: "altru-coder-claw.deleteMessage", conversationId, messageId }),
    loadMoreMessages: (conversationId, before) =>
      vscode.postMessage({ type: "altru-coder-claw.loadMoreMessages", conversationId, before }),

    addReaction: (conversationId, messageId, emoji) =>
      vscode.postMessage({ type: "altru-coder-claw.addReaction", conversationId, messageId, emoji }),
    removeReaction: (conversationId, messageId, emoji) =>
      vscode.postMessage({ type: "altru-coder-claw.removeReaction", conversationId, messageId, emoji }),

    executeAction: (conversationId, messageId, groupId, value) =>
      vscode.postMessage({ type: "altru-coder-claw.executeAction", conversationId, messageId, groupId, value }),

    sendTyping: (conversationId) => vscode.postMessage({ type: "altru-coder-claw.sendTyping", conversationId }),
    sendTypingStop: (conversationId) => vscode.postMessage({ type: "altru-coder-claw.sendTypingStop", conversationId }),
    ...
    botStatus,
    conversationStatus,
    markConversationRead: (conversationId) => vscode.postMessage({ type: "altru-coder-claw.markRead", conversationId }),

    openExternal: (url) => vscode.postMessage({ type: "altru-coder-claw.openExternal", url }),
  }
