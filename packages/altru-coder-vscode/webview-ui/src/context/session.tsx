...
import { ALTRU_CODER_AUTO, parseModelString } from "../../../src/shared/provider-model"
import { findModel as findCatalogModel, flattenModels, localProviders } from "./provider-utils"
...
export const SessionProvider: ParentComponent = (props) => {
  ...
  const catalog = createMemo(() => localProviders(config().provider))
  const catalogModels = createMemo(() => flattenModels(catalog()))
  const fallbackModel = createMemo<ModelSelection>(() => {
    const model = catalogModels()[0]
    if (!model) return { ...ALTRU_CODER_AUTO }
  ...
  const variantList = (sessionID?: string) => {
    const sel = selected(sessionID)
    if (!sel) return []
    const model = findCatalogModel(catalogModels(), sel)
  ...
  function handleExtensionMessage(message: ExtensionMessage): void {
    ...
    switch (message.type) {
      ...
      case "cloudSessionImportFailed":
        ...
        console.error("[Altru Coder New] Cloud session import failed:", message.error)
        break
  ...
  function handlePartUpdated(
    ...
    if (!effectiveMessageID) {
      console.warn("[Altru Coder New] Part updated without messageID:", part.id, part.type)
      return
    }
  ...
  function sendMessage(
    ...
    if (!server.isConnected()) {
      console.warn("[Altru Coder New] Cannot send message: not connected")
      return
    }
  ...
  function sendCommand(
    ...
    if (!server.isConnected()) {
      console.warn("[Altru Coder New] Cannot send command: not connected")
      return
    }
  ...
  function abort() {
    const sessionID = currentSessionID()
    if (!sessionID) {
      console.warn("[Altru Coder New] Cannot abort: no current session")
      return
    }
  ...
  function compact() {
    if (!server.isConnected()) {
      console.warn("[Altru Coder New] Cannot compact: not connected")
      return
    }

    const sessionID = currentSessionID()
    if (!sessionID) {
      console.warn("[Altru Coder New] Cannot compact: no current session")
      return
    }
  ...
  function createSession() {
    if (!server.isConnected()) {
      console.warn("[Altru Coder New] Cannot create session: not connected")
      return
    }
  ...
  function loadSessions() {
    if (!server.isConnected()) {
      console.warn("[Altru Coder New] Cannot load sessions: not connected")
      return
    }
  ...
  function selectSession(id: string) {
    if (!server.isConnected()) {
      console.warn("[Altru Coder New] Cannot select session: not connected")
      return
    }
    if (id.startsWith("cloud:")) {
      console.warn("[Altru Coder New] Cannot select cloud preview session via selectSession")
      return
    }
  ...
  function selectCloudSession(cloudSessionId: string) {
    if (!server.isConnected()) {
      console.warn("[Altru Coder New] Cannot select cloud session: not connected")
      return
    }
    const key = `cloud:${cloudSessionId}`
  ...
  function deleteSession(id: string) {
    if (!server.isConnected()) {
      console.warn("[Altru Coder New] Cannot delete session: not connected")
      return
    }
  ...
  function renameSession(id: string, title: string) {
    if (!server.isConnected()) {
      console.warn("[Altru Coder New] Cannot rename session: not connected")
      return
    }
  ...
  const contextUsage = createMemo<ContextUsage | undefined>(() => {
    ...
    for (let i = msgs.length - 1; i >= 0; i--) {
      ...
      const sel = selected()
      const model = sel ? findCatalogModel(catalogModels(), sel) : undefined
