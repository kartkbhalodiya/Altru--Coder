 ...
 * Uses altru-coder-ui Popover component (Phase 4.5 of UI implementation plan).
 *
...
import { findModel as findCatalogModel, flattenModels, localProviders } from "../../context/provider-utils"
...
import ProviderSelectDialog from "../settings/ProviderSelectDialog"
...
export interface ModelSelectorBaseProps {
  ...
  clearLabel?: string
  /** Include the altru-coder-auto/small model in the list — defaults to false */
...
export const ModelSelectorBase: Component<ModelSelectorBaseProps> = (props) => {
  ...
  // Only show models the user saved in local provider config. This avoids
  // rendering the full backend provider snapshot for IDs like openrouter/nvidia.
  const visibleModels = createMemo(() => {
    if (props.models) return props.models
    return catalogModels().filter((m) => props.includeAutoSmall || !isSmall(m))
  })

  const activeModel = () => {
    const items = props.models
    if (items) return items.find((m) => m.providerID === props.value?.providerID && m.id === props.value?.modelID)
    return findCatalogModel(visibleModels(), props.value)
  }
  ...
  const triggerLabel = () =>
    buildTriggerLabel(
      ...
      hasProviders(),
      {
        ...
        noProviders: language.t("dialog.model.addFirst"),
          ...
          >
            ...
            <div class="model-selector-list" role="listbox" ref={listRef}>
              <Show when={rows().length === 0 && !props.allowClear}>
                <div class="model-selector-empty model-selector-empty--action">
                  <span>{language.t("dialog.model.addFirst.description")}</span>
                  <Button type="button" size="small" variant="secondary" icon="plus-small" onClick={addModel}>
