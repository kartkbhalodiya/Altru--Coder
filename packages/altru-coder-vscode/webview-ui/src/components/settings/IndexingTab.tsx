...
import {
  formatAltruCoderEmbeddingModelLabel,
  getAltruCoderEmbeddingModel,
  normalizeAltruCoderEmbeddingModelId,
} from "@altru-coder/altru-coder-indexing/embedding-models"
import { Select } from "@altru-coder/altru-coder-ui/select"
import { Switch } from "@altru-coder/altru-coder-ui/switch"
...
import { useAltruCoderEmbeddingModels } from "../../context/altru-coder-embedding-models"
...
const allProviders: { value: ProviderId; label: string }[] = [
  { value: "altru-coder", label: "Altru Coder" },
...
function providerFields(provider: ProviderId | undefined): Array<{ key: string; label: string; placeholder: string }> {
  if (provider === "altru-coder") return []
...
const IndexingTab: Component = () => {
  ...
  const embeds = useAltruCoderEmbeddingModels()
  ...
  const altruDefault = () => embeds.catalog().defaultModel
  const altruModels = createMemo(() =>
    embeds.catalog().models.map((model) => ({
      value: model.id,
      label: formatAltruCoderEmbeddingModelLabel(model),
    })),
  )
  const knownAltruCoderModel = (model: string | undefined) => getAltruCoderEmbeddingModel(model, embeds.catalog())?.id
  ...
  const saveProvider = (next: ProviderId | undefined) => {
    if (next === "altru-coder") {
      const model = knownAltruCoderModel(cfg().model) ?? (altruDefault() || undefined)
      updateIndexing({
        provider: next,
        model,
        dimension: undefined,
  ...
  const saveEnabled = (enabled: boolean) => {
    if (enabled && !cfg().provider && altruAvailable()) {
      updateIndexing({
        enabled,
        provider: "altru-coder",
        model: knownAltruCoderModel(cfg().model) ?? altruDefault(),
      })
      return
    }
  ...
  const saveGlobalEnabled = (enabled: boolean) => {
    if (enabled && !globalCfg().provider && !cfg().provider && altruAvailable()) {
      updateGlobalConfig({
        indexing: {
          enabled,
          provider: "altru-coder",
  ...
  return (
    <div style={{ display: "flex", "flex-direction": "column", gap: "16px" }}>
      ...
      <Card>
        ...
        <Show when={selectedProvider() === "altru-coder"}>
          <Show when={altruModels().length > 0}>
            <SettingsRow
              title={language.t("settings.indexing.altruCoderModel.title")}
              description={language.t("settings.indexing.altruCoderModel.description")}
            >
              <Select
                options={altruModels()}
                current={altruModels().find((item) => item.value === knownAltruCoderModel(cfg().model))}
                value={(item) => item.value}
                label={(item) => item.label}
                onSelect={(item) => updateIndexing({ model: item?.value ?? altruDefault(), dimension: undefined })}
                variant="secondary"
                size="small"
                triggerVariant="settings"
        ...
        <Show when={selectedProvider() === "altru-coder" && !altruAvailable()}>
          <SettingsRow
            title={language.t("settings.indexing.altruCoderSignIn.title")}
            description={language.t("settings.indexing.altruCoderSignIn.description")}
            last
          >
            <span />
          </SettingsRow>
