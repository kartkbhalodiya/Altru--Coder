...
const ProvidersTab: Component = () => {
  ...
  const catalogProviders = createMemo(() => {
    ...
    return PROVIDER_PRESETS.filter((item) => !connected.has(item.id) && !disabled.has(item.id))
  })

  const disabledProviders = createMemo(() => config().disabled_providers ?? [])
  const disabledIds = createMemo(() => new Set(disabledProviders()))
  const providers = createMemo(() => providersWithAltruCoderFallback(provider.providers()))
  ...
  return (
    <div>
      <Show when={!disabledIds().has(ALTRU_CODER_PROVIDER_ID)}>
        {/* Altru Coder Gateway — always at the top, not editable */}
        <Card>
          <div
            style={{
              display: "flex",
          ...
          >
            <ProviderLogo providerID={ALTRU_CODER_PROVIDER_ID} width={20} height={20} />
            <span
              style={{
                "font-size": "var(--altru-coder-font-size-14)",
                "font-weight": "500",
      ...
      {/* Connected providers (excluding Altru Coder) */}
            ...
            {(item) => (
              ...
              >
                <div style={{ display: "flex", "align-items": "center", gap: "12px", "min-width": 0 }}>
                  <ProviderLogo providerID={item.id} width={20} height={20} />
                  <span
                    style={{
                      "font-size": "var(--altru-coder-font-size-14)",
                      "font-weight": "500",
      ...
      <h4 style={{ "margin-top": "24px", "margin-bottom": "8px" }}>
        {language.t("settings.providers.section.catalog")}
      </h4>
      <Card>
        <div class="provider-catalog-grid">
            ...
            {(item) => (
              ...
              >
                ...
                <span class="provider-catalog-copy">
                  <span class="provider-catalog-name">{item.name}</span>
                  <span class="provider-catalog-note">{item.note}</span>
                </span>
                <span class="provider-catalog-action" aria-hidden="true">
          ...
          <button
            type="button"
            class="provider-catalog-tile provider-catalog-tile--custom"
            title={`${language.t("common.connect")} ${language.t("provider.custom.title")}`}
            onClick={() => dialog.show(() => <CustomProviderDialog />)}
          >
            <span class="provider-catalog-icon">
              <ProviderLogo providerID="_custom" width={22} height={22} />
            </span>
            <span class="provider-catalog-copy">
              <span class="provider-catalog-name">
                {language.t("provider.custom.title")} <Tag>{language.t("settings.providers.tag.custom")}</Tag>
              </span>
              <span class="provider-catalog-note">{language.t("settings.providers.custom.description")}</span>
            </span>
            <span class="provider-catalog-action" aria-hidden="true">
              <Icon name="plus-small" size="small" />
            </span>
      ...
      <Card>
        ...
        <For each={disabledProviders()}>
          {(id, index) => (
            ...
            >
              <div style={{ display: "flex", "align-items": "center", gap: "12px", "min-width": 0 }}>
                <ProviderLogo providerID={id} width={20} height={20} />
                <span
                  style={{
                    "font-size": "var(--altru-coder-font-size-14)",
                    "font-weight": "500",
