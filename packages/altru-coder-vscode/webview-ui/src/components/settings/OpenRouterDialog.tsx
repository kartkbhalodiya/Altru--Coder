...
import { OpenrouterPreset } from "../../../../src/shared/providers/openrouter"
...
const base = OpenrouterPreset.models ?? []

function first(models: ProviderPresetModel[]) {
  return models.find((item) => item.id === fallback) ?? models[0]
}
...
export default function OpenRouterDialog(props: Props) {
  const dialog = useDialog()
  const language = useLanguage()
  const provider = useProvider()
  const vscode = useVSCode()
  const [catalog, setCatalog] = createSignal(base)
  const auth = props.existing ? provider.authStates()[OpenrouterPreset.id] : undefined
  ...
  function save() {
    ...
    if (items.length === 0) {
      showToast({
        ...
        description: "Select at least one OpenRouter model.",
      })
      return
    }
    ...
    vscode.postMessage({
      ...
      providerID: OpenrouterPreset.id,
      config: {
        npm: CUSTOM_PROVIDER_PACKAGE,
        name: OpenrouterPreset.name,
        options: { baseURL: OpenrouterPreset.baseURL },
    ...
    showToast({
      variant: "success",
      icon: "circle-check",
      title: language.t("provider.connect.toast.connected.title", { provider: OpenrouterPreset.name }),
      description: language.t("provider.connect.toast.connected.description", { provider: OpenrouterPreset.name }),
    })
  }
    ...
    >
      <div class="provider-model-dialog">
        <div style={{ display: "flex", gap: "12px", "align-items": "center" }}>
          <ProviderLogo providerID={OpenrouterPreset.id} width={20} height={20} />
          <div style={{ "font-size": "var(--altru-coder-font-size-16)", "font-weight": "500" }}>
            {OpenrouterPreset.name}
          </div>
        </div>
