...
const dict: Record<string, string> = { ...appEn, ...amEn, ...uiEn, ...altruEn }
...
// ---------------------------------------------------------------------------
// Mock providers — pre-loaded Altru Coder Gateway model for stories
// ---------------------------------------------------------------------------
...
const MockProviderProvider: ParentComponent = (props) => {
  const value = {
    ...
    connected: () => ["altru-coder"],
    defaults: () => ({}),
    defaultSelection: () => ({ providerID: "altru-coder", modelID: "anthropic/claude-sonnet-4-6" }),
    models: () => MOCK_MODELS,
...
function mockNotificationsValue(items: AltruCoderNotification[] = []) {
  return {
    notifications: () => items,
...
export function mockSessionValue(overrides?: {
  ...
  return {
    ...
    selected: () => ({ providerID: "altru-coder", modelID: "anthropic/claude-sonnet-4-6" }),
    selectModel: noop,
    ...
    getSessionModel: () => ({ providerID: "altru-coder", modelID: "anthropic/claude-sonnet-4-6" }),
...
interface StoryProvidersProps {
  ...
  questions?: QuestionRequest[]
  suggestions?: SuggestionRequest[]
  notifications?: AltruCoderNotification[]
  status?: string
  sessionID?: string
