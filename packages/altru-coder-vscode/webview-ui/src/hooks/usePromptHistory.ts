...
const STORAGE_KEY = "altru-coder.prompt-history.v1"
...
function load(): string[] {
  try {
    ...
    console.warn("[Altru Coder New] prompt history load failed", err)
    return []
  }
}
...
function save(items: string[]) {
  try {
    ...
    console.warn("[Altru Coder New] prompt history save failed", err)
  }
}
