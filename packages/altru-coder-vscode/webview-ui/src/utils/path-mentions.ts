...
export function convertToMentionPath(path: string, cwd: string): string {
  ...
  try {
    ...
    console.error("[Altru Coder New] Failed to decode dropped URI:", err, cleaned)
  }
 ...
 */
export const ALTRU_CODER_FILE_PATH_MIME = "application/x-altru-coder-file-path"

/**
 * Extract file paths from a drop's DataTransfer.
 * Checks (in order):
 * 1. Internal relative-path drag (application/x-altru-coder-file-path)
...
export function extractDropPaths(dt: DataTransfer): string[] | null {
  ...
  const altru = dt.getData(ALTRU_CODER_FILE_PATH_MIME)
  if (altru) {
    const paths = altru.split(/\r?\n/).filter((line) => line.trim() !== "")
