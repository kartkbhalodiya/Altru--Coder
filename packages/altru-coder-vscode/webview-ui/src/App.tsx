...
import { DataProvider } from "@altru-coder/altru-coder-ui/context/data"
...
import { AltruCoderEmbeddingModelsProvider } from "./context/altru-coder-embedding-models"
import type { Message as SDKMessage, Part as SDKPart } from "@altru-coder/sdk/v2"
...
export const DataBridge: Component<{ children: any }> = (props) => {
  ...
  return (
    <DataProvider
      data={data}
      directory={directory()}
      // @ts-expect-error — onPermissionRespond/onQuestion* are extension-specific props not yet in altru-coder-ui's DataProvider types
...
export const MermaidDownloadBridge: Component = () => {
  ...
  onMount(() => {
    ...
    window.addEventListener("altru-coder:save-image", save)
    onCleanup(() => {
      window.removeEventListener("altru-coder:save-image", save)
    })
  })
  ...
  onMount(() => {
    const handler = (event: MessageEvent) => {
      ...
      if (message?.type === "action" && message.action) {
        console.log("[Altru Coder New] App: 🎬 action:", message.action)
        handleViewAction(message.action)
      }
      if (message?.type === "navigate" && message.view && VALID_VIEWS.has(message.view)) {
        console.log("[Altru Coder New] App: 🧭 navigate:", message.view, message.tab ? `tab=${message.tab}` : "")
      ...
      if (message?.type === "openCloudSession" && message.sessionId) {
        console.log("[Altru Coder New] App: ☁️ openCloudSession:", message.sessionId)
        session.selectCloudSession(message.sessionId)
        setCurrentView("newTask")
      }
      handleForked(message)
      if (message?.type === "viewSubAgentSession" && message.sessionID) {
        console.log("[Altru Coder New] App: 🔍 viewSubAgentSession:", message.sessionID)
      ...
      if (message?.type === "migrationState") {
        console.log("[Altru Coder New] App: 🔄 migrationState:", message.needed)
          ...
          >
            ...
            <Match when={currentView() === "profile"}>
              <ProfileView
                ...
                onDashboard={() => setCurrentView("newTask")}
              />
            </Match>
                    ...
                    <FileComponentProvider component={File}>
                      <ProviderProvider>
                        <ConfigProvider>
                          <DisplayProvider>
                            <IndexingProvider>
                              <AltruCoderEmbeddingModelsProvider>
