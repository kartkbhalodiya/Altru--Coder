...
describe("parseServerPort", () => {
  it("parses port from standard CLI startup message", () => {
    expect(parseServerPort("altru-coder server listening on http://127.0.0.1:12345")).toBe(12345)
  })
  ...
  it("parses port when embedded in longer output", () => {
    const output = "[INFO] 2024-01-01 altru-coder server listening on http://127.0.0.1:54321\n[INFO] ready"
...
describe("toErrorMessage", () => {
  ...
  it("strips ANSI codes before matching Error:", () => {
    const ansiError = "\x1b[91m\x1b[1mError: \x1b[0mConfig file at /path/altru-coder.json is not valid JSON(C):"
    const result = toErrorMessage("startup failed", [ansiError])
    expect(result.userMessage).toBe("Config file at /path/altru-coder.json is not valid JSON(C):")
  })
  ...
  it("includes CLI path in userDetails when provided", () => {
    const result = toErrorMessage("startup failed", [], "/usr/local/bin/altru-coder")
    expect(result.userDetails).toContain("CLI path: /usr/local/bin/altru-coder")
  })
...
describe("server workspace helpers", () => {
  ...
  it("disables codebase indexing only when no workspace folder is open", () => {
    expect(resolveIndexingEnv(undefined)).toEqual({ ALTRU_CODER_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
    expect(resolveIndexingEnv([])).toEqual({ ALTRU_CODER_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
