...
const { AltruCoderActionProvider } = await import("../../src/services/code-actions/code-action-provider")

const provider = new AltruCoderActionProvider()
...
describe("AltruCoderActionProvider", () => {
  describe("provideCodeActions", () => {
    ...
    describe("non-empty range, no diagnostics", () => {
      it("returns Add, Explain, Improve actions", () => {
        ...
        expect(titles).toContain("Add to Altru Coder")
        expect(titles).toContain("Explain with Altru Coder")
        expect(titles).toContain("Improve with Altru Coder")
      })

      it("does not include Fix action", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(0) as never)
        expect(result.map((a) => a.title)).not.toContain("Fix with Altru Coder")
      })
      ...
      it("uses correct command IDs", () => {
        ...
        expect(commands).toContain("altru-coder.new.addToContext")
        expect(commands).toContain("altru-coder.new.explainCode")
        expect(commands).toContain("altru-coder.new.improveCode")
      })
    ...
    describe("non-empty range, with diagnostics", () => {
      it("returns Add and Fix actions", () => {
        ...
        const titles = result.map((a) => a.title)
        expect(titles).toContain("Add to Altru Coder")
        expect(titles).toContain("Fix with Altru Coder")
      })
      ...
      it("does not include Explain or Improve actions", () => {
        ...
        const titles = result.map((a) => a.title)
        expect(titles).not.toContain("Explain with Altru Coder")
        expect(titles).not.toContain("Improve with Altru Coder")
      })
      ...
      it("Fix action is preferred", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(1) as never)
        const fix = result.find((a) => a.title === "Fix with Altru Coder")
      ...
      it("uses correct Fix command ID", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(1) as never)
        const fix = result.find((a) => a.title === "Fix with Altru Coder")
        expect(fix?.command?.command).toBe("altru-coder.new.fixCode")
      })
    })
  })
})
