...
} from "../../src/altru-coder-provider/handlers/permission-handler"
...
describe("recoveryDirs", () => {
  ...
  it("returns workspace root plus each unique worktree directory", () => {
    const dirs = new Map([
      ["s1", "/workspace/.altru-coder/worktrees/alpha"],
      ["s2", "/workspace/.altru-coder/worktrees/beta"],
      ["s3", "/workspace/.altru-coder/worktrees/alpha"],
    ])
    expect(recoveryDirs("/workspace", dirs)).toEqual([
      "/workspace",
      "/workspace/.altru-coder/worktrees/alpha",
      "/workspace/.altru-coder/worktrees/beta",
    ])
  })
})
...
describe("fetchAndSendPendingPermissions", () => {
  ...
  it("queries workspace root plus each unique worktree directory", async () => {
    ...
    expect(queries).toContain("/workspace/.altru-coder/worktrees/alpha")
    expect(queries).toContain("/workspace/.altru-coder/worktrees/beta")
  ...
  it("deduplicates directories", async () => {
    const dirs = new Map([
      ["s1", "/workspace/.altru-coder/worktrees/alpha"],
      ["s2", "/workspace/.altru-coder/worktrees/alpha"],
    ])
    const { fake, queries } = ctx({ tracked: ["s1", "s2"], dirs })
    await fetchAndSendPendingPermissions(fake)
    expect(queries.filter((d) => d === "/workspace/.altru-coder/worktrees/alpha")).toHaveLength(1)
  })
