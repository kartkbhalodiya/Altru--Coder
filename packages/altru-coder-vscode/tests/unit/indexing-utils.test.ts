...
import { mapSSEEventToWebviewMessage } from "../../src/altru-coder-provider-utils"
import { configFeatures } from "../../src/features"
import type { EventIndexingStatus, IndexingStatus } from "@altru-coder/sdk/v2/client"
...
describe("indexing feature detection", () => {
  ...
  it("detects supported indexing plugin specifiers when experimental.semantic_indexing is true", () => {
    ...
    expect(
      configFeatures({
        plugin: ["file:///tmp/.opencode/plugin/altru-coder-indexing.js"],
        experimental: { semantic_indexing: true },
      }).indexing,
    ).toBe(true)
    expect(
      configFeatures({
        plugin: ["file:///tmp/node_modules/@altru-coder/altru-coder-indexing/index.js"],
        experimental: { semantic_indexing: true },
      }).indexing,
    ).toBe(true)
  })
  ...
  it("ignores unrelated plugin lists", () => {
    expect(configFeatures({ plugin: ["@altru-coder/altru-coder-gateway"] }).indexing).toBe(false)
