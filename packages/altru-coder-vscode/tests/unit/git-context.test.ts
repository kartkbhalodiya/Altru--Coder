...
async function repo(run: (dir: string) => Promise<void>, commit = true) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "altru-coder-git-context-"))
  try {
    git(dir, ["init"])
    if (commit)
      git(dir, ["-c", "user.name=Altru Coder", "-c", "user.email=altru-coder@example.com", "commit", "--allow-empty", "-m", "init"])
    await run(dir)
  } finally {
...
describe("getGitChangesContext", () => {
  it("reports non-git directories", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "altru-coder-no-git-"))
    try {
  ...
  it("includes tracked diffs", async () => {
    await repo(async (dir) => {
      ...
      git(dir, ["-c", "user.name=Altru Coder", "-c", "user.email=altru-coder@example.com", "commit", "-m", "tracked"])
  ...
  it("includes base diffs with patches", async () => {
    await repo(async (dir) => {
      ...
      git(dir, ["-c", "user.name=Altru Coder", "-c", "user.email=altru-coder@example.com", "commit", "-m", "base"])
      await fs.writeFile(path.join(dir, "base.txt"), "after\n")
      git(dir, ["add", "base.txt"])
      git(dir, ["-c", "user.name=Altru Coder", "-c", "user.email=altru-coder@example.com", "commit", "-m", "change"])
