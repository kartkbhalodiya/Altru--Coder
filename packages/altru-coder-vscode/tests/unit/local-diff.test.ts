...
describe("generatedLike", () => {
  ...
  it("rejects normal source files", () => {
    ...
    expect(generatedLike("packages/altru-coder-vscode/src/extension.ts")).toBe(false)
  })
...
describe("workingTreeSummary", () => {
  it("reports only staged, unstaged, and untracked files relative to HEAD", async () => {
    await withRepo(async (dir) => {
      runSync(dir, ["checkout", "-b", "feature"])
      await fs.writeFile(path.join(dir, "committed.txt"), "committed\n")
      runSync(dir, ["add", "committed.txt"])
      runSync(dir, ["commit", "-m", "feature commit"])

      await fs.writeFile(path.join(dir, "seed.txt"), "seed\nworking\n")
      await fs.writeFile(path.join(dir, "fresh.txt"), "one\ntwo\n")

      const result = await workingTreeSummary(git(), dir)
      const files = result.map((entry) => entry.file).sort()
      expect(files).toEqual(["fresh.txt", "seed.txt"])
      expect(result.find((entry) => entry.file === "committed.txt")).toBeUndefined()
      expect(result.find((entry) => entry.file === "fresh.txt")?.tracked).toBe(false)
      expect(result.find((entry) => entry.file === "seed.txt")?.status).toBe("modified")
    })
  })
})

describe("workingTreeFile", () => {
  it("loads detail against HEAD instead of the base branch", async () => {
    await withRepo(async (dir) => {
      runSync(dir, ["checkout", "-b", "feature"])
      await fs.writeFile(path.join(dir, "seed.txt"), "seed\nfeature\n")
      runSync(dir, ["commit", "-am", "feature commit"])
      ...
      const result = await workingTreeFile(git(), dir, "seed.txt")
      expect(result?.before).toBe("seed\nfeature\n")
      expect(result?.after).toBe("seed\nfeature\nworking\n")
      expect(result?.status).toBe("modified")
    })
