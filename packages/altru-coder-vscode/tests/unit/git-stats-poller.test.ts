...
describe("GitStatsPoller", () => {
  ...
  it("uses working-tree local stats when a localWorktreeDiff source is provided", async () => {
    ...
    expect(branchDiffCalls).toBe(0)
    expect(treeDiffCalls).toBeGreaterThan(0)
    expect(emitted[0]).toEqual({ branch: "feature", files: 1, additions: 2, deletions: 1, ahead: 4, behind: 0 })
  })
