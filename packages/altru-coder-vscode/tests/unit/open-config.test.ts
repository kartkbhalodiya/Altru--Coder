...
import { globalFiles, localFiles } from "../../src/altru-coder-provider/config-file"
import { openConfig } from "../../src/altru-coder-provider/open-config"
...
const env = {
  ...
  ALTRU_CODER_CONFIG_DIR: process.env.ALTRU_CODER_CONFIG_DIR,
  ALTRU_CODER_DISABLE_PROJECT_CONFIG: process.env.ALTRU_CODER_DISABLE_PROJECT_CONFIG,
...
const labels = {
  ...
  sourceHomeAltruCoder: "Home Altru Coder",
  sourceHomeAltrucoder: "Home AltruCoder",
  sourceHomeOpencode: "Home Opencode",
  sourceProjectAltruCoder: "Project Altru Coder",
  sourceProjectAltrucoder: "Project AltruCoder",
...
async function temp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "altru-coder-config-"))
  dirs.push(dir)
  return dir
}
...
describe("config file discovery", () => {
  it("discovers global, env, legacy, and virtual config sources", async () => {
    ...
    process.env.ALTRU_CODER_CONFIG = envfile
    ...
    expect(sources).toContain("sourceHomeAltruCoder")
  ...
  it("marks project files unloaded when project config is disabled", async () => {
    reset()
    const root = await temp()
    process.env.ALTRU_CODER_DISABLE_PROJECT_CONFIG = "1"
    await file(path.join(root, "altru-coder.json"))
    ...
    expect(list.find((item) => item.recommended)?.file).toBe(path.join(root, ".altru-coder", "altru-coder.jsonc"))
  })
})
...
describe("openConfig", () => {
  ...
  it("opens the only editable config without showing the picker", async () => {
    reset()
    const root = await temp()
    const cfg = path.join(root, ".altru-coder", "altru-coder.jsonc")
    await file(cfg)
