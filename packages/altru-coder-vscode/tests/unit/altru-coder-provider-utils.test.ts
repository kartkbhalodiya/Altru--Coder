...
import { mergeFileSearchItems } from "../../src/altru-coder-provider/file-search-items"
...
describe("mergeFileSearchItems", () => {
  ...
  it("normalizes Windows separators for matching and output", () => {
    const result = mergeFileSearchItems({
      query: "altru-coder-vscode",
      files: ["packages\\altru-coder-vscode\\src\\AltruCoderProvider.ts"],
      folders: ["packages\\altru-coder-vscode\\"],
    })
    expect(result).toEqual([
      { path: "packages/altru-coder-vscode/", type: "folder" },
      { path: "packages/altru-coder-vscode/src/AltruCoderProvider.ts", type: "file" },
    ])
  })
})
...
describe("getErrorMessage", () => {
  ...
  it("reads the first Zod issue message from ConfigInvalidError", () => {
    const err = {
      ...
      data: {
        path: "/Users/me/.config/altru-coder/altru-coder.json",
        issues: [
...
describe("getConfigErrorDetails", () => {
  ...
  it("formats a single-issue ConfigInvalidError", () => {
    const err = {
      data: {
        path: "/home/me/.config/altru-coder/altru-coder.json",
        issues: [{ code: "unrecognized_keys", keys: ["indexing"], path: [], message: 'Unrecognized key: "indexing"' }],
      },
    }
    expect(getConfigErrorDetails(err)).toBe('File: /home/me/.config/altru-coder/altru-coder.json\n\n✖ Unrecognized key: "indexing"')
  })
