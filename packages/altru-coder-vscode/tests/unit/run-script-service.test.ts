...
describe("RunScriptService", () => {
  ...
  beforeEach(() => {
    root = tmpdir()
    dir = path.join(root, ".altru-coder")
    fs.mkdirSync(dir)
  })
  ...
  it("builds commands for shell, PowerShell, and CMD scripts", () => {
    ...
    expect(buildRunTaskCommand({ path: "C:\\repo\\.altru-coder\\run-script.ps1", kind: "powershell" })).toEqual({
      command: "powershell.exe",
      args: ["-NoLogo", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "C:\\repo\\.altru-coder\\run-script.ps1"],
    })
    expect(buildRunTaskCommand({ path: "C:\\repo path\\.altru-coder\\run-script.cmd", kind: "cmd" })).toEqual({
      command: "cmd.exe",
      args: ["/d", "/s", "/c", '"C:\\repo path\\.altru-coder\\run-script.cmd"'],
    })
  })
  ...
  it("rejects symlinks pointing outside the .altru-coder directory", () => {
  ...
  it("accepts symlinks pointing inside the .altru-coder directory", () => {
