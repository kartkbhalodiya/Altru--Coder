...
describe("t()", () => {
  it("returns translated string for known key", () => {
    const result = t("altrucoder:autocomplete.statusBar.enabled")
    expect(typeof result).toBe("string")
    expect(result.length).toBeGreaterThan(0)
    expect(result).not.toBe("altrucoder:autocomplete.statusBar.enabled")
  })
  ...
  it("interpolates a single variable", () => {
    const result = t("altrucoder:autocomplete.statusBar.tooltip.noUsableProvider", {
      providers: "OpenAI, Anthropic",
      command: "command:altru-coder.new.settingsButtonClicked",
    })
  ...
  it("interpolates multiple variables", () => {
    const result = t("altrucoder:autocomplete.statusBar.tooltip.completionSummary", {
      count: "5",
      startTime: "10:00",
      endTime: "11:00",
      cost: "$0.05",
  ...
  it("leaves unreferenced vars intact in template", () => {
    const key = "altrucoder:autocomplete.statusBar.tooltip.noUsableProvider"
