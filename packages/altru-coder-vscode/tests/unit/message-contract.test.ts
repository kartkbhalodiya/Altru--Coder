 ...
 * 2. Every WebviewMessage type handled in AltruCoderProvider has a corresponding member in the WebviewMessage union
...
const ALTRU_CODER_PROVIDER_UTILS_FILE = path.join(ROOT, "src/altru-coder-provider-utils.ts")
...
describe("AltruCoderProvider message handler coverage", () => {
  it("all WebviewMessage switch cases in AltruCoderProvider exist in WebviewMessage union", () => {
    const providerContent = readFile(ALTRU_CODER_PROVIDER_FILE)
    ...
    expect(
      unrecognized,
      `AltruCoderProvider switch cases not found in any message type definition: ${unrecognized.join(", ")}`,
    ).toEqual([])
  })
})
...
describe("mapSSEEventToWebviewMessage output types", () => {
  it("all output types from mapSSEEventToWebviewMessage exist in ExtensionMessage", () => {
    const utilsContent = readFile(ALTRU_CODER_PROVIDER_UTILS_FILE)
    const messagesContent = readMessageTypeSources()
