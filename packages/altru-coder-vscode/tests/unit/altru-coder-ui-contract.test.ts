/**
 * Runtime contract tests for altru-coder-vscode's dependencies on @altru-coder/altru-coder-ui.
 *
 * These tests import the upstream UI modules directly and verify at runtime
 * that the exports altru-coder-vscode depends on still exist with the expected shape.
 *
 * Because the upstream modules use SolidJS JSX (jsxImportSource: "solid-js"),
 * they must be loaded from within packages/altru-coder-ui/ where bun picks up the
...
/**
 * Tool names that altru-coder-vscode overrides or uses directly.
 * Sources:
...
describe("DataProvider contract (runtime)", () => {
  ...
  it("DataProvider accepts onOpenFile prop and exports OpenFileFn (source)", () => {
    // onOpenFile and OpenFileFn are `altrucoder_change` additions — TypeScript types
  ...
  it("DataProvider accepts onOpenDiff prop and exports OpenDiffFn (source)", () => {
    // onOpenDiff and OpenDiffFn are `altrucoder_change` additions — TypeScript types
    // erased at runtime, so we verify via source analysis
    const src = fs.readFileSync(DATA_CONTEXT_FILE, "utf-8")
...
describe("Edit tool diff-first click contract (source)", () => {
  const src = fs.readFileSync(ALTRU_CODER_MESSAGE_PART_FILE, "utf-8")
