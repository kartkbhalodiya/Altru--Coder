...
describe("resolveModelSelection", () => {
  ...
  it("uses altru-coder auto as the explicit final fallback", () => {
    const result = resolveModelSelection({
      providers,
      connected: [],
      fallback: ALTRU_CODER_AUTO,
    })
    expect(result).toEqual(ALTRU_CODER_AUTO)
  })

  it("keeps the explicit fallback even when altru-coder is missing from the loaded catalog", () => {
    const result = resolveModelSelection({
