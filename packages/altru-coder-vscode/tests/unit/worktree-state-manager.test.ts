...
describe("WorktreeStateManager", () => {
  ...
  beforeEach(() => {
    ...
    // Pre-create .altru-coder dir so fire-and-forget saves don't race on mkdir
    fs.mkdirSync(path.join(root, ".altru-coder"), { recursive: true })
    logs.length = 0
  ...
  describe("session CRUD", () => {
    ...
    it("drops obsolete session prefs while loading state", async () => {
      const file = path.join(root, ".altru-coder", "agent-manager.json")
      fs.writeFileSync(
        file,
        JSON.stringify({
          worktrees: {},
  ...
  describe("persistence", () => {
    ...
    it("creates .altru-coder directory if missing", async () => {
      ...
      expect(fs.existsSync(path.join(fresh, ".altru-coder", "agent-manager.json"))).toBe(true)
    })
    ...
    it("backs up a corrupt state file before recovery saves", async () => {
      ...
      const files = fs.readdirSync(path.join(root, ".altru-coder"))
      expect(result.status).toBe("failed")
  ...
  describe("tab order", () => {
    ...
    it("does not persist empty tab order", async () => {
      ...
      const content = fs.readFileSync(path.join(root, ".altru-coder", "agent-manager.json"), "utf-8")
  ...
  describe("validate", () => {
    ...
    it("resolves relative paths against root", async () => {
      const relative = ".altru-coder/worktrees/test-branch"
  ...
  describe("legacy .altrucoder migration", () => {
    it("migrates and loads state from .altrucoder when .altru-coder is absent", async () => {
      // Remove the .altru-coder dir created in beforeEach
      fs.rmSync(path.join(root, ".altru-coder"), { recursive: true, force: true })

      // Write state to legacy .altrucoder dir (migration will move it to .altru-coder)
      const legacyDir = path.join(root, ".altrucoder")
    ...
    it("skips migration when .altru-coder state already exists", async () => {
      // .altru-coder state already present — migration should skip agent-manager.json
      fs.writeFileSync(
        path.join(root, ".altru-coder", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-new": {
              branch: "new-branch",
    ...
    it("rewrites stale .altrucoder paths in worktree entries (unix)", async () => {
      fs.writeFileSync(
        path.join(root, ".altru-coder", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-stale": {
              branch: "fix",
              path: "/repo/.altrucoder/worktrees/fix",
              parentBranch: "main",
              createdAt: new Date().toISOString(),
            },
          },
      ...
      expect(manager.getWorktrees()[0].path).toBe(`/repo/.altru-coder/worktrees/fix`)
    })

    it("rewrites stale .altrucoder paths with backslashes (windows)", async () => {
      fs.writeFileSync(
        path.join(root, ".altru-coder", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-win": {
              branch: "fix",
              path: "C:\\.altrucoder\\worktrees\\fix",
              parentBranch: "main",
              createdAt: new Date().toISOString(),
            },
          },
      ...
      expect(manager.getWorktrees()[0].path).toBe("C:\\.altru-coder\\worktrees\\fix")
    })
  })
})
