...
describe("buildTelemetryAuthHeader", () => {
  ...
  it("encodes altru-coder:password in base64", () => {
    const result = buildTelemetryAuthHeader("secret")
    const encoded = Buffer.from("altru-coder:secret").toString("base64")
    expect(result).toBe(`Basic ${encoded}`)
  })

  it("handles empty password", () => {
    const result = buildTelemetryAuthHeader("")
    const encoded = Buffer.from("altru-coder:").toString("base64")
    expect(result).toBe(`Basic ${encoded}`)
  })
})
