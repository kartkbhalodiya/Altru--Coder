...
describe("Agent Manager CSS Prefix", () => {
  it("all class selectors should use am- prefix", () => {
    ...
    // - `altru-coder-diff-theme` is the shared Pierre diff theme utility defined
    //   in webview-ui/src/styles/diff.css and reused across webviews.
    // - `css` is matched from `@import "./diff.css"` file extension, not a
    //   class selector.
    const host = new Set(["vscode-high-contrast", "vscode-high-contrast-light", "altru-coder-diff-theme", "css"])
  ...
  it("all CSS custom properties should use am- prefix", () => {
    ...
    // Allow altru-coder-ui design tokens, vscode theme variables, and third-party
    // library tokens (@pierre/diffs, altru-coder-ui sticky-accordion) used as fallbacks
...
describe("Agent Manager Webview — non-git sessionsLoaded fix", () => {
  ...
  /**
   * Regression: when isGitRepo is false, the Altru Coder server never sends a
...
// ---------------------------------------------------------------------------
// AltruCoderProvider — pendingSessionRefresh race condition fix
// ---------------------------------------------------------------------------

describe("AltruCoderProvider — pending session refresh on reconnect", () => {
  const provider = fs.readFileSync(ALTRU_CODER_PROVIDER_FILE, "utf-8")
  const utils = fs.readFileSync(path.join(ROOT, "src/altru-coder-provider-utils.ts"), "utf-8")
   ...
   * altru-coder-provider-utils) sets it when httpClient is unavailable, and
  ...
  it("loadSessions sets pendingSessionRefresh when client is null", () => {
    ...
    expect(start, "loadSessions must exist in altru-coder-provider-utils").toBeGreaterThan(-1)
...
//
// Regression: PR #7473 moved AltruCoderNotifications into MessageList. It calls
