...
async function createTempWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "altru-coder-ignore-"))
  tempDirs.push(dir)
  return dir
}

describe("FileIgnoreController", () => {
  describe("when .altrucoderignore exists", () => {
    it("applies only .altrucoderignore patterns", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".altrucoderignore"), "secret/**\n*.snap\n")
      ...
      expect(controller.getInstructions()).toContain(".altrucoderignore")
    })

    it("does NOT block .env files unless explicitly listed", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".altrucoderignore"), "dist/\n")
    ...
    it("does NOT apply .gitignore patterns", async () => {
      ...
      // .altrucoderignore pattern should apply
      expect(controller.validateAccess(path.join(workspace, "dist", "bundle.js"))).toBe(false)
    })
  })

  describe("when no .altrucoderignore exists (fallback)", () => {
    ...
    it("blocks .env files via hardcoded sensitive patterns", async () => {
      const workspace = await createTempWorkspace()
      // No .altrucoderignore, no .gitignore
  ...
  describe("Windows cross-drive paths", () => {
    it("does not throw for a Windows-style absolute path from another drive", async () => {
      ...
      const cross =
        "c:/Users/User/AppData/Roaming/Code/User/globalStorage/altrucoder.altru-coder/settings/mcp_settings.json"
    ...
    it("does not throw for file:// URIs with Windows drive letters", async () => {
      ...
      const uri =
        "file:///c:/Users/User/AppData/Roaming/Code/User/globalStorage/altrucoder.altru-coder/settings/mcp_settings.json"
