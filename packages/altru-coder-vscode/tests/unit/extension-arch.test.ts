...
const ALTRU_CODER_PROVIDER_FILE = path.join(ROOT, "src/AltruCoderProvider.ts")
...
describe("Extension — package.json command sync", () => {
  ...
  it("every contributes.commands entry has a registerCommand() call", () => {
    ...
    const dynamic: Record<string, string> = {
      "altru-coder.new.agentManager.jumpTo": "registerCommand(`altru-coder.new.agentManager.jumpTo${",
    }
  ...
  /**
   * All declared commands must use the altru-coder.new. prefix.
   * The legacy altru-coder.* namespace (without .new.) belongs to the old
   * extension and must not be reintroduced.
   */
  it("all declared commands use the altru-coder.new. prefix", () => {
    const bad = declared.filter((cmd) => !cmd.startsWith("altru-coder.new."))
    expect(
      bad,
      `Commands without "altru-coder.new." prefix — use the namespaced form:\n` + bad.map((b) => `  - ${b}`).join("\n"),
    ).toEqual([])
  })
})

// ---------------------------------------------------------------------------
// AltruCoderProvider handler wiring — every new AltruCoderProvider() must get
// setContinueInWorktreeHandler() called before resolving its webview.
//
// Regression: tab panels created via openAltruCoderInNewTab() and the TabPanel
...
describe("Extension — AltruCoderProvider handler wiring", () => {
  const ext = fs.readFileSync(EXTENSION_FILE, "utf-8")

  /**
   * Every `new AltruCoderProvider(` in extension.ts must be followed (before the
   * next `new AltruCoderProvider(`) by a `setContinueInWorktreeHandler` call.
   * This prevents future tab/panel additions from silently missing the handler.
   */
  it("every AltruCoderProvider instance gets setContinueInWorktreeHandler wired", () => {
    const pattern = /new AltruCoderProvider\(/g
    ...
    expect(
      instances.length,
      "expected at least 3 AltruCoderProvider instances (sidebar, tab, deserializer)",
    ...
    for (let i = 0; i < instances.length; i++) {
      ...
      if (!region.includes("setContinueInWorktreeHandler")) {
        ...
        missing.push(`AltruCoderProvider at line ${line}`)
      }
    }

    expect(
      missing,
      `These AltruCoderProvider instances are missing setContinueInWorktreeHandler.\n` +
  ...
  it("openAltruCoderInNewTab wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const fn = ext.indexOf("function openAltruCoderInNewTab")
    expect(fn, "openAltruCoderInNewTab must exist").toBeGreaterThan(-1)
  ...
  it("TabPanel deserializer wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const serializer = ext.indexOf('"altru-coder.new.TabPanel"')
...
// ---------------------------------------------------------------------------
// AltruCoderProvider — continueInWorktree error fallback
//
...
describe("AltruCoderProvider — continueInWorktree error fallback", () => {
  const helper = fs.readFileSync(path.join(ROOT, "src/altru-coder-provider/continue-worktree.ts"), "utf-8")
