...
async function createTempRepo(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "altru-coder-wt-"))
  tempDirs.push(dir)
...
async function createTempRepoWithOrigin(): Promise<{ bare: string; clone: string }> {
  ...
  const seed = await fs.mkdtemp(path.join(os.tmpdir(), "altru-coder-wt-seed-"))
  tempDirs.push(seed)
  ...
  const bare = await fs.mkdtemp(path.join(os.tmpdir(), "altru-coder-wt-bare-"))
  tempDirs.push(bare)
  await simpleGit().clone(seed, bare, ["--bare"])

  const clone = await fs.mkdtemp(path.join(os.tmpdir(), "altru-coder-wt-clone-"))
  tempDirs.push(clone)
...
describe("WorktreeStateManager.updateWorktreeLabel", () => {
  it("persists label on a worktree", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "altru-coder-wt-label-"))
    tempDirs.push(dir)
...
describe("WorktreeManager.createWorktree", () => {
  ...
  it("throws when workspace is not a git repo", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "altru-coder-wt-nogit-"))
    tempDirs.push(dir)
  ...
  it("creates worktrees directory under .altru-coder/worktrees/", async () => {
    const root = await createTempRepo()
    const mgr = createManager(root)

    const result = await mgr.createWorktree({ prompt: "test" })

    expect(result.path).toContain(path.join(".altru-coder", "worktrees"))
  })
...
describe("WorktreeManager.removeWorktree", () => {
  ...
  it("does not throw when worktree path does not exist", async () => {
    ...
    // Should not throw
    await mgr.removeWorktree(path.join(root, ".altru-coder", "worktrees", "nonexistent"))
  })
  ...
  it("removes orphaned directory that git does not know about", async () => {
    ...
    const orphanPath = path.join(root, ".altru-coder", "worktrees", "orphan")
  ...
  it(
    ...
    async () => {
      ...
      const worktreesDir = path.join(root, ".altru-coder", "worktrees")
      const deadline = Date.now() + 5000
      while (Date.now() < deadline) {
        const entries = await fs.readdir(worktreesDir)
        if (!entries.some((e) => e.startsWith(".altru-coder-delete-"))) break
        await new Promise((r) => setTimeout(r, 100))
      }

      // No .altru-coder-delete-* temp dirs should remain
      const entries = await fs.readdir(worktreesDir)
      const orphans = entries.filter((e) => e.startsWith(".altru-coder-delete-"))
...
describe("WorktreeManager.discoverWorktrees orphan cleanup", () => {
  it("cleans up .altru-coder-delete-* dirs left by interrupted deletions", async () => {
    const root = await createTempRepo()
    const mgr = createManager(root)
    ...
    const orphan = path.join(root, ".altru-coder", "worktrees", ".altru-coder-delete-fake-uuid")
...
describe("WorktreeManager metadata", () => {
  ...
  it("writes metadata outside the worktree checkout", async () => {
    ...
    expect(existsSync(path.join(result.path, ".altru-coder", "session-id"))).toBe(false)
    expect(existsSync(path.join(result.path, ".altru-coder", "metadata.json"))).toBe(false)
  ...
  it("reads legacy session-id file when metadata.json is missing", async () => {
    ...
    const dir = path.join(result.path, ".altru-coder")
...
describe("WorktreeManager.discoverWorktrees", () => {
  ...
  it("repairs stale gitdir refs when .altru-coder/worktrees already exists", async () => {
    const root = await createTempRepo()
    const mgr = createManager(root)

    const worktree = path.join(root, ".altru-coder", "worktrees", "partial")
    const gitdir = path.join(root, ".git", "worktrees", "partial", "gitdir")
    await fs.mkdir(worktree, { recursive: true })
    await fs.mkdir(path.dirname(gitdir), { recursive: true })
    await fs.writeFile(gitdir, path.join(root, ".altrucoder", "worktrees", "partial", ".git"), "utf-8")

    await mgr.discoverWorktrees()

    const fixed = await fs.readFile(gitdir, "utf-8")
    expect(fixed).toContain(path.join(root, ".altru-coder", "worktrees", "partial", ".git"))
  })
})
...
describe("WorktreeManager.ensureGitExclude", () => {
  it("adds .altru-coder/worktrees/ to .git/info/exclude", async () => {
    const root = await createTempRepo()
    const mgr = createManager(root)
    ...
    expect(content).toContain(".altru-coder/worktrees/")
  })
  ...
  it("adds only specific legacy Agent Manager paths", async () => {
    ...
    const content = await fs.readFile(path.join(root, ".git", "info", "exclude"), "utf-8")
    expect(content).toContain(".altrucoder/worktrees/")
    expect(content).toContain(".altrucoder/agent-manager.json")
    expect(content).toContain(".altrucoder/setup-script")
  ...
  it("is idempotent -- does not duplicate entries", async () => {
    ...
    const content = await fs.readFile(path.join(root, ".git", "info", "exclude"), "utf-8")
    const count = content.split(".altru-coder/worktrees/").length - 1
    expect(count).toBe(1)
  })
})
...
describe("WorktreeManager.removeWorktree safety", () => {
  it("refuses to remove paths outside the worktrees directory", async () => {
    ...
    // Create a directory outside .altru-coder/worktrees/
