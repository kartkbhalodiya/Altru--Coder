 ...
 * ## Why this exists
 *
 * The sidebar AltruCoderProvider always uses `getWorkspaceDirectory()`, which returns
 ...
 *
 * ## How AltruCoderProvider uses this
 *
 * Each AltruCoderProvider instance can receive an explicit `projectDirectory` via
 * `AltruCoderProviderOptions`. When set:
 *
...
/**
 * Resolve the effective project directory for a AltruCoderProvider instance.
 *
 * @param override - Explicit directory from AltruCoderProviderOptions. `undefined`
 ...
 *   `getWorkspaceDirectory(sessionId)` from AltruCoderProvider).
 */
