...
const PANEL_TITLES: Record<PanelView, string> = {
  settings: "Altru Coder Settings",
  profile: "Altru Coder Profile",
  marketplace: "Altru Coder Marketplace",
 ...
 *
 * Uses a full AltruCoderProvider under the hood so each panel has
...
export class SettingsEditorProvider implements vscode.Disposable {
  ...
  private providers = new Map<PanelView, AltruCoderProvider>()
  ...
  /** Extract the PanelView from a viewType string like "altru-coder.new.settingsPanel". */
  static viewFromType(type: string): PanelView | undefined {
    const match = type.match(/^altru-coder\.new\.(\w+)Panel$/)
  ...
  private wirePanel(panel: vscode.WebviewPanel, view: PanelView, projectDirectory: string | null): void {
    panel.iconPath = {
      light: vscode.Uri.joinPath(this.extensionUri, "assets", "icons", "altru-logo.png"),
      dark: vscode.Uri.joinPath(this.extensionUri, "assets", "icons", "altru-logo.png"),
    }

    // Create a dedicated AltruCoderProvider for this panel so it has full
    // backend connectivity (config, providers, agents, profile, auth).
    const provider = new AltruCoderProvider(this.extensionUri, this.connectionService, this.context, {
      projectDirectory,
    })
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage((msg) => {
      if (msg.type === "webviewReady") {
        // Small delay to let AltruCoderProvider's own webviewReady handler finish first
        setTimeout(() => {
    ...
    panel.onDidDispose(() => {
      console.log(`[Altru Coder New] ${title} panel disposed`)
