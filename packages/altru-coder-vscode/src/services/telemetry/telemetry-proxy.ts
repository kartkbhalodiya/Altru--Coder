...
export class TelemetryProxy {
  ...
  capture(event: TelemetryEventName, properties?: Record<string, unknown>) {
    ...
    fetch(`${this.url}/telemetry/capture`, {
      ...
      body: payload,
    }).catch((err) => console.error("[Altru Coder New] Telemetry capture failed:", err))
  }

  /**
   * Propagate runtime telemetry consent changes to the CLI. The CLI subprocess
   * reads `ALTRU_CODER_TELEMETRY_LEVEL` once at spawn — without this call, toggling
  ...
  setEnabled(enabled: boolean) {
    ...
    }).catch((err) => console.error("[Altru Coder New] Telemetry setEnabled failed:", err))
  }
