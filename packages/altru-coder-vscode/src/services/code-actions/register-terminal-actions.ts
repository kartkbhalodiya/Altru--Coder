...
export function registerTerminalActions(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("altru-coder.new.terminalAddToContext", async (args: any) => {
    ...
    vscode.commands.registerCommand("altru-coder.new.terminalFixCommand", async (args: any) => {
      let content = args?.selection as string | undefined
      if (!content) {
        content = (await getTerminalContents(1)).content
      }
    ...
    vscode.commands.registerCommand("altru-coder.new.terminalExplainCommand", async (args: any) => {
      let content = args?.selection as string | undefined
      if (!content) {
        content = (await getTerminalContents(1)).content
      }
