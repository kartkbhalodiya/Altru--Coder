...
export class RemoteStatusService implements vscode.Disposable {
  ...
  constructor() {
    ...
    this.bar.command = "altru-coder.new.toggleRemote"
    this.sync()
  }

  setClient(c: AltruCoderClient | null): void {
    this.client = c
  }
  ...
  async refresh(): Promise<void> {
    ...
    const res = await this.client.remote.status().catch((err: unknown) => {
      console.warn("[Altru Coder] remote status refresh failed:", err)
      return undefined
    })
    if (!res?.data) return
  ...
  private sync(): void {
    ...
    if (this.state.connected) {
      this.bar.text = "$(radio-tower) Altru Coder Remote"
