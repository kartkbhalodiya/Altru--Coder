...
export class MarketplaceInstaller {
  ...
  async installSkill(
    ...
    const tarball = path.join(os.tmpdir(), `altru-coder-skill-${item.id}-${stamp}.tar.gz`)
...
/**
 * Normalize a marketplace MCP entry from the old AltruCoder format to the CLI's expected format.
 *
