...
import { AltruCoderConnectionService } from "./connection-service"

describe("AltruCoderConnectionService drainPendingPrompts", () => {
  test("ignores stale NotFoundError replies while draining permissions", async () => {
    const service = new AltruCoderConnectionService({} as any)
    const client = {
      project: {
        list: async () => ({ data: [] }),
      },
