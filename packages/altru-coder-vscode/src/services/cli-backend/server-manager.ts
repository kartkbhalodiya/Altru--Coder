...
export function resolveIndexingEnv(folders: readonly WorkspaceFolderLike[] | undefined): Record<string, string> {
  ...
  return { ALTRU_CODER_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" }
}
...
export class ServerManager {
  ...
  async getServer(): Promise<ServerInstance> {
    console.log("[Altru Coder New] ServerManager: 🔍 getServer called")
    if (this.instance) {
      console.log("[Altru Coder New] ServerManager: ♻️ Returning existing instance:", { port: this.instance.port })
      return this.instance
    }

    if (this.startupPromise) {
      console.log("[Altru Coder New] ServerManager: ⏳ Startup already in progress, waiting...")
      return this.startupPromise
    }

    console.log("[Altru Coder New] ServerManager: 🚀 Starting new server instance...")
    this.startupPromise = this.startServer()
    try {
      this.instance = await this.startupPromise
      console.log("[Altru Coder New] ServerManager: ✅ Server started successfully:", { port: this.instance.port })
      return this.instance
    } finally {
  ...
  private async startServer(): Promise<ServerInstance> {
    ...
    console.log("[Altru Coder New] ServerManager: 📍 CLI path:", cliPath)
    console.log("[Altru Coder New] ServerManager: 🔐 Generated password (length):", password.length)
    ...
    console.log("[Altru Coder New] ServerManager: 📄 CLI isFile:", stat.isFile())
    console.log("[Altru Coder New] ServerManager: 📄 CLI mode (octal):", (stat.mode & 0o777).toString(8))

    return new Promise((resolve, reject) => {
      console.log("[Altru Coder New] ServerManager: 🎬 Spawning CLI process:", cliPath, ["serve", "--port", "0"])
      const cfg = vscode.workspace.getConfiguration("altru-coder.new")
      ...
      //     `altru-coder.new.extraCaCerts` (NODE_EXTRA_CA_CERTS).
      ...
      const serverProcess = spawn(cliPath, ["serve", "--port", "0"], {
        ...
        env: {
          ...
          ALTRU_CODER_SERVER_PASSWORD: password,
          ALTRU_CODER_CLIENT: "vscode",
          ALTRU_CODER_ENABLE_QUESTION_TOOL: "true",
          ALTRUCODER_FEATURE: "vscode-extension",
          ...indexingEnv,
          ALTRU_CODER_TELEMETRY_LEVEL: vscode.env.isTelemetryEnabled ? "all" : "off",
          ALTRU_CODER_APP_NAME: "altru-coder",
          ALTRU_CODER_EDITOR_NAME: vscode.env.appName,
          ALTRU_CODER_PLATFORM: "vscode",
          ALTRU_CODER_MACHINE_ID: vscode.env.machineId,
          ALTRU_CODER_APP_VERSION: this.context.extension.packageJSON.version,
          ALTRU_CODER_VSCODE_VERSION: vscode.version,
          ALTRUCODER_EDITOR_NAME: `${vscode.env.appName} ${vscode.version}`,
          ...(!claudeCompat && { ALTRU_CODER_DISABLE_CLAUDE_CODE: "true" }),
        },
        stdio: ["ignore", "pipe", "pipe"],
        detached: true,
      })
      console.log("[Altru Coder New] ServerManager: 📦 Process spawned with PID:", serverProcess.pid)
      ...
      serverProcess.stdout?.on("data", (data: Buffer) => {
        ...
        console.log("[Altru Coder New] ServerManager: 📥 CLI Server stdout:", output)

        const port = parseServerPort(output)
        if (port !== null && !resolved) {
          resolved = true
          console.log("[Altru Coder New] ServerManager: 🎯 Port detected:", port)
      ...
      serverProcess.stderr?.on("data", (data: Buffer) => {
        ...
        console.error("[Altru Coder New] ServerManager: ⚠️ CLI Server stderr:", errorOutput)
        stderrLines.push(errorOutput)
      })

      serverProcess.on("error", (error) => {
        console.error("[Altru Coder New] ServerManager: ❌ Process error:", error)
        if (!resolved) {
          reject(error)
        }
      })
      ...
      serverProcess.on("exit", (code) => {
        console.log("[Altru Coder New] ServerManager: 🛑 Process exited with code:", code)
      ...
      setTimeout(() => {
        if (!resolved) {
          console.error(`[Altru Coder New] ServerManager: ⏰ Server startup timeout (${STARTUP_TIMEOUT_SECONDS}s)`)
  ...
  private getCliPath(): string {
    ...
    const binName = process.platform === "win32" ? "altru-coder.exe" : "altru-coder"
    const cliPath = path.join(this.context.extensionPath, "bin", binName)
    console.log("[Altru Coder New] ServerManager: 📦 Using CLI path:", cliPath)
    return cliPath
  }
  ...
  dispose(): void {
    ...
    console.log("[Altru Coder New] ServerManager: 🔴 Disposing — sending SIGTERM to process group, PID:", proc.pid)
    ...
    const timer = setTimeout(() => {
      if (proc.exitCode === null) {
        console.warn("[Altru Coder New] ServerManager: ⚠️ Process did not exit after SIGTERM, sending SIGKILL")
