import type { AltruCoderClient, GlobalEvent, Event } from "@altru-coder/sdk/v2/client"
...
export class SdkSSEAdapter {
  ...
  constructor(private readonly client: AltruCoderClient) {}
  ...
  connect(): void {
    if (this.abortController) {
      console.log("[Altru Coder New] SSE: ⚠️ Already connected, skipping")
      return
    }

    console.log("[Altru Coder New] SSE: 🔌 connect() called")
    this.abortController = new AbortController()
    console.log('[Altru Coder New] SSE: 🔄 Setting state to "connecting"')
    this.notifyState("connecting")
    void this.consumeLoop(this.abortController.signal).catch((err) => {
      console.error("[Altru Coder New] SSE: Unhandled error in consumeLoop:", err)
   ...
   */
  disconnect(): void {
    console.log("[Altru Coder New] SSE: 🔌 disconnect() called")
  ...
  reconnect(): void {
    if (!this.attemptController) {
      console.log("[Altru Coder New] SSE: ⚠️ reconnect() called but no active attempt")
      return
    }
    console.log("[Altru Coder New] SSE: 🔄 reconnect() — aborting current attempt")
  ...
  private async consumeLoop(signal: AbortSignal): Promise<void> {
    while (!signal.aborted) {
      ...
      try {
        console.log("[Altru Coder New] SSE: 🎬 Calling SDK global.event()...")
        const events = await this.client.global.event({
          ...
          onSseError: (error) => {
            ...
            console.error("[Altru Coder New] SSE: ❌ SDK SSE error callback:", error)
            this.notifyError(error instanceof Error ? error : new Error(String(error)))
          },
        })

        console.log("[Altru Coder New] SSE: ✅ Stream opened successfully")
        ...
        for await (const event of events.stream) {
          ...
          if (type !== "server.heartbeat") {
            console.log("[Altru Coder New] SSE: 📨 Event:", type)
          }
          this.notifyEvent(globalEvent.payload as Event, globalEvent.directory)
        }

        console.log("[Altru Coder New] SSE: 📭 Stream ended normally")
      } catch (error) {
        ...
        if (!aborted) {
          console.error("[Altru Coder New] SSE: ❌ Stream error:", error)
          this.notifyError(error instanceof Error ? error : new Error(String(error)))
        }
      } finally {
      ...
      console.log(`[Altru Coder New] SSE: 🔄 Reconnecting in ${SdkSSEAdapter.RECONNECT_DELAY_MS}ms...`)
      this.notifyState("connecting")
  ...
  private resetHeartbeat(attempt: AbortController): void {
    ...
    this.heartbeatTimer = setTimeout(() => {
      console.log("[Altru Coder New] SSE: ⏰ Heartbeat timeout — aborting stale connection")
      attempt.abort()
  ...
  private notifyEvent(event: Event, directory?: string): void {
    for (const handler of this.handlers) {
      try {
        ...
        console.error("[Altru Coder New] SSE: Error in event handler:", error)
      }
    }
  }
  ...
  private notifyError(error: Error): void {
    for (const handler of this.errorHandlers) {
      try {
        ...
        console.error("[Altru Coder New] SSE: Error in error handler:", err)
      }
    }
  }
  ...
  private notifyState(state: "connecting" | "connected" | "disconnected"): void {
    for (const handler of this.stateHandlers) {
      try {
        ...
        console.error("[Altru Coder New] SSE: Error in state handler:", error)
      }
    }
  }
}
