 ...
 * The network namespace is not yet in the SDK AltruCoderClient type (pending SDK regeneration),
 * so we access it via a type assertion.
 */
async function drainNetworkWaits(client: AltruCoderClient, dir: string) {
...
/**
 * Shared connection service that owns the single ServerManager, AltruCoderClient (SDK), and SdkSSEAdapter.
 * Multiple AltruCoderProvider instances subscribe to it for SSE events and state changes.
 */
export class AltruCoderConnectionService {
  private readonly serverManager: ServerManager
  private client: AltruCoderClient | null = null
   ...
   */
  async getClientAsync(dir?: string): Promise<AltruCoderClient> {
  ...
  /**
   * Subscribe to notification dismiss events broadcast from any AltruCoderProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a notification dismiss event to all subscribed AltruCoderProvider instances.
   */
  ...
  /**
   * Subscribe to language change events broadcast from any AltruCoderProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a language change event to all subscribed AltruCoderProvider instances.
   */
  ...
  /**
   * Subscribe to profile change events broadcast from any AltruCoderProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a profile change event to all subscribed AltruCoderProvider instances.
   */
  ...
  /**
   * Subscribe to migration-complete events broadcast from any AltruCoderProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a migration-complete event to all subscribed AltruCoderProvider instances.
   */
  ...
  /**
   * Subscribe to favorites change events broadcast from any AltruCoderProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a favorites change event to all subscribed AltruCoderProvider instances.
   */
   ...
   * AltruCoderProvider (root + worktree dirs). Used by drainPendingPrompts() to
   ...
   * directory known to any currently-mounted AltruCoderProvider.
   *
  ...
  flushViewed(): void {
    ...
    this.debounceTimer = setTimeout(() => {
      ...
      this.client?.session
        ...
        .catch((err) => console.warn("[Altru Coder New] ConnectionService: viewed flush failed:", err))
    }, 150)
  }
  ...
  private startHealthPoll(baseUrl: string, password: string): void {
    ...
    this.healthPollTimer = setInterval(async () => {
      ...
      if (!healthy && this.state === "connected") {
        console.warn("[Altru Coder New] ConnectionService: ❤️‍🩹 Health check failed — forcing SSE reconnect")
  ...
  private async checkHealth(baseUrl: string, password: string): Promise<boolean> {
    try {
      ...
      const res = await fetch(`${baseUrl}/global/health`, {
        headers: { Authorization: `Basic ${Buffer.from(`altru-coder:${password}`).toString("base64")}` },
  ...
  private async doConnect(workspaceDir: string): Promise<void> {
    ...
    const authHeader = `Basic ${Buffer.from(`altru-coder:${server.password}`).toString("base64")}`
    this.client = createAltruCoderClient({
...
async function drainSuggestions(client: AltruCoderClient, directory: string): Promise<void> {
