...
// equivalents in @altru-coder/sdk. All API types (Session, Event, Agent,
// McpStatus, Config, etc.) should be imported from "@altru-coder/sdk/v2/client".

import type { IndexingStatus as SdkIndexingStatus } from "@altru-coder/sdk/v2/client"
...
export interface AltruCoderOrganization {
  id: string
  name: string
  role: string
}

export interface AltruCoderProfile {
  email: string
  name?: string
  organizations?: AltruCoderOrganization[]
}
...
// Cloud session from the Altru Coder cloud API (cli_sessions_v2)
...
// Full cloud session data for preview (from /altru-coder/cloud/session/:id)
