...
const CONFIG_SECTION = "altru-coder.new.autocomplete"
...
export class AutocompleteServiceManager {
  ...
  constructor(context: vscode.ExtensionContext, connectionService: AltruCoderConnectionService) {
  ...
  private async updateGlobalContext() {
    await vscode.commands.executeCommand(
      "setContext",
      "altrucoder.autocomplete.enableSmartInlineTaskKeybinding",
  ...
  private handleFatalAutocompleteError(status: number | null): void {
    const msg =
      status === 402
        ? t("altrucoder:autocomplete.creditsExhausted.message")
        : t("altrucoder:autocomplete.authError.message")
  ...
  public async showIncompatibilityExtensionPopup() {
    const message = t("altrucoder:autocomplete.incompatibilityExtensionPopup.message")
    const disableCopilot = t("altrucoder:autocomplete.incompatibilityExtensionPopup.disableCopilot")
    const disableInlineAssist = t("altrucoder:autocomplete.incompatibilityExtensionPopup.disableInlineAssist")
    ...
    if (response === disableCopilot) {
      ...
      await vscode.commands.executeCommand("altru-coder.new.autocomplete.disable")
    }
  }
