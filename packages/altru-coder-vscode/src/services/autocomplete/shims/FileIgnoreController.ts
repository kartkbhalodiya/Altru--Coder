...
const ALTRUCODERIGNORE = ".altrucoderignore"
const GITIGNORE = ".gitignore"

/**
 * Patterns for sensitive environment files, applied only when no .altrucoderignore exists.
 */
...
export class FileIgnoreController {
  ...
  async initialize(): Promise<void> {
    ...
    // Try .altrucoderignore first — if it exists, use only that.
    ...
    const altrucoderignorePath = path.join(this.workspacePath, ALTRUCODERIGNORE)
    if (fs.existsSync(altrucoderignorePath)) {
      const altrucoderignoreContent = fs.readFileSync(altrucoderignorePath, "utf-8")
      if (altrucoderignoreContent.trim()) {
        this.ignoreInstance.add(altrucoderignoreContent)
        this.ignoreInstance.add(ALTRUCODERIGNORE)
        this.loadedContents.push({ file: ALTRUCODERIGNORE, content: altrucoderignoreContent })
        return
      }
    }
