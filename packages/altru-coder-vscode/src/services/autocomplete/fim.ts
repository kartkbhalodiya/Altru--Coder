 ...
 * Uses the SDK's altru-coder.fim() SSE endpoint which handles auth and streaming.
 *
...
export async function generateFim(
  ...
  const { stream } = await client.altruCoder.fim(
    {
      prefix,
      suffix,
      model: modelId,
 ...
 */
export function hasValidCredentials(connectionService: AltruCoderConnectionService): boolean {
