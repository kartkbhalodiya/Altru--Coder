 ...
 */
export const INLINE_COMPLETION_ACCEPTED_COMMAND = "altrucoder.autocomplete.inline-completion.accepted"
...
export class AutocompleteInlineCompletionProvider implements vscode.InlineCompletionItemProvider {
  ...
  private connectionService: AltruCoderConnectionService
  ...
  public async provideInlineCompletionItems_Internal(
    ...
    try {
      ...
      if (matchingResult !== null) {
        ...
        vscode.commands.executeCommand("setContext", "altru-coder.new.autocomplete.hasSuggestions", true)
  ...
  private async hasBalance(): Promise<boolean> {
    try {
      ...
      const result = await client.altruCoder.profile().catch(() => null)
