 ...
 * .altrucoderignore / .gitignore files are modified) and shares a single
...
export class ChatTextAreaAutocomplete {
  private connection: AltruCoderConnectionService
  readonly telemetry: AutocompleteTelemetry
  private ignore: FileIgnoreController | null = null
  private dir = ""
  private watcher: vscode.FileSystemWatcher | undefined

  constructor(connectionService: AltruCoderConnectionService, telemetry?: AutocompleteTelemetry) {
    this.connection = connectionService
    this.telemetry = telemetry ?? new AutocompleteTelemetry("chat-textarea")
    this.watcher = vscode.workspace.createFileSystemWatcher("**/{.altrucoderignore,.gitignore}")
    const invalidate = () => {
  ...
  async getCompletion(userText: string, visibleCodeContext?: VisibleCodeContext): Promise<{ suggestion: string }> {
    const cfg = vscode.workspace.getConfiguration("altru-coder.new.autocomplete")
