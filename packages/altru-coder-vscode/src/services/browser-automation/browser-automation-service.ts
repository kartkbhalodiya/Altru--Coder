...
import type { AltruCoderClient, McpStatus } from "@altru-coder/sdk/v2/client"
...
export class BrowserAutomationService implements vscode.Disposable {
  ...
  private static readonly MCP_SERVER_NAME = "altru-coder-playwright"

  constructor(private readonly connectionService: AltruCoderConnectionService) {
    // Listen for settings changes
    this.disposables.push(
      vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration("altru-coder.new.browserAutomation")) {
  ...
  private async register(): Promise<void> {
    ...
    if (!client) {
      console.error("[Altru Coder New] BrowserAutomationService: No SDK client available")
    ...
    try {
      ...
      if (serverStatus?.status === "connected") {
        ...
        console.error(
          "[Altru Coder New] BrowserAutomationService: MCP server failed:",
      ...
      console.error("[Altru Coder New] BrowserAutomationService: Failed to register MCP server:", error)
      this.setState("failed")
    }
  }
  ...
  private async unregister(): Promise<void> {
    ...
    if (client) {
      try {
        ...
        console.error("[Altru Coder New] BrowserAutomationService: Failed to disconnect MCP server:", error)
      }
    }
  ...
  private getClient(): AltruCoderClient | null {
    try {
  ...
  private setState(state: BrowserAutomationState): void {
    ...
    console.log(`[Altru Coder New] BrowserAutomationService: State ${this.state} → ${state}`)
    this.state = state
