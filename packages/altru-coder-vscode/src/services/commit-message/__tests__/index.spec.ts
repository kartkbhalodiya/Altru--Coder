...
describe("commit-message service", () => {
  ...
  let mockConnectionService: AltruCoderConnectionService
  ...
  describe("registerCommitMessageService", () => {
    ...
    it("registers the altru-coder.new.generateCommitMessage command", () => {
  ...
  describe("command execution", () => {
    ...
    it("shows error when backend fails to connect", async () => {
      ...
      expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
        "Failed to connect to Altru Coder backend. Please try again.",
      )
    })
