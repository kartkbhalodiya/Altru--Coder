 ...
 *
 * Uses a full AltruCoderProvider so the viewer has backend connectivity
...
export class SubAgentViewerProvider implements vscode.Disposable {
  ...
  private providers = new Map<string, AltruCoderProvider>()
  ...
  openPanel(sessionID: string, title?: string): void {
    ...
    const panel = vscode.window.createWebviewPanel("altru-coder.new.SubAgentViewerPanel", label, vscode.ViewColumn.One, {
      enableScripts: true,
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage(async (msg) => {
      ...
      try {
        ...
        console.error("[Altru Coder New] SubAgentViewerProvider: Failed to load session:", err)
      }
    })
    ...
    panel.onDidDispose(() => {
      console.log("[Altru Coder New] Sub-agent viewer panel disposed:", sessionID)
