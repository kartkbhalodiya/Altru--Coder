 ...
 *
 * Reads legacy Altru Coder v5.x data from VS Code SecretStorage and the extension's
...
const MIGRATION_STATUS_KEY = "altru-coder.legacyMigrationStatus"
...
export async function clearLegacyData(context: vscode.ExtensionContext): Promise<void> {
  ...
  const legacyStateKeys = [
    "altru-coder.allowedCommands",
    "altru-coder.deniedCommands",
    "altru-coder.autoApprovalEnabled",
    "altru-coder.fuzzyMatchThreshold",
    "altru-coder.diffEnabled",
    "altru-coder.language",
    "altru-coder.customModes",
    "altru-coder.firstInstallCompleted",
    "altru-coder.telemetrySetting",
  ...
  // Clear legacy VS Code settings registered under the "altru-coder" configuration scope.
  ...
  const cfg = vscode.workspace.getConfiguration("altru-coder")
...
async function migrateProvider(
  ...
  // The profile endpoint requires type:"oauth". The legacy extension stored the same Altru Coder
  // API token — write it in the OAuth format the new extension expects (matching device-auth:
  // access + refresh + 1-year expiry).
  if (mapping.id === "altru-coder") {
  ...
  // For providers that support an organization ID (e.g. Altru Coder Gateway), migrate using OAuth
...
async function migrateDefaultModel(settings: LegacyProviderSettings, client: AltruCoderClient): Promise<MigrationResultItem> {
  const provider = settings.apiProvider
  if (!provider) {
...
async function migrateAutocomplete(settings: LegacyAutocompleteSettings): Promise<MigrationResultItem> {
  try {
    const config = vscode.workspace.getConfiguration("altru-coder.new.autocomplete")
...
function readLegacySettings(context: vscode.ExtensionContext): LegacySettings {
  ...
  return {
    autoApprovalEnabled: context.globalState.get<boolean>("altru-coder.autoApprovalEnabled"),
    allowedCommands: context.globalState.get<string[]>("altru-coder.allowedCommands"),
    deniedCommands: context.globalState.get<string[]>("altru-coder.deniedCommands"),
    ...
    language: context.globalState.get<string>("altru-coder.language"),
