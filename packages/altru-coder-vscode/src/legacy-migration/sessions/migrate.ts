...
export async function migrate(
  ...
  try {
    ...
    const project = await client.altrucoder.sessionImport.project(payload.project, { throwOnError: true })
    const projectID = project.data?.id ?? payload.project.id
    const session = await client.altrucoder.sessionImport.session(
      {
        ...payload.session,
        projectID,
    ...
    for (const msg of payload.messages) {
      await client.altrucoder.sessionImport.message(msg, { throwOnError: true })
    }

    for (const part of payload.parts) {
      await client.altrucoder.sessionImport.part(part, { throwOnError: true })
    }
