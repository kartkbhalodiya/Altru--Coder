import type { Session, Agent, Event, ProviderListResponse } from "@altru-coder/sdk/v2/client"
import { prettifyError } from "zod/v4"
import type { CloudSessionMessage, IndexingStatus } from "./services/cli-backend/types"
import type { PartBatch, PartUpdate } from "./altru-coder-provider/session-stream-scheduler"
import type { PartRemove } from "./shared/stream-messages"

export { SessionStreamScheduler } from "./altru-coder-provider/session-stream-scheduler"
...
function safeStringify(value: unknown): string | undefined {
  try {
    ...
    console.warn("[Altru Coder New] getErrorMessage: JSON.stringify failed", err)
  }
  return undefined
}
...
export async function runWithMessageConfirmation<T>(
  ...
  try {
    ...
    if (await state.wait(id)) {
      console.warn(`[Altru Coder New] ${label} ended after server accepted it; ignoring transport error`, {
...
/**
 * Shared interface for the subset of AltruCoderProvider state needed by session-refresh helpers.
 * Extracted here so the logic can be tested without importing AltruCoderProvider (and vscode).
 */
  ...
  const extra = await Promise.all(
    [...worktreeDirs].map((dir) =>
      list(dir).catch((err: unknown) => {
        console.error(`[Altru Coder] Failed to list sessions for ${dir}:`, err)
        failed.add(dir)
        return [] as Session[]
      }),
    ),
