/**
 * Permission handlers — extracted from AltruCoderProvider.
 *
...
import type { AltruCoderClient, PermissionRequest } from "@altru-coder/sdk/v2/client"
...
export async function handlePermissionResponse(
  ...
  if (!target) {
    console.error("[Altru Coder New] AltruCoderProvider: No sessionID for permission response")
    ctx.postMessage({ type: "permissionError", permissionID: permissionId })
    return
  }
  ...
  if (approvedAlways.length > 0 || deniedAlways.length > 0) {
    const saveResult = await ctx.client.permission
      ...
      .catch((error: unknown) => {
        ...
        console.error("[Altru Coder New] AltruCoderProvider: Failed to save always-rules:", error)
        ctx.postMessage({ type: "permissionError", permissionID: permissionId })
  ...
  const replyResult = await ctx.client.permission
    ...
    .catch((error: unknown) => {
      if (isNotFoundError(error)) return "stale" as const
      console.error("[Altru Coder New] AltruCoderProvider: Failed to respond to permission:", error)
      ctx.postMessage({ type: "permissionError", permissionID: permissionId })
      return "error" as const
    })
...
export async function fetchAndSendPendingPermissions(ctx: PermissionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[Altru Coder New] AltruCoderProvider: Failed to fetch pending permissions:", error)
  }
}
