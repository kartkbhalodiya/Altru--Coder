/**
 * Question handlers — extracted from AltruCoderProvider.
 *
...
export async function fetchAndSendPendingQuestions(ctx: QuestionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[Altru Coder New] AltruCoderProvider: Failed to fetch pending questions:", error)
  }
}
...
export async function handleQuestionReply(
  ...
  try {
    ...
    console.error("[Altru Coder New] AltruCoderProvider: Failed to reply to question:", error)
    ctx.postMessage({ type: "questionError", requestID })
    return false
  }
}
...
export async function handleQuestionReject(
  ...
  try {
    ...
    console.error("[Altru Coder New] AltruCoderProvider: Failed to reject question:", error)
    ctx.postMessage({ type: "questionError", requestID })
    return false
  }
}
