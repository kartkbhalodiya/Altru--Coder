/**
 * Cloud session handlers — extracted from AltruCoderProvider.
 *
...
export async function handleRequestCloudSessions(
  ...
  try {
    const result = await ctx.client.altruCoder.cloudSessions({
    ...
    console.error("[Altru Coder New] AltruCoderProvider: Failed to fetch cloud sessions:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleRequestCloudSessionData(ctx: CloudSessionContext, sessionId: string): Promise<void> {
  ...
  try {
    const result = await ctx.client.altruCoder.cloud.session.get({ id: sessionId })
    ...
    console.error("[Altru Coder New] Failed to load cloud session data:", err)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId: sessionId,
...
export async function handleImportAndSend(
  ...
  try {
    const result = await ctx.client.altruCoder.cloud.session.import({
      sessionId: cloudSessionId,
      directory: dir,
    })
    session = result.data as Session | undefined
  } catch (error) {
    console.error("[Altru Coder New] AltruCoderProvider: ❌ Cloud session import failed:", error)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId,
  ...
  try {
    ...
    console.error("[Altru Coder New] Failed to send message after cloud import:", err)
    ctx.postMessage({
