/**
 * Legacy migration handlers — extracted from AltruCoderProvider.
 *
 * Manages the migration wizard for users upgrading from Altru Coder v5.x.
...
export async function checkAndShowMigrationWizard(ctx: MigrationContext): Promise<void> {
  ...
  console.log("[Altru Coder New] AltruCoderProvider: 🔄 Legacy data detected, showing migration wizard")
  ctx.postMessage({
    type: "migrationState",
    needed: true,
    data: {
...
export async function handleStartLegacyMigration(
  ...
  try {
    ...
    console.error("[Altru Coder New] AltruCoderProvider: ❌ Migration failed", error)
    ctx.postMessage({
