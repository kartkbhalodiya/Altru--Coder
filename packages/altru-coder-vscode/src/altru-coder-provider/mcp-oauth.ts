  ...
  void vscode.env.openExternal(vscode.Uri.parse(url)).then(
    (opened) => {
      ...
      void vscode.window.showErrorMessage(
        "MCP sign-in failed to open the browser. Check the Altru Coder logs for the authentication URL.",
      )
    },
    (error) => {
      console.error("[Altru Coder New] Failed to open MCP OAuth URL:", error)
      void vscode.window.showErrorMessage(
        "MCP sign-in failed to open the browser. Check the Altru Coder logs for the authentication URL.",
      )
    },
...
export async function connectMcpServer(
  ...
  try {
    ...
    console.error("[Altru Coder New] Failed to connect MCP:", name, error)
    await refreshStatus()
  }
}
...
export async function disconnectMcpServer(
  ...
  try {
    ...
    console.error("[Altru Coder New] Failed to disconnect MCP:", name, error)
    await refreshStatus()
  }
}
...
export async function authenticateMcpServer(
  ...
  try {
    ...
    console.error("[Altru Coder New] Failed to authenticate MCP:", name, error)
