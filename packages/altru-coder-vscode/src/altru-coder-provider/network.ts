 ...
 * user to press Enter; `altru-coder run` auto-retries with backoff. This module implements auto-reply
 ...
 */
export function handleNetworkEvent(type: string, props: Props, client: AltruCoderClient | null, dir: GetDir) {
  ...
  if (type === "session.network.restored" && props.requestID) {
    ...
    if (!entry) return
    console.log("[Altru Coder New] network: auto-replying to restore", props.requestID)
