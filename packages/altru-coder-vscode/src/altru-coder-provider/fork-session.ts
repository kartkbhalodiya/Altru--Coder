...
export async function handleForkSession(ctx: ForkContext, sessionId: string, messageId?: string): Promise<void> {
  const status =
    ...
    (await Promise.resolve()
      ...
      .catch((e) => {
        console.error("[Altru Coder New] refreshForkStatus failed:", e)
  ...
  await forkSession(
    {
      ...
      registerSession: () => {},
      log: (...args) => console.log("[Altru Coder New] AltruCoderProvider:", ...args),
    },
    sessionId,
    undefined,
    messageId,
