...
export function getWebviewFontSize(): number {
  const raw = vscode.workspace.getConfiguration("altru-coder.new").get<number>("fontSize", 13)
  return clamp(raw)
}

function fontStyle(): string {
  const base = getWebviewFontSize()
  const vars = SIZES.map((size) => `--altru-coder-font-size-${size}: ${(base * size) / 13}px;`).join("\n      ")
  return `:root {
      ${vars}
      --altru-coder-font-scale: ${base / 13};
      --font-size-x-small: var(--altru-coder-font-size-10);
      --font-size-small: var(--altru-coder-font-size-11);
      --font-size-base: var(--altru-coder-font-size-13);
      --font-size-large: var(--altru-coder-font-size-16);
    }`
}
...
export function buildWebviewHtml(
  ...
  return `<!DOCTYPE html>
<html lang="en" data-theme="altru-coder-vscode">
<head>
  <meta charset="UTF-8">
