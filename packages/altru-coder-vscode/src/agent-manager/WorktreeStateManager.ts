 ...
 *
 * Persists to `.altru-coder/agent-manager.json`. Decouples worktrees from sessions
...
import { ALTRU_CODER_DIR, migrateAgentManagerData, type MigrationResult } from "./constants"
...
export class WorktreeStateManager {
  ...
  constructor(root: string, log: (msg: string) => void) {
    this.root = root
    this.file = path.join(root, ALTRU_CODER_DIR, STATE_FILE)
    this.log = log
  }
  ...
  async load(): Promise<StateLoadResult> {
    // Migrate Agent Manager data from .altrucoder → .altru-coder before first read
  ...
  private apply(content: string): void {
    ...
    for (const [id, wt] of Object.entries(data.worktrees ?? {})) {
      // Rewrite stale .altrucoder paths while preserving the separator style already stored.
      const fixed =
        wt.path?.replace(/([/\\])\.altrucoder([/\\])/g, (_match, leadingSep, trailingSep) => {
          return `${leadingSep}.altru-coder${trailingSep}`
        }) ?? wt.path
