...
/** Altru Coder config directory name (project-level and inside worktrees). */
export const ALTRU_CODER_DIR = ".altru-coder"

/** Legacy config directory name for backward compatibility reads. */
export const LEGACY_DIR = ".altrucoder"

/** Agent Manager files that should be migrated from .altrucoder/ to .altru-coder/. */
...
export interface MigrationResult {
  /** Number of git worktree refs that were rewritten from .altrucoder → .altru-coder. */
  refsFixed: number
}

/**
 * Migrate Agent Manager data from .altrucoder/ to .altru-coder/.
 *
 * Moves individual Agent Manager files/directories (worktrees, state,
 * setup scripts) from the legacy .altrucoder/ into .altru-coder/. Skips items
 * that already exist in .altru-coder/ (the new location wins). This is safe
 * because Agent Manager exclusively owns these files.
 *
 * Fixes git worktree internal references (.git/worktrees/{name}/gitdir)
 * whenever .altru-coder/worktrees/ exists so partially migrated repos recover too.
 *
...
export async function migrateAgentManagerData(root: string, log: (msg: string) => void): Promise<MigrationResult> {
  ...
  const target = path.join(root, ALTRU_CODER_DIR)

  if (await isDirectory(legacy)) {
    // Ensure .altru-coder/ exists
    try {
    ...
    for (const item of AGENT_MANAGER_ITEMS) {
      ...
      if (await exists(dst)) {
        log(`Skipping ${item}: already exists in ${ALTRU_CODER_DIR}`)
        continue
      }

      try {
        await fs.promises.rename(src, dst)
        log(`Migrated ${item} from ${LEGACY_DIR} to ${ALTRU_CODER_DIR}`)
      } catch (err) {
...
/**
 * After moving worktrees from .altrucoder/ to .altru-coder/, fix git internal refs.
 *
 ...
 * gitdir files that reference the old .altrucoder path.
 *
...
async function fixGitWorktreeRefs(root: string, log: (msg: string) => void): Promise<number> {
  ...
  const newSegment = path.join(root, ALTRU_CODER_DIR) + path.sep
  let fixed = 0
