...
export interface TerminalRoutingDeps {
  ...
  /** Loopback URL + basic-auth password for the running `altru-coder serve`. */
...
export class TerminalRouter {
  ...
  private buildWsUrl(ptyID: string, cwd: string): string {
    ...
    const token = Buffer.from(`altru-coder:${config.password}`).toString("base64")
