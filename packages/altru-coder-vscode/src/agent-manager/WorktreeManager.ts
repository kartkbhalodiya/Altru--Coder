 ...
 *
 * Ported from altrucoder/src/core/altrucoder/agent-manager/WorktreeManager.ts.
 * Handles creation, discovery, and cleanup of worktrees stored in
 * {projectRoot}/.altru-coder/worktrees/
 */
...
const TEMP_PREFIX = ".altru-coder-delete-"
...
import { ALTRU_CODER_DIR, LEGACY_DIR, migrateAgentManagerData } from "./constants"

const SESSION_ID_FILE = "session-id"
const METADATA_FILE = "metadata.json"
const GIT_METADATA_FILE = "altru-coder-agent-manager-metadata.json"
...
export class WorktreeManager {
  ...
  constructor(root: string, log: (msg: string) => void, ops?: GitOps) {
    this.root = root
    this.dir = path.join(root, ALTRU_CODER_DIR, "worktrees")
    this.git = simpleGit(root)
    this.ops = ops
    this.log = log
  }

  /** Run once before first read/write to migrate Agent Manager data from .altrucoder → .altru-coder. */
  ...
  private async removeWorktreeImpl(worktreePath: string, branch?: string): Promise<void> {
    ...
    const temp = path.join(path.dirname(worktreePath), `.altru-coder-delete-${randomUUID()}`)
    try {
  ...
  /** Remove orphaned .altru-coder-delete-* temp dirs left by interrupted deletions. */
  ...
  async readMetadata(
    ...
    // Check .altru-coder/ first, then legacy .altrucoder/
    for (const dirName of [ALTRU_CODER_DIR, LEGACY_DIR]) {
  ...
  async ensureGitExclude(): Promise<void> {
    ...
    const items = [
      [".altru-coder/worktrees/", "Altru Coder agent worktrees"],
      [".altru-coder/agent-manager.json", "Altru Coder Agent Manager state"],
      [".altru-coder/setup-script", "Altru Coder worktree setup script"],
      [".altru-coder/setup-script.sh", "Altru Coder worktree setup script"],
      [".altru-coder/setup-script.ps1", "Altru Coder worktree setup script"],
      [".altru-coder/setup-script.cmd", "Altru Coder worktree setup script"],
      [".altru-coder/setup-script.bat", "Altru Coder worktree setup script"],
      [".altrucoder/worktrees/", "Altru Coder legacy agent worktrees"],
      [".altrucoder/agent-manager.json", "Altru Coder Agent Manager legacy state"],
      [".altrucoder/setup-script", "Altru Coder legacy worktree setup script"],
      [".altrucoder/setup-script.sh", "Altru Coder legacy worktree setup script"],
      [".altrucoder/setup-script.ps1", "Altru Coder legacy worktree setup script"],
      [".altrucoder/setup-script.cmd", "Altru Coder legacy worktree setup script"],
      [".altrucoder/setup-script.bat", "Altru Coder legacy worktree setup script"],
    ] as const
