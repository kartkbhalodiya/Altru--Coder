 ...
 *  used to spike `altru-coder serve`; now that the detail path runs in the
 ...
 * `altru-coder serve` process, which leaks native memory on every `Bun.spawn` on
...
/**
 * Hot polling path for the active workspace. Returns the exact staged,
 * unstaged, and untracked working-tree changes relative to HEAD. This avoids
 * showing committed branch history in the sidebar badge / Changes panel.
 */
export async function workingTreeSummary(git: GitOps, dir: string, log?: Log): Promise<WorktreeDiffEntry[]> {
  const items = await list(git, dir, "HEAD", log)
  return items.map(summarize)
}
...
export async function diffFile(
  ...
  const anc = await ancestor(git, dir, base, log)
  if (!anc) return null
  return await detail(git, dir, anc, file, log)
}

export async function workingTreeFile(
  git: GitOps,
  dir: string,
  file: string,
  log?: Log,
): Promise<WorktreeDiffEntry | null> {
  return await detail(git, dir, "HEAD", file, log)
}
