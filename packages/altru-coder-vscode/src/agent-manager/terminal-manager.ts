 ...
 *
 * Maps Agent Manager terminal IDs to backend PTY IDs (from `altru-coder serve`).
...
export class TerminalManager {
   ...
   *  the server-side PTY would linger until `altru-coder serve` exits. */
  async close(terminalId: string): Promise<void> {
    ...
    try {
      ...
      if (error) {
        ...
        this.deps.log(`Terminal close failed (${terminalId}): ${msg} — PTY may linger until altru-coder serve exits`)
        return
      }
   ...
   *     server-side PTYs are then only reaped when `altru-coder serve` itself
  ...
  async dispose(): Promise<void> {
    ...
    const client = (() => {
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err)
        this.deps.log(
          `Terminal dispose: SDK client unavailable (${msg}); relying on altru-coder serve process-group kill to reap PTYs`,
        )
        return undefined
      }
    })()
    ...
    if (failed > 0) {
      this.deps.log(`Terminal dispose: ${failed}/${snapshot.length} PTYs may linger until altru-coder serve exits`)
    }
    this.entries.clear()
  }
}
