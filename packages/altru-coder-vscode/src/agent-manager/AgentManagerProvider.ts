...
import type { AltruCoderClient, Session } from "@altru-coder/sdk/v2/client"
 ...
 * sessions are stored in `.altru-coder/agent-manager.json`. The UI shows two
...
export class AgentManagerProvider implements Disposable {
  public static readonly viewType = "altru-coder.new.AgentManagerPanel"
  ...
  constructor(
    ...
    this.outputChannel = host.createOutput("Altru Coder Agent Manager")
    ...
    this.unsubTool = this.connectionService.onEventFiltered(
      (event) => (event as { type?: string }).type === "altrucoder.agent_manager.start",
  ...
  private async initializeState(): Promise<void> {
    ...
    // When the .altrucoder → .altru-coder migration rewrote git worktree refs, nudge
  ...
  private openWorktreeFile(sessionId: string, filePath: string, line?: number, column?: number): void {
    ...
    try {
      ...
      console.error("[Altru Coder New] AgentManagerProvider: Cannot resolve file path:", err)
      return
    }
   ...
   * Called from AltruCoderProvider when the sidebar sends "continueInWorktree".
   */
