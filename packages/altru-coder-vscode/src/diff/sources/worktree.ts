...
import { workingTreeSummary, workingTreeFile } from "../../agent-manager/local-diff"
...
/**
 * Diffs between the local working tree and HEAD. Each fetch returns
 ...
 * extension host — no `altru-coder serve` round-trip.
 */
export function createWorktreeDiffSource(connection: AltruCoderConnectionService): DiffSource {
  const output = vscode.window.createOutputChannel("Altru Coder Diff: Workspace")
  ...
  return {
    ...
    async fetch(): Promise<DiffSourceFetch> {
      ...
      const entries = await workingTreeSummary(git, current.directory, log)
    ...
    async fetchFile(file: string): Promise<DiffFile | null> {
      ...
      try {
        const entry = await workingTreeFile(git, current.directory, file, log)
