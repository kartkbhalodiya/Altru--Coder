/**
 * Provider action handlers extracted from AltruCoderProvider to stay under max-lines.
 * These are pure async functions that operate on the SDK client — no vscode dependency.
 */
import type { Config, AltruCoderClient } from "@altru-coder/sdk/v2"
...
export async function fetchProviderData(client: AltruCoderClient, dir: string) {
  const authRequest =
  ...
  return {
    ...
    disposeGlobal: async (reason: string) => {
      ...
      await client.global.dispose().catch((error: unknown) => {
        console.warn(`[Altru Coder New] AltruCoderProvider: global.dispose() after ${reason} failed:`, error)
      })
    },
...
async function removeAuth(ctx: ActionContext, id: string, configured: boolean) {
  try {
    ...
    console.warn(`[Altru Coder New] auth.remove failed for configured provider ${id} (non-fatal):`, err)
  }
}
