 ...
 *
 * Minimal inline port of `@altru-coder/event-service` (cloud monorepo). Connects
 * to the altru-coder events Cloudflare Worker using a two-step ticket flow:
...
import type { AltruCoderChatEventMap, AltruCoderChatEventName } from "./types"

const WS_SUBPROTOCOL = "altru-coder.events.v1"
...
export class EventServiceClient {
  ...
  on<N extends AltruCoderChatEventName>(
    ...
    // altru-coder-chat worker edge before broadcast.
  ...
  private handleMessage(data: string): void {
    ...
    if (m.type === "error") {
      console.warn("[Altru Coder New] event-service server error", m)
    }
  }
