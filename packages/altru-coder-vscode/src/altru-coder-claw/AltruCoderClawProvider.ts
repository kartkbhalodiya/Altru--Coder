/**
 * AltruCoderClaw panel provider for the VS Code extension.
 *
 * Owns the Altru Coder Chat HTTP client + event-service WebSocket connection
...
import type { AltruCoderConnectionService } from "../services/cli-backend"
...
import { AltruCoderChatApiError, AltruCoderChatClient } from "./altru-coder-chat-client"
...
import type {
  ...
  AltruCoderClawInMessage,
  AltruCoderClawOutMessage,
  AltruCoderClawState,
  Message,
  MessageCreatedEvent,
  MessageDeletedEvent,
...
export class AltruCoderClawProvider implements vscode.Disposable {
  static readonly viewType = "altru-coder.new.AltruCoderClawPanel"
  ...
  private chat: AltruCoderChatClient | null = null
  ...
  constructor(
    ...
    private readonly connection: AltruCoderConnectionService,
  ) {}
  ...
  private attach(panel: vscode.WebviewPanel): void {
    ...
    panel.iconPath = {
      light: vscode.Uri.joinPath(this.uri, "assets", "icons", "altru-logo.png"),
      dark: vscode.Uri.joinPath(this.uri, "assets", "icons", "altru-logo.png"),
    }
    ...
    panel.webview.html = buildWebviewHtml(panel.webview, {
      scriptUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "altru-coder-claw.js")),
      styleUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "altru-coder-claw.css")),
      iconsBaseUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "assets", "icons")),
      title: "AltruCoderClaw",
    })

    const msgSub = panel.webview.onDidReceiveMessage((msg: AltruCoderClawInMessage) => this.onMessage(msg))
    ...
    const unsub = this.connection.onLanguageChanged((locale) => {
      this.post({ type: "altru-coder-claw.locale", locale })
    })
  ...
  private post(msg: AltruCoderClawOutMessage): void {
    this.panel?.webview.postMessage(msg)
  }

  private async onMessage(msg: AltruCoderClawInMessage): Promise<void> {
    switch (msg.type) {
      case "altru-coder-claw.ready":
        await this.init()
        return
      case "altru-coder-claw.openExternal": {
  ...
  private get locale(): string {
    const override = vscode.workspace.getConfiguration("altru-coder.new").get<string>("language")
  ...
  private async init(): Promise<void> {
    ...
    try {
      this.post({ type: "altru-coder-claw.state", state: { phase: "loading", locale: this.locale } })
      ...
      const state: AltruCoderClawState = {
        phase: "ready",
        locale: this.locale,
        status: this.status,
      ...
      this.post({ type: "altru-coder-claw.state", state })
      this.startPolling()
      this.startBotNudge()
    } finally {
   ...
   */
  private async bootstrap(client: AltruCoderClient, gen: number): Promise<boolean> {
    ...
    if (!this.sandboxId) {
      this.post({ type: "altru-coder-claw.state", state: { phase: "noInstance", locale: this.locale } })
      return false
    }
  ...
  private async resolveStatus(client: AltruCoderClient, gen: number): Promise<boolean> {
    const statusRes = await client.altruCoder.claw.status().catch(() => null)
    if (this.stale(gen)) return false
  ...
  private async fetchChatToken(gen: number): Promise<ChatToken | null> {
    ...
    try {
      ...
      console.error("[Altru Coder New] AltruCoderClaw chat token fetch failed:", message)
      // Token fetch typically fails when the instance hasn't been upgraded
      // to support altru-coder-chat — surface that as the upgrade prompt.
      this.post({ type: "altru-coder-claw.state", state: { phase: "needsUpgrade", locale: this.locale } })
      return null
    }
  }
  ...
  private async openChatStream(envelope: ChatToken, gen: number): Promise<boolean> {
    ...
    const events = new EventServiceClient({
      ...
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "altru-coder-claw.error", error: "Authentication expired" })
      },
    })
    this.events = events

    const chat = new AltruCoderChatClient({
      baseUrl: envelope.altruChatUrl,
      getToken: () => tokens.get(),
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "altru-coder-claw.error", error: "Authentication expired" })
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.error("[Altru Coder New] AltruCoderClaw event-service connect failed:", message)
      this.post({
        type: "altru-coder-claw.state",
  ...
  private async loadInitialSnapshots(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Altru Coder New] AltruCoderClaw listConversations failed:", message)
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Altru Coder New] AltruCoderClaw getBotStatus failed:", message)
    }
    ...
    if (!this.activeConversationId && this.conversations.length > 0) {
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : String(err)
        console.warn("[Altru Coder New] AltruCoderClaw getConversationStatus failed:", message)
      }
  ...
  private async resolveClient(): Promise<AltruCoderClient | null> {
    if (this.connection.getConnectionState() !== "connected") {
      try {
        ...
        console.debug("[Altru Coder New] AltruCoderClaw connect deferred:", (err as Error)?.message ?? err)
        return null
      }
    }
    try {
      ...
      console.debug("[Altru Coder New] AltruCoderClaw getClient deferred:", (err as Error)?.message ?? err)
      return null
    }
  }
  ...
  private subscribeSandboxContext(): void {
    ...
    const ctx = `/altru-coder-claw/${this.sandboxId}`
  ...
  private subscribeConversationContext(conversationId: string): void {
    ...
    const ctx = `/altru-coder-claw/${this.sandboxId}/${conversationId}`
    this.events.subscribe([ctx])
  ...
  private attachEventHandlers(events: EventServiceClient, _chat: AltruCoderChatClient): void {
    ...
    this.chatSubs.push(
      events.on("conversation.created", (ctx, e: ConversationCreatedEvent) => {
        if (!this.sandboxId || ctx !== `/altru-coder-claw/${this.sandboxId}`) return
    ...
    this.chatSubs.push(
      events.on("conversation.left", (ctx, e: ConversationLeftEvent) => {
        ...
        if (this.activeConversationId === e.conversationId) {
          ...
          this.post({ type: "altru-coder-claw.activeConversation", conversationId: null })
          this.post({
          ...
          this.post({ type: "altru-coder-claw.conversationStatus", status: null })
        }
        this.broadcastConversations({ replace: true })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("bot.status", (ctx, e: BotStatusEvent) => {
        ...
        this.post({ type: "altru-coder-claw.botStatus", status: this.botStatus })
      }),
    )
      ...
      events.on("message.created", (ctx, e: MessageCreatedEvent) => {
        ...
        if (e.clientId) {
          ...
          if (idx !== -1) {
            ...
            this.post({
              type: "altru-coder-claw.messageReplaced",
    ...
    this.chatSubs.push(
      events.on("action.delivery_failed", (ctx, e: ActionDeliveryFailedEvent) => {
        ...
        this.broadcastMessages({ replace: true })
        this.post({ type: "altru-coder-claw.error", error: "Couldn't reach the bot — please try again" })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("conversation.status", (ctx, e: ConversationStatusEvent) => {
        ...
        this.post({ type: "altru-coder-claw.conversationStatus", status: this.conversationStatus })
      }),
    )
  }
  ...
  private async selectConversation(conversationId: string): Promise<void> {
    ...
    this.post({ type: "altru-coder-claw.activeConversation", conversationId })
    this.typingMembers = []
  ...
  private async createConversation(title?: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to create conversation") })
    }
  }
  ...
  private async renameConversation(conversationId: string, title: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to rename conversation") })
      void this.refreshConversations()
    }
  }
  ...
  private async leaveConversation(conversationId: string): Promise<void> {
    ...
    try {
      ...
      if (this.activeConversationId === conversationId) {
        ...
        this.messages = []
        this.post({ type: "altru-coder-claw.activeConversation", conversationId: null })
        this.post({ type: "altru-coder-claw.messages", conversationId, messages: [], hasMore: false, replace: true })
      }
      this.broadcastConversations({ replace: true })
    } catch (err) {
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to leave conversation") })
    }
  }
  ...
  private async loadMoreConversations(): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to load conversations") })
    }
  }
  ...
  private async sendMessage(
    ...
    // altru-coder-chat validates clientId as a ULID (Crockford Base32); generate
    ...
    if (conversationId === this.activeConversationId) {
      ...
      this.post({ type: "altru-coder-claw.messageOptimistic", conversationId, message: optimistic })
    }
    ...
    try {
      ...
      console.error("[Altru Coder New] AltruCoderClaw sendMessage failed:", err instanceof Error ? err.message : err)
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to send message") })
      if (conversationId === this.activeConversationId) {
        this.messages = this.messages.filter((m) => m.id !== pendingId)
        this.post({ type: "altru-coder-claw.messageRemoved", conversationId, messageId: pendingId })
      }
    }
  }
  ...
  private async editMessage(conversationId: string, messageId: string, content: ContentBlock[]): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to edit message") })
      if (snapshot && conversationId === this.activeConversationId) {
  ...
  private async deleteMessage(conversationId: string, messageId: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to delete message") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async loadMoreMessages(conversationId: string, before: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to load messages") })
    }
  }
  ...
  private async addReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to add reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async removeReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to remove reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async executeAction(
    ...
    try {
      ...
      this.post({ type: "altru-coder-claw.error", error: this.formatError(err, "Failed to execute action") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async refreshConversations(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Altru Coder New] AltruCoderClaw refreshConversations failed:", message)
    }
  }
  ...
  private async refreshActiveMessages(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Altru Coder New] AltruCoderClaw refreshActiveMessages failed:", message)
    }
  }
  ...
  private upsertTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "altru-coder-claw.typing", conversationId: this.activeConversationId, memberId })
    }
  ...
  private removeTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "altru-coder-claw.typingStop", conversationId: this.activeConversationId, memberId })
    }
  }
  ...
  private formatError(err: unknown, fallback: string): string {
    if (err instanceof AltruCoderChatApiError) {
  ...
  private startBotNudge(): void {
    ...
    this.botNudge = setInterval(() => {
      if (!this.chat || !this.sandboxId) return
      this.chat.requestBotStatus(this.sandboxId).catch((err: unknown) => {
        console.debug("[Altru Coder New] AltruCoderClaw requestBotStatus failed:", (err as Error)?.message ?? err)
      })
    }, BOT_STATUS_NUDGE_MS)
  }
  ...
  private async poll(): Promise<void> {
    try {
      ...
      const res = await client.altruCoder.claw.status()
      if (res?.data) {
        this.status = res.data as ClawStatus
        this.post({ type: "altru-coder-claw.status", data: this.status })
      }
    } catch (err) {
      console.debug("[Altru Coder New] AltruCoderClaw poll failed:", (err as Error)?.message ?? err)
    }
  }
