/**
 * HTTP client for the altru-coder-chat Cloudflare Worker.
 *
 * Minimal inline port of `@altru-coder/altru-chat/client` (cloud monorepo) tailored
 ...
 * the altru-coder-chat worker is the source of truth and validates at its edge.
 */
...
export type AltruCoderChatClientConfig = {
  baseUrl: string
  getToken: () => Promise<string>
  onUnauthorized?: () => void
}

export class AltruCoderChatApiError extends Error {
  constructor(
    public readonly status: number,
    public readonly body: unknown,
  ) {
    super(`AltruCoderChat request failed: ${status}${formatBodyDetail(body)}`)
    this.name = "AltruCoderChatApiError"
  }
}
...
export class AltruCoderChatClient {
  private readonly baseUrl: string
  private readonly getToken: () => Promise<string>
  private readonly onUnauthorized: (() => void) | undefined
  private readonly sendQueues: SendQueue = new Map()

  constructor(config: AltruCoderChatClientConfig) {
  ...
  private async request<T>(path: string, opts: HttpOpts = {}): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new AltruCoderChatApiError(res.status, body)
    }
