/**
 * AltruCoderChat access-token cache.
 *
 * Mirrors the web client pattern (see `apps/web/src/app/(app)/claw/altru-chat/token.ts`
 * in the cloud monorepo). The token is minted by the Altru Coder gateway
 * (`altru-coder.claw.chatCredentials`) and kept in memory with a 5-minute freshness
 * buffer. The gateway operation is a historical name — it used to return
 * Stream Chat credentials and was repurposed when Altru Coder migrated to its own
 * altru-coder-chat service.
 *
...
export class TokenManager {
  ...
  constructor(private readonly getClient: () => AltruCoderClient | null) {}
  ...
  async getOrFetch(): Promise<ChatToken> {
    ...
    if (this.lastFailedAt && Date.now() - this.lastFailedAt < RETRY_BACKOFF_MS) {
      throw new Error("Altru Coder chat token fetch on cooldown after recent failure")
    }
    if (!this.inflight) {
  ...
  private async fetch(): Promise<ChatToken> {
    ...
    if (!client) throw new Error("Altru Coder backend not connected")
    const res = await client.altruCoder.claw.chatCredentials()
    ...
    if (!res || errResponse || !res.data) {
      ...
      throw new Error(`altru-coder-chat credentials fetch failed${detail ? `: ${detail}` : ""}`)
    }
    const data = res.data as Partial<ChatToken>
    const missing: string[] = []
    if (!data.token) missing.push("token")
    if (!data.expiresAt) missing.push("expiresAt")
    if (!data.altruChatUrl) missing.push("altruChatUrl")
    if (!data.eventServiceUrl) missing.push("eventServiceUrl")
    if (missing.length > 0) {
      throw new Error(
        `Malformed altru-coder-chat credentials response: missing ${missing.join(", ")} (received keys: ${Object.keys(data).join(", ") || "<empty>"})`,
      )
    }
    return {
      token: data.token!,
      expiresAt: data.expiresAt!,
      altruChatUrl: data.altruChatUrl!,
