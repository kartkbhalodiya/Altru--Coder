 ...
 * then copies it into packages/altru-coder-vscode/bin/altru-coder.
 *
...
const targetBinPath = join(targetBinDir, "altru-coder")
...
function sourceBinaryPath(): string {
  return join(opencodeDir, "dist", `@altru-coder/cli-${process.platform}-${process.arch}`, "bin", "altru-coder")
}
...
async function rebuild() {
  ...
  try {
    ...
    log(`Binary updated (${elapsed}s): ${relative(packagesDir, source)} -> bin/altru-coder`)
  } catch (err) {
...
// Watch for changes
log(`Watching ${relative(altruVscodeDir, opencodeSrcDir)}/ for changes...`)
