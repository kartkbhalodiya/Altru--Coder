#!/usr/bin/env bun
/**
 * Build the Altru Coder VS Code extension and launch it in a development host.
 *
 * Usage:
...
const base = join(tmpdir(), `altru-coder-vscode-dev-${hash}`)
...
function cleanEnv(input: NodeJS.ProcessEnv) {
  ...
  for (const key of ["XDG_DATA_HOME", "XDG_CACHE_HOME", "XDG_CONFIG_HOME", "XDG_STATE_HOME", "ALTRU_CODER_TEST_HOME"]) {
    const value = env[key]
...
async function launch() {
  ...
  const args = ["--new-window", `--extensions-dir=${extDir}`, `--user-data-dir=${userDir}`, "--skip-release-notes"]
