...
/**
 * Ensures the VS Code extension has a CLI binary at `packages/altru-coder-vscode/bin/altru-coder`.
 *
 * Strategy:
 * 1) If `bin/altru-coder` already exists -> ok.
 ...
 * 4) Copy the resulting binary into `packages/altru-coder-vscode/bin/altru-coder` and chmod +x.
 *
 * This script is intended to be run from `packages/altru-coder-vscode` as part of build/package.
 */

const altruVscodeDir = join(import.meta.dir, "..")
const packagesDir = join(altruVscodeDir, "..")
const opencodeDir = join(packagesDir, "opencode")
const coreDir = join(packagesDir, "core")

const targetBinDir = join(altruVscodeDir, "bin")
...
async function findAltruCoderBinaryInOpencodeDist(): Promise<string | null> {
  ...
  const preferred = join(distDir, `@altru-coder`, tag, "bin", binName)
  try {
    statSync(preferred)
    return preferred
  } catch {
  ...
  // Fallback: find any dist/**/bin/altru-coder or altru-coder.exe
  const queue = [distDir]
  while (queue.length) {
    const dir = queue.pop()
    if (!dir) continue
    ...
    for (const e of entries) {
      ...
      if (e.isFile() && (e.name === "altru-coder" || e.name === "altru-coder.exe") && basename(dirname(p)) === "bin") {
        return p
      }
    }
  }
...
async function ensureBuiltBinary(): Promise<string> {
  const found = await findAltruCoderBinaryInOpencodeDist()
  if (found) return found

  log(
    `No prebuilt binary found under ${relative(altruVscodeDir, join(opencodeDir, "dist"))} - attempting build via bun.`,
  )
  ...
  const built = await findAltruCoderBinaryInOpencodeDist()
  if (!built) {
    throw new Error(
      `CLI build completed but no binary was found in ${join(opencodeDir, "dist")} (expected dist/**/bin/altru-coder).`,
    )
  }
  return built
}
...
async function main() {
  ...
  if (exists && !rebuild) {
    ...
    log(
      `CLI binary already present at ${relative(altruVscodeDir, targetBinPath)} (${Math.round(st.size / 1024 / 1024)}MB). Use --force to rebuild.`,
    )
    return
  }
  ...
  if (exists && rebuild) {
    ...
    if (existsSync(distDir)) {
      ...
      log(`Removed ${relative(altruVscodeDir, distDir)} to force rebuild.`)
    }
  }
  ...
  log(`Copied CLI binary from ${relative(packagesDir, sourceBinPath)} -> ${relative(altruVscodeDir, targetBinPath)}`)
}
