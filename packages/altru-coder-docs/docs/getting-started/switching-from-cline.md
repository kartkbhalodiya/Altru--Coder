...
# Migrating from Cline to Altru Coder

A practical guide for developers switching from Cline to Altru Coder.

## Why Switch?

**Speed.** The Altru Coder Platform is designed to reduce friction across the entire development workflow. Beyond feature parity with Cline, Altru Coder includes Code Reviews, App Builder, Cloud Agents, and one-click Deploy.

**Multi-platform.** Work in VS Code, JetBrains IDEs, CLI, or the web. Your Sessions sync across all of them automatically.

**Specialized modes.** Instead of one agent doing everything, Altru Coder has five specialized modes optimized for different parts of your workflow.

**500+ models.** More providers, more options, better pricing flexibility. Switch models mid-conversation if you want.

---

## Understanding Altru Coder's Mode System

Cline uses a single agent with a Plan/Act toggle. Altru Coder takes a different approach: five specialized modes, each optimized for a specific part of development. You select modes from a dropdown in the interface in the Altru Coder Extension, or with slash commands in the CLI.

### Altru Coder's Five Modes
...
**Altru Coder:** Select **Ask** from the dropdown
...
**Why this matters:** In Cline, you might accidentally make changes while exploring. In Altru Coder, Ask and Architect modes can't write files, so you're safe to explore without worry. Altru Coder's **Orchestrator** mode lets you intelligently generate and execute subtasks that are optimized for each mode.
...
2. Search "Altru Coder"
3. Click Install
4. Find the Altru Coder icon in your sidebar
...
4. Find Altru Coder icon in right sidebar
...
Visit [app.altru-coder.ai](https://app.altru-coder.ai/) and sign in. This gives you access to:

- **Cloud Agents:** Run Altru Coder without a local machine
- **App Builder:** Build and preview apps directly in your browser
- **Altru Coder Deploy:** One-click deployments
...
1. Click "Try Altru Coder for Free" in the Altru Coder panel
2. Sign in with OAuth at altru-coder.ai
3. You'll be redirected back to your IDE

### Configure your provider

**Option 1: Use Altru Coder Gateway (easiest)**

1. Open settings (⚙️ icon)
2. Select "Altru Coder Gateway" as provider
...
## Beyond the IDE: Altru Coder's Platform Features

One of the biggest differences from Cline is that Altru Coder isn't just an IDE extension. It's a platform with multiple interfaces that can all share your Sessions and context.

### Cloud Agents

Run Altru Coder from [app.altru-coder.ai/cloud](https://app.altru-coder.ai/cloud) without needing your local machine. Great for:
...
Build live apps and sites directly from the web with a real-time preview. Similar to Lovable, but integrated with your Altru Coder Sessions. Deploy with one click when you're ready.

### Altru Coder Deploy

One-click deployments from directly within Altru Coder. Go from code to production without leaving your workflow.
...
Semantic search across your repositories using cloud-hosted embeddings. Altru Coder indexes your codebase to deliver more relevant, context-aware responses.
...
| Cline Feature | Altru Coder Equivalent | Notes |
|---|---|---|
...
- **One-click Deploy:** Ship directly from Altru Coder
- **Code Reviews:** AI-powered PR analysis
...
**Q: Can I use both Cline and Altru Coder side-by-side?** Yes. They're separate extensions.
...
1. Install Altru Coder in your primary IDE
...
3. Try Cloud Agents at [app.altru-coder.ai](https://app.altru-coder.ai/)
4. Install the Altru Coder CLI with `npm install -g @altru-coder/cli`
