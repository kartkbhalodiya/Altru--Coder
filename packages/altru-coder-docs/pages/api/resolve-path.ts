...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (fs.existsSync(candidatePath)) {
    return res.status(200).json({ filePath: `packages/altru-coder-docs/pages/${sanitizedPath}.md` })
  }

  if (fs.existsSync(candidateIndexPath)) {
    return res.status(200).json({ filePath: `packages/altru-coder-docs/pages/${sanitizedPath}/index.md` })
  }
