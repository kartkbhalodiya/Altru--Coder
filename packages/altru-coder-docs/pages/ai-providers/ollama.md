---
title: "Using Ollama with Altru Coder | Run Local Models"
description: "Run local AI models with Ollama in Altru Coder for offline, private coding. Setup guide for VS Code and the CLI."
sidebar_label: Ollama
---

# Using Ollama With Altru Coder

Altru Coder supports running models locally using Ollama. This provides privacy, offline access, and potentially lower costs, but requires more setup and a powerful computer.
...
For the Altru Coder agent the current recommendation is `qwen3-coder:30b`. `qwen3-coder:30b` sometimes fails to call tools correctly (it is much more likely to have this problem than the full `qwen3-coder:480b` model). As a mixture-of-experts model, this could be because it activated the wrong experts. Whenever this happens, try changing your prompt or use the Enhance Prompt button.

An alternative to `qwen3-coder:30b` is `devstral:24b`. For other features of Altru Coder such as Enhance Prompt or Commit Message Generation smaller models may suffice.

## Setting up Ollama

To set up Ollama for use with Altru Coder, follow the instructions below.
...
By default, API requests time out after 10 minutes. Local models can be slow, if you hit this timeout you can consider increasing it here: VS Code Extensions panel > Altru Coder gear menu > Settings > API Request Timeout.
...
- Open the Altru Coder panel ({% altruCodeIcon size="1em" /%}).
...
If your Ollama model doesn't appear in the Altru Coder model picker, register it as a custom model in your config file:
