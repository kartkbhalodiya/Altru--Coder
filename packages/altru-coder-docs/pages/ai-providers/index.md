---
title: "AI Providers"
description: "Configure and connect different AI model providers to Altru Coder"
---

# AI Providers

Altru Coder supports a wide variety of AI providers, giving you flexibility in how you power your AI-assisted development workflow. Choose from cloud providers, local models, or AI gateways based on your needs.

## Getting Started

The fastest way to get started is with **Altru Coder's built-in provider**, which requires no configuration. Simply sign in and start coding.
...
|---|---|
| Ease of use | [Altru Coder (built-in)](/docs/ai-providers/altrucoder) |
...
{% callout type="note" %}
In the **VSCode (Legacy)** version, API keys use VS Code's Secret Storage. In the current **VSCode & CLI** version, keys are set via environment variables or referenced in `altru-coder.json` config files. See individual provider pages for setup instructions for each platform.
{% /callout %}

## Disabling Built-in Providers

You can prevent specific providers from loading using `disabled_providers` in your `altru-coder.json` (or `altru-coder.jsonc`). This is useful to hide models from built-in or detected providers that you don't intend to use.
...
Both fields accept provider IDs — the lowercase identifier used in the `provider/model` format (e.g. `altru-coder`, `anthropic`, `openai`, `google`, `groq`).

## Next Steps

- **New to Altru Coder?** Start with the [Altru Coder provider](/docs/ai-providers/altrucoder) - no setup required
