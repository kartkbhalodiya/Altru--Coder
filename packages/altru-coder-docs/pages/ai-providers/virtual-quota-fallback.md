---
title: "Virtual Quota Fallback in Altru Coder"
description: "Configure automatic provider fallback in Altru Coder to handle quota limits and keep your AI coding workflow uninterrupted."
...
- **Usage Tracking:** You can set custom limits for each provider based on the number of tokens or requests per minute, hour, or day. Altru Coder tracks the usage for each provider against these limits.
...
Before configuring this provider, you must have at least one other API provider already configured as a separate profile in Altru Coder. This provider is only useful if there are other profiles for it to manage.
...
<!-- <img src="/docs/img/providers/virtualQuotaSelectDropdown.png" alt="virtuaQuotaFallback dropdown selection in Altru Coder settings" width="600" /> -->
...
<!-- <img src="/docs/img/providers/virtualQuotaFullConfig.png" alt="virtuaQuotaFallback configuration in Altru Coder settings" width="600" /> -->
...
**Result:**
Altru Coder will send all requests to Chutes AI. Once your usage exceeds 5,000 tokens within an hour, it will automatically switch to your OpenAI account. The system will switch back to Chutes AI in the next hour when its quota window has reset.
