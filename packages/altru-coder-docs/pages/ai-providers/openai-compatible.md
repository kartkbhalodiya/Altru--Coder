---
title: "Using OpenAI-Compatible Providers with Altru Coder"
...
# Using OpenAI Compatible Providers With Altru Coder

Altru Coder supports a wide range of AI model providers that offer APIs compatible with the OpenAI API standard. This means you can use models from providers _other than_ OpenAI, while still using a familiar API interface. This includes providers like:
...
Use Altru Coder's native `azure` provider instead. If your Azure deployment name differs from the model name you select in Altru Coder, map it with the model `id` field in `altru-coder.json`.
{% /callout %}
...
You'll find these settings in the Altru Coder settings panel (click the {% codicon name="gear" /%} icon):
...
- **Base URL** — The provider's OpenAI-compatible API endpoint (e.g., `https://api.your-provider.com/v1`). Altru Coder auto-fetches available models when a valid URL is entered. For Azure OpenAI GPT-5, use the native `azure` provider instead.
...
For additional model configuration (token limits, tool calling, variants), edit the `altru-coder.jsonc` config file directly — see the **CLI** tab or the [Custom Models](/docs/code-with-ai/agents/custom-models) guide.

### Automatic Model Detection

When configuring a custom OpenAI-compatible provider, Altru Coder can automatically detect available models from your provider's `/v1/models` endpoint.

Once you enter a valid **Base URL** and **API Key**, Altru Coder will query the provider and present a searchable model picker with all available models. You can:
...
Define a custom provider in your `altru-coder.json` config file (`~/.config/altru-coder/altru-coder.json` or `./altru-coder.json`). The provider key (e.g., `"vllm"`) is your chosen identifier — it can be any name you like.
...
Altru Coder supports full endpoint URLs in the Base URL field, providing greater flexibility for provider configuration:
...
- **Azure GPT-5 rejects `max_tokens`:** Azure GPT-5 deployments must use Altru Coder's native `azure` provider. Generic OpenAI-compatible custom providers send `max_tokens`, which Azure GPT-5 rejects because it expects `max_completion_tokens`.
- **Unexpected Results:** If you're getting unexpected results, try a different model.

By using an OpenAI-compatible provider, you can leverage the flexibility of Altru Coder with a wider range of AI models. Remember to always consult your provider's documentation for the most accurate and up-to-date information.
