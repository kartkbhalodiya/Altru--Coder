---
title: "Using Human Relay with Altru Coder | No API Key Needed"
description: "Use Altru Coder with web-based AI chats like ChatGPT and Claude without an API key by manually relaying messages via Human Relay."
sidebar_label: Human Relay
---

# Human Relay Provider

The Human Relay provider allows you to use Altru Coder with web-based AI models like ChatGPT or Claude without needing an API key. Instead, it relies on you to manually relay messages between Altru Coder and the AI's web interface.

## How it Works

1.  **Select Human Relay**: Choose "Human Relay" as your API provider in Altru Coder's settings. No API key is required.
2.  **Initiate a Request**: Start a chat or task with Altru Coder as usual.
...
6.  **Paste Back to Altru Coder**: Return to the dialog box in VS Code, paste the AI's response into the designated field, and click "Confirm".
7.  **Continue**: Altru Coder will process the response as if it came directly from an API.
