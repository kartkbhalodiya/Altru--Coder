---
title: "Using ChatGPT Plus/Pro with Altru Coder"
description: "Use your ChatGPT Plus or Pro subscription with Altru Coder. Setup guide for authenticating without a separate API key."
sidebar_label: ChatGPT Plus/Pro
---

# Using ChatGPT Subscriptions With Altru Coder
...
5. Back in Altru Coder settings, pick a model from the dropdown.
6. Save.
...
If OpenAI is already connected from an API key, environment variable, or `altru-coder.json` config, you can still sign in with ChatGPT from the OpenAI provider row. Altru Coder uses the ChatGPT sign-in for Codex models until you disconnect it, then falls back to your existing OpenAI API configuration.
...
```bash
altru-coder auth login --provider codex
```
...
Then set your default model to one of the OpenAI Codex models available in Altru Coder:
...
- **Sign Out:** To disconnect in VS Code, use the "Disconnect" button in the provider settings. In the CLI, run `altru-coder auth logout` and choose OpenAI.

## Limitations

- **You can't use arbitrary OpenAI API models.** This provider only exposes the models listed in Altru Coder's Codex model catalog.
- **You can't export/migrate your sign-in state with settings export.** OAuth tokens are stored in VS Code SecretStorage, which isn't included in Altru Coder's settings export.
