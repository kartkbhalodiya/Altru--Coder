---
title: "Using Claude Code with Altru Coder"
description: "Use your Claude Code subscription with Altru Coder. Setup guide for authenticating and selecting Claude models."
sidebar_label: Claude Code
---

{% callout type="warning" title="Important Notice" %}
In January 2026, Anthropic decided to restrict Claude Code CLI to official Claude Code clients. Claude Code credentials cannot be used in Altru Coder or other third-party harnesses.

For continued use of Anthropic models in Altru Coder, please use the [Anthropic API provider](/docs/ai-providers/anthropic) with an API key instead.
{% /callout %}

# Using Claude Code With Altru Coder

Claude Code is Anthropic's official CLI that provides direct access to Claude models from your terminal. Using Claude Code with Altru Coder lets you leverage your existing CLI setup without needing separate API keys.
...
If this environment variable is set on your system, the `claude` tool may use it for authentication instead of the interactive `/login` method. When Altru Coder executes the tool, it will accurately reflect that an API key is being used, as this is the underlying behavior of the `claude` CLI itself.
{% /callout %}
...
Claude Code credentials no longer work in Altru Coder. Please use the [Anthropic provider](/docs/ai-providers/anthropic) with an API key instead.
{% /callout %}
...
- **Enhance Prompt Feature:** Full compatibility with Altru Coder's Enhance Prompt feature, allowing you to automatically improve and refine your prompts before sending them to Claude.
