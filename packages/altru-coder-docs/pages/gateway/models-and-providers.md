...
description: "Learn about the AI models available through the Altru Coder AI Gateway, including model IDs and how to use them."
---

# Models & Providers

The Altru Coder AI Gateway provides access to hundreds of AI models through a single unified API. You can switch between models by changing the model ID string -- no code changes required.
...
const result = streamText({
  model: altru-coder.chat("anthropic/claude-sonnet-4.6"),
  prompt: "Hello!",
})
```
...
```
GET https://api.altru-coder.ai/api/gateway/models
```
...
Auto virtual models automatically select the best underlying model based on the task type. The selection is controlled by the `x-altrucoder-mode` request header.

{% callout type="info" title="Underlying models can change" %}
The mappings below reflect the current routing. The underlying models behind each `altru-coder-auto/*` tier are updated server-side as better options become available or as providers change pricing and availability — the tier IDs themselves remain stable.
{% /callout %}
...
```json
{
  "model": "altru-coder-auto/frontier",
...
curl -X POST "https://api.altru-coder.ai/api/gateway/chat/completions" \
  ...
  -H "x-altrucoder-mode: plan" \
  -H "Content-Type: application/json" \
  -d '{"model": "altru-coder-auto/balanced", "messages": [{"role": "user", "content": "Design a database schema"}]}'
```
