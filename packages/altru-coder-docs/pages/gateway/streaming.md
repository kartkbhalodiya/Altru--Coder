---
title: "Streaming"
description: "Learn how to implement real-time streaming responses with the Altru Coder AI Gateway using Server-Sent Events (SSE)."
---

# Streaming

The Altru Coder AI Gateway supports streaming responses from all models using Server-Sent Events (SSE). Streaming allows your application to display tokens as they're generated, providing a more responsive user experience.
...
const response = await fetch("https://api.altru-coder.ai/api/gateway/chat/completions", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${process.env.ALTRU_CODER_API_KEY}`,
