---
title: "API Reference"
description: "Complete API reference for the Altru Coder AI Gateway, including chat completions, FIM completions, and model listing endpoints."
---

# API Reference

The Altru Coder AI Gateway provides an OpenAI-compatible API. All endpoints use the base URL:
...
```bash
curl -X POST "https://api.altru-coder.ai/api/fim/completions" \
  -H "Authorization: Bearer $ALTRU_CODER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
