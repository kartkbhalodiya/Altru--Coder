...
{% browserFrame url="app.altru-coder.ai/gastown/town/rigs/main" caption="A bead in review — the refinery is evaluating the polecat's work" %}
...
{% browserFrame url="app.altru-coder.ai/gastown/town/merges" caption="The merge queue — review detail showing refinery feedback" %}
...
## Combining with Altru Coder Review

Gas Town's refinery works independently, but combining it with [Altru Coder Review](/docs/code-with-ai/platforms/cloud-agent) creates an even stronger pipeline:

1. **Agent writes** → agent-level refinery reviews (fast, automated)
2. **Code lands as PR** → Altru Coder Review provides human-readable review (deeper, contextual)
...
{% browserFrame url="app.altru-coder.ai/gastown/town/merges" caption="The merge queue — all reviews at a glance" %}
