---
title: "Browser Use"
description: "Using Altru Coder to interact with web browsers"
---

# Browser Use

Altru Coder provides browser automation capabilities that let you interact with websites directly from your coding workflow. This feature supports testing web applications, automating browser tasks, and capturing screenshots without leaving your editor.
...
Browser automation is built into the extension and requires no manual setup. Enable it from **Settings → Browser** and Altru Coder handles the rest automatically.

{% /tab %}
{% tab label="CLI" %}

Altru Coder uses [Playwright](https://playwright.dev/) for browser automation. Add it to your `altru-coder.jsonc` configuration:
...
By default, Altru Coder uses a built-in browser that:

- Launches automatically when you ask Altru Coder to visit a website
- Captures screenshots of web pages
- Allows Altru Coder to interact with web elements
...
1. Ask Altru Coder to visit a website
2. Altru Coder launches the browser and shows you a screenshot
3. Request additional actions (clicking, typing, scrolling)
4. Altru Coder closes the browser when finished

For example:

- `Open the browser and view our site.`
- `Can you check if my website at https://altrucoder.ai is displaying correctly?`
...
Altru Coder launches a browser automatically when asked and returns screenshots after each action so you can see what's happening. It can navigate to URLs, click elements, fill in forms, scroll, hover, select from dropdowns, and drag and drop — all driven by natural language instructions in chat.
...
Browser automation is configured in your `altru-coder.jsonc` file. No additional settings are required — Playwright manages the browser lifecycle automatically.
...
To change Browser / Computer Use settings in Altru Coder:

1. Click the gear icon {% codicon name="gear" /%} in Altru Coder
2. Open `Browser / Computer Use`

### Enable/Disable Browser Use

**Purpose**: Master toggle that enables Altru Coder to interact with websites using a Puppeteer-controlled browser.
...
**Purpose**: Determines the resolution of the browser session Altru Coder uses.
...
**Purpose**: Connect Altru Coder to an existing Chrome browser instead of using the built-in browser.
...
Connect to a visible Chrome window to observe Altru Coder's interactions in real-time:
