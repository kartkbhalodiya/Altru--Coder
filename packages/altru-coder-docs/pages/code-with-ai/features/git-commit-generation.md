...
Generate descriptive commit messages automatically based on your staged git changes. Altru Coder analyzes your staged files and creates conventional commit messages that follow best practices.
...
2. In the VS Code Source Control panel, look for the `Altru Coder` logo next to the commit message field)
...
Configuration is handled through the extension's settings or the shared `altru-coder.jsonc` config file.

Customize the commit prompt from **Settings > Commit Message**. Altru Coder saves this prompt to the current project's config so each repository can follow its own commit conventions without changing your global settings.
...
- [Settings Management](/docs/getting-started/settings) - Manage all your Altru Coder preferences
