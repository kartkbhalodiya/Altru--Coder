...
Checkpoints automatically version your workspace files during Altru Coder tasks, enabling non-destructive exploration of AI suggestions and easy recovery from unwanted changes.
...
Checkpoints (called **snapshots** in the new extension) are enabled by default. They are configured via the `snapshot` key in your config file (`altru-coder.jsonc` or `~/.config/altru-coder/altru-coder.jsonc`):
...
Checkpoints are controlled by the `snapshot` boolean in your `altru-coder.jsonc` configuration file:
...
Access checkpoint settings in Altru Coder settings under the "Checkpoints" section:
...
2. Check or uncheck the "Enable automatic checkpoints" checkbox
   ...
   {% image src="/docs/img/checkpoints/checkpoints.png" alt="Checkpoint settings in Altru Coder configuration" width="500" /%}
...
Altru Coder captures snapshots of your project's state using a shadow Git repository, separate from your main version control system. These snapshots, called checkpoints, automatically record changes throughout your AI-assisted workflow—whenever tasks begin, files change, or commands run.
...
- **Scope**: Checkpoints only capture changes made during active Altru Coder tasks
...
1. **Snapshot Git Repository**: A dedicated Git repository created outside your project at `~/.local/share/altru-coder/snapshot/<project-id>/<worktree-hash>/`. This stores all snapshot tree objects without affecting your project's Git history. Each worktree gets its own snapshot repository, identified by a hash of the worktree path.
...
When restoration executes, Altru Coder:
...
#### .altrucoderignore Behavior

The `.altrucoderignore` file (which controls AI access to files) is separate from checkpoint tracking:

- Files excluded by `.altrucoderignore` but not by `.gitignore` will still be checkpointed
- Changes to AI-inaccessible files can still be restored through checkpoints

This separation is intentional, as `.altrucoderignore` limits which files the AI can access, not which files should be tracked for version history.
