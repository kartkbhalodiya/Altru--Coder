...
This guide walks you through setting up Mistral's Codestral model for free autocomplete in Altru Coder. Mistral offers a free tier that's perfect for getting started with AI-powered code completions.
...
- A [Altru Coder account](https://app.altru-coder.ai) (free to create)
...
## Step 4: Add Your Key via BYOK in Altru Coder

1. Log into the [Altru Coder platform](https://app.altru-coder.ai).
2. Navigate to the [Bring Your Own Key (BYOK) page](https://app.altru-coder.ai/byok), available in the sidebar under **Account**.
...
Once your BYOK key is saved, Altru Coder's autocomplete will automatically use your Codestral key through the Altru Coder Gateway. No additional configuration is needed in the extension.

1. Open VS Code with the Altru Coder extension installed.
...
The autocomplete status bar in VS Code shows the current provider ("Altru Coder Gateway") and tracks cumulative cost. With BYOK, requests are billed directly by Mistral at their rates (Codestral has a free tier) and show as $0.00 on your Altru Coder balance.
...
```
Your Editor → Altru Coder Gateway (with your key) → Mistral
```

- The Altru Coder Gateway detects your BYOK key and routes autocomplete requests using it.
- You are billed directly by Mistral — Altru Coder does not add any markup.
- If your BYOK key is invalid, the request will fail (it does not fall back to Altru Coder's keys).

## Troubleshooting

- **Autocomplete not appearing?** Check that autocomplete is enabled in Altru Coder settings (it is on by default). Also verify you are signed into Altru Coder in the extension.
...
- **Seeing charges on your Altru Coder balance?** If you haven't configured BYOK, autocomplete defaults to using your Altru Coder credits. Add your Codestral key via BYOK to route requests through your own Mistral account.
...
{% youtube url="https://www.youtube.com/embed/0aqBbB8fPho" caption="Setting up Mistral for free autocomplete in Altru Coder" /%}

## Step 1: Open Altru Coder Settings

In VS Code, open the Altru Coder panel and click the **Settings** icon (gear) in the top-right corner.

![Open Altru Coder Settings](/docs/img/mistral-setup/01-open-altru-coder-settings.png)
...
{% callout type="note" %}
When creating an autocomplete profile, you don't need to select a specific model—Altru Coder will automatically use the appropriate Codestral model optimized for code completions.
{% /callout %}
...
## Step 9: Paste API Key in Altru Coder

Return to Altru Coder settings and paste your API key into the **Mistral API Key** field.
