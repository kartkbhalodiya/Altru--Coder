...
Instead of maintaining a static list that's perpetually behind, we built something better — a real-time leaderboard showing which models Altru Coder users are actually having success with right now.

## Check the Live Models List

**[👉 See what's working today at altru-coder.ai/models](https://altru-coder.ai/models)**
...
- Set per-agent defaults and a global default in the **Settings** panel (Models tab), or directly in the `altru-coder.jsonc` config file.
...
- For non-interactive use, pass `--model` flag to `altru-coder run` (e.g., `altru-coder run --model claude-sonnet-4-20250514`).
- Set the global default with the `model` key in `altru-coder.jsonc`, or configure per-agent models in the `agent` section.
...
- **Via config file** — edit `altru-coder.jsonc`:
...
The AI model space moves fast. Bookmark [altru-coder.ai/models](https://altru-coder.ai/models) and check back when you're evaluating options. What's best today might not be best next month — and that's actually exciting.
