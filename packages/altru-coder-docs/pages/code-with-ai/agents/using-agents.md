---
title: "Using Agents"
description: "Understanding and using different agents in Altru Coder"
---

# Using Agents

Agents in Altru Coder are specialized personas that tailor the assistant's behavior to your current task. Each agent offers different capabilities, expertise, and access levels to help you accomplish specific goals.
...
{% youtube url="https://youtu.be/cS4vQfX528w" caption="Explaining the different modes in Altru Coder" /%}
...
4. **Accept suggestions:** Click on mode switch suggestions that Altru Coder offers when appropriate

   {% image src="/docs/img/modes/modes-2.png" alt="Accepting a mode switch suggestion from Altru Coder" width="400" /%}
...
| **Tool Access** | Read-only tools plus restricted file editing (plan files in `.altru-coder/plans/` only) |
...
{% callout type="tip" %}
**Keep debugging separate from main tasks:** When using Debug mode, ask Altru Coder to "start a new task in Debug mode with all of the necessary context needed to figure out X" so that the debugging process uses its own context window and doesn't pollute the main task.
{% /callout %}
