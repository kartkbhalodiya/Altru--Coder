...
description: "Using Altru Coder in Visual Studio Code"
---

# VS Code Extension

Altru Coder is available as two VS Code extensions: the **VSCode (Legacy)** extension and the current **VSCode** version built on Altru Coder's shared agent runtime.
...
The extension includes its own embedded runtime. No separate Altru Coder CLI installation is required.
...
- **[JSONC config files](/docs/getting-started/settings)** — Portable settings in `altru-coder.jsonc` instead of VS Code settings
- **[Granular permissions](/docs/getting-started/settings/auto-approving-actions)** — Per-tool permission rules with glob patterns
- **[Agents](/docs/code-with-ai/agents/using-agents)** — Customizable agents (`.altru-coder/agents/*.md`) replacing the modes system
...
Settings apply across extension surfaces, including the sidebar and Agent Manager. The standalone CLI uses the same `~/.config/altru-coder/altru-coder.jsonc` (global) and `./altru-coder.jsonc` (project) files when used directly.
...
- **[Custom Modes](/docs/customize/custom-modes)** — Define custom modes with `.altrucodermodes` YAML files
