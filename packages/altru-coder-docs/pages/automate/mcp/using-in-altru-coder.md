---
title: "Using MCP in Altru Coder"
description: "How to use MCP servers in Altru Coder"
---

# Using MCP in Altru Coder

Model Context Protocol (MCP) extends Altru Coder's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with Altru Coder.
...
MCP server configurations are stored inside the main Altru Coder config file. There are two levels:

1. **Global Configuration**: `~/.config/altru-coder/altru-coder.jsonc` — applies to all projects.
2. **Project-level Configuration**: `altru-coder.jsonc` in your project root, or `.altru-coder/altru-coder.jsonc` for a cleaner setup.

**Precedence**: Project-level configuration takes precedence over global configuration.

### Editing MCP Settings

You can edit MCP settings from the Altru Coder settings UI:
...
MCP servers are configured under the `mcp` key in `altru-coder.jsonc`:
...
Remote servers support OAuth 2.0 authentication. If the server supports it, Altru Coder will automatically start the OAuth flow when you connect. You can also disable OAuth with `"oauth": false`.

{% /tab %}
{% tab label="CLI" %}

The CLI accepts several config filenames. The recommended file is `altru-coder.json`:

| Scope | Recommended Path | Also supported |
|---|---|---|
| **Global** | `~/.config/altru-coder/altru-coder.json` | `altru-coder.jsonc`, `opencode.json`, `opencode.jsonc`, `config.json` |
| **Project** | `./altru-coder.json` or `./.altru-coder/altru-coder.json` | `altru-coder.jsonc`, `opencode.jsonc`, `opencode.json` |
...
| **Project** | `.altrucoder/mcp.json` | In your project root. Auto-detected by Altru Coder. |
...
_Example of MCP Server config in Altru Coder (STDIO Transport)_
...
- **Remote (HTTP/SSE)**: Servers hosted over HTTP/HTTPS. Altru Coder tries `StreamableHTTP` first, then falls back to `SSE` automatically.
...
| Command | Description |
|---|---|
| `altru-coder mcp list` | List all configured MCP servers |
| `altru-coder mcp add` | Add an MCP server |
| `altru-coder mcp auth` | Authenticate with an MCP server |
| `altru-coder mcp logout` | Log out from an MCP server |
| `altru-coder mcp debug` | Debug an MCP server connection |
...
Run `altru-coder mcp list` to verify the server status.
...
You can edit both global and project-level MCP configuration files directly from the Altru Coder settings.

1. Click the {% codicon name="gear" /%} icon in the top navigation of the Altru Coder pane to open `Settings`.
2. Click the `Agent Behaviour` tab on the left side
3. Select the `MCP Servers` sub-tab
4. Click the appropriate button:
   - **`Edit Global MCP`**: Opens the global `mcp_settings.json` file.
   - **`Edit Project MCP`**: Opens the project-specific `.altrucoder/mcp.json` file. If this file doesn't exist, Altru Coder will create it for you.

{% image src="/docs/img/using-mcp-in-altru-coder/mcp-installed-config.png" alt="Edit Global MCP and Edit Project MCP buttons" width="600" caption="Edit Global MCP and Edit Project MCP buttons" /%}
...
{% image src="/docs/img/using-mcp-in-altru-coder/using-mcp-in-altru-coder-5.png" alt="Delete confirmation box" width="400" caption="Delete confirmation box" /%}
...
{% image src="/docs/img/using-mcp-in-altru-coder/using-mcp-in-altru-coder-6.png" alt="Network Timeout pulldown" width="400" caption="Network Timeout pulldown" /%}
...
**In your config file:** Add the tool name (or a wildcard pattern) to the `permission` key in `altru-coder.jsonc`:
...
{% image src="/docs/img/using-mcp-in-altru-coder/using-mcp-in-altru-coder-7.png" alt="Always allow checkbox for MCP tools" width="120" caption="Always allow checkbox for MCP tools" /%}

When enabled, Altru Coder will automatically approve this specific tool without prompting. Note that the global "Use MCP servers" setting takes precedence - if it's disabled, no MCP tools will be auto-approved.
...
Altru Coder does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **Altru Coder Marketplace:** Browse community-contributed MCP server configurations and agent skills in the [Altru Coder Marketplace](https://github.com/Altru-Coder/altru-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask Altru Coder:** You can ask Altru Coder to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend Altru Coder with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, Altru Coder will automatically detect available tools and resources. To use them:

1. Type your request in the Altru Coder chat interface
2. Altru Coder will identify when an MCP tool can help with your task
...
- **Server Not Responding:** Check if the server process is running. Use `altru-coder mcp debug <server-name>` to inspect the connection.
- **Permission Errors:** Ensure API keys and credentials are set in your `altru-coder.jsonc` config or via `{env:VARIABLE_NAME}` references.
...
- **Permission Errors:** Ensure proper API keys and credentials are configured in your `mcp_settings.json` (for global settings) or `.altrucoder/mcp.json` (for project settings).
- **Tool Not Available:** Confirm the server is properly implementing the tool and it's not disabled in settings
