...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Altru Coder and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Altru Coder) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Altru Coder
...
- Single-client scenarios (one Altru Coder instance per server)
...
1. The client (Altru Coder) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Altru Coder
...
STDIO servers run locally on the same machine as Altru Coder, which has several important implications:
...
- **Execution**: Starts and stops with Altru Coder (child process lifecycle)
...
- Start when needed by Altru Coder
- Not require network configuration
- Need to be installed alongside Altru Coder or via a package manager
...
## Configuring Transports in Altru Coder

For detailed information on configuring STDIO and SSE transports in Altru Coder, including example configurations, see the [Understanding Transport Types](using-in-altru-coder#understanding-transport-types) section in the Using MCP in Altru Coder guide.
