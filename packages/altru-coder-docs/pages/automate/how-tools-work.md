...
Altru Coder uses tools to interact with your code and environment. These specialized helpers perform specific actions like reading files, making edits, running commands, or searching your codebase. Tools provide automation for common development tasks without requiring manual execution.

## Tool Workflow

Describe what you want to accomplish in natural language, and Altru Coder will:
...
**Altru Coder:** (Proposes the `write` tool)
...
**Altru Coder:** (Proposes the `write_to_file` tool as shown in the image above)
...
**Altru Coder:** (Confirms file creation)
...
Every tool use is subject to a permission check. The default action for any tool with no matching rule in your config is **`ask`** — meaning Altru Coder will pause and prompt you before executing it.
...
Every tool use requires your explicit approval. When Altru Coder proposes a tool, you'll see:
...
| `websearch` | Searches the web (Altru Coder/OpenRouter users) | Web |
| `codesearch` | Semantic code search (Altru Coder/OpenRouter users) | Web |
