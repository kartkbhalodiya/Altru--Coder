...
The Agent Manager is a control panel for running and orchestrating multiple Altru Coder agents, with support for parallel worktree-isolated sessions.

The Agent Manager is a **full-panel editor tab** built directly into the extension. It uses the extension's embedded runtime, so no separate Altru Coder CLI installation or CLI authentication setup is required. It supports:
...
- Command Palette: "Altru Coder: Open Agent Manager"
...
Managed worktrees are created under `.altru-coder/worktrees/` in your project. Altru Coder also stores Agent Manager UI state in `.altru-coder/agent-manager.json`.
...
2. Enter a branch name (or let Altru Coder generate one)
...
3. Altru Coder creates one worktree + session per version and runs them in parallel
...
Altru Coder can start Agent Manager sessions from chat with the experimental `agent_manager` tool. Enable it in **Settings > Experimental > Agent Manager Tool**, or set `experimental.agent_manager_tool` to `true` in `altru-coder.jsonc`.
...
Create a script file in `.altru-coder/` using the appropriate filename for your platform:
...
Altru Coder runs the script automatically whenever a new worktree is created. It uses `sh` for POSIX scripts, PowerShell for `.ps1`, and `cmd.exe` for `.cmd` / `.bat`, so executable permissions are not required.
...
# Copy a nested env file that Altru Coder does not auto-copy.
...
Before the setup script runs, Altru Coder automatically copies root-level `.env` files from the main repo into the new worktree.
...
- Files that already exist in the worktree, because Altru Coder never overwrites them

Use `.altru-coder/setup-script` for anything outside the automatic copy rules, including nested env files, ignored local config, local certificates, local database files, generated config directories, or tool-specific files required to run the project.
...
For example, on macOS / Linux create `.altru-coder/run-script`:
...
Agent Manager state is persisted in `.altru-coder/agent-manager.json`. It stores worktrees, sections, session tabs, ordering, collapsed state, diff preferences, and cached PR metadata. Git branches and worktree directories remain on disk separately.

Closing a managed worktree removes it from Agent Manager, deletes its `.altru-coder/worktrees/` directory, and deletes the local branch. Closing an imported external worktree removes the Agent Manager entry but leaves the external directory and branch untouched.
