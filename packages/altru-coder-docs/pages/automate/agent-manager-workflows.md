...
- **Run script** — create `.altru-coder/run-script` (or `.ps1` / `.cmd` / `.bat` on Windows) and trigger it with `Cmd+E` / `Ctrl+E`. Runs in whichever worktree is selected. Gets `WORKTREE_PATH` and `REPO_PATH` in the environment.
...
export COMPOSE_PROJECT_NAME="altru_coder_${name}"
...
Use `.altru-coder/setup-script` to make new worktrees runnable without manual setup. It runs after Altru Coder copies root-level `.env` and `.env.*` files, and before the agent starts in the new worktree.

Altru Coder's env copy is intentionally narrow:
...
Put the remaining project-specific setup in `.altru-coder/setup-script`, for example copying `apps/web/.env.local`, creating a per-worktree database, or installing dependencies. The setup script receives `WORKTREE_PATH` and `REPO_PATH` in the environment.
...
- **`altru-coder review` in CI** — automated PR review. See [Code Reviews](/docs/automate/code-reviews/overview) for the setup.
