...
Auto-Launch Configuration allows you to automatically start a Altru Coder task when opening a workspace, with support for specific profiles and modes. This was originally developed as an internal test feature, but we decided to expose it to users in case anyone finds it useful!
...
When you open a workspace in VS Code, Altru Coder automatically checks for a launch configuration JSON file. If found, it:
...
1. Create a `.altrucoder` directory in your workspace root (if it doesn't exist)
2. Create a `launchConfig.json` file inside the `.altrucoder` directory
...
- **`mode`** (string): The Altru Coder mode to use for this task. Available modes:
...
- Auto-launch triggers approximately 500ms after Altru Coder extension activation
...
1. Verify file location: `.altrucoder/launchConfig.json` in workspace root
