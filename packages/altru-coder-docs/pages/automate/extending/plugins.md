---
title: "Plugins"
description: "Extend the Altru Coder CLI with custom hooks, tools, auth providers, and more"
platform: new
---

# Plugins

Plugins extend Altru Coder by hooking into events, adding custom tools, registering auth or model providers, and customizing runtime behavior. They are TypeScript or JavaScript modules loaded at startup, and work in both the Altru Coder CLI and the VS Code extension.
...
- Global: `~/.config/altru-coder/plugin/`
- Project: `.altru-coder/plugin/`, `.altrucoder/plugin/`, or `.opencode/plugin/`
...
### From the `altru-coder plugin` command

Install an npm plugin and patch your config in one step:

```bash
# Install into the current project's config
altru-coder plugin my-plugin

# Install into your global config
altru-coder plugin my-plugin --global

# Replace an existing entry
altru-coder plugin my-plugin --force
```

The command resolves the package, reads its `package.json` for plugin entrypoints, and writes the entry into the appropriate config file (currently `.opencode/opencode.jsonc` / `.opencode/tui.jsonc` for local installs, or `~/.config/altru-coder/opencode.jsonc` / `~/.config/altru-coder/tui.jsonc` for `--global`) while preserving JSONC comments.
...
- **Install scripts are disabled** for npm plugins. Altru Coder installs packages with lifecycle scripts such as `install` and `postinstall` blocked.
- **Local plugins** are loaded directly from the plugin directory. If your plugin imports external packages, add a `package.json` to your config directory (see [Dependencies](#dependencies)) — Altru Coder runs `bun install` on startup so imports resolve.

### Load order

Plugins from all sources run on every session. They load in this order:

1. Internal built-ins (Altru Coder Gateway auth, Codex auth, Copilot auth, Cloudflare, etc.)
2. Global config plugin array (`~/.config/altru-coder/altru-coder.json`)
3. Global plugin directory (`~/.config/altru-coder/plugin/`)
4. Project config plugin array (`altru-coder.json` / `opencode.json`)
5. Project plugin directory (`.altru-coder/plugin/` and friends)
...
Set the `ALTRU_CODER_PURE=1` environment variable to skip all external plugins — only built-in plugins will load. Useful for reproducible CI runs or debugging.
...
| `client` | A Altru Coder SDK client (`@altru-coder/sdk`) for calling the local server. |
| `$` | [Bun's shell API](https://bun.com/docs/runtime/shell). |
| `serverUrl` | URL of the local Altru Coder server. |
...
Workspace adaptors let plugins add custom workspace targets to Altru Coder's workspace creation flow. This API is experimental and may change.
...
const WorkspacePlugin: Plugin = async ({ experimental_workspace }) => {
  experimental_workspace.register("folder", {
    ...
    configure(config) {
      return { ...config, directory: `/tmp/altru-${Date.now()}` }
    },
    async create(config) {
...
Published npm plugins should declare separate package entrypoints for each runtime they support. Altru Coder detects install targets from `package.json`:
...
```bash
bun add -d @altru-coder/plugin
```

```ts
import type { Plugin } from "@altru-coder/plugin"
import { tool } from "@altru-coder/plugin/tool"
```

Altru Coder automatically creates a `package.json` in config directories that contain a `plugin/` folder and installs `@altru-coder/plugin` so types resolve out of the box.
...
Altru Coder runs `bun install` at startup so your plugins can import the packages:

```ts
// .altru-coder/plugin/escape-bash.ts
...
| `shell.env` | Inject environment variables into every shell command Altru Coder runs. |
...
Altru Coder fills provider/model IDs from the returned catalog and uses the returned models in the picker and provider router.
...
The `event` hook fires for every event on Altru Coder's internal bus. Common event types include:
...
```ts
// .altru-coder/plugin/database.ts
import type { Plugin } from "@altru-coder/plugin"
import { tool } from "@altru-coder/plugin/tool"
...
For tools that don't need the full plugin context, drop them in a `tool/` or `tools/` folder inside any config directory — for example `.altru-coder/tool/database.ts` or `~/.config/altru-coder/tool/database.ts`. The filename becomes the tool name, and each file exports a `tool()` definition directly. The layout is identical to the [OpenCode custom tools guide](https://opencode.ai/docs/custom-tools); substitute `.altru-coder/` (or `.altrucoder/` / `.opencode/`) for `.opencode/`.
...
```ts
// .altru-coder/plugin/notify.ts
import type { Plugin } from "@altru-coder/plugin"

const Notify: Plugin = async ({ $ }) => ({
  event: async ({ event }) => {
    if (event.type === "session.idle") {
      await $`osascript -e 'display notification "Session complete!" with title "Altru Coder"'`
    }
  },
})
...
```ts
// .altru-coder/plugin/env-guard.ts
import type { Plugin } from "@altru-coder/plugin"
...
```ts
// .altru-coder/plugin/inject-env.ts
import type { Plugin } from "@altru-coder/plugin"
...
Prefer `client.app.log()` over `console.log` so entries land in Altru Coder's log pipeline:
...
```ts
// .altru-coder/plugin/compaction.ts
import type { Plugin } from "@altru-coder/plugin"
...
By default, Altru Coder sends a synthetic "continue" turn after compaction so the agent resumes the interrupted task. Use `experimental.compaction.autocontinue` to disable that turn for specific sessions or providers:
...
Plugins can also target the Altru Coder TUI itself — registering slash commands, routes, slots, dialogs, and keybinds. TUI plugins are SolidJS modules exported from `"./tui"` in your plugin package, or theme-only packages declared with `oc-themes`.

TUI plugins live in a separate module namespace (`@altru-coder/plugin/tui`) and have their own API surface (`TuiPluginApi`). Because the TUI API is larger and still evolving, this guide doesn't cover it exhaustively — use the types in `@altru-coder/plugin/tui` as the reference, and look at the built-in TUI plugins under `packages/opencode/src/cli/cmd/tui/feature-plugins/` for working examples.
...
- **Plugin failed to load** — check the CLI logs with `altru-coder --print-logs --log-level DEBUG`. Load failures are also surfaced as session errors in the TUI and VS Code extension.
...
- **Plugin loads in dev but not in CI** — verify `ALTRU_CODER_PURE` is not set, and that npm-installed plugins are cached under `packages/` in the current CLI XDG cache directory (`~/.cache/opencode/packages/` by default, or `$XDG_CACHE_HOME/opencode/packages/` when `XDG_CACHE_HOME` is set). Run with `--log-level DEBUG` to see install output.
- **Reset the plugin cache** — delete the plugin package folder under the CLI's `packages/` cache directory (or the `node_modules` cache under your config directory) and restart Altru Coder.

---

## Reference

- Types: [`@altru-coder/plugin`](https://github.com/Altru-Coder/altrucoder/tree/main/packages/plugin) — `Plugin`, `Hooks`, `PluginInput`, `ToolDefinition`, `AuthHook`, `ProviderHook`.
- Example plugin: [`packages/plugin/src/example.ts`](https://github.com/Altru-Coder/altrucoder/blob/main/packages/plugin/src/example.ts)
- CLI command: [`altru-coder plugin`](/docs/code-with-ai/platforms/cli-reference#altru-coder-plugin)
