...
The `execute_command` tool runs CLI commands on the user's system. It allows Altru Coder to perform system operations, install dependencies, build projects, start servers, and execute other terminal-based tasks needed to accomplish user objectives.
...
- Integrates with AltruCoderIgnore system for file access control
- Handles terminal escape sequences for clean output

## Limitations

- Command access may be restricted by AltruCoderIgnore rules and security validations
...
1. **Command Validation and Security Checks**:
   ...
   - Checks against AltruCoderIgnore rules for file access permissions
...
2. **Second Priority: Security Validation**
   ...
   - Commands are checked against AltruCoderIgnore rules for file access control
...
- When setting up a new project, Altru Coder runs initialization commands like `npm init -y` followed by installing dependencies.
- When building a web application, Altru Coder executes build commands like `npm run build` to compile assets.
- When deploying code, Altru Coder runs git commands to commit and push changes to a repository.
- When troubleshooting, Altru Coder executes diagnostic commands to gather system information.
- When starting a development server, Altru Coder launches the appropriate server command (e.g., `npm start`).
- When running tests, Altru Coder executes the test runner command for the project's testing framework.
