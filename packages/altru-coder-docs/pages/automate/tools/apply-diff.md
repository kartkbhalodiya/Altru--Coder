...
- When Altru Coder needs to make precise changes to existing code without rewriting entire files.
...
- Validates file access against `.altrucoderignore` rules.
...
2.  **AltruCoderIgnore Check**: Validates if the target file path is allowed by `.altrucoderignore` rules.
...
Altru Coder uses this strategy for applying diffs:
