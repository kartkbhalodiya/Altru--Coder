...
The `use_mcp_tool` tool enables interaction with external tools provided by connected Model Context Protocol (MCP) servers. It extends Altru Coder's capabilities with domain-specific functionality through a standardized protocol.
...
This tool allows Altru Coder to access specialized functionality provided by external MCP servers. Each MCP server can offer multiple tools with unique capabilities, extending Altru Coder beyond its built-in functionality. The system validates arguments against schemas, manages server connections, and processes responses of various content types (text, image, resource).
...
- **Global Configuration**: Managed through the Altru Coder extension settings in VS Code. These apply across all projects unless overridden.
- **Project-level Configuration**: Defined in a `.altrucoder/mcp.json` file within your project's root directory.
...
- Since `.altrucoder/mcp.json` can be committed to version control, it simplifies sharing configurations with your team.
...
3. **Response Processing**:
   ...
   - Results are formatted for display in the Altru Coder interface
