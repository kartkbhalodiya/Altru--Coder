...
The `read_file` tool examines the contents of files in a project. It allows Altru Coder to understand code, configuration files, and documentation to provide better assistance.
...
- When Altru Coder needs to understand existing code structure
- When Altru Coder needs to analyze configuration files
- When Altru Coder needs to extract information from text files
- When Altru Coder needs to see code before suggesting changes
...
- When asked to explain or improve code, Altru Coder first reads the relevant files to understand the current implementation.
- When troubleshooting configuration issues, Altru Coder reads config files to identify potential problems.
- When working with documentation, Altru Coder reads existing docs to understand the current content before suggesting improvements.
...
If the file is excluded by rules in a `.altrucoderignore` file:
...
```
Error: Access denied to file '.env' due to .altrucoderignore rules.
```
