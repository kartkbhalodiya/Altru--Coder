...
The `list_files` tool displays the files and directories within a specified location. It helps Altru Coder understand your project structure and navigate your codebase effectively.
...
- When Altru Coder needs to understand your project structure
- When Altru Coder explores what files are available before reading specific ones
- When Altru Coder maps a codebase to better understand its organization
- Before using more targeted tools like `read_file` or `search_files`
- When Altru Coder needs to check for specific file types (like configuration files) across a project
...
- Marks files ignored by `.altrucoderignore` with a lock symbol (🔒) when `showAltruCoderIgnoredFiles` is enabled
...
5. **Result Filtering**:
   ...
   - Respects `.gitignore` rules when in recursive mode
   - Handles `.altrucoderignore` patterns, either hiding files or marking them with a lock symbol
6. **Formatting**:
   - Marks directories with a trailing slash (`/`)
   - Sorts results to show directories before their contents for logical hierarchy
   - Marks ignored files with a lock symbol (🔒) when `showAltruCoderIgnored` is enabled
...
- Files ignored by `.altrucoderignore` are marked with a lock symbol (🔒) when `showAltruCoderIgnored` is enabled
...
When `.altrucoderignore` files are used and `showAltruCoderIgnored` is enabled:
...
- When starting a new task, Altru Coder may list the project files to understand its structure before diving into specific code.
- When asked to find specific types of files (like all JavaScript files), Altru Coder first lists directories to know where to look.
- When providing recommendations for code organization, Altru Coder examines the current project structure first.
- When setting up a new feature, Altru Coder lists related directories to understand the project conventions.
