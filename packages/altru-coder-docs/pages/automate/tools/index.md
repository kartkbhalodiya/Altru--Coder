...
description: Learn how Altru Coder's tools automate your development workflow
---

# Tool Use Overview

Altru Coder implements a sophisticated tool system that allows AI models to interact with your development environment in a controlled and secure manner. This document explains how tools work, when they're called, and how they're managed.
...
| **Browser Group** | Web browser automation | `altru-coder-playwright_*` (via built-in Playwright MCP) | Browser testing and interaction |
...
These tools help Altru Coder understand your code and project:
...
These tools help Altru Coder make changes to your code:
...
For multiple replacements in one file, Altru Coder uses repeated `edit` calls or a patch-style edit when the model supports it.

### Execute Tools

These tools help Altru Coder run commands:

- `bash` - Runs shell commands with configurable timeout and working directory

### Web Tools

These tools help Altru Coder access web content:

- `webfetch` - Fetches a URL and returns the content
- `websearch` - Searches the web (available to Altru Coder/OpenRouter users)
- `codesearch` - Semantic code search (available to Altru Coder/OpenRouter users)

### Browser Tools

The VS Code extension has a built-in browser automation tool powered by [Playwright MCP](https://www.npmjs.com/package/@playwright/mcp). Enable it in Settings → Browser Automation. When enabled, it registers an MCP server named `altru-coder-playwright` and exposes tools such as:

- `altru-coder-playwright_browser_navigate` - Navigate to a URL
- `altru-coder-playwright_browser_click` - Click an element
- `altru-coder-playwright_browser_type` - Type text into an element
- `altru-coder-playwright_browser_screenshot` - Capture a screenshot
- `altru-coder-playwright_browser_snapshot` - Capture an accessibility snapshot
