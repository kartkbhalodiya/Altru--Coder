...
The `list_code_definition_names` tool provides a structural overview of your codebase by listing code definitions from source files at the top level of a specified directory. It helps Altru Coder understand code architecture by displaying line numbers and definition snippets.
...
- When Altru Coder needs to understand your codebase architecture quickly
- When Altru Coder needs to locate important code constructs across multiple files
...
- When starting a new task, Altru Coder first lists key code definitions to understand the overall structure of your project.
- When planning refactoring work, Altru Coder uses this tool to identify classes and functions that might be affected.
- When exploring unfamiliar codebases, Altru Coder maps the important code constructs before diving into implementation details.
- When adding new features, Altru Coder identifies existing patterns and relevant code definitions to maintain consistency.
- When troubleshooting bugs, Altru Coder maps the codebase structure to locate potential sources of the issue.
- When planning architecture changes, Altru Coder identifies all affected components across files.
