...
- When Altru Coder needs to create a new file from scratch
- When Altru Coder needs to completely rewrite an existing file
...
- Access Control: Validates against `.altrucoderignore` restrictions before making changes
...
1. **Parameter Validation**: Validates the required parameters and permissions
   ...
   - Validates the file is allowed (not restricted by `.altrucoderignore`)
...
- When creating a new project, Altru Coder generates multiple files but lets you review each before committing changes.
- When setting up configuration files, Altru Coder shows the proposed configuration in a diff view for approval.
- When generating documentation, Altru Coder creates markdown files but lets you make final adjustments in the diff view.
- When developing a prototype, Altru Coder shows complete source files in a diff view where you can fine-tune before saving.
