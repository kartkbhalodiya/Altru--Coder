...
The `access_mcp_resource` tool retrieves data from resources exposed by connected Model Context Protocol (MCP) servers. It allows Altru Coder to access files, API responses, documentation, or system information that provides additional context for tasks.
...
- When Altru Coder needs additional context from external systems
- When Altru Coder needs to access domain-specific data from specialized MCP servers
- When Altru Coder needs to retrieve reference documentation hosted by MCP servers
- When Altru Coder needs to integrate real-time data from external APIs via MCP
...
4. **Response Processing**:
   ...
   - Returns the processed resource data to Altru Coder for use in the current task
...
- When helping with API development, Altru Coder retrieves endpoint specifications from MCP resources to ensure correct implementation.
- When assisting with data visualization, Altru Coder accesses current data samples from connected MCP servers.
- When working in specialized domains, Altru Coder retrieves technical documentation to provide accurate guidance.
- When generating industry-specific code, Altru Coder references compliance requirements from documentation resources.
