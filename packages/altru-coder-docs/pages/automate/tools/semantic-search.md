...
- When Altru Coder needs to find code related to specific functionality across your project
...
- When implementing a new feature, Altru Coder searches for "authentication middleware" to understand existing patterns before writing new code.
- When debugging an issue, Altru Coder searches for "error handling in API calls" to find related error patterns across the codebase.
- When refactoring code, Altru Coder searches for "database transaction patterns" to ensure consistency across all database operations.
- When onboarding to a new codebase, Altru Coder searches for "configuration loading" to understand how the application bootstraps.
