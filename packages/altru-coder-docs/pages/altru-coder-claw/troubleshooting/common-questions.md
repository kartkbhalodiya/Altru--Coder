...
description: "Answers to common AltruCoderClaw troubleshooting questions"
---
...
2. Click **OpenClaw Doctor** on your [dashboard](/docs/altru-coder-claw/dashboard)
...
1. Check that the channel token is configured in [Settings](/docs/altru-coder-claw/dashboard#channels)
...
You can directly access the files in `/root/.openclaw/` on the [AltruCoderClaw Dashboard](https://app.altru-coder.ai/claw) using the file browser of the edit files dialog. This can be a useful way to examine or update the config files (especially `openclaw.json`) if you run into an issue. There may also be backups in the form of `openclaw.bak` files that you can manually restore from if needed.
...
3. If the issue persists, join the [Altru Coder Discord](https://altru-coder.ai/discord) and share details in the AltruCoderClaw channel
