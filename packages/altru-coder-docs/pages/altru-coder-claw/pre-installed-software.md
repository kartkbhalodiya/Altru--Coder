...
description: "Default system utilities, languages, and CLI tools included in the AltruCoderClaw Docker image"
---

# Pre-installed Software

Every AltruCoderClaw instance ships with a curated set of system utilities, language runtimes, package managers, and CLI tools. This page documents everything that comes pre-installed in the AltruCoderClaw Docker image so you know what's available out of the box. Where a specific version is listed it reflects the pin in the Dockerfile as of March 2026. Entries marked **unpinned** install the latest available version at image build time and may differ between releases.

## Base Image

AltruCoderClaw is built on **Debian Bookworm** (`debian:bookworm-slim`). Since it's Debian-based, you can use `apt` to install additional packages at any time:
...
{% callout type="tip" %}
These tools receive updates when you **Upgrade & Redeploy** your instance from the [AltruCoderClaw Dashboard](/docs/altru-coder-claw/dashboard#redeploy). Check the changelog for image update announcements.
{% /callout %}
...
- [Machine Specs](/docs/altru-coder-claw/dashboard#machine-specs)
