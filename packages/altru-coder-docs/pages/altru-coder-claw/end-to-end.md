...
description: "Start-to-finish guide for configuring your AltruCoderClaw instance"
---

# Setup walkthrough

This guide walks you through a full AltruCoderClaw setup — from creating accounts to scheduling your first automated workflow. Plan for about 60 minutes.

## Planning your setup

For most users, a useful AltruCoderClaw configuration involves:
...
We recommend creating **separate accounts** for your AltruCoderClaw rather than connecting it to your personal accounts. This applies to Google, GitHub, and any other services you connect. A dedicated account improves isolation — your personal data stays separate, and you can control exactly what access the Claw has by sharing or delegating only what you want.

### Chat platform options

- **[Altru Coder Chat](https://app.altru-coder.ai)** — available in the web app and coming soon to iOS and Android; requires zero configuration
- **[Telegram](/docs/altru-coder-claw/chat-platforms/telegram)** — easy to set up, private by default
- **[Discord](/docs/altru-coder-claw/chat-platforms/discord)** — moderate setup
- **[Slack](/docs/altru-coder-claw/chat-platforms/slack)** — most involved setup

{% callout type="warning" title="Chain-of-connection security" %}
If your Claw has access to sensitive data (like your email), be careful which chat platform you connect it to. On broadly-accessible platforms like Slack or Discord, anyone on the server could potentially message your Claw and access that data. If you're connecting sensitive integrations, use a private platform like Altru Coder Chat or Telegram.
{% /callout %}
...
If you are doing a [1-1 configuration call with Altru Coder](https://altru-coder.ai/altru-coder-claw/config-service), please complete these steps before the call.
...
1. **Create a Google Account for your Claw** — Go to [google.com](https://www.google.com/) and create a new Google/Gmail account dedicated to your AltruCoderClaw. Something like `yourname.bot@gmail.com` works well.
...
2. **Set up Google Cloud** — Visit [console.cloud.google.com](https://console.cloud.google.com). Accept the terms of service and click "Start my free tier". You may need to add a credit card for identity verification.
   ...
   {% callout type="info" %}
   Nothing AltruCoderClaw does costs any money with Google.
   {% /callout %}

3. **Install Docker** — AltruCoderClaw configures Google by running a Docker container on your machine. Download Docker at [docker.com](https://www.docker.com/), then open it. You don't need to sign in or create a Docker account.
...
Your Claw needs a way to communicate with you. **[Altru Coder Chat](https://app.altru-coder.ai)** requires no setup — just open the web app. For other platforms, follow the relevant guide:

- [Telegram](/docs/altru-coder-claw/chat-platforms/telegram) — about 2 minutes
- [Discord](/docs/altru-coder-claw/chat-platforms/discord) — about 10 minutes
- [Slack](/docs/altru-coder-claw/chat-platforms/slack) — about 15 minutes; always use the manifest

{% callout type="tip" %}
If you're not sure which to pick, Altru Coder Chat (no setup) or Telegram (2 minutes) are the easiest options.
{% /callout %}
...
1. In the AltruCoderClaw dashboard, go to **Settings → Google Account** and copy the Docker command shown.
...
For full details, see the [Google setup guide](/docs/altru-coder-claw/development-tools/google).
...
For full details, see the [GitHub setup guide](/docs/altru-coder-claw/development-tools/github).
...
**Push notifications:** by default, your Claw wakes up on every incoming email. If you'd prefer a digest (e.g., once at 7am), disable push notifications in **Settings → Google Account** on the [dashboard](/docs/altru-coder-claw/dashboard) — otherwise it processes each email as it arrives.
...
By default, AltruCoderClaw asks for confirmation before every tool call. To let it act freely, go to the [AltruCoderClaw dashboard](https://app.altru-coder.ai/claw) and enable auto-approval in the **Default Permissions** section.
...
Or ask your Claw to build a custom skill from scratch — it has a built-in skill-builder skill for exactly this. You can explore popular skills and use case inspiration at the [AltruCoderClaw Bytes library](https://altru-coder.ai/altru-coder-claw/bytes).

## Manage inference

**Model picker:** Balanced is a good starting point. Frontier is more capable but significantly more expensive.

You can also use your AltruCoderPass credits — find this under **Profile** in the dashboard.
