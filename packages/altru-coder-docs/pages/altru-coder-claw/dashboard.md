---
title: "AltruCoderClaw Dashboard Reference"
description: "Managing your AltruCoderClaw instance from the dashboard"
---

# AltruCoderClaw Dashboard

This page covers everything you can do from the AltruCoderClaw dashboard. For getting started, see [AltruCoderClaw Overview](/docs/altru-coder-claw/overview).

{% image src="/docs/img/altru-coder-claw/dashboard.png" alt="Connect account screen" width="800" caption="The AltruCoderClaw Dashboard" /%}
...
For access to the full catalog of 335+ models, use the `/model` and `/models` commands in the [Control UI Chat](/docs/altru-coder-claw/control-ui#changing-models).

### Channels

You can connect Telegram, Discord, and Slack by entering bot tokens in the Settings tab. See [Connecting Chat Platforms](/docs/altru-coder-claw/chat-platforms) for setup instructions.
...
See [Version Pinning](/docs/altru-coder-claw/control-ui/version-pinning) for details.
...
> 💡 **Tip**
> If your instance is in a crash loop and you can't access the Control UI, try **Restore Config** from the AltruCoderClaw dashboard first before redeploying.
...
When your instance is running you can access the [OpenClaw Control UI](/docs/altru-coder-claw/control-ui) — a browser-based dashboard for managing your agent, channels, sessions, exec approvals, and more:

1. Click **Open** to launch the OpenClaw web interface in a new tab

See the [Control UI reference](/docs/altru-coder-claw/control-ui) for a full overview of its capabilities.

{% callout type="warning" %}
Do not use the **Update** feature in the OpenClaw Control UI to update AltruCoderClaw. Use **Redeploy** from the AltruCoderClaw Dashboard instead. Updating via the Control UI will not apply the correct AltruCoderClaw platform image and may break your instance.
{% /callout %}
...
You need to **approve** each request before the user or device can interact with your agent. See [Pairing Requests](/docs/altru-coder-claw/chat-platforms#pairing-requests) for details.

## Changelog

The dashboard shows recent AltruCoderClaw platform updates. Each entry is tagged as a **feature** or **bugfix**, and some include a deploy hint:
...
- [OpenClaw Control UI](/docs/altru-coder-claw/control-ui)
