---
title: "Slack"
description: "Using AltruCoderClaw with Slack"
---

# Slack

This page covers everything you need to use AltruCoderClaw with Slack: connecting your bot, controlling who can DM it, and adding it to channels.

## Connecting AltruCoderClaw to Slack

{% youtube url="https://youtu.be/Q5bt-qH-_pY" title="Slack Setup Guide" caption="How to connect your AltruCoderClaw agent to Slack" /%}

Create a Slack app from the OpenClaw manifest and link it to your AltruCoderClaw dashboard.
...
4. Customize the manifest before creating:
   ...
   - Update the slash command if desired (e.g., `/altru-coder-claw`)
5. Click **Create**
...
### Step 3: Connect Slack to AltruCoderClaw

1. In the [AltruCoderClaw UI](https://app.altru-coder.ai/claw), find the Slack integration section (may show "not configured")
2. Enter both tokens:
   - The `xapp-` app-level token
   - The `xoxb-` bot user OAuth token
3. Click **Save**
4. Scroll to the top of the AltruCoderClaw UI and click **Redeploy**. Wait for the instance to come back up

### Step 4: Pair Slack with AltruCoderClaw

1. In Slack, DM the app and send any message — this triggers the pairing flow
2. The app will return a pairing code
3. Return to [app.altru-coder.ai/claw](https://app.altru-coder.ai/claw) and confirm the pairing code and approve
...
By default, AltruCoderClaw can respond to any DMs and will not respond in Slack channels, even if added.

## Making AltruCoderClaw DM-Only (from you)

By default, AltruCoderClaw will respond to DMs from any user in Slack.
...
Tell your AltruCoderClaw agent:
...
## Adding AltruCoderClaw to a Slack Channel

By default, AltruCoderClaw will not respond in Slack channels, even if added. To have AltruCoderClaw participate in a Slack channel:
...
Tell your AltruCoderClaw agent (via DM):
