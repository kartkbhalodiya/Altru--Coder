...
description: "Configure native OS notifications for Altru Coder"
platform: legacy
---
...
Altru Coder uses system notifications to inform you about:
...
Altru Coder's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. Altru Coder uses macOS's built-in command to display notifications.
...
**How it works:** Altru Coder first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
