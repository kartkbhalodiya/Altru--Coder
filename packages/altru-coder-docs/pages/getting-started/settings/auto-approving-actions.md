...
description: "Configure automatic approval settings for Altru Coder operations"
---

# Auto-Approving Actions

{% callout type="danger" %}
**Security Warning:** Auto-approve settings bypass confirmation prompts, giving Altru Coder direct access to your system. This can result in data loss, file corruption, or worse. Command line access is particularly dangerous, as it can potentially execute harmful operations that could damage your system or compromise security. Only enable auto-approval for actions you fully trust.
{% /callout %}
...
The UI reads and writes to the same `altru-coder.jsonc` config files used by the CLI, so changes made in either place are reflected in both.
...
| `"ask"` | Altru Coder pauses and asks for approval before running the tool |
...
When a tool is set to `"ask"`, Altru Coder pauses and displays a permission prompt with two options:
...
For full details and examples, see [MCP Tool Permissions](/docs/automate/mcp/using-in-altru-coder#auto-approve-tools).
...
The CLI uses a granular, per-tool permission system configured in `altru-coder.jsonc`. Instead of broad categories like "read" or "write," each tool has its own permission level with glob-pattern rules for fine-grained control.
...
Permissions are configured under the `permission` key in `altru-coder.jsonc`. The following tool-specific permission levels are available:
...
When a tool is set to `"ask"`, Altru Coder pauses and displays a permission prompt. You have three options:
...
2. Select which actions Altru Coder can perform without asking permission
3. Use the master toggle (leftmost checkbox) to quickly enable/disable all permissions

[![AltruCoder Task Timeline](https://img.youtube.com/vi/NBccFnYDQ-k/maxresdefault.jpg)](https://youtube.com/shorts/NBccFnYDQ-k?feature=shared)
...
|---|---|---|
| **Read files and directories** | Lets Altru Coder access files without asking | Medium |
| **Edit files** | Lets Altru Coder modify files without asking | **High** |
...
| **Use MCP servers** | Lets Altru Coder use configured MCP services | Medium-High |
| **Switch modes** | Changes between Altru Coder modes automatically | Low |
...
{% callout type="info" %}
Allow Altru Coder to automatically perform operations without requiring approval. Enable these settings only if you fully trust the AI and understand the associated security risks.
{% /callout %}
...
**Description:** When enabled, Altru Coder will automatically view directory contents and read files without requiring you to click the Approve button.

**Risk level:** Medium

While this setting only allows reading files (not modifying them), it could potentially expose sensitive data. Still recommended as a starting point for most users, but be mindful of what files Altru Coder can access.

#### Read Outside Workspace

**Setting:** "Allow reading files outside the workspace"

**Description:** When enabled, Altru Coder can read files outside the current workspace directory without asking for approval.

**Risk level:** Medium-High

This setting extends read permissions beyond your project folder. Consider the security implications:

- Altru Coder could access sensitive files in your home directory
...
**Recommendation:** Keep disabled unless you specifically need Altru Coder to read files outside your project.
...
This setting allows Altru Coder to modify your files without confirmation. The delay timer is crucial:
...
**Description:** When enabled, Altru Coder can create or modify files outside the current workspace directory without asking for approval.

**Risk level:** Very High

Use with caution and in controlled environments. It allows Altru Coder to:
...
**Description:** When enabled, Altru Coder can overwrite or modify files that are normally protected by the [`.altrucoderignore`](/docs/customize/custom-rules) file.
...
This setting allows Altru Coder to permanently remove files without confirmation.

**Safeguards:**

- Altru Coder still respects `.altrucoderignore` rules
...
Allows Altru Coder to control a headless browser without confirmation. This can include:
...
The delay controls how long Altru Coder waits before trying again:
...
Allows Altru Coder to change between different modes (Code, Architect, etc.) without asking for permission. This primarily affects the AI's behavior rather than system access.
...
Enables Altru Coder to create and complete subtasks automatically. This relates to workflow organization rather than system access.
...
Add command prefixes (e.g., `git`, `npm`, `ls`) that Altru Coder can run without asking. Use `*` to allow all commands (use with caution).
...
**Description:** Automatically selects the first AI-suggested answer for a follow-up question after a configurable timeout. This speeds up your workflow by letting Altru Coder proceed without manual intervention.
...
- Overnight runs where you want Altru Coder to continue working
...
This setting allows Altru Coder to automatically update task progress and todo lists during work sessions. This includes:
...
This is particularly useful when combined with the Subtasks permission, as it allows Altru Coder to maintain a complete picture of project progress without constant approval requests.
...
**"You Only Live Once"** mode enables _all_ auto-approve permissions at once using the master toggle. This gives Altru Coder complete autonomy to read files, write code, execute commands, and perform any operation without asking for permission.
...
This is the fastest way to work with Altru Coder, but also the riskiest. Use it only when you fully trust the AI and are prepared for the consequences.
{% /callout %}
