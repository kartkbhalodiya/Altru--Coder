...
description: "Use your own API keys with Altru Coder Gateway while retaining platform features"
---

# Bring Your Own Key (BYOK)

Bring Your Own Key (BYOK) lets you use your own API keys when using the Altru Coder Gateway, while retaining Altru Coder platform features like Code Reviews and Cloud Agents.

A user or organization may want to use BYOK to:

- Utilize new models quickly, Altru Coder Gateway supports most new models in minutes
...
Altru Coder Gateway supports BYOK keys for these providers.
...
These providers offer coding-focused subscriptions or dedicated endpoints. Bring the API key issued by your plan to use its included models through the Altru Coder Gateway:
...
1. Log into the Altru Coder platform and select the account or organization you want to add the BYOK key to.
...
- When you use the **Altru Coder Gateway** provider, Altru Coder checks if there's a BYOK key for the selected model's provider.
- If a matching BYOK key exists, the request is routed using your key.
- If the key is invalid, the request fails. It does not fall back to using Altru Coder's keys.
...
- BYOK works with the Altru Coder Gateway provider. Users should ensure that is set as the active [provider](/docs/ai-providers).
