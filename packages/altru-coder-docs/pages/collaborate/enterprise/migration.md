---
title: "Migration"
description: "Migrate your team to Altru Coder Enterprise"
---

# Migration

Switch to **Altru Coder Teams** or **Altru Coder Enterprise** from other AI coding tools and experience transparent pricing, no vendor lock-in, and superior team management capabilities.

## Why Teams Switch to Altru Coder
...
**Altru Coder Teams** and **Altru Coder Enterprise** show you exactly what each AI request costs - no markup, no hidden fees, complete transparency.

### No Rate Limiting

**Other tools** slow you down with rate limits and model switching when you need AI most.

**Altru Coder Teams** and **Altru Coder Enterprise** never limit your usage - pay for what you use, use what you need.

### True Team Management

**Other solutions** offer basic user management with limited visibility.

**Altru Coder Teams** provides comprehensive team analytics, role-based permissions, and detailed usage insights, while **Altru Coder Enterprise** adds advanced governance, audit logging, and enterprise-level security controls.
...
### What You Gain with Altru Coder Teams or Altru Coder Enterprise
...
3. **Calculate current costs** (if visible) vs. Altru Coder pricing
4. **Plan migration timeline** to minimize disruption

**Step 2: Altru Coder Setup**

1. **Create organization** at [app.altru-coder.ai](https://app.altru-coder.ai)
2. **Subscribe to Teams ($15/user/month)** or **Enterprise ([Contact Sales](https://altru-coder.ai/contact-sales))**
3. **Configure team settings** and usage policies
4. **Purchase initial AI credits** based on usage estimates

**Step 3: Team Migration**

1. **Invite team members** to Altru Coder
2. **Install Altru Coder extension** alongside Cursor initially
3. **Migrate projects gradually** starting with non-critical work
4. **Train team** on Altru Coder features and workflows

**Step 4: Full Transition**

1. **Monitor usage patterns** in Altru Coder dashboard
...
| Cursor Feature | Altru Coder Equivalent |
|---|---|
...
2. **Install Altru Coder** extension for team members
3. **Start with simple tasks** in Altru Coder
4. **Compare results** and team satisfaction

**Phase 2: Gradual Transition (Week 3-4)**

1. **Use Altru Coder** for new projects
...
Altru Coder uses a compatible rules system that supports Cursor and Windsurf patterns. Migrating your custom rules and configurations is straightforward and typically takes 5-10 minutes per project.

**Quick Overview:**

- **Project rules**: `.cursor/rules/*.mdc` → `.altrucoder/rules/*.md` (remove YAML frontmatter, keep Markdown content)
- **Legacy rules**: `.cursorrules` → `.altrucoder/rules/legacy-rules.md`
- **AGENTS.md**: Works identically in Altru Coder (no conversion needed)
- **Global rules**: Recreate in `~/.altrucoder/rules/*.md` directory

Altru Coder also supports mode-specific rules (`.altrucoder/rules-{mode}/`), which Cursor and Windsurf don't have. This allows different rules for different workflows (e.g., Code mode vs Debug mode).
...
**Altru Coder Transparent Pricing**
...
- Altru Coder extension installation and setup
...
- **Email:** migrations@altru-coder.ai
...
Ready to make the switch? Contact our migration team at migrations@altru-coder.ai to plan your transition to transparent AI coding.
