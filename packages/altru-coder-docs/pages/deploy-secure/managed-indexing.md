...
Altru Coder's **Managed Indexing** feature provides semantic search across your repositories using cloud-hosted embeddings. When enabled, Altru Coder indexes your codebase to deliver more relevant, context-aware responses during development.
...
1. Create a `.altrucoder/config.json` file in the root of your repository (if it doesn't already exist).
...
- **Storage** — Embeddings are stored in Altru Coder Cloud. Your actual code is never stored, only the vector embeddings.
- **Team sharing** — For teams and enterprise accounts, indexes are shared among all team members.

### Index Retention

Indexes are stored for **7 days**. If a branch or repository index hasn't been updated within that window, it will be garbage collected. The next time you open the project in VS Code with Altru Coder running, it will be re-indexed automatically.
...
A minimal UI is available at [app.altru-coder.ai](https://app.altru-coder.ai) to:
