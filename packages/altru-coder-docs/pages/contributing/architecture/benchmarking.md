---
title: "Benchmarking"
description: "Design for benchmarking Altru Coder against models and other agents"
---
...
This document proposes a benchmarking system for Altru Coder with two primary goals:

1. **Compare models against one another** using the same agent -- measuring task completion, token cost, and total time
2. **Compare agents against one another** using the same model -- e.g., Altru Coder vs Claude Code, or Altru Coder v1.0 vs v1.1
...
The key engineering deliverable is a **Altru Coder Harbor adapter** that runs Altru Coder CLI autonomously in containerized environments and emits ATIF-compliant trajectories.
...
As Altru Coder evolves, we need systematic answers to questions like:
...
- How does Altru Coder compare to Claude Code, Codex, or other agents on the same tasks?
...
1. Run Altru Coder against standardized benchmark datasets in a reproducible, containerized environment
2. Compare model performance (same agent, different models) on task completion, token cost, and wall-clock time
3. Compare agent performance (same model, different agents or Altru Coder versions) on the same metrics
...
Harbor is the standard evaluation framework used by many frontier labs. Rather than building our own harness, we write a Altru Coder adapter and plug into the existing ecosystem.
...
```bash
opik harbor run -d terminal-bench@head -a altru-coder -m anthropic/claude-opus-4
```
...
This makes it easy to create task sets that target specific Altru Coder capabilities -- for example, a set of refactoring tasks, or a set of multi-file debugging scenarios. Custom sets can be published to the Harbor registry or kept private.

See the [Harbor task tutorial](https://www.tbench.ai/docs/task-tutorial) for a step-by-step guide.

## Deliverables

### 1. Altru Coder Harbor Adapter

The primary engineering deliverable. This adapter:

- **Installs Altru Coder CLI** in a Docker container
- **Configures autonomous execution** using `altru-coder run --auto`, which disables all permission prompts so the agent runs fully unattended
- **Translates Harbor task prompts** into Altru Coder CLI invocations
- **Emits ATIF-compliant trajectories** capturing every step, tool call, and metric

The adapter follows the same pattern as existing Harbor agents (see the [OpenHands adapter](https://harborframework.com/docs/agents/trajectory-format#openhands-example) for reference). The key implementation detail is the `populate_context_post_run` method that converts Altru Coder's execution log into ATIF format.
...
Documentation and examples for creating Altru Coder-specific task sets:
...
Configure the Opik-Harbor integration for Altru Coder benchmark runs:

- Set up `opik harbor run` with the Altru Coder adapter
...
Run the same Altru Coder agent against Terminal-Bench with different models:

```bash
# Run with Claude Opus
opik harbor run -d terminal-bench@2.0 -a altru-coder -m anthropic/claude-opus-4

# Run with GPT-5
opik harbor run -d terminal-bench@2.0 -a altru-coder -m openai/gpt-5

# Run with Gemini 3 Pro
opik harbor run -d terminal-bench@2.0 -a altru-coder -m google/gemini-3-pro
```
...
### Comparing Altru Coder Versions

Test a new release against the previous version:

```bash
# Run current release
opik harbor run -d terminal-bench@2.0 -a altru-coder@v2.0 -m anthropic/claude-opus-4

# Run candidate release
opik harbor run -d terminal-bench@2.0 -a altru-coder@v2.1-rc1 -m anthropic/claude-opus-4
```
...
```bash
# Run against a custom Altru Coder-specific dataset
opik harbor run -d altru-coder-refactoring@1.0 -a altru-coder -m anthropic/claude-opus-4
```
