...
Many MCP servers require authentication to access protected resources. Currently, Altru Coder only supports static credential configuration (API keys, tokens) which must be manually entered and stored. This creates friction for users and security concerns for enterprises.

The MCP specification defines an OAuth 2.1-based authorization flow that enables secure, user-friendly authentication without requiring users to manually manage credentials. This document specifies how Altru Coder will implement the MCP Authorization specification to support OAuth-enabled MCP servers.
...
- **MCP Client** (Altru Coder) - Acts as OAuth 2.1 Client, obtains tokens on behalf of users
...
For Client ID Metadata Documents, Altru Coder needs to host a metadata document. We will use static hosting on altrucoder.ai:

- Host at `https://altrucoder.ai/.well-known/oauth-client/vscode-extension.json`
...
```json
{
  "client_id": "https://altrucoder.ai/.well-known/oauth-client/vscode-extension.json",
  "client_name": "Altru Coder",
  "client_uri": "https://altrucoder.ai",
  "logo_uri": "https://altrucoder.ai/logo.png",
  "redirect_uris": ["http://127.0.0.1:0/callback", "vscode://altrucoder.altru-coder/oauth/callback"],
...
2. **VS Code URI Handler** (Fallback)
   - Register `vscode://altrucoder.altru-coder/oauth/callback` URI handler
...
- [ ] Host Altru Coder client metadata at altrucoder.ai
