...
|---|---|---|
| `altru-coder-auto/frontier` | Best paid models | Paid |
| `altru-coder-auto/balanced` | Strong performance, lower cost | Paid |
| `altru-coder-auto/free` | Best available free models | Free |
| `altru-coder-auto/small` | Internal — background tasks | Varies |
...
3. **Background tasks** — Altru Coder uses small models for things like generating session titles and commit messages. These should be invisible and reliable, not dependent on the user's model selection or credit status.
...
**Who it's for**: Users who want to try Altru Coder without a credit card, students, hobbyists, and anyone exploring AI-assisted coding.
...
**Who it's for**: Not user-facing. Used internally by Altru Coder for lightweight background tasks (session titles, commit messages, conversation summaries).
...
**Implementation**: The `getSmallModel()` function in `packages/opencode/src/provider/provider.ts` prioritizes `altru-coder-auto/small` when the Altru Coder provider is active. If the user's provider doesn't have a dedicated small model, it falls back globally to `altru-coder-auto/small` when available.
...
Auto: Small does not appear in the model picker. It is filtered out by the UI (see `ALTRU_CODER_AUTO_SMALL_IDS` in the VS Code extension).

### Defaults

- **Authenticated users**: Default to `altru-coder-auto/balanced` (defined in `packages/altru-coder-gateway/src/api/constants.ts`)
- **Unauthenticated users**: Default to `altru-coder-auto/free`
...
Auto Model uses a split client/server architecture. The actual model-to-mode mappings are not hardcoded in the client — they're served dynamically from the Altru Coder API, making it possible to update routing without client releases.

### Server side (Altru Coder API)

The Altru Coder API at `api.altru-coder.ai` defines which underlying models each `altru-coder-auto/*` tier routes to per mode. Each auto model is returned with an `opencode.variants` field — a map of mode-specific provider options:
...
This is fetched via `packages/altru-coder-gateway/src/api/models.ts` which parses the `opencode.variants` field from the API response.

### Client side

The client-side chain works as follows:

1. **Model fetching**: `packages/opencode/src/provider/model-cache.ts` caches Altru Coder Gateway models with a 5-minute TTL, fetching from the Altru Coder API.

2. **Variant passthrough**: `packages/opencode/src/provider/transform.ts` — the `variants()` function passes through server-defined variants for Altru Coder Gateway models directly, rather than computing them locally.

3. **Variant storage**: `packages/opencode/src/provider/provider.ts` stores `variants` on the model object when the provider is `altru-coder`.
...
| File | Role |
|---|---|
| `packages/altru-coder-gateway/src/api/constants.ts` | Default model constants (`DEFAULT_MODEL`, `DEFAULT_FREE_MODEL`) |
| `packages/altru-coder-gateway/src/api/models.ts` | Fetches models from Altru Coder API, parses `opencode.variants` |
| `packages/opencode/src/provider/model-cache.ts` | Caches Altru Coder Gateway models with 5-min TTL |
| `packages/opencode/src/provider/provider.ts` | Preserves variants for altru-coder provider; `getSmallModel()` prioritizes `altru-coder-auto/small` |
| `packages/opencode/src/provider/transform.ts` | Passes through server-defined variants for Altru Coder Gateway models |
...
- Unauthenticated users default to `altru-coder-auto/free` with no configuration required
