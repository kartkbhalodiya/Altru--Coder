---
title: "Custom Modes"
description: "Create and configure custom modes in Altru Coder"
---

# Custom Modes

Altru Coder allows you to create **custom modes** (also called **agents**) to tailor Altru Coder's behavior to specific tasks or workflows. Custom modes can be either **global** (available across all projects) or **project-specific** (defined within a single project).

{% callout type="info" %}
The current VS Code extension (built on the Altru Coder CLI) uses **agent Markdown files** to define custom modes. The legacy extension used `custom_modes.yaml` / `.altrucodermodes`. See the tabs below for the relevant approach.
{% /callout %}
...
### 1. Ask Altru Coder! (Recommended)

Ask Altru Coder to create an agent for you:

```
Create a new agent called "docs-writer" that can only read files and edit Markdown files.
```

Altru Coder will generate the agent definition and write it to `.altru-coder/agent/` in your project.
...
```
.altru-coder/agents/my-agent.md
.altru-coder/agent/my-agent.md
...
```
~/.config/altru-coder/agent/my-agent.md
```
...
**Example agent file** (`.altru-coder/agents/docs-writer.md`):
...
### 4. Config File (`altru-coder.jsonc`)

Define agents under the `agent` key in your project's `altru-coder.jsonc`:
...
2. Global config (`~/.config/altru-coder/altru-coder.jsonc`)
3. Project config (`altru-coder.jsonc` at project root)
4. `.altru-coder/` / `.opencode/` directory configs and agent `.md` files
5. Environment variable overrides (`ALTRU_CODER_CONFIG_CONTENT`)
...
```jsonc
// altru-coder.jsonc — override the built-in "code" agent
{
  "agent": {
    "code": {
      "model": "openai/gpt-4o",
...
Or as a `.md` file (`.altru-coder/agents/code.md`):
...
If you have existing `.altrucodermodes` or `custom_modes.yaml` files from the VSCode extension, the extension automatically migrates them on startup. The migration converts:
...
| OS | Path |
|---|---|
| macOS | `~/Library/Application Support/Code/User/globalStorage/altrucoder.altru-coder/settings/custom_modes.yaml` |
| Linux | `~/.config/Code/User/globalStorage/altrucoder.altru-coder/settings/custom_modes.yaml` |
| Windows | `%APPDATA%\Code\User\globalStorage\altrucoder.altru-coder\settings\custom_modes.yaml` |

Project-level `.altrucodermodes` and workspace-scoped files are handled by the CLI backend that the extension delegates to — see the [CLI tab](#cli) for the full load-order table. After the extension migrates on startup, the legacy file is no longer consulted; remove new modes through the extension UI instead of editing `custom_modes.yaml` directly.
...
**Legacy `custom_modes.yaml` is not loaded from `~/.config/altru-coder/`.** If you're migrating from the legacy VSCode extension, global custom modes are read from `~/.altrucoder/cli/global/settings/custom_modes.yaml` (not from the CLI's XDG config directory). The recommended approach is to convert legacy modes to agent `.md` files and place them in `~/.config/altru-coder/agent/` instead — see [Markdown files](#3-markdown-files-with-yaml-frontmatter) and [Migration](#migration-from-vscode-extension-modes) below.
{% /callout %}
...
### 2. Using `altru-coder agent create`
...
The TUI also **remembers the last model you picked for each agent** across sessions. A config-pinned `model` acts as the default when no manual pick exists. To reset a pick and let the config take over, use the model picker (`Ctrl+X m`) and select a different model, or remove the saved pick from `~/.local/state/altru-coder/model.json`.
...
If you have existing `.altrucodermodes` or `custom_modes.yaml` files from the VSCode extension, the CLI automatically migrates them on startup. The migration converts:
...
| 2 | `~/.altrucoder/cli/global/settings/custom_modes.yaml` | YAML | Global |
| 3 | `~/.altrucodermodes` | YAML | Global |
| 4 | `<project>/.altrucodermodes` | YAML | Project (wins on conflict) |

{% callout type="info" %}
`~/.config/altru-coder/` is the XDG config directory for the new agent format — legacy `custom_modes.yaml` placed there will **not** be loaded. Use `~/.config/altru-coder/agent/*.md` or `~/.config/altru-coder/altru-coder.jsonc` for new agent definitions instead.
{% /callout %}
...
Each mode—including custom ones—features **Sticky Models**. This means Altru Coder automatically remembers and selects the last model you used with a particular mode. This lets you assign different preferred models to different tasks without constant reconfiguration, as Altru Coder switches between models when you change modes.
...
{% image src="/docs/img/custom-modes/custom-modes-2.png" alt="Custom mode creation interface in Altru Coder" width="600" caption="Custom mode creation interface in Altru Coder" /%}

_Altru Coder's interface for creating and managing custom modes._

## What's Included in a Custom Mode?

Custom modes are defined by several key properties. Understanding these concepts will help you tailor Altru Coder's behavior effectively.

| UI Field / YAML Property | Conceptual Description |
|---|---|
| **Slug** (`slug`) | A unique internal identifier for the mode. Used by Altru Coder to reference the mode, especially for associating mode-specific instruction files. |
| **Name** (`name`) | The display name for the mode as it appears in the Altru Coder user interface. Should be human-readable and descriptive. |
...
| **Role Definition** (`roleDefinition`) | Defines the core identity and expertise of the mode. This text is placed at the beginning of the system prompt and defines Altru Coder's personality and behavior when this mode is active. |
...
| **When to Use** (`whenToUse`) | _(Optional)_ Provides guidance for Altru Coder's automated decision-making, particularly for mode selection and task orchestration. Used by the Orchestrator mode for task coordination. |
| **Custom Instructions** (`customInstructions`) | _(Optional)_ Specific behavioral guidelines or rules for the mode. Added near the end of the system prompt to further refine Altru Coder's behavior. |
...
Modes are managed from the Modes area in Altru Coder. Depending on your UI layout, you can open this from the mode selector in the chat panel or from the notebook icon.
...
5. Altru Coder packages the mode's configuration and any rules into the YAML file
...
4. Choose the import level:
   - **Project:** Available only in current workspace (saved to `.altrucodermodes` file)
...
- **Project agents:** `.altru-coder/agents/*.md` (or `.opencode/agents/*.md`)
- **Global agents:** `~/.config/altru-coder/agents/*.md`
...
### Ask Altru Coder! (Recommended)

You can also have Altru Coder create an agent file for you. For example:

```
Create a new agent called "Documentation Writer". It should only be able to read files and write Markdown files.
```

Altru Coder will create the appropriate `.altru-coder/agents/docs-writer.md` file with the right frontmatter.
...
You can quickly create a basic custom mode by asking Altru Coder to do it for you. For example:

```
Create a new mode called "Documentation Writer". It should only be able to read files and write Markdown files.
```

Altru Coder will guide you through the process, prompting for necessary information and creating the mode using the preferred YAML format.
...
The interface provides fields for Name, Slug, Description, Save Location, Role Definition, When to Use (optional), Available Tools, and Custom Instructions. After filling these, click the "Create Mode" button. Altru Coder will save the new mode in YAML format.

### 3. Manual Configuration (YAML & JSON)

You can directly edit the configuration files to create or modify custom modes. This method offers the most control over all properties. Altru Coder now supports both YAML (preferred) and JSON formats.

- **Global Modes:** Edit `custom_modes.yaml` (primary). `custom_modes.json` is a legacy fallback and may still exist in older setups.
- **Project Modes:** Edit `.altrucodermodes` in your project root (YAML preferred; JSON still supported for compatibility).
...
If you see both YAML and JSON mode files, this is usually from legacy configuration. Altru Coder reads YAML first and does not keep both files synchronized line-by-line. In practice, edit YAML unless you have a specific reason to stay on JSON.
{% /callout %}
...
- **Usage:** Used internally and in file/directory names for mode-specific rules (e.g., `.altru-coder/rules-{slug}/`)
...
- **Purpose:** The display name shown in the Altru Coder UI
...
- **Purpose:** Provides guidance for Altru Coder's automated decision-making, particularly for mode selection and task orchestration
- **Format:** A string describing ideal scenarios or task types for this mode
- **Usage:** Used by Altru Coder for automated decisions and not displayed in the mode selector UI
...
While JSON is still fully supported, new modes created via the UI or by asking Altru Coder will default to YAML.
...
### Project Modes (`.altrucodermodes`)

- No automatic startup migration occurs for project-specific files
- Altru Coder can read `.altrucodermodes` files in either YAML or JSON format
- When editing through the UI, JSON files will be converted to YAML format
- For manual conversion, you can ask Altru Coder to help reformat configurations
...
### Preferred Method: Directory (`.altru-coder/rules-{mode-slug}/`)
...
### Fallback Method: Single File (`.altru-coderrules-{mode-slug}`)

```
.
├── .altru-coderrules-docs-writer  # Example for mode slug "docs-writer"
└── ... (other project files)
```

**Rules Directory Scope:**

- **Global modes:** Rules are stored in `~/.altru-coder/rules-{slug}/`
- **Project modes:** Rules are stored in `{workspace}/.altru-coder/rules-{slug}/`
...
1. **Project-level mode configurations** (from `.altrucodermodes` - YAML or JSON)
2. **Global mode configurations** (from `custom_modes.yaml`, then `custom_modes.json` if YAML not found)
3. **Default mode configurations**

**Important:** When modes with the same slug exist in both `.altrucodermodes` and global settings, the `.altrucodermodes` version completely overrides the global one for ALL properties.

## Overriding Default Modes

You can override Altru Coder's built-in modes (like 💻 Code, 🪲 Debug, ❓ Ask, 🏗️ Architect, 🪃 Orchestrator) by creating a custom mode with the same slug.
...
**Let Altru Coder Build Your Regex Patterns**

Instead of writing complex regex manually, ask Altru Coder:

```
Create a regex pattern that matches JavaScript files but excludes test files
```

Altru Coder will generate the pattern. Remember to adapt it for YAML (usually single backslashes) or JSON (double backslashes).
...
### Basic Documentation Writer (`.altru-coder/agents/docs-writer.md`)
...
### Test Engineer (`.altru-coder/agents/test-engineer.md`)
...
### Security Reviewer (`.altru-coder/agents/security-review.md`)
...
### Config File Example (`altru-coder.jsonc`)
...
- **Agent not appearing:** Ensure the `.md` file is in a recognized directory (`.altru-coder/agents/`, `.altru-coder/agent/`, `.opencode/agents/`). Check that the `mode` property is `primary` or `all` if you expect it in the agent picker.
...
- **Legacy modes are auto-migrated:** If you have `.altrucodermodes` files, they'll be converted on startup — no manual migration needed
...
- **Keep prompts focused:** The markdown body is your system prompt — write it as if briefing a colleague
- **Use `mode: subagent`** for helper agents that shouldn't be directly selectable by users
- **Test with `altru-coder agent create`** to see how the CLI generates agent definitions, then customize from there
- **Legacy modes are auto-migrated:** If you have `.altrucodermodes` files, they'll be converted on startup — no manual migration needed
...
Ready to explore more? Check out the [Show and Tell](https://github.com/Altru-Coder/altrucoder/discussions/categories/show-and-tell) to discover and share custom modes and agents created by the community!
