---
title: "Skills"
description: "Extend Altru Coder capabilities with skills"
---

# Skills

Altru Coder implements [Agent Skills](https://agentskills.io/home), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
## How Skills Work in Altru Coder
...
1. **Discovery**: Skills are scanned from designated directories when Altru Coder initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
Global skills are located in the `.altru-coder` directory within your Home directory:

- Mac and Linux: `~/.altru-coder/skills/`
- Windows: `\Users\<yourUser>\.altru-coder\skills\`
...
Located in `.altru-coder/skills/` within your project:
...
You can configure extra skill locations and remote skill URLs in your `altru-coder.jsonc` config (project or global):
...
Global skills are located in the `.altrucoder` directory within your Home directory.

- Mac and Linux: `~/.altrucoder/skills/`
- Windows: `\Users\<yourUser>\.altrucoder\`
...
Located in `.altrucoder/skills/` within your project:
...
```bash
# For Code mode only
mkdir -p ~/.altrucoder/skills-code/typescript-patterns

# For Architect mode only
mkdir -p ~/.altrucoder/skills-architect/microservices
```
...
When multiple skills share the same name, project-level skills (`.altru-coder/skills/`) take precedence over global skills (`~/.altru-coder/skills/`). Skills from compatibility directories (`.claude/skills/`, `.agents/skills/`) and additional configured paths are loaded alongside project and global skills.
...
When multiple skills share the same name, Altru Coder uses these priority rules:
...
- In the **CLI**: Skills are loaded when you start a new session or run `altru-coder run`
...
Skills are discovered when Altru Coder initializes:

- When VSCode starts
- When you reload the VSCode window (`Cmd+Shift+P` → "Developer: Reload Window")

Skills directories are monitored for changes to `SKILL.md` files. However, the most reliable way to pick up new skills is to reload VS or the Altru Coder extension.
...
In Altru Coder, the `name` field **must match** the parent directory name:
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.altru-coder/skills/api-design
   ```
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.altrucoder/skills/api-design
   ```
...
- **[Altru Coder Marketplace repository](https://github.com/Altru-Coder/altru-marketplace)** — Browse community skills on GitHub and manually download them into your skills directory
...
- **Altru Coder Marketplace** — Browse skills directly in the Altru Coder extension via the Marketplace tab, or explore the [Altru Coder Marketplace repository](https://github.com/Altru-Coder/altru-marketplace) on GitHub
- [Agent Skills Specification](https://agentskills.io/home) — The open specification that skills follow, enabling interoperability across different AI agents
...
3. **Check file location**: Ensure `SKILL.md` is directly inside the skill directory (e.g., `.altru-coder/skills/my-skill/SKILL.md`), not nested further.

4. **Check config paths**: If using `skills.paths` or `skills.urls`, verify the paths and URLs are correct in your `altru-coder.jsonc`.
...
1. **Check the Output panel**: Open `View` → `Output` → Select "Altru Coder" from dropdown. Look for skill-related errors.
...
Have you created a skill that others might find useful? Share it with the community by contributing to the [Altru Coder Marketplace](https://github.com/Altru-Coder/altru-marketplace)!

{% tabs %}
{% tab label="VSCode" %}

While the new platform does not yet have a built-in marketplace UI, skills from the [Altru Coder Marketplace repository](https://github.com/Altru-Coder/altru-marketplace) can be manually downloaded into your `.altru-coder/skills/` directory or loaded via `skills.urls` in config.
...
3. **Fork the marketplace repository**: Visit [github.com/Altru-Coder/altru-marketplace](https://github.com/Altru-Coder/altru-marketplace) and create a fork
...
- Follow the [contribution guidelines](https://github.com/Altru-Coder/altru-marketplace/blob/main/CONTRIBUTING.md) in the marketplace repository

For more details on contributing to Altru Coder, see the [Contributing Guide](/docs/contributing).
