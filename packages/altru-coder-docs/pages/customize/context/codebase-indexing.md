...
Codebase Indexing enables semantic code search across your entire project using AI embeddings. Instead of searching for exact text matches, it understands the _meaning_ of your queries, helping Altru Coder find relevant code even when you don't know specific function names or file locations.
...
4. **Provides the [`semantic_search`](/docs/automate/tools/semantic-search) tool** to Altru Coder for intelligent code discovery
...
- **Enhanced AI Understanding**: Altru Coder can better comprehend and work with your codebase
...
1. Open Altru Coder **Settings** → **Experimental**.
...
Alternatively, set `experimental.semantic_indexing` to `true` in your `altru-coder.jsonc`:
...
1. Open Altru Coder **Settings** → **Indexing**, or click the indexing indicator at the bottom of the prompt input panel.
...
You can also edit the `indexing` section in `altru-coder.jsonc` directly:
...
- **LanceDB** — embedded, file-based. No server to run. Stores data under your Altru Coder data directory by default.
...
Set the flag in your `altru-coder.jsonc`:
...
Open a Altru Coder TUI session and run:
...
All changes are written to your `altru-coder.jsonc` config and take effect immediately.
...
- `lancedb` — `{ directory? }` — embedded, file-based. No server to run. Uses a default Altru Coder data directory when omitted.
...
3. If you don't see the icon, open Altru Coder settings ({% codicon name="gear" /%}) and search for **Codebase Indexing**.
...
- Files matching `.gitignore` and [`.altrucoderignore`](/docs/customize/context/altrucoderignore) patterns
...
- **API Keys**: Stored in your `altru-coder.jsonc` config. Treat that file as a secret in shared environments.
...
- Check that `indexing.enabled` is `true` in your `altru-coder.jsonc`
...
Once indexed, Altru Coder can use the [`semantic_search`](/docs/automate/tools/semantic-search) tool to find relevant code:
...
The tool provides Altru Coder with:
...
- **Secure storage**: API keys are stored in your local `altru-coder.jsonc` configuration
- **Fully local option**: Use **Ollama + LanceDB** for completely local processing
- **Access control**: Respects existing file permissions and `.altrucoderignore`
