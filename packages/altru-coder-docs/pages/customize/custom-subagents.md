...
Altru Coder includes two built-in subagents:
...
Add agents to the `agent` section of your `altru-coder.jsonc` config file. Any key that doesn't match a built-in agent name creates a new custom agent.
...
- **Global**: `~/.config/altru-coder/agents/`
- **Project-specific**: `.altru-coder/agents/`
...
2. **Global config** (`~/.config/altru-coder/config.json`)
3. **Project config** (`altru-coder.jsonc` in the project root)
4. **Global agent markdown files** (`~/.config/altru-coder/agents/*.md`)
5. **Project agent markdown files** (`.altru-coder/agents/*.md`)
