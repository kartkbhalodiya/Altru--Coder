...
The Altru Coder **memory bank** feature has been deprecated in favor of AGENTS.md.
...
1. Examine the contents in `.altrucoder/rules/memory-bank/`
2. Move that content into your project's `AGENTS.md` file (or ask Altru Coder to do it for you)
   {% /callout %}

## What is AGENTS.md?

AGENTS.md is an open standard for configuring AI agent behavior in software projects. It's a simple Markdown file placed at the root of your project that contains instructions for AI coding assistants. The standard is supported by multiple AI coding tools, including Altru Coder, Cursor, and Windsurf.
...
When working in a subdirectory, Altru Coder will load both the root AGENTS.md and any subdirectory AGENTS.md files, with subdirectory files taking precedence for conflicting instructions.

## File Protection

Both `AGENTS.md` and `AGENT.md` are **write-protected files** in Altru Coder. This means:
...
## How AGENTS.md Works in Altru Coder

### Loading Behavior

When you start a task in Altru Coder:

1. Altru Coder checks for `AGENTS.md` or `AGENT.md` at the project root
...
| **[Instructions](/docs/customize/custom-rules)** | Project | `instructions` key in project `altru-coder.jsonc` | 2 |
| **AGENTS.md** | Project | `AGENTS.md` at project root | 3 |
| **[Instructions](/docs/customize/custom-rules)** | Global | `instructions` key in global `altru-coder.jsonc` | 4 |
| **[Skills](/docs/customize/skills)** | Both | `.altru-coder/skills/`, config `skills` key | Loaded on demand |
...
AGENTS.md works alongside Altru Coder's other configuration systems:

| Feature | Scope | Location | Purpose | Priority |
|---|---|---|---|---|
| **[Mode-specific Custom Rules](/docs/customize/custom-rules)** | Project | `.altrucoder/rules-{mode}/` | Mode-specific rules and constraints | 1 (Highest) |
| **[Custom Rules](/docs/customize/custom-rules)** | Project | `.altrucoder/rules/` | Altru Coder-specific rules and constraints | 2 |
| **[AGENTS.md](/docs/customize/agents-md)** | Project | `AGENTS.md` | Universal standard for any AI coding tool | 3 |
| **[Global Custom Rules](/docs/customize/custom-rules)** | Global | `~/.altrucoder/rules/` | Global Altru Coder rules | 4 |
...
```bash
export ALTRU_CODER_DISABLE_EXTERNAL_SKILLS=true
```
...
```json
{
  "altrucoder.useAgentRules": false
}
```
...
- **[Custom Rules](/docs/customize/custom-rules)** - Altru Coder-specific rules with more control
