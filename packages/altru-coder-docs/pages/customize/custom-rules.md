---
title: "Custom Rules"
description: "Define custom rules for Altru Coder behavior"
---

# Custom Rules

Custom rules provide a powerful way to define project-specific and global behaviors and constraints for the Altru Coder AI agent. With custom rules, you can ensure consistent formatting, restrict access to sensitive files, enforce coding standards, and customize the AI's behavior for your specific project needs or across all projects.
...
Altru Coder supports two types of custom rules:
...
Project rules are configured via the `instructions` key in your project's `altru-coder.jsonc` file. You can edit this file directly or use the **Settings** webview to manage the `instructions` configuration. Each entry points to a file path or glob pattern:

```jsonc
// altru-coder.jsonc
{
  "instructions": [".altru-coder/rules/formatting.md", ".altru-coder/rules/*.md"],
}
```

You can also place rule files in the **`.altru-coder/`** directory structure:
...
Global rules are configured via the `instructions` key in your global `altru-coder.jsonc` config file (typically at `~/.config/altru-coder/altru-coder.jsonc`).

{% callout type="note" title="Migration" %}
The extension is backward compatible with `.altrucoder/rules/` directories. Existing rules will continue to work, but migrating to `altru-coder.jsonc` is recommended.
{% /callout %}
...
Project rules are configured via the `instructions` key in your project's `altru-coder.jsonc` file. Each entry points to a file path or glob pattern:
...
{% callout type="note" title="Migration" %}
The CLI is backward compatible with `.altrucoder/rules/` directories. Existing rules will continue to work, but migrating to `altru-coder.jsonc` is recommended.
{% /callout %}
...
Custom rules are primarily loaded from the **`.altrucoder/rules/` directory**. This is the recommended approach for organizing your project-specific rules. Each rule is typically placed in its own Markdown file with a descriptive name:
...
Rules are managed by editing the `instructions` array in your `altru-coder.jsonc` config file. You can also use the **Settings** webview in VS Code to edit the configuration.

- **Add a rule**: Add a file path or glob pattern to the `instructions` array
- **Remove a rule**: Remove the entry from the array
- **Disable a rule temporarily**: Comment out the line in `altru-coder.jsonc` (JSONC supports `//` comments)

```jsonc
// altru-coder.jsonc
{
  "instructions": [
    ".altru-coder/rules/formatting.md",
    // ".altru-coder/rules/experimental.md"  -- temporarily disabled
    ".altru-coder/rules/naming_conventions.md",
  ],
}
```
...
Rules are managed by editing the `instructions` array in your `altru-coder.jsonc` config file directly.
...
Altru Coder provides a built-in interface for managing your custom rules without manually editing files in the `.altrucoder/rules/` directories. To access the UI, click on the <Codicon name="law" /> icon in the **bottom right corner** of the Altru Coder window.
...
Rules are loaded in the order they appear in the `instructions` array in `altru-coder.jsonc`:

1. **Global instructions** from the global `altru-coder.jsonc` config
2. **Project instructions** from the project's `altru-coder.jsonc`
...
If `.altrucoder/rules/` directories exist in your project, their contents are automatically included for backward compatibility. To fully migrate, move your rule files and reference them in `altru-coder.jsonc`.
{% /callout %}
...
1. **Global rules** from `~/.altrucoder/rules/` directory
2. **Project rules** from `.altrucoder/rules/` directory
3. **Legacy fallback files** (for backward compatibility):
   - `.roorules`
   - `.clinerules`
   - `.altrucoderrules` (deprecated)
...
{% callout type="note" %}
We strongly recommend keeping your rules in the `.altrucoder/rules/` folder as it provides better organization and is the preferred approach for future versions. The legacy file-based approach is maintained for backward compatibility but may be subject to change in future releases.
{% /callout %}
...
1. First, it checks for `.altrucoder/rules-${mode}/` directory
2. If that doesn't exist or is empty, it falls back to `.altrucoderrules-${mode}` file (deprecated)
...
1. Create a `altru-coder.jsonc` file in your project root (if it doesn't exist)
2. Create a `.altru-coder/rules/` directory (or any directory you prefer)
3. Write your rule as a Markdown file in that directory
4. Add the file path or a glob pattern to the `instructions` array in `altru-coder.jsonc`

```jsonc
// altru-coder.jsonc
{
  "instructions": [".altru-coder/rules/my-new-rule.md"],
}
```

Rules are applied on the next interaction. You can also edit `altru-coder.jsonc` through the **Settings** webview in VS Code.
...
{% image src="/docs/img/custom-rules/rules-ui.png" alt="Rules tab in Altru Coder" width="400" /%}

The easiest way to create and manage rules is through the built-in UI:

1. Access the rules management interface from the Altru Coder panel
...
1. Create the `.altrucoder/rules/` directory if it doesn't already exist
...
Rules will be automatically applied to all future Altru Coder interactions. Any new changes will be applied immediately.
...
{% image src="/docs/img/custom-rules/custom-rules.png" alt="Altru Coder ignores request to read sensitive file" width="600" /%}
...
3. **Check rule locations**:
   ...
   - Ensure rules are in supported locations:
     - Global rules: `~/.altrucoder/rules/` directory
     - Project rules: `.altrucoder/rules/` directory
     - Legacy files: `.altrucoderrules`, `.roorules`, or `.clinerules`
