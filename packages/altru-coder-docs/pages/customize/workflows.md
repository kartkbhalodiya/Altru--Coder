---
title: "Workflows"
description: "Create automated workflows with Altru Coder"
platform: new
---

# Workflows

Workflows (also called **slash commands** in the new extension) automate repetitive tasks by defining step-by-step instructions for Altru Coder to execute.

{% image src="/docs/img/screenshot-tests/altru-coder-vscode/visual-regression/settings/agent-behaviour-workflows-chromium-linux.png" alt="Workflows tab in Altru Coder" width="420" caption="Workflows tab in Altru Coder" /%}
...
Workflows are Markdown files stored as **slash commands** in `.altru-coder/commands/`:

- **Global commands**: `~/.config/altru-coder/commands/` (available in all projects)
- **Project commands**: `[project]/.altru-coder/commands/` (project-specific)
...
For example, a file at `.altru-coder/commands/submit-pr.md` is invoked with `/submit-pr`.
...
The new extension automatically migrates legacy workflows from `.altrucoder/workflows/` to the new command format on startup. You can also manually move files and remove the `.md` extension from invocations.

{% /tab %}
{% tab label="VSCode (Legacy)" %}

Workflows are markdown files stored in `.altrucoder/workflows/`:

- **Global workflows**: `~/.altrucoder/workflows/` (available in all projects)
- **Project workflows**: `[project]/.altrucoder/workflows/` (project-specific)
...
Create a file called `submit-pr.md` in your `.altru-coder/commands` directory:
...
Create a file called `submit-pr.md` in your `.altrucoder/workflows` directory:
