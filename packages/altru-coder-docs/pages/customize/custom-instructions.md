...
description: "Provide custom instructions to guide Altru Coder"
---

# Custom Instructions

Custom Instructions allow you to personalize how Altru Coder behaves, providing specific guidance that shapes responses, coding style, and decision-making processes. Both the **VSCode** and **CLI** versions support custom instructions, though the mechanisms differ.

## What Are Custom Instructions?

Custom Instructions define specific Extension behaviors, preferences, and constraints beyond Altru Coder's basic role definition. Examples include coding style, documentation standards, testing requirements, and workflow guidelines.
...
Altru Coder automatically discovers instruction files at your project root and in parent directories (via `findUp`). The following filenames are recognized:

- **`AGENTS.md`** — The primary instruction file for Altru Coder
...
- **Altru Coder:** `~/.config/altru-coder/AGENTS.md`
...
The `instructions` key in `altru-coder.jsonc` accepts an array of paths, globs, or URLs pointing to additional instruction files. You can manage these in **Settings → Agent Behaviour → Rules** subtab.
...
## Legacy `.altrucoderrules` Support

If your project contains `.altrucoderrules` files from the VSCode extension, these are still loaded via auto-migration. However, migrating to `AGENTS.md` is recommended for new projects.
...
Each agent can have its own custom prompt defined in its `.md` file (the markdown body) or via the `agent.<name>.prompt` key in `altru-coder.jsonc`:
...
Or as the markdown body in `.altru-coder/agents/code.md`:
...
The `instructions` key in `altru-coder.jsonc` accepts an array of paths, globs, or URLs pointing to additional instruction files. Configure these in your `altru-coder.jsonc`:
...
{% image src="/docs/img/custom-instructions/custom-instructions.png" alt="Altru Coder Modes tab showing global custom instructions interface" width="600" caption="Altru Coder Modes tab showing global custom instructions interface" /%}

1.  **Open Modes Tab:** Click the <Codicon name="notebook" /> icon in the Altru Coder top menu bar
...
Mode-specific instructions can be set using the Modes Tab
    ...
    {% image src="/docs/img/custom-instructions/custom-instructions-3.png" alt="Altru Coder Modes tab showing mode-specific custom instructions interface" width="600" caption="Altru Coder Modes tab showing mode-specific custom instructions interface" /%}
    * **Open Tab:** Click the <Codicon name="notebook" /> icon in the Altru Coder top menu bar
...
- Preferred: `.altru-coder/rules-{mode-slug}/` (directory)
- Fallback: `.altrucoderrules-{mode-slug}` (single file)

{% callout type="info" title="Legacy Naming Note" %}
Only `.altrucoderrules-{mode-slug}` is recognized as the legacy fallback. Older naming like `.clinerules-{mode-slug}` is not supported.
{% /callout %}
