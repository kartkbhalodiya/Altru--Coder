...
void mock.module("open", () => ({
  default: async (url: string) => {
    ...
    if (openShouldFail) {
      // altrucoder_change start - buffer the error until the consumer attaches
...
// altrucoder_change start - reset mock state and wait for OAuth redirect signal in CI
...
test("BrowserOpenFailed event is published when open() throws", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      // altrucoder_change start - attach rejection handler before stopping callback server
      ...
      await waitFor(() => events.length > 0) // altrucoder_change
...
test("BrowserOpenFailed event is NOT published when open() succeeds", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => { // altrucoder_change
      ...
      await waitFor(() => openCalledWith !== undefined) // altrucoder_change
      await new Promise((resolve) => setTimeout(resolve, 600)) // altrucoder_change - let authenticate await callbackPromise before stop rejects it
