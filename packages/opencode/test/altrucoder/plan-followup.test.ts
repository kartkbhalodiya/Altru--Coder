...
import { formatTodos, generateHandover, PlanFollowup, PlanFollowupRuntime } from "../../src/altrucoder/plan-followup"
...
describe("plan follow-up", () => {
  ...
  test("ask - hides custom answer row on VS Code where the main prompt input handles typed replies", () =>
    withInstance(async () => {
      ...
      try {
        ...
        process.env.ALTRU_CODER_CLIENT = prev
      }
    }))
  ...
  test("ask - retargets prompt queue so injected message is visible in scope", () =>
    withInstance(async () => {
      const { AltruCoderSessionPromptQueue } = await import("../../src/altrucoder/session/prompt-queue")
      ...
      AltruCoderSessionPromptQueue.retarget(seeded.sessionID, original)
      ...
      const scoped = AltruCoderSessionPromptQueue.scope(seeded.sessionID, all)
