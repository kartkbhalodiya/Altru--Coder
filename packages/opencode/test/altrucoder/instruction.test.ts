...
const withConfigDir =
  ...
  <A, E, R>(self: Effect.Effect<A, E, R>) =>
    Effect.gen(function* () {
      const original = process.env["ALTRU_CODER_CONFIG_DIR"]
      if (value === undefined) delete process.env["ALTRU_CODER_CONFIG_DIR"]
      else process.env["ALTRU_CODER_CONFIG_DIR"] = value
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          if (original === undefined) delete process.env["ALTRU_CODER_CONFIG_DIR"]
          else process.env["ALTRU_CODER_CONFIG_DIR"] = original
        }),
      )
      return yield* self
    })

describe("Instruction.systemPaths ALTRU_CODER_CONFIG_DIR profile fallback", () => {
  it.live("prefers ALTRU_CODER_CONFIG_DIR AGENTS.md over global when both exist", () =>
    Effect.gen(function* () {
  ...
  it.live("falls back to global AGENTS.md when ALTRU_CODER_CONFIG_DIR has no AGENTS.md", () =>
    Effect.gen(function* () {
  ...
  it.live("uses global AGENTS.md when ALTRU_CODER_CONFIG_DIR is not set", () =>
    Effect.gen(function* () {
      const globalTmp = yield* tmpWithFiles({ "AGENTS.md": "# Global Instructions" })
      const projectTmp = yield* tmpdirScoped()
