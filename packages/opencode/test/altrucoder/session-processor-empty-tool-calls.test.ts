...
import { AltruCoderSessionProcessor } from "../../src/altrucoder/session/processor"
        ...
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === AltruCoderSessionProcessor.REASONING_LENGTH_WARNING,
          )

          expect(warning?.ignored).toBe(true)

          const modelMsgs = yield* MessageV2.toModelMessagesEffect([{ info: handle.message, parts }], mdl)
          expect(JSON.stringify(modelMsgs)).not.toContain(AltruCoderSessionProcessor.REASONING_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === AltruCoderSessionProcessor.OUTPUT_LENGTH_WARNING,
          )
          ...
          expect(json).not.toContain(AltruCoderSessionProcessor.OUTPUT_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
