    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".altru-coder", "agent", "skip.md"),
          `---
mode: "banana"
---
Broken agent prompt`,
        ...
        await Filesystem.write(
          path.join(dir, ".altru-coder", "agent", "keep.md"),
          `---
model: test/model
---
Valid agent prompt`,
...
Broken agent prompt`,
  ...
  test("skips invalid command markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".altru-coder", "command", "skip.md"),
          `---
subtask: "banana"
---
Broken command template`,
        ...
        await Filesystem.write(
          path.join(dir, ".altru-coder", "command", "keep.md"),
          `---
...
Broken command template`,
  ...
  test("collects warnings for invalid agent markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".altru-coder", "agent", "broken.md"),
          `---
mode: "banana"
---
Broken agent`,
  ...
  test("collects warnings for invalid command markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".altru-coder", "command", "broken.md"),
          `---
subtask: "banana"
---
Broken command`,
  ...
  test("collects warnings for invalid JSON in .altru-coder directory config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, ".altru-coder", "altru-coder.json"), "{ not valid json !!!")
      },
    })
  ...
  test("collects warnings for invalid schema in .altru-coder directory config", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(cfg).toBeDefined()
        expect(warns.some((w) => w.path.includes("altru-coder.json") && w.message.includes("invalid"))).toBe(true)
      },
    })
  })
