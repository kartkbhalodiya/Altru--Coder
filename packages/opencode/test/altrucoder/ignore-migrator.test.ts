...
import { IgnoreMigrator } from "../../src/altrucoder/ignore-migrator"
...
# Comment 2`
  ...
  *.env  `
  ...
  describe("migrate", () => {
    test("returns empty permission for project without .altrucoderignore", async () => {
    ...
    test("loads project .altrucoderignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
    ...
    test("handles complex .altrucoderignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".altrucoderignore"),
            `# Secrets
secrets/
*.env
!.env.example
...
`,
  ...
  describe("loadIgnoreConfig", () => {
    test("returns empty object for project without .altrucoderignore", async () => {
      await using tmp = await tmpdir()
    ...
    test("returns permission config for project with .altrucoderignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".altrucoderignore"), "secrets/")
        },
