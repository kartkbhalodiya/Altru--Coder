...
import { AltruCoderCompactionPayloadRecovery } from "../../src/altrucoder/session/compaction-payload-recovery"
...
describe("AltruCoderCompactionPayloadRecovery", () => {
  test("detects Altru Coder gateway payload-size errors", () => {
    const error = new MessageV2.ContextOverflowError({
      message: "Request Entity Too Large",
      responseBody: "Request Entity Too Large\n\nFUNCTION_PAYLOAD_TOO_LARGE",
    }).toObject()

    expect(AltruCoderCompactionPayloadRecovery.matches(error)).toBe(true)
  })
  ...
  test("strips media and marks completed tool outputs compacted", async () => {
    ...
    await Effect.runPromise(
      AltruCoderCompactionPayloadRecovery.strip({
      ...
      return Stream.make({ type: "start" } as LLM.Event).pipe(
        Stream.concat(
          Stream.fail(
            new APICallError({
              message: "Request Entity Too Large",
              url: "https://api.altru-coder.ai/api/openrouter/responses",
              requestBodyValues: {},
              statusCode: 413,
