...
import { detectRepo, findRepoFrom } from "../../../src/altrucoder/cli/dev-setup"
...
describe("findRepoFrom", () => {
  test("returns the repo root when started from a nested path", async () => {
    ...
    const nested = path.join(tmp.path, "packages", "opencode", "src", "altrucoder", "cli")
  ...
  test("returns undefined when no sentinel is found up to filesystem root", async () => {
    ...
    // above tmp does not contain a altrucoder repo at its own root.
    const found = await findRepoFrom(tmp.path)
...
describe("detectRepo", () => {
  test("honours ALTRU_CODER_DEV_REPO without touching the filesystem", async () => {
    // Pointing at a path that does not exist — the hint must be returned as-is
    // (this proves no walk happens when ALTRU_CODER_DEV_REPO is set).
    const hint = "/definitely/does/not/exist/altru-repo"
    const got = await withEnv("ALTRU_CODER_DEV_REPO", hint, () => detectRepo())
    expect(got).toBe(hint)
  })

  test("ALTRU_CODER_DEV_REPO wins over the filesystem walk", async () => {
    // Build a synthetic repo, point ALTRU_CODER_DEV_REPO at it, and run from an
    // unrelated cwd. Even though the walk from cwd (inside the real altru-coder
    // checkout) would succeed, the hint must take priority.
    await using tmp = await tmpdir()
    await makeSynthRepo(tmp.path)

    const got = await withEnv("ALTRU_CODER_DEV_REPO", tmp.path, () => detectRepo())
