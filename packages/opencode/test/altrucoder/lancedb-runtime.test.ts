...
const entry = "file:///tmp/altru-cache/node_modules/@lancedb/lancedb/dist/index.js"
const add = mock(async () => ({ directory: "/tmp/altru-cache", entrypoint: entry }))
...
const env = "ALTRU_CODER_LANCEDB_PATH"
const prev = process.env[env]

describe("LanceDBRuntime", () => {
  beforeEach(async () => {
    const { LanceDBRuntime } = await import("../../src/altrucoder/lancedb")
    LanceDBRuntime.clear()
    add.mockClear()
    add.mockImplementation(async () => ({ directory: "/tmp/altru-cache", entrypoint: entry }))
  })
  ...
  test("retries after a failed install", async () => {
    ...
    add
      ...
      .mockImplementationOnce(async () => ({ directory: "/tmp/altru-cache", entrypoint: entry }))
