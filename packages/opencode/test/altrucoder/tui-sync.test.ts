...
import { AltruCoderSessionTuiSync } from "../../src/altrucoder/session/tui-sync"
...
function syncVariant(input: { current: string | undefined; message: Message; parts?: readonly { type: string }[] }) {
  if (!AltruCoderSessionTuiSync.model({ role: input.message.role, parts: input.parts })) return input.current
  return input.message.model?.variant ?? "default"
}

describe("AltruCoderSessionTuiSync.model", () => {
  test("syncs normal user messages", () => {
    expect(AltruCoderSessionTuiSync.model({ role: "user", parts: [{ type: "text" }] })).toBe(true)
  })

  test("skips compaction marker user messages", () => {
    expect(AltruCoderSessionTuiSync.model({ role: "user", parts: [{ type: "compaction" }] })).toBe(false)
  })

  test("skips messages before parts load", () => {
    expect(AltruCoderSessionTuiSync.model({ role: "user" })).toBe(false)
  })
  ...
  test("skips messages checked with stored parts", () => {
    ...
    expect(AltruCoderSessionTuiSync.model({ role: msg.role, parts })).toBe(false)
  })

  test("skips non-user messages", () => {
    expect(AltruCoderSessionTuiSync.model({ role: "assistant", parts: [{ type: "text" }] })).toBe(false)
  })
