...
afterEach(() => {
  delete process.env["ALTRU_CODER_EXPERIMENTAL_HTTPAPI"]
})
...
describe("POST /permission/:requestID/reply", () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const response = await server.request("/permission/permission_missing/reply", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-altru-coder-directory": tmp.path },
  ...
  test("returns 404 for unknown replies when experimental HttpApi is enabled", async () => {
    process.env["ALTRU_CODER_EXPERIMENTAL_HTTPAPI"] = "1"
    await using tmp = await tmpdir({ git: true })
