...
afterAll(async () => {
  ...
  for (const file of ["altru-coder.jsonc", "altru-coder.json", "config.json", "opencode.json", "opencode.jsonc"]) {
    ...
    withDir({ git: true }, () =>
      Effect.gen(function* () {
        const fiber = yield* ask({
          ...
          permission: "bash",
          patterns: ["altru-coder-permission-8353 test"],
          metadata: { rules: ["altru-coder-permission-8353 *", "altru-coder-permission-8353 test"] },
          always: ["altru-coder-permission-8353 *", "altru-coder-permission-8353 test"],
          ruleset: [],
        }).pipe(Effect.forkScoped)
        ...
        yield* saveAlwaysRules({
          ...
          approvedAlways: ["altru-coder-permission-8353 test"],
        })
        yield* reply({ requestID: PermissionID.make("permission_saved_always"), reply: "always" })
        yield* Fiber.join(fiber)

        const cfg = yield* Effect.promise(() => Config.get())
        expect(cfg.permission?.bash).toMatchObject({ "altru-coder-permission-8353 test": "allow" })
        expect(cfg.permission?.bash).not.toMatchObject({ "altru-coder-permission-8353 *": "allow" })

        const broad = yield* ask({
          id: PermissionID.make("permission_saved_always_broad"),
          sessionID: SessionID.make("session_saved_always"),
          permission: "bash",
          patterns: ["altru-coder-permission-8353 install"],
          metadata: {},
          always: [],
          ruleset: [],
        }).pipe(Effect.forkScoped)
