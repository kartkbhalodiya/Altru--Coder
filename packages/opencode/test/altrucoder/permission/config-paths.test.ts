...
import { ConfigProtection } from "../../../src/altrucoder/permission/config-paths"
...
describe("ConfigProtection.isRequest", () => {
  ...
  const legacy = AltruCoderPaths.globalDirs().map((d) => path.resolve(d))
  ...
  test("returns false for file-tool external_directory targeting global config", () => {
    const result = ConfigProtection.isRequest({
      permission: "external_directory",
      patterns: [config + "/*"],
      metadata: { filepath: path.join(config, "altru-coder.json"), parentDir: config },
    })
  ...
  test("returns true for edit targeting relative config path via patterns", () => {
    const result = ConfigProtection.isRequest({
      permission: "edit",
      patterns: [".altru-coder/command/foo.md"],
    })
    expect(result).toBe(true)
  })
  ...
  test("returns false for edit targeting excluded subdir (plans)", () => {
    const result = ConfigProtection.isRequest({
      permission: "edit",
      patterns: [".altru-coder/plans/plan.md"],
    })
    expect(result).toBe(false)
  })
  ...
  test("returns false for read permission", () => {
    const result = ConfigProtection.isRequest({
      permission: "read",
      patterns: [".altru-coder/config.json"],
    })
    expect(result).toBe(false)
  })
  ...
  test("returns true for edit targeting root config files", () => {
    for (const file of ["altru-coder.json", "altru-coder.jsonc", "AGENTS.md"]) {
      const result = ConfigProtection.isRequest({
        permission: "edit",
        patterns: [file],
      })
