...
import { ModesMigrator } from "../../src/altrucoder/modes-migrator"
...
describe("ModesMigrator", () => {
  ...
  describe("convertPermissions", () => {
    ...
    test("converts fileRegex groups to restricted permissions", () => {
      const groups: ModesMigrator.AltruCoderMode["groups"] = [
        "read",
    ...
    test("handles tuple without fileRegex", () => {
      const groups: ModesMigrator.AltruCoderMode["groups"] = [["edit", {}]]
      const permissions = ModesMigrator.convertPermissions(groups)
  ...
  describe("convertMode", () => {
    test("converts full mode to agent config", () => {
      const mode: ModesMigrator.AltruCoderMode = {
        slug: "translate",
        name: "Translate",
  ...
  - slug: translate
    ...
    groups:
      - read`,
          ...
          // Create project .altrucodermodes (should win)
          await Bun.write(
            path.join(dir, ".altrucodermodes"),
            `customModes:
  - slug: translate
