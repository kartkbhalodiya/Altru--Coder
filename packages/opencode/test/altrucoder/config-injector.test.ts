...
import { AltruCoderConfigInjector } from "../../src/altrucoder/config-injector"
import { tmpdir } from "../fixture/fixture"
import path from "path"
import fs from "fs/promises"

describe("AltruCoderConfigInjector", () => {
  describe("buildConfig", () => {
    test("returns empty config when no modes exist", async () => {
      await using tmp = await tmpdir()

      const result = await AltruCoderConfigInjector.buildConfig({ projectDir: tmp.path, skipGlobalPaths: true })
  ...
  - slug: code
    ...
    test("includes workflows as commands in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".altrucoder", "workflows")
          await Bun.write(
    ...
    test("includes rules in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Bun.write(path.join(dir, ".altrucoder", "rules", "main.md"), "# Rules")
        },
      })

      const result = await AltruCoderConfigInjector.buildConfig({
        projectDir: tmp.path,
  ...
  - slug: translate
    ...
    test("includes ignore patterns in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".altrucoderignore"), "secrets/\n*.env")
        },
      })
  ...
  - slug: translate
    ...
    test("handles negation patterns in ignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".altrucoderignore"), "*.env\n!.env.example")
        },
      })
  ...
  describe("getEnvVars", () => {
    test("returns empty object for empty config", () => {
      const envVars = AltruCoderConfigInjector.getEnvVars("{}")
    ...
    test("returns ALTRU_CODER_CONFIG_CONTENT for non-empty config", () => {
      const config = JSON.stringify({ agent: { test: {} } })
      const envVars = AltruCoderConfigInjector.getEnvVars(config)

      expect(envVars).toEqual({
        ALTRU_CODER_CONFIG_CONTENT: config,
      })
    })
  })
})
