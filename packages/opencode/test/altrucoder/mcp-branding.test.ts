...
//
// History: upstream OpenCode has repeatedly overwritten the Altru Coder-branded
...
// barrel into mcp/index.ts") which Altru Coder picked up via the v1.4.7 merge (PR
// #9346, commit 57630eaf1). The original fix was PR #7174.
//
// This test asserts the surviving Altru Coder-branded strings directly against the
// source so that the next upstream churn on this file fails the Altru Coder test
// suite instead of shipping an "opencode mcp auth" popup to end users.

const mcpSource = path.join(__dirname, "..", "..", "src", "mcp", "index.ts")

describe("Altru Coder MCP branding", () => {
  test("auth toast tells the user to run `altru-coder mcp auth`, never `opencode mcp auth`", async () => {
    const src = await Bun.file(mcpSource).text()
    expect(src).toContain("Run: altru-coder mcp auth ${key}")
    expect(src).not.toContain("Run: opencode mcp auth")
  })

  test("MCP `Client` instances identify themselves as `altru-coder`", async () => {
    const src = await Bun.file(mcpSource).text()
    ...
    // must use the Altru Coder brand.
