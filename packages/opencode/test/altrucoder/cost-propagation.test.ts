// Verifies AltruCoderCostPropagation.propagate() serializes concurrent writes to
...
import { AltruCoderCostPropagation } from "../../src/altrucoder/session/cost-propagation"
...
describe("AltruCoderCostPropagation.propagate", () => {
  it.live("sums deltas correctly under parallel execution", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        yield* Effect.all(
          deltas.map((d) => AltruCoderCostPropagation.propagate(sessions, chat.id, assistant.id, d)),
  ...
  it.live("is a no-op when amount is non-positive", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const sessions = yield* Session.Service
        const { chat, assistant } = yield* seed()
        yield* AltruCoderCostPropagation.propagate(sessions, chat.id, assistant.id, 0)
        yield* AltruCoderCostPropagation.propagate(sessions, chat.id, assistant.id, -1.5)
        const parent = yield* Effect.sync(() => MessageV2.get({ sessionID: chat.id, messageID: assistant.id }))
        if (parent.info.role !== "assistant") return
