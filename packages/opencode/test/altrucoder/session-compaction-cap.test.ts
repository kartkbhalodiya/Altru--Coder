...
import { AltruCoderSession } from "../../src/altrucoder/session"
    ...
    () =>
      provideTmpdirServer(
        Effect.fnUntraced(function* ({ llm }) {
          ...
          const turnClose = yield* Deferred.make<AltruCoderSession.CloseReason>()
          const unsub = yield* bus.subscribeCallback(AltruCoderSession.Event.TurnClose, (evt) => {
          ...
          expect(yield* llm.calls).toBe(AltruCoderSessionPrompt.MAX_COMPACTION_ATTEMPTS * 2 + 1)
...
describe("AltruCoderSessionPrompt.guardCompactionAttempt", () => {
  it.effect("returns { exhausted: false } and does not mutate state below the cap", () =>
    Effect.sync(() => {
      ...
      const result = AltruCoderSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_under",
        attempts: AltruCoderSessionPrompt.MAX_COMPACTION_ATTEMPTS - 1,
        closeReasons,
        message: msg,
      })
  ...
  it.effect("sets close reason and attaches error once attempts reach the cap", () =>
    Effect.sync(() => {
      ...
      const result = AltruCoderSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_cap",
        attempts: AltruCoderSessionPrompt.MAX_COMPACTION_ATTEMPTS,
        closeReasons,
        message: msg,
      })
