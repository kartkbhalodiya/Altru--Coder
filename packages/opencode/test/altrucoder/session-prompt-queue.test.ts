...
import { AltruCoderSessionPromptQueue } from "@/altrucoder/session/prompt-queue"
import { Suggestion } from "../../src/altrucoder/suggestion"
...
describe("session prompt queue", () => {
  test("scopes queued turns without moving prior assistant history", async () => {
    ...
    const ids = await Effect.runPromise(
      AltruCoderSessionPromptQueue.enqueue(
        sessionID,
        two,
        Effect.sync(() => AltruCoderSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)),
        Effect.succeed([]),
      ),
    )
  ...
  test("retarget keeps older queued prompts hidden", async () => {
    ...
    const ids = await Effect.runPromise(
      AltruCoderSessionPromptQueue.enqueue(
        ...
        base,
        Effect.sync(() => {
          AltruCoderSessionPromptQueue.retarget(sessionID, injected)
          return AltruCoderSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)
        }),
        Effect.succeed([]),
      ),
    )
  ...
  test("hasFollowup reports true only for prompts enqueued after the active slot started", async () => {
    ...
    const first = Effect.runPromise(
      AltruCoderSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "first:start", value: AltruCoderSessionPromptQueue.hasFollowup(sessionID) })
          firstStarted.resolve()
          yield* Effect.promise(() => firstReleased.promise)
          observed.push({ where: "first:end", value: AltruCoderSessionPromptQueue.hasFollowup(sessionID) })
          return "first"
        }),
    ...
    const second = Effect.runPromise(
      AltruCoderSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "second:start", value: AltruCoderSessionPromptQueue.hasFollowup(sessionID) })
    ...
    expect(AltruCoderSessionPromptQueue.hasFollowup(sessionID)).toBe(true)
    ...
    const third = Effect.runPromise(
      AltruCoderSessionPromptQueue.enqueue(
        ...
        Effect.sync(() => {
          observed.push({ where: "third:start", value: AltruCoderSessionPromptQueue.hasFollowup(sessionID) })
          return "third"
        }),
    ...
    expect(AltruCoderSessionPromptQueue.hasFollowup(sessionID)).toBe(false)
        ...
        fn: async () => {
          ...
          const ids = await Effect.runPromise(
            AltruCoderSessionPromptQueue.enqueue(
              ...
              Effect.succeed(AltruCoderSessionPromptQueue.scope(session.id, []).map((item) => item.info.id)),
              Effect.succeed([]),
            ),
          )
          expect(ids).toEqual([])
          expect(AltruCoderSessionPromptQueue.hasFollowup(session.id)).toBe(false)
        },
      })
    } finally {
      server.stop(true)
