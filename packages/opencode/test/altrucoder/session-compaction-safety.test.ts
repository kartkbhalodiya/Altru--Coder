// Unit tests for AltruCoderSessionPrompt history-trim / media-strip helpers.
...
describe("AltruCoderSessionPrompt.hasCompletedSummary", () => {
  test("returns false for empty array", () => {
    expect(AltruCoderSessionPrompt.hasCompletedSummary([])).toBe(false)
  })
  ...
  test("returns false when only summary has an error", () => {
    ...
    expect(AltruCoderSessionPrompt.hasCompletedSummary(msgs)).toBe(false)
  })
  ...
  test("returns true when summary has finish and no error", () => {
    ...
    expect(AltruCoderSessionPrompt.hasCompletedSummary(msgs)).toBe(true)
  })
})

describe("AltruCoderSessionPrompt.trimBeforeLastSummary", () => {
  test("returns input unchanged when no summary present", () => {
    const msgs = [user("msg_u1"), assistant("msg_a1", "msg_u1", [], { finish: "end_turn" })]
    const result = AltruCoderSessionPrompt.trimBeforeLastSummary(msgs)
  ...
  test("trims to summary's text-user parent (reference-session bug)", () => {
    ...
    const result = AltruCoderSessionPrompt.trimBeforeLastSummary(filtered)
  ...
  test("is idempotent", () => {
    ...
    const first = AltruCoderSessionPrompt.trimBeforeLastSummary(msgs)
    const second = AltruCoderSessionPrompt.trimBeforeLastSummary(first)
...
describe("AltruCoderSessionPrompt.stripHistoricalMedia", () => {
  test("replaces image file part in historical user message with placeholder text", () => {
    ...
    const result = AltruCoderSessionPrompt.stripHistoricalMedia(msgs)
  ...
  test("does not mutate the input", () => {
    ...
    const result = AltruCoderSessionPrompt.stripHistoricalMedia(input)
...
describe("AltruCoderSessionPrompt.maybeStripHistoricalMedia", () => {
  test("returns input unchanged when no completed summary exists", () => {
    const msgs = [
      user("msg_hist", [filePart("msg_hist", "image/png", "hist.png")]),
      user("msg_last", [textPart("msg_last", "follow-up")]),
    ]
    const result = AltruCoderSessionPrompt.maybeStripHistoricalMedia(msgs)
    expect(result).toBe(msgs)
  })
