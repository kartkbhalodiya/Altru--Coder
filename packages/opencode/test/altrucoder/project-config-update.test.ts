...
async function writeConfig(dir: string, config: unknown) {
  await Filesystem.write(path.join(dir, "altru-coder.json"), JSON.stringify(config, null, 2))
}

test("project config update creates .altru-coder/altru-coder.json and reloads it", async () => {
...
test("project config update skips empty delete-only writes when no config exists", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      await expect(fs.access(path.join(tmp.path, ".altru-coder", "altru-coder.json"))).rejects.toThrow()
    },
  })
})

test("project config update prefers existing root altru-coder.json", async () => {
  await using tmp = await tmpdir()
...
test("project config update patches ancestor .altru-coder/altru-coder.json from nested directory", async () => {
  await using tmp = await tmpdir()
  const child = path.join(tmp.path, "nested", "workspace")
  await fs.mkdir(child, { recursive: true })
  await fs.mkdir(path.join(tmp.path, ".altru-coder"), { recursive: true })
  await writeConfig(path.join(tmp.path, ".altru-coder"), { username: "alice" })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(merged.model).toBe("updated/model")
      expect(merged.username).toBe("alice")
      await expect(fs.access(path.join(child, ".altru-coder", "altru-coder.json"))).rejects.toThrow()
    },
  })
})
