...
import { IngestQueue } from "../../../src/altru-coder-sessions/ingest-queue"
...
describe("share ingest queue", () => {
  ...
  test("altru_coder_meta uses stable key and coalesces", async () => {
    const sent: unknown[] = []
    const sched = scheduler(() => clock.now)
    ...
    expect(sent.length).toBe(1)
    expect((sent[0] as any).data.length).toBe(1)
    expect((sent[0] as any).data[0].type).toBe("altru_coder_meta")
