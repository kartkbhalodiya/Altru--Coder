...
import { RemoteSender } from "../../../src/altru-coder-sessions/remote-sender"
import type { RemoteWS } from "../../../src/altru-coder-sessions/remote-ws"
import type { RemoteProtocol } from "../../../src/altru-coder-sessions/remote-protocol"
import { SessionPrompt } from "../../../src/session/prompt"
import { Question } from "../../../src/question"
import { Permission } from "../../../src/permission"
import { Suggestion } from "../../../src/altrucoder/suggestion" // altrucoder_change
...
describe("RemoteSender", () => {
  ...
  test("send_message normalizes string model without prefix", async () => {
    ...
    expect(prompt).toHaveBeenCalledWith({
      sessionID: "ses_x",
      parts: [{ type: "text", text: "hello" }],
      model: { providerID: "altru-coder", modelID: "anthropic/claude-sonnet-4-20250514" },
    })
  })

  test("send_message keeps altrucoder-prefixed model unchanged before internal conversion", async () => {
    const { conn } = fakeConn()
    const prompt = spyOn(SessionPrompt, "prompt").mockResolvedValue({} as never)
    const sender = RemoteSender.create({
      conn,
    ...
    sender.handle({
      type: "command",
      id: "req_model_altrucoder",
      command: "send_message",
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: "altrucoder/gpt-5-mini",
      },
    })
    ...
    expect(prompt).toHaveBeenCalledWith({
      sessionID: "ses_x",
      parts: [{ type: "text", text: "hello" }],
      model: { providerID: "altru-coder", modelID: "gpt-5-mini" },
    })
  })
  ...
  test("send_message rejects structured model on remote path", () => {
    ...
    sender.handle({
      ...
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: "altrucoder", modelID: "gpt-5-mini" },
      },
    })
  ...
  test("send_message does not special-case altru-coder-prefixed model", async () => {
    const { conn, sent } = fakeConn()
    const prompt = spyOn(SessionPrompt, "prompt").mockResolvedValue({} as never)
    const sender = RemoteSender.create({
      conn,
    ...
    sender.handle({
      type: "command",
      id: "req_model_altru_coder",
      command: "send_message",
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: "altru-coder/gpt-5-mini",
      },
    })

    await new Promise((r) => setTimeout(r, 0))

    expect(sent[0]).toEqual({ type: "response", id: "req_model_altru_coder", result: {} })
    expect(prompt).toHaveBeenCalledWith({
      sessionID: "ses_x",
      parts: [{ type: "text", text: "hello" }],
      model: { providerID: "altru-coder", modelID: "altru-coder/gpt-5-mini" },
    })
  })
  // altrucoder_change end
