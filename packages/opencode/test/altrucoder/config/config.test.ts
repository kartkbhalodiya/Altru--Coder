...
describe("altrucoder indexing config", () => {
  afterEach(async () => {
    ...
    try {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const config = await load()
          const input = AltruCoderIndexing.input(config.indexing, global.indexing)
  ...
  test("global indexing enabled applies when project indexing is disabled", async () => {
    const input = AltruCoderIndexing.input({ enabled: false }, { enabled: true })
    expect(input.enabled).toBe(true)
  })
})
