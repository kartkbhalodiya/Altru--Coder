...
import { McpMigrator } from "../../src/altrucoder/mcp-migrator"
...
describe("McpMigrator", () => {
  describe("convertServer", () => {
    test("converts local server with command and args", () => {
      const server: McpMigrator.AltruCoderMcpServer = {
        command: "npx",
  ...
  describe("migrate", () => {
    ...
    test("migrates servers from project .altru-coder/mcp.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".altru-coder")
          await Bun.write(
    ...
    test("reads from legacy .altrucoder/mcp.json when .altru-coder/mcp.json is absent", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".altrucoder")
          await Bun.write(
            path.join(settingsDir, "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    // Regression: malformed .altrucoder/mcp.json must not prevent .altru-coder/mcp.json from loading
    test("loads .altru-coder/mcp.json even when .altrucoder/mcp.json is malformed", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".altrucoder", "mcp.json"), "{ corrupt json !!!")
          await Bun.write(
            path.join(dir, ".altru-coder", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test(".altru-coder/mcp.json overrides .altrucoder/mcp.json for same server name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".altrucoder", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test("does not read project-level .altru-coder/mcp_settings.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".altru-coder")
          await Bun.write(
