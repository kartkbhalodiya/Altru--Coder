...
import { buildAltruCoderHeaders, getFeatureHeader, getEditorNameHeader } from "@altru-coder/altru-coder-gateway"
import { HEADER_FEATURE, ENV_FEATURE, ENV_VERSION, DEFAULT_EDITOR_NAME } from "@altru-coder/altru-coder-gateway"
...
describe("getEditorNameHeader", () => {
  ...
  it("returns default editor name without version when ALTRUCODER_VERSION is not set", () => {
    delete process.env[ENV_VERSION]
    expect(getEditorNameHeader()).toBe(DEFAULT_EDITOR_NAME)
  })

  it("appends version when ALTRUCODER_VERSION is set", () => {
    process.env[ENV_VERSION] = "1.2.3"
    expect(getEditorNameHeader()).toBe(`${DEFAULT_EDITOR_NAME} 1.2.3`)
  })
})

describe("buildAltruCoderHeaders", () => {
  const original = process.env[ENV_FEATURE]
  ...
  it("includes feature header when env var is set", () => {
    ...
    const headers = buildAltruCoderHeaders()
  ...
  it("always includes editor name header", () => {
    delete process.env[ENV_FEATURE]
    const headers = buildAltruCoderHeaders()
    expect(headers["X-ALTRUCODER-EDITORNAME"]).toBe(getEditorNameHeader())
  })
