...
describe("parseAltruCoderErrorCode", () => {
  ...
  it("extracts PROMOTION_MODEL_LIMIT_REACHED from { code } (top-level)", () => {
    ...
    expect(parseAltruCoderErrorCode(error)).toBe("PROMOTION_MODEL_LIMIT_REACHED")
  })
  ...
  it("returns undefined for non-Altru Coder error codes", () => {
    const error = makeAPIError({
      statusCode: 429,
      responseBody: JSON.stringify({ error: { code: "SOME_OTHER_ERROR" } }),
    })
    expect(parseAltruCoderErrorCode(error)).toBeUndefined()
  })
...
describe("isAltruCoderError", () => {
  it("returns true for PAID_MODEL_AUTH_REQUIRED", () => {
    const error = makeAPIError({
      statusCode: 401,
      responseBody: JSON.stringify({ error: { code: "PAID_MODEL_AUTH_REQUIRED" } }),
    })
    expect(isAltruCoderError(error)).toBe(true)
  })
  ...
  it("returns false for regular 429 errors without Altru Coder code", () => {
    const error = makeAPIError({
      statusCode: 429,
      isRetryable: true,
      message: "Too Many Requests",
    })
    expect(isAltruCoderError(error)).toBe(false)
  })
...
describe("SessionRetry.retryable with Altru Coder errors", () => {
