...
process.env.ALTRU_CODER_SESSION_RETRY_LIMIT = "2"
...
afterEach(() => {
  delete process.env.ALTRU_CODER_SESSION_RETRY_LIMIT
})
...
describe("session processor retry limit", () => {
  ...
  it.effect("only positive integers enable the limit", () =>
    Effect.promise(async () => {
      ...
      delete process.env.ALTRU_CODER_SESSION_RETRY_LIMIT
      expect(Flag.ALTRU_CODER_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.ALTRU_CODER_SESSION_RETRY_LIMIT = "0"
      expect(Flag.ALTRU_CODER_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.ALTRU_CODER_SESSION_RETRY_LIMIT = "-1"
      expect(Flag.ALTRU_CODER_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.ALTRU_CODER_SESSION_RETRY_LIMIT = "abc"
      expect(Flag.ALTRU_CODER_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.ALTRU_CODER_SESSION_RETRY_LIMIT = "2"
      expect(Flag.ALTRU_CODER_SESSION_RETRY_LIMIT).toBe(2)
    }),
  )
  ...
  it.effect("reads env at access time (dynamic getter)", () =>
    Effect.promise(async () => {
      ...
      delete process.env.ALTRU_CODER_SESSION_RETRY_LIMIT
      expect(Flag.ALTRU_CODER_SESSION_RETRY_LIMIT).toBeUndefined()
      process.env.ALTRU_CODER_SESSION_RETRY_LIMIT = "5"
      expect(Flag.ALTRU_CODER_SESSION_RETRY_LIMIT).toBe(5)
      delete process.env.ALTRU_CODER_SESSION_RETRY_LIMIT
    }),
  )
})
