...
//
// Unit tests for AltruCoderSnapshotTrack.wrap — the slow-repo guard that sits
...
import { AltruCoderSnapshotTrack } from "../../src/altrucoder/snapshot/track"
...
const makeHooks = (
  answer: AltruCoderSnapshotTrack.Answer | Promise<AltruCoderSnapshotTrack.Answer>,
): { hooks: AltruCoderSnapshotTrack.Hooks; calls: Calls } => {
  const calls: Calls = { ask: 0, persist: 0, progress: [] }
  let counter = 0
  const hooks: AltruCoderSnapshotTrack.Hooks = {
    async ask() {
      calls.ask += 1
      return answer
    },
...
describe("AltruCoderSnapshotTrack.wrap", () => {
  test("returns the hash when inner resolves before the timeout", async () => {
    const state = AltruCoderSnapshotTrack.makeState()
    const { hooks, calls } = makeHooks("continue")

    const result = await Effect.runPromise(
      AltruCoderSnapshotTrack.wrap({
...
describe("AltruCoderSnapshotTrack progress indicator", () => {
  ...
  test("slow-but-succeeding path (under timeout) starts then ends the indicator", async () => {
    ...
    expect(withoutFrame(firstText.text)).toBe(
      AltruCoderSnapshotTrack.formatProgress(AltruCoderSnapshotTrack.PROGRESS_INITIALIZING, "").trim(),
    )
  ...
  test("timed-out path keeps the initializing label (no text escalation)", async () => {
    ...
    const base = AltruCoderSnapshotTrack.formatProgress(AltruCoderSnapshotTrack.PROGRESS_INITIALIZING, "").trim()
    for (const evt of events) {
...
describe("AltruCoderSnapshotTrack constants", () => {
  test("TIMEOUT_MS defaults to 10s and respects env override", () => {
    ...
    // with ALTRU_CODER_SNAPSHOT_TRACK_TIMEOUT_MS, which this test suite does not set.
    expect(AltruCoderSnapshotTrack.TIMEOUT_MS).toBe(10_000)
  })

  test("exposes stable answer labels", () => {
    expect(AltruCoderSnapshotTrack.ANSWER_CONTINUE).toBe("Continue with snapshots")
    expect(AltruCoderSnapshotTrack.ANSWER_DISABLE).toBe("Disable for this project")
  })
})
