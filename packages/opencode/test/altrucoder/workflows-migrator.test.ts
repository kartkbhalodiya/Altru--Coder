...
import { WorkflowsMigrator } from "../../src/altrucoder/workflows-migrator"
...
Actual description here.`
  ...
  describe("discoverWorkflows", () => {
    test("discovers project workflows", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".altru-coder", "workflows")
    ...
    test("discovers workflows from legacy .altrucoder/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
    ...
    test("discovers global workflows from ~/.altru-coder/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".altru-coder", "workflows", "global.md"), "# Global\n\nGlobal workflow")
  ...
  describe("convertToCommand", () => {
    test("converts workflow to command format", () => {
      const workflow: WorkflowsMigrator.AltruCoderWorkflow = {
        name: "code-review",
    ...
    test("project workflows override global workflows with same name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const projectDir = path.join(dir, ".altru-coder", "workflows")
