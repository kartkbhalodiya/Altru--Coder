...
import { ConfigValidation } from "../../src/altrucoder/config-validation"
...
describe("ConfigValidation.check", () => {
  ...
  test("validates valid JSONC config", async () => {
    await using tmp = await tmpdir({ git: true })
    const filepath = path.join(tmp.path, "altru-coder.json")
  ...
  test("validates valid markdown command", async () => {
    await using tmp = await tmpdir({ git: true })
    const filepath = path.join(tmp.path, ".altru-coder", "command", "test-cmd.md")
...
Do something useful`,
  ...
  test("reports schema error for command with invalid field types", async () => {
    await using tmp = await tmpdir({ git: true })
    const filepath = path.join(tmp.path, ".altru-coder", "command", "bad.md")
...
Do something`,
  ...
  test("validates valid markdown agent", async () => {
    await using tmp = await tmpdir({ git: true })
    const filepath = path.join(tmp.path, ".altru-coder", "agent", "helper.md")
    await Filesystem.write(
      filepath,
      `---
...
You are a helpful agent.`,
  ...
  test("skips plan files (excluded subdir)", async () => {
    await using tmp = await tmpdir({ git: true })
    const filepath = path.join(tmp.path, ".altru-coder", "plans", "plan.md")
