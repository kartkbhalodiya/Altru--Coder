...
describe("Project.fromDirectory", () => {
  ...
  test("should handle git repository with commits", async () => {
    ...
    const altruFile = path.join(tmp.path, ".git", "altru-coder")
    expect(await Bun.file(altruFile).exists()).toBe(true)
  })
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  test("worktree should share project ID with main repo", async () => {
    ...
    try {
      ...
      const cache = path.join(tmp.path, ".git", "altru-coder")
...
describe("Project.fromDirectory with bare repos", () => {
  test("worktree from bare repo should cache in bare repo, not parent", async () => {
    ...
    try {
      ...
      const correctCache = path.join(barePath, "altru-coder") // altrucoder_change
      const wrongCache = path.join(parentDir, ".git", "altru-coder") // altrucoder_change
  ...
  test("different bare repos under same parent should not share project ID", async () => {
    ...
    try {
      ...
      const cacheA = path.join(bareA, "altru-coder")
      const cacheB = path.join(bareB, "altru-coder")
      const wrongCache = path.join(parentDir, ".git", "altru-coder")
