...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.ALTRU_CODER_TEST ?? "")'), {
      env: {
        ALTRU_CODER_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "ALTRU_CODER_TEST_SHELL"], {
      shell: true,
      env: {
        ALTRU_CODER_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("ALTRU_CODER_TEST_SHELL=ok")
  })
