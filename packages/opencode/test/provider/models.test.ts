...
// test/preload.ts pins ALTRU_CODER_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.ALTRU_CODER_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.ALTRU_CODER_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.ALTRU_CODER_MODELS_PATH = undefined
  Flag.ALTRU_CODER_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.ALTRU_CODER_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.ALTRU_CODER_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
...
// altrucoder_change - skip: upstream tests assert raw-fixture passthrough but Altru Coder's
// ModelsDev.get() filters/injects providers based on Config.get() (altru-coder-allowed gating,
// apertis options, altru-coder provider injection). The test setup doesn't provide an Instance
