...
import { Auth } from "../../src/auth" // altrucoder_change
...
// altrucoder_change start - upstream #24416 fixture adapted for @altru-coder/plugin
async function markPluginDependenciesReady(dir: string) {
  await mkdir(path.join(dir, "node_modules"), { recursive: true })
  await Bun.write(
    path.join(dir, "package-lock.json"),
    JSON.stringify({ packages: { "": { dependencies: { "@altru-coder/plugin": "0.0.0" } } } }),
  )
}
...
function paid(providers: Awaited<ReturnType<typeof list>>) {
  ...
  if (!item) return 0 // altrucoder_change - Altru Coder drops opencode provider without apiKey/auth
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // altrucoder_change start - upstream #24416 avoids real plugin dependency installs
