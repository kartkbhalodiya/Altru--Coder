...
test("returns default native agents when no config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(names).toContain("code") // altrucoder_change
      expect(names).toContain("plan")
      expect(names).toContain("debug") // altrucoder_change
      expect(names).toContain("orchestrator") // altrucoder_change
      expect(names).toContain("ask") // altrucoder_change
...
// altrucoder_change start - renamed from "build" to "code"
test("code agent has correct default properties", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(evalPerm(code, "bash")).toBe("ask") // altrucoder_change - safe-bash default is ask
    },
  })
})
// altrucoder_change end

// altrucoder_change start - ask agent tests
...
test("plan agent denies edits except .altru-coder/plans/* and .opencode/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".altru-coder/plans/foo.md", plan!.permission).action).toBe("allow")
...
test("agent steps/maxSteps config sets steps property", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const code = await load(tmp.path, (svc) => svc.get("code")) // altrucoder_change
      const plan = await load(tmp.path, (svc) => svc.get("plan"))
      expect(code?.steps).toBe(50) // altrucoder_change
...
test("agent name can be overridden", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        code: { name: "Coder" }, // altrucoder_change
      },
    },
  })
  await Instance.provide({
...
test("agent prompt can be set from config", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        code: { prompt: "Custom system prompt" }, // altrucoder_change
      },
    },
  })
  await Instance.provide({
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".altru-coder", "skill", "perm-skill") // altrucoder_change: .altru-coder is primary
      await Bun.write(
...
`,
  ...
  const home = process.env.ALTRU_CODER_TEST_HOME
  process.env.ALTRU_CODER_TEST_HOME = tmp.path
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await load(tmp.path, (svc) => svc.get("build"))
        const skillDir = path.join(tmp.path, ".altru-coder", "skill", "perm-skill") // altrucoder_change: .altru-coder is primary
...
test("defaultAgent returns build when no default_agent config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(agent).toBe("code") // altrucoder_change
    },
  })
})
...
test("defaultAgent returns plan when code is disabled and default_agent not set", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const agent = await load(tmp.path, (svc) => svc.defaultAgent())
      // altrucoder_change - code is disabled, so it should return plan (next primary agent)
      expect(agent).toBe("plan")
    },
  })
})
...
test("defaultAgent throws when all primary agents are disabled", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        // altrucoder_change start - disable all primary agents
        code: { disable: true },
        plan: { disable: true },
        debug: { disable: true },
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // altrucoder_change - all primary agents are disabled
      await expect(load(tmp.path, (svc) => svc.defaultAgent())).rejects.toThrow("no primary visible agent found")
    },
  })
})

// altrucoder_change start - Backward compatibility tests for "build" -> "code" rename
