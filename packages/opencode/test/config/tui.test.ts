...
afterEach(async () => {
  delete process.env.ALTRU_CODER_CONFIG
  delete process.env.ALTRU_CODER_TUI_CONFIG
  // altrucoder_change start
  await fs.rm(path.join(Global.Path.config, "altru-coder.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "altru-coder.jsonc"), { force: true }).catch(() => {})
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const local = path.join(dir, ".altru-coder") // altrucoder_change
      await fs.mkdir(local, { recursive: true })

      await Bun.write(
        path.join(Global.Path.config, "altru-coder.json"), // altrucoder_change
        JSON.stringify(
          {
      ...
      await Bun.write(
        path.join(local, "altru-coder.json"), // altrucoder_change
        JSON.stringify(
          {
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".altru-coder"), { recursive: true }) // altrucoder_change
      await Bun.write(
        path.join(dir, ".altru-coder", "tui.json"), // altrucoder_change
...
test("migrates tui-specific keys from altru-coder.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "altru-coder.json"))) // altrucoder_change
  expect(server.theme).toBeUndefined()
  expect(server.keybinds).toBeUndefined()
  expect(server.tui).toBeUndefined()
  expect(await Filesystem.exists(path.join(tmp.path, "altru-coder.json.tui-migration.bak"))).toBe(true) // altrucoder_change
...
test("migrates project legacy tui keys even when global tui.json already exists", async () => {
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "altru-coder.json")))
  expect(server.theme).toBeUndefined()
  expect(server.tui).toBeUndefined()
})
...
test("skips migration when altru-coder.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "altru-coder.jsonc.tui-migration.bak"))).toBe(false) // altrucoder_change
  const source = await Filesystem.readText(path.join(tmp.path, "altru-coder.jsonc")) // altrucoder_change
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "altru-coder.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "altru-coder.json.tui-migration.bak"))).toBe(false) // altrucoder_change
})

test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "altru-coder.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "altru-coder.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  const backup = await Filesystem.readText(path.join(tmp.path, "altru-coder.jsonc.tui-migration.bak")) // altrucoder_change
...
// altrucoder_change start
test("migrates legacy tui keys across multiple altru-coder.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "altru-coder.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "altru-coder.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
      // altrucoder_change end
    },
  })
...
test("project config takes precedence over ALTRU_CODER_TUI_CONFIG (matches ALTRU_CODER_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.ALTRU_CODER_TUI_CONFIG = custom
    },
  })
...
test("ALTRU_CODER_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from ALTRU_CODER_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await Bun.write(path.join(customDir, "altru-coder.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.ALTRU_CODER_CONFIG = path.join(customDir, "altru-coder.json") // altrucoder_change
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
...
// altrucoder_change start
test("loads .altru-coder/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".altru-coder"), { recursive: true })
      await Bun.write(path.join(dir, ".altru-coder", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
      // altrucoder_change end
    },
  })
  const config = await getTuiConfig(tmp.path)
