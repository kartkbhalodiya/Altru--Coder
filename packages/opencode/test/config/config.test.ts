...
const managedConfigDir = process.env.ALTRU_CODER_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "altru-coder.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "altru-coder.json") {
...
test("updates config and preserves empty shell sentinel", async () => {
  // altrucoder_change - upstream hardcodes project config to config.json; Altru Coder writes to altru-coder.json
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opencode.ai/config.json",
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "altru-coder.json"))
...
test("updates global config and omits empty shell key in json", async () => {
  // altrucoder_change - globalConfigFile() prefers altru-coder.json over opencode.json
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opencode.ai/config.json",
...
test("loads JSONC config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "altru-coder.jsonc"),
        `{
        // This is a comment
        "$schema": "https://app.altru-coder.ai/config.json",
        "model": "test/model",
...
test("prefers .altru-coder directory config over legacy .altrucoder", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, ".altrucoder", "altru-coder.json"),
        JSON.stringify({
          $schema: "https://app.altru-coder.ai/config.json",
          model: "legacy/model",
        }),
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "altru-coder.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["ALTRU_CODER_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:ALTRU_CODER_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["ALTRU_CODER_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["ALTRU_CODER_CONSOLE_TOKEN"]
    }
  }
})
...
test("validates config schema and reports warning on invalid fields", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // altrucoder_change - invalid schema surfaces as warnings, not a throw
      await load()
...
test("reports warning for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "altru-coder.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // altrucoder_change - invalid JSON surfaces as a warning, not a throw
      await load()
      const warnings = await Config.warnings()
      expect(warnings.length).toBeGreaterThan(0)
    },
...
test("loads config from .altru-coder directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".altru-coder")
...
test("loads agents from .altru-coder/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".altru-coder")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .altru-coder/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".altru-coder")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .altru-coder/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".altru-coder")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("prefers .altru-coder commands over legacy .altrucoder commands", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, ".altrucoder", "command", "hello.md"),
        `---
...
Hello from legacy command`,
      ...
      await Filesystem.write(
        path.join(dir, ".altru-coder", "command", "hello.md"),
        `---
...
test("does not try to install dependencies in read-only ALTRU_CODER_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.ALTRU_CODER_CONFIG_DIR
  process.env.ALTRU_CODER_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ALTRU_CODER_CONFIG_DIR
    else process.env.ALTRU_CODER_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable ALTRU_CODER_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "altru-coder.json"),
        JSON.stringify({ $schema: "https://app.altru-coder.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .altru-coder config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".altru-coder")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .altru-coder config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "altru-coder.json"),
        JSON.stringify({
          $schema: "https://app.altru-coder.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .altru-coder config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "altru-coder.json"),
        JSON.stringify({
          $schema: "https://app.altru-coder.ai/config.json",
...
// Note: preload.ts sets ALTRU_CODER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("permission config preserves user key order", async () => {
  ...
  // altrucoder_change start — isolate from global config to prevent cross-test contamination
  ...
  try {
    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, "altru-coder.json"), // altrucoder_change
          JSON.stringify({
...
test("project config can override MCP server enabled status", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // altrucoder_change start — base config in .json, override in .jsonc (jsonc loads second and wins)
...
test("local .altru-coder config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .altru-coder directory config enables it
      const opencodeDir = path.join(dir, ".altru-coder")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "altru-coder.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "altru-coder.json") // altrucoder_change
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.altru-coder/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.altru-coder/plugin/demo.ts", "file:///project/.altru-coder/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.altru-coder/plugin/demo.ts"])
  })
...
describe("ALTRU_CODER_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"]
    process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .altru-coder/ directories when flag is set", async () => {
    // altrucoder_change - test .altru-coder/ directory (the test body writes to .altru-coder/command/)
    const originalEnv = process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"]
    process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .altru-coder directory with a command
          const opencodeDir = path.join(dir, ".altru-coder", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .altru-coder should NOT be in directories list  // altrucoder_change
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ALTRU_CODER_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["ALTRU_CODER_CONFIG_DIR"]
      process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["ALTRU_CODER_CONFIG_DIR"]
      } else {
        process.env["ALTRU_CODER_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("ALTRU_CODER_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ALTRU_CODER_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["ALTRU_CODER_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["ALTRU_CODER_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from ALTRU_CODER_CONFIG_DIR, not project
...
describe("ALTRU_CODER_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in ALTRU_CODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ALTRU_CODER_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["ALTRU_CODER_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["ALTRU_CODER_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["ALTRU_CODER_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in ALTRU_CODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ALTRU_CODER_CONFIG_CONTENT"]
