...
const originalWorkspacesFlag = Flag.ALTRU_CODER_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  ALTRU_CODER_AUTH_CONTENT: process.env.ALTRU_CODER_AUTH_CONTENT,
...
beforeEach(() => {
  Database.close()
  Flag.ALTRU_CODER_EXPERIMENTAL_WORKSPACES = true
  restoreEnv()
})
...
afterEach(async () => {
  ...
  Flag.ALTRU_CODER_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
describe("workspace-old CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      process.env.ALTRU_CODER_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      ...
      expect(JSON.parse(recorded.calls.create[0].env.ALTRU_CODER_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.ALTRU_CODER_WORKSPACE_ID).toBe(workspaceID)
      expect(recorded.calls.create[0].env.ALTRU_CODER_EXPERIMENTAL_WORKSPACES).toBe("true")
...
describe("workspace-old sync state", () => {
  test("startWorkspaceSyncing is disabled by the experimental workspace flag", async () => {
    await withInstance(async (dir) => {
      Flag.ALTRU_CODER_EXPERIMENTAL_WORKSPACES = false
