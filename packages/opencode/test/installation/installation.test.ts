...
describe("installation", () => {
  describe("latest", () => {
    ...
    test("reads npm versions via registry", async () => {
      ...
      expect(calls).toContain(`https://registry.npmjs.org/@altru-coder%2Fcli/${InstallationChannel}`) // altrucoder_change
    })
    ...
    test("reads brew formulae API versions", async () => {
      const layer = testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("Altru-Coder/tap/altru")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("altru-coder")) return "altru-coder"
