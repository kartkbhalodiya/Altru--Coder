...
import * as launch from "../../src/lsp/launch" // altrucoder_change - spy on spawn
...
import { TsCheck } from "../../src/altrucoder/ts-check" // altrucoder_change

// altrucoder_change - Typescript.spawn ignores ctx, so a cast is fine here.
...
describe("lsp.spawn", () => {
  ...
  // altrucoder_change start - enable flag so spawn() is reached past the TsClient short-circuit
  it.live("would spawn builtin LSP for files inside instance when lsp is true", () =>
    provideTmpdirInstance(
      (dir) =>
        LSP.Service.use((lsp) =>
          Effect.gen(function* () {
            const saved = Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TOOL
            Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TOOL = true
            const spy = spyOn(LSPServer.Typescript, "spawn").mockResolvedValue(undefined)
            ...
            try {
              ...
              Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TOOL = saved
              spy.mockRestore()
            }
          }),
        ),
  ...
  // altrucoder_change start - Typescript spawn is gated behind ALTRU_CODER_EXPERIMENTAL_LSP_TOOL.
  test("spawns tsgo LSP when ALTRU_CODER_EXPERIMENTAL_LSP_TOOL is enabled", async () => {
    const saved = Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TOOL
    Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TOOL = true
  ...
  test("Typescript.spawn returns undefined when ALTRU_CODER_EXPERIMENTAL_LSP_TOOL is off", async () => {
    const saved = Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TOOL
    Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TOOL = false
    try {
