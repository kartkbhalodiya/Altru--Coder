...
import { ToastProvider } from "../../../../src/cli/cmd/tui/ui/toast" // altrucoder_change
import { Instance } from "../../../../src/project/instance" // altrucoder_change
...
function createFetch() {
  ...
  const fetch = (async (input: RequestInfo | URL) => {
    ...
    switch (url.pathname) {
      ...
      case "/global/config": // altrucoder_change
      case "/mcp":
      case "/provider/auth":
      case "/session/status":
        return json({})
...
describe("tui sync", () => {
  test("refresh scopes sessions by default and lists project sessions when disabled", async () => {
    ...
    const { app, kv, sync, session } = await Instance.provide({ directory: tmp.path, fn: mount }) // altrucoder_change
    ...
    try {
      ...
      await disposeAllInstances() // altrucoder_change
