...
const wrapper = path.join(root, "bin", "altru-coder")
...
describe("npm install artifact behavior", () => {
  test("keeps the CLI wrapper contract", async () => {
    ...
    expect(text).toContain("const envPath = process.env.ALTRU_CODER_BIN_PATH")
    expect(text).toContain('const base = "@altru-coder/cli-" + platform + "-" + arch')
  ...
  test("links npm bin commands to the wrapper during local install", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "altru-coder-install-artifact-"))
    try {
      ...
      await fs.copyFile(wrapper, path.join(bin, "altru-coder"))
      await Bun.write(
        path.join(pkg, "package.json"),
        JSON.stringify(
          {
            name: "altru-coder-install-artifact-repro",
            version: "1.0.0",
            bin: {
      ...
      const commands = ["altru-coder", "altrucoder"]
      ...
      const hidden = path.join(prefix, "node_modules", ".bin", ".altru-coder")
