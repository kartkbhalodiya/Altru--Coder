...
import { SessionNetwork } from "../../src/session/network" // altrucoder_change
...
it.live("session.processor effect tests reset reasoning state across retries", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
        // altrucoder_change start — auto-reply to network reconnection prompts triggered by reset()
        ...
        offAsk() // altrucoder_change — cleanup subscriber
      }),
    { git: true, config: (url) => providerCfg(url) },
  ),
)
