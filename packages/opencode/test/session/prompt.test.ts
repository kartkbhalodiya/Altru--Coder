...
// altrucoder_change start - make timing-sensitive prompt tests wait on observable state
...
// altrucoder_change - skipped: tracked in #9958
    ...
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        const tool = yield* Effect.promise(async () => {
          ...
          while (Date.now() < end) {
            const msgs = await Effect.runPromise(MessageV2.filterCompactedEffect(chat.id))
            const assistant = msgs.findLast((item) => item.info.role === "assistant" && item.info.agent === "code") // altrucoder_change
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      // altrucoder_change start - hold the model response instead of cancelling an infinite stream
      Effect.fnUntraced(function* ({ llm }) {
...
unix(
  // altrucoder_change - skip flaky cancel test on Windows CI
...
// altrucoder_change start - handleSubtask propagates child session cost to wrapper (#6321)
it.live(
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      // altrucoder_change start - gate the failing stream so both callers join the same run
      Effect.fnUntraced(function* ({ llm }) {
        const gate = defer<void>()
        const prompt = yield* SessionPrompt.Service
        const sessions = yield* Session.Service
...
// altrucoder_change start - #9492: the upstream fork-based shape of this test
...
// can be exercised. The Altru Coder queue semantics (in-flight stream drains, second
// LLM request ends with the queued user message) are covered end-to-end in
// packages/opencode/test/altrucoder/session-prompt-queue.test.ts — keep this
...
it.live(
  ...
  30_000, // altrucoder_change - Windows CI process startup can exceed 3s
)
      ...
      (_dir) =>
        Effect.gen(function* () {
          ...
          if (Exit.isFailure(exit)) {
            ...
            expect(NamedError.Unknown.isInstance(err)).toBe(true)
            if (NamedError.Unknown.isInstance(err)) {
              expect(err.data.message).toContain("code") // altrucoder_change - "build" renamed to "code"
            }
          }
        }),
      { git: true },
