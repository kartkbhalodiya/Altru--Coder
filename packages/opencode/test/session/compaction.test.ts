...
describe("SessionNs.getUsage", () => {
  ...
  // altrucoder_change start - Test for OpenRouter provider cost
  ...
  test("uses upstreamInferenceCost for Altru Coder provider", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    const provider = { id: "altru-coder" } as Provider.Info
    const result = SessionNs.getUsage({
      model,
      provider,
      usage: {
    ...
    // Should use upstreamInferenceCost for Altru Coder provider (BYOK)
  ...
  test("uses regular cost when upstreamInferenceCost is missing for Altru Coder", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    // When upstream cost is missing for Altru Coder, fall back to regular cost field
