...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.ALTRU_CODER_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.ALTRU_CODER_TEST_HOME = prev
      }),
  )

const discovered = <T extends { location: string }>(list: readonly T[]) =>
  list.filter((s) => s.location !== Skill.BUILTIN_LOCATION) // altrucoder_change

describe("skill", () => {
  // altrucoder_change start
  it.live("discovers skills from .altru-coder/skill/ directory", () =>
      ...
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".altru-coder", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
...
`,
          ...
          const list = discovered(yield* skill.all()) // altrucoder_change
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".altru-coder", "skill", "dir-skill", "SKILL.md"), // altrucoder_change: .altru-coder is primary
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".altru-coder", "skill", "dir-skill")) // altrucoder_change: .altru-coder is primary
  ...
  // altrucoder_change start
  it.live("discovers multiple skills from .altru-coder/skill/ directory", () =>
    // altrucoder_change end
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".altru-coder", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".altru-coder", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".altru-coder", "skill", "no-frontmatter", "SKILL.md"), // altrucoder_change: .altru-coder is primary
              `# No Frontmatter
...
`,
          ...
          const skill = yield* Skill.Service
          expect(discovered(yield* skill.all())).toEqual([]) // altrucoder_change
        }),
      { git: true },
    ),
  )
