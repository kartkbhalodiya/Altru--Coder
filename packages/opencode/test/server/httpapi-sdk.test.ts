...
type Sdk = ReturnType<typeof createAltruCoderClient>
...
function app(backend: Backend, input?: { password?: string; username?: string }) {
  Flag.ALTRU_CODER_EXPERIMENTAL_HTTPAPI = backend === "httpapi"
...
// altrucoder_change start - skip variant for Altru Coder-overlaid routes not yet wired into the HttpApi bridge
...
describe("HttpApi SDK", () => {
  ...
  // altrucoder_change start - /config/providers and /agent 500 on HttpApi backend; Altru Coder overlays not yet migrated onto the bridge
  httpapi.skip(
  ...
  parity("matches generated SDK basic auth behavior across backends", (backend) =>
    withStandardProject(backend, ({ directory }) =>
      Effect.gen(function* () {
        ...
        // altrucoder_change start - match Hono AuthMiddleware username default ("altru-coder")
        const bad = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("altru-coder", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("altru-coder", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
        // altrucoder_change end
