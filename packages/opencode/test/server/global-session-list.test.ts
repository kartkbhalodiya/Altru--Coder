...
import { RemoteSender } from "../../src/altru-coder-sessions/remote-sender" // altrucoder_change
...
describe("session.listGlobal", () => {
  ...
  // altrucoder_change start - project-family filter across worktrees (stale .git/altru project ID)
  test("filters by project family across worktrees when project IDs drift", async () => {
    ...
    try {
      ...
      // Now write a stale project ID to .git/altru — this overrides the root's cached ID
      await Bun.write(path.join(first.path, ".git", "altru-coder"), "stale-project-id")

      const root = await Instance.provide({
        directory: first.path,
        fn: async () => svc.create({ title: "root-session" }),
      })
      await Bun.file(path.join(first.path, ".git", "altru-coder")).delete()
