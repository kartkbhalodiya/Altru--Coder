    ...
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          ...
          "x-altru-coder-directory": "/secret/path",
      ...
      expect(forwarded?.headers["x-altru-coder-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-altru-coder-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-altru-coder-directory", headerDir),
        HttpClient.execute,
      )
