...
function app(experimental: boolean) {
  Flag.ALTRU_CODER_EXPERIMENTAL_HTTPAPI = experimental
...
// altrucoder_change - HttpApi vs legacy Hono parity is structurally broken on Altru Coder:
...
//      Many Altru Coder-specific Model fields (ai_sdk_provider, prompt, recommendedIndex,
//      isFree) and Command fields (agent, model, subtask) hit this.
//   2. The two backends share Altru Coder's ConfigService cache; reading /config twice in
...
// Skip until either the Altru Coder schemas migrate to NullOr or the parity test learns
// to ignore Altru Coder-specific extra fields.
