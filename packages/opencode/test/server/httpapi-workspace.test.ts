...
const originalWorkspaces = Flag.ALTRU_CODER_EXPERIMENTAL_WORKSPACES
const originalHttpApi = Flag.ALTRU_CODER_EXPERIMENTAL_HTTPAPI
const it = testEffect(
...
afterEach(async () => {
  mock.restore()
  Flag.ALTRU_CODER_EXPERIMENTAL_WORKSPACES = originalWorkspaces
  Flag.ALTRU_CODER_EXPERIMENTAL_HTTPAPI = originalHttpApi
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-altru-coder-workspace": "internal",
          },
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-altru-coder-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-altru-coder-workspace")
      } finally {
