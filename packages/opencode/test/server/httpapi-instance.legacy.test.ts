...
describe("instance HttpApi", () => {
  ...
  test.skip("serves catalog read endpoints through Hono bridge", async () => {
    ...
    const [commands, agents, skills, lsp, formatter] = await Promise.all([
      app().request(InstancePaths.command, { headers: { "x-altru-coder-directory": tmp.path } }),
      app().request(InstancePaths.agent, { headers: { "x-altru-coder-directory": tmp.path } }),
      app().request(InstancePaths.skill, { headers: { "x-altru-coder-directory": tmp.path } }),
      app().request(InstancePaths.lsp, { headers: { "x-altru-coder-directory": tmp.path } }),
      app().request(InstancePaths.formatter, { headers: { "x-altru-coder-directory": tmp.path } }),
    ])
  ...
  test("serves project git init through Hono bridge", async () => {
    ...
    const current = await app().request("/project/current", { headers: { "x-altru-coder-directory": tmp.path } })
  ...
  test("serves project update through Hono bridge", async () => {
    ...
    const response = await app().request(`/project/${project.id}`, {
      method: "PATCH",
      headers: { "x-altru-coder-directory": tmp.path, "content-type": "application/json" },
    ...
    const list = await app().request("/project", { headers: { "x-altru-coder-directory": tmp.path } })
