...
- Author package entrypoint is `@altru-coder/plugin/tui`.
...
- Import types from `@altru-coder/plugin/tui`.
- `@altru-coder/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `ALTRU_CODER_CONFIG_DIR`, OpenCode installs `@altru-coder/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@altru-coder/plugin/tui`.
...
- `--pure` / `ALTRU_CODER_PURE` skips external TUI plugins only.
