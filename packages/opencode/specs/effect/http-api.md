...
- `ALTRU_CODER_EXPERIMENTAL_HTTPAPI` selects the backend at server startup. Default is still `hono`.
...
- An Effect `HttpApi` OpenAPI surface exists (`OpenApi.fromApi(PublicApi)` in `cli/cmd/generate.ts --httpapi`, `ALTRU_CODER_SDK_OPENAPI=httpapi` in `packages/sdk/js/script/build.ts`) but is opt-in. The default SDK generation is still Hono.
...
- Instance context is provided by `httpapi/server.ts` using `directory`, `workspace`, and `x-altru-coder-directory`.
...
3. Mount the new paths in `src/server/routes/instance/index.ts` only inside the `ALTRU_CODER_EXPERIMENTAL_HTTPAPI` block.
...
2. The route is mounted by default, not only behind `ALTRU_CODER_EXPERIMENTAL_HTTPAPI`.
...
- Cover `directory` and `x-altru-coder-directory` instance selection.
...
14. [ ] Switch OpenAPI/SDK generation to Effect routes and compare SDK output. Effect path is implemented and opt-in via `--httpapi` / `ALTRU_CODER_SDK_OPENAPI=httpapi`. Close the schema-shape gaps in `public.ts` (branded `pattern`, per-property `description`, `Event.*` / `SyncEvent.*` naming, dedup collisions), then flip `packages/sdk/js/script/build.ts` default.
15. [ ] Flip `backend.ts` default from `hono` to `effect-httpapi`, keep `ALTRU_CODER_EXPERIMENTAL_HTTPAPI` (or its inverse) as a short fallback flag, then delete replaced Hono route files.

## Checklist

- [x] Add first `HttpApi` JSON route slices.
- [x] Bridge selected `HttpApi` routes behind `ALTRU_CODER_EXPERIMENTAL_HTTPAPI`. (Now backend-fork-at-startup rather than in-Hono path mounting.)
...
- [x] Implement Effect `HttpApi` OpenAPI generation behind `--httpapi` / `ALTRU_CODER_SDK_OPENAPI=httpapi`.
