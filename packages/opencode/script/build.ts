...
import { createRequire } from "module" // altrucoder_change

const __filename = fileURLToPath(import.meta.url)
const __dirname = path.dirname(__filename)
const dir = path.resolve(__dirname, "..")
const require = createRequire(import.meta.url) // altrucoder_change
...
import { LanceDBRuntime } from "../src/altrucoder/lancedb" // altrucoder_change
...
// altrucoder_change - packages/app was removed; the web UI embed step is no longer applicable

// altrucoder_change start - codebase indexing
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    plugins: [plugin],
    // altrucoder_change start - skip sourcemaps for release builds (each .js.map adds ~50 MB per target → ~600 MB total)
    sourcemap: Script.release ? "none" : "external",
    // altrucoder_change end
    external: ["node-gyp", ...LanceDBRuntime.external], // altrucoder_change
    format: "esm",
    minify: true,
    splitting: true,
    compile: {
      ...
      outfile: `dist/${name}/bin/altru-coder`, // altrucoder_change
      execArgv: [`--user-agent=altru-coder/${Script.version}`, "--use-system-ca", "--"], // altrucoder_change
      windows: {},
    },
    // altrucoder_change start - packages/app was removed; no embedded web UI
    files: {},
    entrypoints: ["./src/index.ts", parserWorker, workerPath],
    // altrucoder_change end
    define: {
      ALTRU_CODER_VERSION: `'${Script.version}'`,
      ALTRU_CODER_MIGRATIONS: JSON.stringify(migrations),
      OTUI_TREE_SITTER_WORKER_PATH: bunfsRoot + workerRelativePath,
      ALTRU_CODER_WORKER_PATH: workerPath,
      ALTRU_CODER_CHANNEL: `'${Script.channel}'`,
      ALTRU_CODER_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
      ALTRU_CODER_BUILD_KIND: Script.release ? `'release'` : `'source'`, // altrucoder_change
    },
  })
  ...
  if (item.os === "linux") {
    ...
    if (interpreter) {
      try {
        await $`patchelf --set-interpreter ${interpreter} dist/${name}/bin/altru-coder`
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/altru-coder` // altrucoder_change
...
if (Script.release) {
  const archives: string[] = [] // altrucoder_change
  for (const key of Object.keys(binaries)) {
    const archive = key.replace(pkg.name, "altru-coder") // altrucoder_change
    if (key.includes("linux")) {
      const out = path.resolve("dist", `${archive}.tar.gz`) // altrucoder_change
      await $`tar -czf ${out} *`.cwd(`dist/${key}/bin`) // altrucoder_change
      archives.push(out) // altrucoder_change
    } else {
      const out = path.resolve("dist", `${archive}.zip`) // altrucoder_change
      await $`zip -r ${out} *`.cwd(`dist/${key}/bin`) // altrucoder_change
      archives.push(out) // altrucoder_change
    }
  }
  await $`gh release upload v${Script.version} ${archives} --clobber` // altrucoder_change
}
