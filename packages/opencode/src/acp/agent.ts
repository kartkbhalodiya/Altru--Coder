...
import type { AssistantMessage, Event, AltruCoderClient, SessionMessageResponse, ToolPart } from "@altru-coder/sdk/v2"
import { applyPatch } from "diff"
import { InstallationVersion } from "@opencode-ai/core/installation/version"

import { fetchDefaultModel } from "@altru-coder/altru-coder-gateway" // altrucoder_change
...
export async function init({ sdk: _sdk }: { sdk: AltruCoderClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `altru-coder auth login` in the terminal",
      name: "Login with Altru Coder",
      id: "altru-coder-login",
    }
    ...
    if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
      authMethod._meta = {
        "terminal-auth": {
          command: "opencode",
          args: ["auth", "login"],
          label: "Altru Coder Login", // altrucoder_change
        },
      }
    }
    ...
    return {
      ...
      agentInfo: {
        name: "Altru Coder", // altrucoder_change
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  // altrucoder_change start
  const altruProvider = providers.find((p) => p.id === "altru-coder")
  if (altruProvider) {
    const [best] = Provider.sort(Object.values(altruProvider.models))
    if (best) {
      return {
  ...
  // altrucoder_change start
  // Only fall back to the Altru Coder provider if it was present in the available
  ...
  if (providers.some((p) => p.id === "altru-coder")) {
    const freeModel = await fetchDefaultModel()
    return { providerID: ProviderID["altru-coder"], modelID: ModelID.make(freeModel) }
  }
