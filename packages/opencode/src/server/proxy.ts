...
type Msg = string | ArrayBuffer // altrucoder_change - narrower than upstream's Uint8Array variant; onMessage always converts before enqueue
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-altru-coder-proxy-url")
      const queue: Msg[] = []
...
export function websocket(
  ...
  next.set("x-altru-coder-proxy-url", ProxyUtil.websocketTargetURL(target))
