...
function createHono(opts: CorsOptions, selection: ServerBackend.Selection = ServerBackend.force(select(), "hono")) {
  ...
  if (Flag.ALTRU_CODER_WORKSPACE_ID) {
    return {
      app: app
        .use(InstanceMiddleware(Flag.ALTRU_CODER_WORKSPACE_ID ? WorkspaceID.make(Flag.ALTRU_CODER_WORKSPACE_ID) : undefined))
        .use(FenceMiddleware)
...
export async function openapi() {
  ...
  const result = await generateSpecs(app, {
    documentation: {
      info: {
        title: AltruCoderServer.DOC_TITLE, // altrucoder_change
        version: "1.0.0",
        description: AltruCoderServer.DOC_DESCRIPTION, // altrucoder_change
      },
      openapi: "3.1.1",
    },
  })
