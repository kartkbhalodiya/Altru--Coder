...
import { SaveAlwaysRulesBody } from "../groups/permission" // altrucoder_change
...
export const permissionHandlers = HttpApiBuilder.group(InstanceHttpApi, "permission", (handlers) =>
  Effect.gen(function* () {
    ...
    const reply = Effect.fn("PermissionHttpApi.reply")(function* (ctx: {
      ...
      if (!ok) return yield* new HttpApiError.NotFound({}) // altrucoder_change
      return true
    })
    ...
    return handlers.handle("list", list).handle("reply", reply).handle("saveAlwaysRules", saveAlwaysRules) // altrucoder_change
  }),
)
