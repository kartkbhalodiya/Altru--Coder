...
const embeddedUIPromise = Flag.ALTRU_CODER_DISABLE_EMBEDDED_WEB_UI
...
// altrucoder_change - upstream's proxy-to-app.opencode.ai fallback was removed; Altru Coder serves the embedded UI only
function embeddedUI() {
  if (Flag.ALTRU_CODER_DISABLE_EMBEDDED_WEB_UI) return Promise.resolve(null)
...
export async function serveUI(request: Request) {
  ...
  // altrucoder_change - no proxy fallback to app.opencode.ai; embedded UI only
  return Response.json({ error: "Not Found" }, { status: 404 })
}
