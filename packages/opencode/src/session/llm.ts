...
import { DEFAULT_HEADERS } from "@/altrucoder/const"
import { getAltruCoderProjectId } from "@/altrucoder/project-id"
import { HEADER_PROJECTID, HEADER_MACHINEID, HEADER_TASKID } from "@altru-coder/altru-coder-gateway"
import { Identity } from "@altru-coder/altru-coder-telemetry"
...
export interface Interface {
  ...
  readonly raw: (input: StreamRequest) => Effect.Effect<Result> // altrucoder_change - raw streamText result for Altru Coder helpers
}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      system.push(
        [
          // altrucoder_change start - soul defines core identity and personality
      ...
      if (isOpenaiOauth) {
        // altrucoder_change start - prepend soul to instructions
      ...
      const params = yield* plugin.trigger(
        ...
        },
        {
          ...
          // altrucoder_change start - gpt-5 via @ai-sdk/openai-compatible proxies (e.g. LiteLLM)
      ...
      // altrucoder_change start - resolve project ID and machine ID for altru-coder provider
      const isAltruCoder = input.model.api.npm === "@altru-coder/altru-coder-gateway"
      const altruProjectId = yield* isAltruCoder
        ? Effect.promise(() => getAltruCoderProjectId().catch(() => undefined))
        : Effect.succeed(undefined)
      const machineId = yield* isAltruCoder
      ...
      return streamText({
        ...
        abortSignal: input.abort,
        headers: {
          ...(input.model.providerID.startsWith("altru-coder") // altrucoder_change
            ? {
                "x-altru-coder-project": opencodeProjectID,
                "x-altru-coder-session": input.sessionID,
                "x-altru-coder-request": input.user.id,
                "x-altru-coder-client": Flag.ALTRU_CODER_CLIENT,
              }
            : {
                "x-session-affinity": input.sessionID,
                ...(input.parentSessionID ? { "x-parent-session-id": input.parentSessionID } : {}),
                "User-Agent": `opencode/${InstallationVersion}`,
                ...(input.model.providerID !== "anthropic" ? DEFAULT_HEADERS : undefined), // altrucoder_change
              }),
          // altrucoder_change start - headers for altru-coder provider
          ...(isAltruCoder && input.agent.name ? { "x-altrucoder-mode": input.agent.name.toLowerCase() } : {}),
          ...(isAltruCoder && altruProjectId ? { [HEADER_PROJECTID]: altruProjectId } : {}),
          ...(isAltruCoder && machineId ? { [HEADER_MACHINEID]: machineId } : {}),
          ...(isAltruCoder ? { [HEADER_TASKID]: input.sessionID } : {}),
          // altrucoder_change end
          ...input.model.headers,
          ...headers,
        },
        ...
        // altrucoder_change - disable AI SDK span recording (ai.* / gen_ai.*)
    ...
    // altrucoder_change - expose raw streamText result for Altru Coder helpers; Effect.orDie collapses AuthError into a defect
...
// altrucoder_change start - keep raw async stream wrapper for Altru Coder callsites during Effect migration
