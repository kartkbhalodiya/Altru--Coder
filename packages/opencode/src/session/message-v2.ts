...
import { SessionNetwork } from "./network" // altrucoder_change
...
// altrucoder_change start - shared editor context schema (used by MessageV2.User and SessionPrompt.PromptInput)
...
// altrucoder_change start - strip bloated metadata fields from stored parts to prevent multi-MB payloads
...
export function stripPartMetadata(part: Part): Part {
  // altrucoder_change - exported for testing
...
// altrucoder_change - apply stripping inside helpers so all read paths are covered
...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  for (const msg of input) {
    ...
    if (msg.info.role === "assistant") {
      ...
      for (const part of msg.parts) {
        // altrucoder_change start - keep local UI warnings out of future prompts
        if (part.type === "text" && !part.ignored)
          // altrucoder_change end
...
export function parts(message_id: MessageID) {
  ...
  return rows.map(
    (row) =>
      // altrucoder_change - apply stripping to parts fetched individually as well to cover all read paths
      stripPartMetadata({
        ...row.data,
        id: row.id,
        sessionID: row.session_id,
...
export function fromError(
  ...
  switch (true) {
    ...
    case SessionNetwork.disconnected(e): // altrucoder_change start
      return new APIError(
        {
          message: SessionNetwork.message(e), // altrucoder_change end
          isRetryable: true,
          metadata: {
