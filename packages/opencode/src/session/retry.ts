...
import { isAltruCoderError } from "@/altrucoder/altru-coder-errors" // altrucoder_change
...
export function retryable(error: Err) {
  ...
  if (MessageV2.APIError.isInstance(error)) {
    ...
    // altrucoder_change start - Current Altru Coder errors require user action (login/signup), don't retry
    if (isAltruCoderError(error)) return undefined
    ...
    // altrucoder_change start - FreeUsageLimitError is not retryable: retrying the same
...
export function policy(opts: {
  ...
  return Schedule.fromStepWithMetadata(
    Effect.succeed((meta: Schedule.InputMetadata<unknown>) => {
      // altrucoder_change start — enforce retry limit
      ...
      return Effect.gen(function* () {
        // altrucoder_change start — handle network disconnect via offline handler
