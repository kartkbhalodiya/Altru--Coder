...
import { AltruCoderSessionPrompt } from "@/altrucoder/session/prompt" // altrucoder_change
import { AltruCoderSessionPromptQueue } from "@/altrucoder/session/prompt-queue" // altrucoder_change
...
// altrucoder_change
export const shouldAskPlanFollowup = AltruCoderSessionPrompt.shouldAskPlanFollowup
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cancel = Effect.fn("SessionPrompt.cancel")(function* (sessionID: SessionID) {
      ...
      yield* AltruCoderSessionPromptQueue.cancel(sessionID) // altrucoder_change - drop queued follow-up loops on abort
      AltruCoderSessionPrompt.abortPlanFollowup(sessionID) // altrucoder_change - abort pending plan-followup handover work
    ...
    const insertReminders = Effect.fn("SessionPrompt.insertReminders")(function* (input: {
      ...
      if (!Flag.ALTRU_CODER_EXPERIMENTAL_PLAN_MODE) {
        // altrucoder_change start - inject plan file path so agent writes to .altru-coder/plans/
        yield* Effect.promise(() =>
          AltruCoderSessionPrompt.insertPlanReminders({ agent: input.agent, session: input.session, userMessage }),
        )
        // altrucoder_change end
        const wasPlan = input.messages.some((msg) => msg.info.role === "assistant" && msg.info.agent === "plan")
        if (wasPlan && input.agent.name === "code") {
          // altrucoder_change - renamed from "build" to "code"
          userMessage.parts.push({
            id: PartID.ascending(),
            messageID: userMessage.info.id,
            sessionID: userMessage.info.sessionID,
            type: "text",
            text: CODE_SWITCH, // altrucoder_change - renamed from BUILD_SWITCH to CODE_SWITCH
            synthetic: true,
          })
        }
      ...
      if (input.agent.name !== "plan" && assistantMessage?.info.agent === "plan") {
        ...
        const part = yield* sessions.updatePart({
          ...
          sessionID: userMessage.info.sessionID,
          type: "text",
          text: `${CODE_SWITCH}\n\nA plan file exists at ${plan}. You should execute on the plan defined within it`, // altrucoder_change - renamed from BUILD_SWITCH to CODE_SWITCH
          synthetic: true,
        })
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
          ...
          permission
            .ask({
              ...
              ruleset: Permission.merge(
                ...
                AltruCoderSessionPrompt.guardPermissions({ agent: input.agent, session: input.session }),
              ),
              hardRuleset: AltruCoderSessionPrompt.hardPermissions({ agent: input.agent }),
              // altrucoder_change end
            })
            .pipe(Effect.orDie),
      })
    ...
    const handleSubtask = Effect.fn("SessionPrompt.handleSubtask")(function* (input: {
      ...
      // altrucoder_change start - shared reader for the child session id written by task.ts ctx.metadata (#6321)
      const childID = () => {
            ...
            permission
              .ask({
                ...
                ruleset: Permission.merge(
                  ...
                  AltruCoderSessionPrompt.guardPermissions({ agent: taskAgent, session }),
                ),
                hardRuleset: AltruCoderSessionPrompt.hardPermissions({ agent: taskAgent }),
                // altrucoder_change end
              })
              .pipe(Effect.orDie),
        })
        .pipe(
          ...
          Effect.onInterrupt(() =>
            Effect.gen(function* () {
              ...
              // altrucoder_change start - propagate partial subagent cost on cancel (#6321)
              const cid = childID()
              if (cid) {
                assistantMessage.cost = yield* AltruCoderCostPropagation.childCost(sessions, SessionID.make(cid))
              }
              // altrucoder_change end
      ...
      assistantMessage.finish = "tool-calls"
      assistantMessage.time.completed = Date.now()
      // altrucoder_change start - include subagent total cost on the wrapper message (#6321)
      ...
      const summaryUserMsg: MessageV2.User = {
        ...
        editorContext: lastUser.editorContext, // altrucoder_change — preserve editor context
      }
    ...
    const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
      ...
      const info: MessageV2.User = {
        ...
        system: input.system,
        format: input.format,
        editorContext: input.editorContext, // altrucoder_change
      }
          ...
          switch (url.protocol) {
            ...
            case "file:": {
              ...
              if (mime === "application/x-directory") {
                ...
                const exit = yield* execRead(args, { includeDirectoryFiles: true }).pipe(Effect.exit) // altrucoder_change inline folder files
    ...
    const prompt: (input: PromptInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.prompt")(
      function* (input: PromptInput) {
        const session = yield* sessions.get(input.sessionID)
        yield* revert.cleanup(session)
        // altrucoder_change start - persist queued prompts immediately while serializing each follow-up loop
        yield* AltruCoderSessionPrompt.recoverDanglingAssistant({ sessionID: input.sessionID, status, sessions })
        yield* AltruCoderSessionPrompt.recoverProviderFinishError({ sessionID: input.sessionID, status, sessions })
        ...
        // altrucoder_change start — unblock tools waiting on user input so any in-flight
        ...
        return yield* AltruCoderSessionPromptQueue.enqueue(
          input.sessionID,
          message.info.id,
    ...
    const lastAssistant = Effect.fnUntraced(function* (sessionID: SessionID) {
      // altrucoder_change start - retry when cancel races before shellImpl writes messages
    ...
    // altrucoder_change — mutable close-reason per session, set by runLoop and read by loop
    const closeReasons = new Map<string, AltruCoderSession.CloseReason>()

    const runLoop: (sessionID: SessionID) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.run")(
      function* (sessionID: SessionID) {
        // altrucoder_change — cache environment details per turn (prompt caching)
        const envCache: AltruCoderSessionPrompt.EnvCache = {}
        closeReasons.delete(sessionID) // altrucoder_change
        let compactionAttempts = 0 // altrucoder_change - cap compaction attempts per turn to avoid infinite loops
        const ctx = yield* InstanceState.context
        ...
        while (true) {
          ...
          msgs = AltruCoderSessionPromptQueue.scope(sessionID, msgs) // altrucoder_change - hide later queued prompts
          msgs = AltruCoderSessionPrompt.trimBeforeLastSummary(msgs) // altrucoder_change - trim on any completed summary (e.g. manual /compact against a text user)
          ...
          // altrucoder_change start - carry local review command marker into LLM telemetry
          const telemetry = AltruCoderSessionProcessor.extractReviewTelemetry(
          ...
          // altrucoder_change start - keep provider-executed tools from forcing a re-loop
          ...
          // altrucoder_change start - plan_exit is a hard stop before another model call
          if (
            lastAssistant?.finish &&
            hasToolCalls &&
            lastAssistant.parentID === lastUser.id &&
            lastUser.id < lastAssistant.id &&
            AltruCoderSessionPrompt.shouldAskPlanFollowup({ messages: msgs, abort: AbortSignal.any([]) })
          ) {
          ...
          if (
            ...
            !hasToolCalls &&
            lastAssistant.parentID === lastUser.id && // altrucoder_change - unrelated later assistants do not answer this turn
            lastUser.id < lastAssistant.id
          ) {
            // altrucoder_change start - ask follow-up when plan_exit tool was called
            const action = yield* Effect.promise((signal) =>
              AltruCoderSessionPrompt.askPlanFollowup({ sessionID, messages: msgs, abort: signal }),
            )
            if (action === "continue") continue
          ...
          if (task?.type === "compaction") {
            ...
            // altrucoder_change start - compaction.process only returns "stop" after
          ...
          if (
            ...
            // altrucoder_change start
            const guard = AltruCoderSessionPrompt.guardCompactionAttempt({
              sessionID,
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            // altrucoder_change start — ephemeral context injection + post-summary
            ...
            AltruCoderSessionPrompt.injectEditorContext({ msgs, lastUser, sessionID, cache: envCache })
            msgs = AltruCoderSessionPrompt.maybeStripHistoricalMedia(msgs)
            // altrucoder_change end
            ...
            const mode = AltruCoderSessionPrompt.modeInstructions({ agent }) // altrucoder_change
            const system = [...env, ...instructions, ...(mode ? [mode] : []), ...(skills ? [skills] : [])]
            const format = lastUser.format ?? { type: "text" as const }
            if (format.type === "json_schema") system.push(STRUCTURED_OUTPUT_SYSTEM_PROMPT) // altrucoder_change
            const result = yield* handle.process({
              // altrucoder_change
              // altrucoder_change start - keep Ask/Plan tool filtering hardened against session allows
              user: lastUser,
              agent,
              permission: AltruCoderSessionPrompt.guardPermissions({ agent, session }),
              // altrucoder_change end
              sessionID,
            ...
            if (finished && !handle.message.error) {
              ...
              if (handle.message.finish === "error") {
                AltruCoderSessionProcessor.providerFinishError(handle.message)
                yield* sessions.updateMessage(handle.message)
                closeReasons.set(sessionID, "error")
                return "break" as const
              }
            ...
            // altrucoder_change start — break out so a newer queued prompt can take over
            ...
            if (AltruCoderSessionPromptQueue.hasFollowup(sessionID)) {
    ...
    const loop: (input: LoopInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.loop")(function* (
      ...
      yield* AltruCoderSessionPrompt.recoverDanglingAssistant({ sessionID: input.sessionID, status, sessions })
      yield* AltruCoderSessionPrompt.recoverProviderFinishError({ sessionID: input.sessionID, status, sessions })
      yield* bus.publish(AltruCoderSession.Event.TurnOpen, { sessionID: input.sessionID })
      return yield* Effect.onExit(
        state.ensureRunning(input.sessionID, lastAssistant(input.sessionID), runLoop(input.sessionID)),
        Effect.fnUntraced(function* (exit) {
          yield* bus.publish(AltruCoderSession.Event.TurnClose, {
            sessionID: input.sessionID,
            reason: AltruCoderSessionPrompt.resolveCloseReason({
              sessionID: input.sessionID,
              closeReasons,
              exit,
            }),
    ...
    const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
      ...
      // altrucoder_change start - mark local review commands for completion telemetry
      const telemetry = AltruCoderSessionProcessor.reviewTelemetry(input.command)
      if (telemetry) {
...
export const PromptInput = Schema.Struct({
  ...
  // altrucoder_change start - reuse shared editor context schema
