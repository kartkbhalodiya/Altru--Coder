...
export namespace SessionNetwork {
  ...
  class StateService extends Context.Service<StateService, { readonly get: () => Effect.Effect<StateShape> }>()(
    "@altru-coder/SessionNetwork.State",
  ) {}
  ...
  export function disconnected(err: unknown) {
    ...
    // altrucoder_change - recognize AbortSignal.timeout() errors
    for (const item of chain(err)) {
  ...
  export function message(err: unknown) {
    // altrucoder_change - check for timeout first
    for (const item of chain(err)) {
  ...
  export const reply = fn(
    ...
    async (input) => {
      ...
      // altrucoder_change start — reconnect failed remote MCP servers after network recovery
      void MCP.status()
        .then((statuses) => {
