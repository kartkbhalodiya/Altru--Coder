...
// altrucoder_change - drop unused inArray/lt (listGlobal delegated to AltruCoderSession)
import { eq, and, gte, isNull, desc, like, or } from "drizzle-orm"
import { SyncEvent } from "../sync"
import { PartTable, SessionTable } from "./session.sql"
// altrucoder_change - ProjectTable removed (unused)
import { Storage } from "@/storage/storage"
import * as Log from "@opencode-ai/core/util/log"
import { MessageV2 } from "./message-v2"
import type { InstanceContext } from "../project/instance"
import { InstanceState } from "@/effect/instance-state"
import { Instance } from "@/project/instance" // altrucoder_change - children() uses Instance.current to scope by project_id
...
// altrucoder_change start - legacy promise helpers + altrucoder extensions
import { makeRuntime } from "@/effect/run-service"
import { AltruCoderSession, altruSessionFork } from "@/altrucoder/session"
...
const Summary = Schema.Struct({
  ...
  diffs: optionalOmitUndefined(Schema.Array(Snapshot.SummaryFileDiff)), // altrucoder_change - lightweight diff without patch
})
...
export const GlobalInfo = Schema.Struct({
  ...
  worktreeName: Schema.optional(Schema.String), // altrucoder_change - basename of the specific worktree directory
})
...
export const CreateInput = Schema.optional(
  Schema.Struct({
    ...
    platform: Schema.optional(Schema.String), // altrucoder_change - per-session platform override for telemetry attribution
...
export const Event = {
  ...
  TurnOpen: AltruCoderSession.Event.TurnOpen,
  TurnClose: AltruCoderSession.Event.TurnClose,
  // altrucoder_change end
}

export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".altru-coder", "plans") // altrucoder_change
...
export const getUsage = (input: {
  ...
  provider?: Provider.Info // altrucoder_change
}) => {
  ...
  // altrucoder_change start - Use provider-reported cost when available for OpenRouter/Altru Coder
  const reported = AltruCoderSession.providerCost({
...
export interface Interface {
  ...
  readonly create: (input?: {
    ...
    title?: string
    permission?: Permission.Ruleset
    platform?: string // altrucoder_change - per-session platform override for telemetry attribution
    workspaceID?: WorkspaceID
  }) => Effect.Effect<Info>
...
export const layer: Layer.Layer<Service, never, Bus.Service | Storage.Service | SyncEvent.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      if (!Flag.ALTRU_CODER_EXPERIMENTAL_WORKSPACES) {
    ...
    // altrucoder_change start - scope by project_id when instance context is available
    ...
    const remove: Interface["remove"] = Effect.fnUntraced(function* (sessionID: SessionID) {
      try {
        ...
        // altrucoder_change start
        yield* Effect.promise(() => AltruCoderSession.removeSession(sessionID)).pipe(Effect.ignore)
        AltruCoderSession.clearPlatformOverride(sessionID)
        if (hasInstance) {
    ...
    const updateMessage = <T extends MessageV2.Info>(msg: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        // altrucoder_change start - ignore FK errors when session was deleted while processor was still running
        yield* Effect.sync(() =>
          AltruCoderSession.runSyncSafe(
    ...
    const create = Effect.fn("Session.create")(function* (input?: {
      ...
      const session = yield* createNext({
        ...
        title: input?.title,
        permission: input?.permission,
        workspaceID: input?.workspaceID ?? workspace, // altrucoder_change - allow explicit override
      })
      // altrucoder_change start - store platform override for session ingest
      if (input?.platform) {
        AltruCoderSession.setPlatformOverride(session.id, input.platform)
      }
      // altrucoder_change end
      return session
    })
...
function* listByProject(
  ...
  // altrucoder_change start - AltruCoderSession.filters keeps sessions visible across project_id changes
  ...
  const conditions =
    input.path !== undefined
      ...
      : AltruCoderSession.filters({ projectID: input.projectID, directory: input.directory })
  // altrucoder_change end
...
// altrucoder_change start - delegate to AltruCoderSession.listGlobal (adds projectID worktree family + directories[])
export function* listGlobal(input?: {
  ...
  yield* AltruCoderSession.listGlobal<GlobalInfo>({ ...input, fromRow })
}
// altrucoder_change end

// altrucoder_change start - keep legacy promise helpers for Altru Coder callsites
...
export const fork = altruSessionFork
...
export async function updateMessage<T extends MessageV2.Info>(msg: T): Promise<T> {
  MessageV2.Info.zod.parse(msg) // altrucoder_change
...
export async function updatePart<T extends MessageV2.Part>(part: T): Promise<T> {
  MessageV2.Part.zod.parse(part) // altrucoder_change
