...
import { TsClient } from "../altrucoder/ts-client" // altrucoder_change
...
const filterExperimentalServers = (servers: Record<string, LSPServer.Info>) => {
  if (Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TY) {
    if (servers["pyright"]) {
      log.info("LSP server pyright is disabled because ALTRU_CODER_EXPERIMENTAL_LSP_TY is enabled")
    ...
    const getClients = Effect.fnUntraced(function* (file: string) {
      ...
      return yield* Effect.promise(async () => {
        ...
        for (const server of Object.values(s.servers)) {
          ...
          // altrucoder_change start - use lightweight tsgo-based client when persistent LSP is not enabled
          if (server.id === "typescript" && !Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TOOL) {
            const existing = s.clients.find((x) => x.root === root && x.serverID === server.id)
            if (existing) {
