...
import { TsCheck } from "../altrucoder/ts-check" // altrucoder_change
...
// altrucoder_change start - tsgo native LSP or lightweight diagnostic client
// When ALTRU_CODER_EXPERIMENTAL_LSP_TOOL is enabled, spawn tsgo --lsp --stdio as a
...
export const Typescript: Info = {
  ...
  async spawn(root) {
    if (!Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TOOL) return undefined
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.ALTRU_CODER_DISABLE_LSP_DOWNLOAD) return
...
export const Ty: Info = {
  ...
  async spawn(root) {
    if (!Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TY) {
      return undefined
    }
  ...
  async spawn(root) {
    ...
    return {
      process: spawn(
        java,
        [
          "-Djava.import.generatesMetadataFilesAtProjectRoot=false", // altrucoder_change
          "-jar",
          launcherJar,
          "-configuration",
          configFile,
