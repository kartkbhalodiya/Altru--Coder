...
import { DiffFull } from "../altrucoder/snapshot/diff-full" // altrucoder_change
import { AltruCoderSnapshotTrack } from "../altrucoder/snapshot/track" // altrucoder_change
import type { MessageID, SessionID } from "../session/schema" // altrucoder_change
...
// altrucoder_change start - lightweight FileDiff without `patch` for session.summary.diffs (keeps DB payload small)
...
export interface Interface {
  ...
  // altrucoder_change start - accept optional sessionID/messageID so the slow-repo prompt can target
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Snapshot.state")(function* (ctx) {
        ...
        const enabled = Effect.fnUntraced(function* () {
          ...
          // altrucoder_change start - ACP guard: disable snapshots for ACP clients
          if (Flag.ALTRU_CODER_CLIENT === "acp") return false
          // altrucoder_change end
        ...
        const diffFull = Effect.fnUntraced(function* (from: string, to: string) {
          return yield* locked(
            Effect.gen(function* () {
              ...
              // altrucoder_change start — route patches through git (DiffFull.batch) instead of the
    ...
    // altrucoder_change start - per-instance state for the slow-repo track wrapper
    const trackState = AltruCoderSnapshotTrack.makeState()
    // altrucoder_change end
    ...
    return Service.of({
      ...
      // altrucoder_change start - timeout + interactive "disable for this project" prompt
      track: Effect.fn("Snapshot.track")(function* (opts) {
        return yield* AltruCoderSnapshotTrack.wrap({
      ...
      diffFull: Effect.fn("Snapshot.diffFull")(function* (from: string, to: string) {
        // altrucoder_change start - cache full diffs at the service boundary
        if (from === to) return []
        const key = `${from}:${to}`
