...
const AgentSchema = Schema.StructWithRest(
  Schema.Struct({
    model: Schema.optional(Schema.NullOr(ConfigModelID)), // altrucoder_change - nullable for delete sentinel
    variant: Schema.optional(Schema.String).annotate({
      description: "Default model variant for this agent (applies only when using the agent's configured model).",
    }),
    temperature: Schema.optional(Schema.NullOr(Schema.Finite)), // altrucoder_change - nullable for delete sentinel
    top_p: Schema.optional(Schema.NullOr(Schema.Finite)), // altrucoder_change - nullable for delete sentinel
    prompt: Schema.optional(Schema.NullOr(Schema.String)), // altrucoder_change - nullable for delete sentinel
...
const normalize = (agent: Schema.Schema.Type<typeof AgentSchema>): Schema.Schema.Type<typeof AgentSchema> => {
  ...
  // altrucoder_change start - preserve null delete sentinel (?? would collapse null to maxSteps)
...
export async function load(dir: string, warnings?: Warning[]) {
  ...
  for (const item of await Glob.scan("{agent,agents}/**/*.md", {
    ...
    // altrucoder_change start - use Effect schema (propertyOrder: original) + non-fatal handleInvalid
    try {
      result[config.name] = ConfigParse.effectSchema(Info, config, item) as Info
    } catch (err) {
      if (ConfigError.InvalidError.isInstance(err)) {
        await AltruCoderConfig.handleInvalid("agent", item, err.data.issues ?? [], err, warnings)
        continue
      }
      throw err
    }
