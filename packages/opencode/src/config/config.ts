...
import { Env } from "../env"
import { applyEdits, findNodeAtLocation, modify, parseTree } from "jsonc-parser" // altrucoder_change - parseTree/findNodeAtLocation used in patchJsonc
...
import { AltruCoderDefaultPlugins } from "@/altrucoder/config/default-plugins"
...
const { caught: caughtWarning } = AltruCoderConfig
// altrucoder_change end
...
// altrucoder_change start - indexing configuration
export const Indexing = AltruCoderIndexingConfig
...
// altrucoder_change - AltruCoderIndexingConfig is still a Zod schema; bridge via ZodOverride
const IndexingRef = Schema.Any.annotate({ [ZodOverride]: AltruCoderIndexingConfig })
...
export const Info = Schema.Struct({
  ...
  // altrucoder_change start
  // NOTE: Any new altrucoder_change key added to Config.Info must also be mirrored in
  // apps/web/src/app/config.json/extras.ts in the cloud repo, otherwise
  // $schema: https://app.altru-coder.ai/config.json will not recognize it.
  remote_control: Schema.optional(Schema.Boolean).annotate({
    description: "Enable remote control of sessions via Altru Coder Cloud. Equivalent to running /remote on startup.",
  }),
  ...
  indexing: Schema.optional(IndexingRef).annotate({ description: "Codebase indexing configuration" }), // altrucoder_change
  ...
  // altrucoder_change end
  // altrucoder_change start - nullable for delete sentinel
  ...
  // altrucoder_change end
  // altrucoder_change start - renamed from "build" to "code" + nullable for delete sentinel
  ...
  agent: Schema.optional(
    Schema.StructWithRest(
      Schema.Struct({
        ...
        plan: Schema.optional(ConfigAgent.Info),
        build: Schema.optional(ConfigAgent.Info),
        debug: Schema.optional(ConfigAgent.Info), // altrucoder_change
        orchestrator: Schema.optional(ConfigAgent.Info), // altrucoder_change
        ask: Schema.optional(ConfigAgent.Info), // altrucoder_change
        // subagent
  ...
  provider: Schema.optional(Schema.Record(Schema.String, Schema.NullOr(ConfigProvider.Info))).annotate({
    // altrucoder_change - nullable for delete sentinel
  ...
  commit_message: AltruCoderConfig.CommitMessageSchema, // altrucoder_change
  ...
  experimental: Schema.optional(
    Schema.Struct({
      ...
      codebase_search: Schema.optional(Schema.Boolean).annotate({ description: "Enable AI-powered codebase search" }), // altrucoder_change
      // altrucoder_change start
      ...
      // altrucoder_change end
      // altrucoder_change start - enable telemetry by default
...
type State = {
  ...
  warnings: Warning[] // altrucoder_change
...
export interface Interface {
  ...
  readonly warnings: () => Effect.Effect<Warning[]> // altrucoder_change
}
...
function patchJsonc(input: string, patch: unknown, path: string[] = []): string {
  if (!isRecord(patch)) {
    // altrucoder_change - null means "delete this key" — pass undefined to jsonc-parser's modify()
  ...
  // altrucoder_change start — when the existing JSONC node at this path is a
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://app.altru-coder.ai/config.json" // altrucoder_change
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://app.altru-coder.ai/config.json",') // altrucoder_change
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      yield* Effect.promise(() => AltruCoderConfig.migrateBashPermission()) // altrucoder_change
      let result: Info = {}
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json")))
      // altrucoder_change start
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "altru-coder.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "altru-coder.jsonc")))
      // altrucoder_change end
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://app.altru-coder.ai/config.json" // altrucoder_change
    ...
    const ensureGitignore = Effect.fn("Config.ensureGitignore")(function* (dir: string) {
      ...
      if (!hasIgnore) {
        yield* fs
          .writeFileString(
            gitignore,
            // altrucoder_change start - added pnpm-lock.yaml and yarn.lock (not in upstream)
            [
              "node_modules",
              "package.json",
              "package-lock.json",
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        // altrucoder_change start - warning accumulator and legacy Altru Coder config
        const warnings: Warning[] = []
        const auth = yield* authSvc.all().pipe(Effect.orDie)

        let result: Info = {}
        const legacy = yield* Effect.promise(() =>
          AltruCoderConfig.loadLegacyConfigs({
        ...
        const orgModes = yield* Effect.promise(() => AltruCoderConfig.loadOrganizationModes(auth))
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ALTRU_CODER_CONFIG_CONTENT") return "local"
        ...
        const merge = Effect.fnUntraced(function* (source: string, next: Info, kind?: ConfigPlugin.Scope) {
          ...
          const scoped = AltruCoderConfig.scopeIndexing(next, scope)
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            // altrucoder_change start - warn instead of fail on wellknown errors
            const next = yield* Effect.tryPromise({
              try: async () => {
                ...
                if (!remoteConfig.$schema) remoteConfig.$schema = "https://app.altru-coder.ai/config.json"
                return remoteConfig
              },
              catch: (err) => err,
            }).pipe(
        ...
        // altrucoder_change start - capture global config failures as warnings
        ...
        if (Flag.ALTRU_CODER_CONFIG) {
          // altrucoder_change start - capture ALTRU_CODER_CONFIG failures as warnings
          yield* merge(
            Flag.ALTRU_CODER_CONFIG,
            yield* loadFile(Flag.ALTRU_CODER_CONFIG).pipe(
              Effect.catchDefect((err: unknown) => {
                caughtWarning(warnings, Flag.ALTRU_CODER_CONFIG!, err)
                return Effect.succeed({} as Info)
              }),
            ),
          )
          // altrucoder_change end
          log.debug("loaded custom config", { path: Flag.ALTRU_CODER_CONFIG })
        }

        if (!Flag.ALTRU_CODER_DISABLE_PROJECT_CONFIG) {
          // altrucoder_change start - also discover altru-coder.json project files
          for (const name of ["altru-coder", "opencode"] as const) {
        ...
        if (Flag.ALTRU_CODER_CONFIG_DIR) {
          log.debug("loading config from ALTRU_CODER_CONFIG_DIR", { path: Flag.ALTRU_CODER_CONFIG_DIR })
        }
        ...
        for (const dir of unique(directories)) {
          if (AltruCoderConfig.isConfigDir(dir, Flag.ALTRU_CODER_CONFIG_DIR)) {
            for (const file of AltruCoderConfig.ALL_CONFIG_FILES) {
          ...
          // altrucoder_change start - propagate parse errors to the Warning accumulator
        ...
        if (process.env.ALTRU_CODER_CONFIG_CONTENT) {
          // altrucoder_change start - capture ALTRU_CODER_CONFIG_CONTENT parse failures as warnings
          const source = "ALTRU_CODER_CONFIG_CONTENT"
          yield* merge(
            source,
            yield* loadConfig(process.env.ALTRU_CODER_CONFIG_CONTENT, {
              dir: ctx.directory,
              source,
            }).pipe(
              Effect.tap(() => Effect.sync(() => log.debug("loaded custom config from ALTRU_CODER_CONFIG_CONTENT"))),
              Effect.catchDefect((err: unknown) => {
                caughtWarning(warnings, source, err)
                return Effect.succeed({} as Info)
              }),
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ALTRU_CODER_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ALTRU_CODER_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        // altrucoder_change start - include altru-coder.json/altru-coder.jsonc in managed dir loading
        ...
        if (Flag.ALTRU_CODER_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ALTRU_CODER_PERMISSION))
        }
        ...
        if (Flag.ALTRU_CODER_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ALTRU_CODER_DISABLE_PRUNE) {
          result.compaction = { ...result.compaction, prune: false }
        }
        // altrucoder_change start — inject Altru Coder default plugins into both plugin list and origins
        AltruCoderDefaultPlugins.apply(result, { disabled: Flag.ALTRU_CODER_DISABLE_DEFAULT_PLUGINS, log })
        // altrucoder_change end
    ...
    const update = Effect.fn("Config.update")(function* (config: Info, options?: { dispose?: boolean }) {
      // altrucoder_change start - delegate Altru Coder project config update behavior.
      const ctx = yield* InstanceState.context
      yield* AltruCoderConfig.updateProjectConfig({
        fs,
    ...
    const updateGlobal = Effect.fn("Config.updateGlobal")(function* (
      ...
      if (!file.endsWith(".jsonc")) {
        ...
        // altrucoder_change - use `patch` (writableGlobal) so empty-string sentinels are stripped via undefined
        const merged = AltruCoderConfig.mergeConfig(writable(existing), patch)
...
// altrucoder_change start - keep async wrappers for Altru Coder callsites during Effect migration
