...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/altru" // altrucoder_change
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "altru-coder") // altrucoder_change
    default:
      return "/etc/altru" // altrucoder_change
  }
}

export function managedConfigDir() {
  return process.env.ALTRU_CODER_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
