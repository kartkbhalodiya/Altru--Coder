...
const MARKER_START = "# >>> altru-coder-dev launcher >>>"
const MARKER_END = "# <<< altru-coder-dev launcher <<<"

export const DevSetupCommand = cmd({
  command: "dev-setup",
  describe: "install a `altru-coder-dev` shell alias for this checkout",
  builder: (y) =>
    y
      .option("shell", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    UI.empty()
    UI.println(`  ${B}Altru Coder CLI dev launcher setup${N}`)
    UI.empty()
    ...
    UI.println(
      `  ${action === "install" ? "This will add" : "This will replace"} one line so ${H}altru-coder-dev${N} runs this checkout from any directory:`,
    )
    UI.empty()
    UI.println(`      ${snippet}`)
    UI.empty()
...
// Deprecated: kept so `altru-coder dev-alias` keeps working for scripts/docs.
...
function aliasLine(shell: Shell, repo: string): string {
  const sh = path.join(repo, "bin", "altru-coder-dev")
  const bat = path.join(repo, "bin", "altru-coder-dev.cmd")
  const q = (s: string) => `'${s.replaceAll("'", `'\\''`)}'`
  if (shell === "fish") return `alias altru-coder-dev ${q(sh)}`
  if (shell === "powershell") return `function altru-coder-dev { & '${bat.replaceAll("'", "''")}' @args }`
  return `alias altru-coder-dev=${q(sh)}`
}
...
async function applyBlock(rc: string, snippet: string): Promise<{ created: boolean; backup?: string }> {
  ...
  const backup = `${rc}.altru-coder-dev.bak.${stamp()}`
...
export async function detectRepo(): Promise<string> {
  const hint = process.env.ALTRU_CODER_DEV_REPO
  if (hint) return hint
  ...
  // For a local build it's <repo>/packages/opencode/dist/<target>/bin/altru-coder, so
  ...
  throw new Error(
    "cannot locate altrucoder source checkout; set ALTRU_CODER_DEV_REPO=/path/to/altrucoder or run ./bin/altru-coder-dev dev-setup from the repo",
  )
}
