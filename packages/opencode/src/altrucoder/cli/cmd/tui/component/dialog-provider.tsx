...
/**
 * Altru Coder-specific overrides for the provider dialog.
 *
...
import type { ProviderAuthAuthorization } from "@altru-coder/sdk/v2"
import { AltruCoderAutoMethod } from "@/altrucoder/components/dialog-altru-coder-auto-method"
...
/**
 * If the provider is Altru Coder Gateway, renders the custom `AltruCoderAutoMethod`
...
export function renderAutoMethod(opts: {
  ...
  if (opts.providerID !== "altru-coder") return undefined
  return () => (
    <AltruCoderAutoMethod
 ...
 * provider is Altru Coder Gateway. Returns `undefined` otherwise.
 */
export function renderApiDescription(
  providerID: string,
  theme: { textMuted: RGBA; text: RGBA; primary: RGBA },
): (() => JSX.Element) | undefined {
  if (providerID !== "altru-coder") return undefined
  return () => (
    <box gap={1}>
      <text fg={theme.textMuted}>
        Altru Coder Gateway gives you access to all the best coding models at the cheapest prices with a single API key.
      </text>
      <text fg={theme.text}>
        Go to <span style={{ fg: theme.primary }}>https://altru-coder.ai/gateway</span> to get a key
      </text>
    </box>
  )
}
