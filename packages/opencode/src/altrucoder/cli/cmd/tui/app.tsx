...
/**
 * Altru Coder-specific TUI app customizations.
 *
...
import { isAltruCoderError, showAltruCoderErrorToast } from "@/altrucoder/altru-coder-errors"
import { registerAltruCoderCommands } from "@/altrucoder/altru-coder-commands"
import { initializeTUIDependencies } from "@altru-coder/altru-coder-gateway/tui"

// Re-export so upstream can render the route without importing directly
export { AltruCoderClawView } from "@/altrucoder/claw/view"
...
export const APP_TITLE = "Altru Coder CLI"

/** Public docs URL shown in the command palette. */
export const DOCS_URL = "https://altru-coder.ai/docs"

/** Human-readable product name used in user-facing messages. */
export const APP_NAME = "Altru Coder"
...
/**
 * Returns the terminal title for altru-coder-claw routes.
...
export function getTerminalTitle(
  ...
  if (route.data.type === "altru-coder-claw") return `${base} | AltruCoderClaw`
  return undefined
}
...
/**
 * Intercepts Altru Coder-specific errors and shows a warning toast.
 * Returns `true` if the error was handled, `false` otherwise.
 */
export function handleSessionError(error: unknown, toast: ReturnType<typeof useToast>): boolean {
  if (error && typeof error === "object" && isAltruCoderError(error as any)) {
    showAltruCoderErrorToast(error as any, toast)
    return true
  }
  return false
}
 ...
 *
 * - Injects TUI dependencies into altru-coder-gateway
 * - Registers Altru Coder Gateway commands (profile, teams, altru-coder-claw, etc.)
...
export function init() {
  ...
  // Inject TUI dependencies for altru-coder-gateway
  ...
  // Register Altru Coder Gateway commands (profile, teams, altru-coder-claw, remote, etc.)
  registerAltruCoderCommands(useSDK)
