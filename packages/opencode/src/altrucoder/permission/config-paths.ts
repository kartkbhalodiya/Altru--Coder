...
import { AltruCoderPaths } from "@/altrucoder/paths"

export namespace ConfigProtection {
  /**
   * Config directory prefixes (relative paths, forward-slash normalized).
   * Matches .altru-coder/, .altrucoder/, .opencode/ at any depth within the project.
   */
  const CONFIG_DIRS = [".altru-coder/", ".altrucoder/", ".opencode/"]
  ...
  export function isRelative(pattern: string): boolean {
    ...
    for (const dir of CONFIG_DIRS) {
      const bare = dir.slice(0, -1) // e.g. ".altru-coder"
      // Match at root (e.g. ".altru-coder/foo") or nested (e.g. "packages/sub/.altru-coder/foo")
  ...
  function fallback(p: string): boolean {
    ...
    return keys(p).some(
      (key) =>
        key.endsWith("/config/altru") ||
        key.includes("/config/altru-coder/") ||
        key.endsWith("/.config/altru") ||
        key.includes("/.config/altru-coder/"),
    )
  }
  ...
  export function isAbsolute(filepath: string): boolean {
    ...
    // ~/.config/altru-coder/ (XDG config)
    for (const dir of configs()) {
      if (within(filepath, dir)) return true
    }

    // ~/.altru-coder/ and ~/.altrucoder/ (legacy global dirs)
    for (const dir of AltruCoderPaths.globalDirs()) {
      if (within(filepath, dir)) return true
    }
