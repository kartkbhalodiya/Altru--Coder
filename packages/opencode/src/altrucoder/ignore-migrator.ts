...
export namespace IgnoreMigrator {
  const log = Log.create({ service: "altrucoder.ignore-migrator" })

  const ALTRUCODERIGNORE_FILE = ".altrucoderignore"
  const GLOBAL_ALTRUCODERIGNORE = path.join(os.homedir(), ".altrucoder", ALTRUCODERIGNORE_FILE)
  ...
  /**
   * Parse .altrucoderignore content into patterns.
  ...
  /**
   * Load patterns from a .altrucoderignore file
   */
  ...
  /**
   * Migrate .altrucoderignore to Opencode permission config
   */
  export async function migrate(options: { projectDir: string; skipGlobalPaths?: boolean }): Promise<MigrationResult> {
    ...
    // 1. Load global .altrucoderignore (lower priority)
    if (!options.skipGlobalPaths) {
      const globalPatterns = await loadIgnoreFile(GLOBAL_ALTRUCODERIGNORE, "global")
      allPatterns.push(...globalPatterns)

      if (globalPatterns.length > 0) {
        log.debug("loaded global .altrucoderignore", { count: globalPatterns.length })
      }
    }

    // 2. Load project .altrucoderignore (higher priority - added last)
    const projectIgnorePath = path.join(options.projectDir, ALTRUCODERIGNORE_FILE)
    const projectPatterns = await loadIgnoreFile(projectIgnorePath, "project")
    allPatterns.push(...projectPatterns)

    if (projectPatterns.length > 0) {
      log.debug("loaded project .altrucoderignore", { count: projectPatterns.length })
    }
  ...
  /**
   * Load .altrucoderignore and return permission config.
  ...
  export async function loadIgnoreConfig(
    ...
    try {
      ...
      if (result.patternCount > 0) {
        log.info("loaded .altrucoderignore patterns", {
      ...
      log.warn("failed to load .altrucoderignore", { error: err })
      return {}
    }
  }
}
