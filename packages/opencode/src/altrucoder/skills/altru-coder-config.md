# Altru Coder CLI Configuration Reference

All config lives in `altru-coder.json` (or `altru-coder.jsonc`). Precedence low-to-high: remote well-known, global (`~/.config/altru-coder/altru-coder.json`), env `ALTRU_CODER_CONFIG`, project `./altru-coder.json`, `.altru-coder/altru-coder.json`, `ALTRU_CODER_CONFIG_CONTENT`, managed (see Config File Locations). Deep-merged; later wins.

This also covers where Altru Coder looks for config files, commands, agents, and skills across project, global, and legacy paths such as `.altru-coder/`, `.altrucoder/`, `.opencode/`, and `~/.config/altru-coder/`, plus Agent Manager setup/run scripts in the VS Code extension.

## Commands (`.altru-coder/command/*.md`)

Markdown files with YAML frontmatter. The filename (minus `.md`) becomes the command name invoked via `/name`. Commands can live in `.altru-coder/`, `.altrucoder/`, `.opencode/`, and global config roots, with both `command/` and `commands/` directory names supported. See Config File Locations for the full search order.
...
1. `~/.config/altru-coder/`
2. `~/.altru-coder/`
3. `~/.altrucoder/`
4. `~/.opencode/`
5. The `ALTRU_CODER_CONFIG_DIR` directory (if the env var is set)
6. project `.altru-coder/`, `.altrucoder/`, and `.opencode/` directories from the current working directory up to the worktree root
...
## Agents (`.altru-coder/agent/*.md`)

Also loaded from `.altrucoder/` and `.opencode/` directories (legacy), and plural `agents/` variants.
...
Markdown files in `.altru-coder/workflows/` or `.altrucoder/workflows/` (project-level) and `~/.altru-coder/workflows/` or `~/.altrucoder/workflows/` (global). These are automatically converted to commands at startup. The filename (minus `.md`) becomes the command name. Project workflows override global ones with the same name.

## Agent Manager Setup And Run Scripts

Agent Manager setup/run scripts are project files in the main repository's `.altru-coder/` directory. They are not `altru-coder.json` settings and should not be configured inside generated `.altru-coder/worktrees/<name>/` checkouts.

Agent Manager worktrees usually live under `.altru-coder/worktrees/`. Think of each worktree as a separate checkout on its own branch: it enables parallel edits, but dependencies, build output, caches, databases, and generated files can consume significant disk space across many worktrees.
...
|---|---|
| macOS / Linux | `.altru-coder/setup-script`, `.altru-coder/setup-script.sh` |
| Windows | `.altru-coder/setup-script.ps1`, `.altru-coder/setup-script.cmd`, `.altru-coder/setup-script.bat` |
...
- For port conflicts (`EADDRINUSE`, browser/tests hitting the wrong worktree), inspect the app's dev-server config as well as `.altru-coder/run-script`. If fixing it requires application changes, ask whether the user wants the app made configurable or only wants a run-script workaround.
...
For example, this configuration will hide all models from the built-in Altru Coder Gateway as well as any from the OpenAI provider which may be enabled automatically through environment variables.

```jsonc
{
  "$schema": "https://app.altru-coder.ai/config.json",
  "disabled_providers": ["altru-coder", "openai"],
}
```

The provider ID is the lowercase name used in the `provider/model` format (e.g., `altru-coder`, `openai`, `anthropic`, `google`, `groq`).
...
Skills are markdown files at `skills/<name>/SKILL.md` (or `skill/<name>/SKILL.md`) with `name` and `description` in frontmatter. Discovered inside `.altru-coder/`, `.altrucoder/`, and `.opencode/` directories.
...
|---|---|---|---|
| Switch theme | `<leader>t` | `/themes` | Pick from 35+ built-in themes (altru-coder, catppuccin, dracula, github, gruvbox, nord, tokyonight, etc.) |
| Toggle appearance (dark/light) | — | — | Ctrl+P → "Toggle appearance" |

Custom themes: place JSON files in `~/.config/altru-coder/themes/` or `.altru-coder/themes/`.
...
### Config files (altru-coder.json)

| Scope | Path |
|---|---|
| Project | `./altru-coder.json`, `./altru-coder.jsonc`, `./opencode.json` (legacy), `./opencode.jsonc` (legacy) |
| Global | `~/.config/altru-coder/altru-coder.json`, `~/.config/altru-coder/altru-coder.jsonc`, `~/.config/altru-coder/opencode.json` (legacy), `~/.config/altru-coder/opencode.jsonc` (legacy), `~/.config/altru-coder/config.json` (legacy) |
| Managed | Linux: `/etc/altru-coder/`, macOS: `/Library/Application Support/altru-coder/`, Windows: `%ProgramData%\altru-coder\` — loads `altru-coder.json`, `altru-coder.jsonc`, `opencode.json`, `opencode.jsonc` (enterprise, highest priority) |

Each config directory (`.altru-coder/`, `.altrucoder/`, `.opencode/`) can also contain `altru-coder.json`, `altru-coder.jsonc`, `opencode.json`, or `opencode.jsonc`.

### Config directories

Three directory names are scanned: `.altru-coder` (modern), `.altrucoder` (legacy), `.opencode` (legacy). All three are checked at each level:

- **Project**: walks up from CWD to the git worktree root, checking for all three at each directory level
- **Home**: `~/.altru-coder/`, `~/.altrucoder/`, `~/.opencode/`
- **XDG global**: `~/.config/altru-coder/` (always loaded, lowest file-based precedence)
...
Example: `~/.config/altru-coder/command/*.md` (modern global), `~/.altrucoder/command/*.md` (legacy global), `.opencode/commands/*.md` (legacy project) all load commands.
...
|---|---|
| `ALTRU_CODER_CONFIG` | Path to an additional config file (loaded after global) |
| `ALTRU_CODER_CONFIG_DIR` | Path to an additional config directory (appended to search list) |
| `ALTRU_CODER_CONFIG_CONTENT` | Inline JSON config string (high precedence, after project dirs) |
| `ALTRU_CODER_DISABLE_PROJECT_CONFIG` | Skip all project-level config (files and directories) |
