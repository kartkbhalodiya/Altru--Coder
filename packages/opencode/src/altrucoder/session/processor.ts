...
export namespace AltruCoderSessionProcessor {
  const log = Log.create({ service: "session.processor.altru-coder" })
  ...
  export const PROVIDER_FINISH_ERROR_MESSAGE =
    "The provider ended the response with an error before returning details. Start a new message to retry; Altru Coder will compact the oversized conversation first if needed."
  ...
  /**
   * Returns the Altru Coder-specific retry policy options (limit + offline handler).
  ...
  export function retryOpts(input: {
    ...
    return {
      limit: Flag.ALTRU_CODER_SESSION_RETRY_LIMIT,
