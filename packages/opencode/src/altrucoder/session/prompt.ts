...
import { PlanFollowup } from "@/altrucoder/plan-followup"
import { AltruCoderSession } from "@/altrucoder/session"
import { Permission } from "@/permission"
import { environmentDetails, type EditorContext } from "@/altrucoder/editor-context"
...
export namespace AltruCoderSessionPrompt {
  ...
  export function shouldAskPlanFollowup(input: { messages: MessageV2.WithParts[]; abort: AbortSignal }) {
    ...
    if (!["cli", "vscode"].includes(Flag.ALTRU_CODER_CLIENT)) return false
   ...
   * OneDrive ReparsePoint directories in some Node versions (altrucoder#9755).
   */
  ...
  export function resolveCloseReason(input: {
    sessionID: string
    closeReasons: Map<string, AltruCoderSession.CloseReason>
    exit: Exit.Exit<any, any>
  }): AltruCoderSession.CloseReason {
