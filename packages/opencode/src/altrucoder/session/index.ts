...
export namespace AltruCoderSession {
  const log = Log.create({ service: "session.altru-coder" })
  ...
  // ---------------------------------------------------------------------------
  // Provider-reported cost (OpenRouter / Altru Coder)
  // ---------------------------------------------------------------------------

  /**
   * Extract provider-reported cost from OpenRouter metadata when available.
   * For the Altru Coder provider (BYOK), prefers `upstreamInferenceCost` over the
  ...
  export function providerCost(input: {
    ...
    const isAltruCoder = (input.provider?.id ?? input.providerID) === "altru-coder"
    const upstream = openrouterUsage.costDetails?.upstreamInferenceCost
    const regular = openrouterUsage.cost

    // Altru Coder is always BYOK, so prefer upstream cost. For OpenRouter, use regular cost.
    const cost = isAltruCoder && upstream !== undefined ? upstream : regular
  ...
  export async function shareSession(id: string): Promise<{ url: string }> {
    const { AltruCoderSessions } = await import("@/altru-coder-sessions/altru-coder-sessions")
    return AltruCoderSessions.share(id)
  }

  export async function unshareSession(id: string): Promise<void> {
    const { AltruCoderSessions } = await import("@/altru-coder-sessions/altru-coder-sessions")
    await AltruCoderSessions.unshare(id)
  }

  export async function removeSession(id: string): Promise<void> {
    const { AltruCoderSessions } = await import("@/altru-coder-sessions/altru-coder-sessions")
    await AltruCoderSessions.remove(id).catch(() => {})
  }
...
export const altruSessionFork = fn(
  ...
  async (input) => {
    ...
    await AltruCoderSession.remapChildren(session.id)
    return session
  },
)
