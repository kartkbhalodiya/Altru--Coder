...
async function getHelpText(name: string, cmd: Cmd): Promise<string> {
  const inst = yargs([])
    .scriptName(name ? `altru-coder ${name}` : "altru-coder")
    .wrap(null)
  if (cmd.builder) {
...
async function getSubcommands(
  ...
  const inst = yargs([]).scriptName(`altru-coder ${name}`).wrap(null)
  builder(inst)
  ...
  try {
    ...
    for (const [sub, handler] of Object.entries(handlers as Record<string, any>)) {
      ...
      const full = `${name} ${sub}`
      const subInst = yargs([]).scriptName(`altru-coder ${full}`).wrap(null)
...
function formatMarkdown(
  ...
  for (const section of sections) {
    parts.push(`## ${section.name ? `altru-coder ${section.name}` : "altru-coder"}`)
    parts.push("")
    if (section.hidden) {
    ...
    for (const sub of section.subs) {
      parts.push(`### altru-coder ${sub.name}`)
      parts.push("")
      if (sub.hidden) {
        parts.push("> **Internal command** — not intended for direct use.")
        parts.push("")
...
function formatText(
  ...
  for (const section of sections) {
    parts.push(rule)
    const display = section.name ? `altru-coder ${section.name}` : "altru-coder"
    ...
    for (const sub of section.subs) {
      const sublabel = sub.hidden ? `--- altru-coder ${sub.name} [internal] ---` : `--- altru-coder ${sub.name} ---`
      parts.push(sublabel)
      parts.push("")
      parts.push(sub.help)
      parts.push("")
...
export async function generateCommandTable(options?: { commands?: Cmd[] }) {
  ...
  for (const cmd of cmds) {
    ...
    const display = raw.startsWith("$0") ? "altru-coder" + raw.slice(2) : "altru-coder " + raw
