...
/**
 * AltruCoderClaw SolidJS reactive helpers — Altru Coder Chat protocol.
 *
 * Exposes:
 *   - `createClawStatus`: polled instance status
 *   - `createClawChat`: Altru Coder Chat connection + active-conversation state
 *
...
/**
 * Poll the AltruCoderClaw instance status every `interval` ms.
 */
export function createClawStatus(sdk: SDK, interval = 10_000) {
  ...
  onMount(() => {
    const poll = async () => {
      const res = await sdk.client.altruCoder.claw.status().catch(() => null)
...
export function createClawChat(sdk: SDK) {
  ...
  onMount(async () => {
    ...
    const statusRes = await sdk.client.altruCoder.claw.status().catch(() => null)
    ...
    const credsRes = await sdk.client.altruCoder.claw.chatCredentials().catch((e: unknown) => {
    ...
    if (!envelope.altruChatUrl) missing.push("altruChatUrl")
    ...
    try {
      log.info("connecting altru-coder-chat")
      chat = await connect({
        envelope,
