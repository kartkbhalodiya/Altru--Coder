...
/**
 * AltruCoderClaw chat panel
 *
 * Renders a scrollable message list, a slash-command autocomplete popup,
 * and a textarea for chatting with the AltruCoderClaw bot via Altru Coder Chat.
...
export function ClawChat(props: {
  ...
  // `bot:altru-coder-claw:{sandboxId}` — the bot is the only non-self member in 1:1
  // chats, so we resolve bot ids to the user-configured `botName` (which
  // falls back to "AltruCoderClaw" upstream) and render any human collaborators
