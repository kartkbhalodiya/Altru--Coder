...
/**
 * AltruCoderClaw full-screen view
 *
 * Main layout component for the /altru-coder-claw route.
...
export function AltruCoderClawView() {
  ...
  // Bot display name — sourced from the AltruCoderClaw platform status (set by the
  // user during onboarding via patchBotIdentity). Falls back to the literal
  // "AltruCoderClaw" while loading or for instances that skipped onboarding,
  // matching the web UI's fallback chain.
  const botName = createMemo(() => status()?.botName ?? "AltruCoderClaw")
  ...
  // AltruCoderClaw view commands — single source of truth for both the global
  // command palette / keybinds and the in-chat slash autocomplete.
  // The list is reactive on `chat.connected` so the slash menu only
  // exposes `/new` and `/conversations` once we're connected to altru-coder-chat.
  const altruCommands = createMemo<CommandOption[]>(() => {
    const ready = chat.connected()
    return [
      {
        value: "altru-coder-claw.back",
        title: "Back",
        description: "Return to the previous view",
        category: "AltruCoderClaw",
        slash: { name: "back" },
      ...
      {
        value: "altru-coder-claw.new",
        title: "New conversation",
        description: "Start a new AltruCoderClaw conversation",
        category: "AltruCoderClaw",
        slash: { name: "new" },
        enabled: ready,
        hidden: !ready,
      ...
      {
        value: "altru-coder-claw.conversations",
        title: "Conversations",
        description: "Browse, rename, and delete AltruCoderClaw conversations",
        category: "AltruCoderClaw",
  ...
  command.register(() => altruCommands())
  ...
  const clawSlashes = createMemo<ClawSlashOption[]>(() => {
    const visible = altruCommands().filter((c) => c.enabled !== false && !c.hidden && c.slash)
