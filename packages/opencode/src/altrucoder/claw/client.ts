...
/**
 * AltruCoderClaw Altru Coder Chat client wrapper for the TUI.
 *
 * Hosts the HTTP and WebSocket clients, exposes a multi-conversation API
 * (list/create/select/rename/delete), and translates Altru Coder Chat events for
...
import { AltruCoderChatClient } from "./altru-coder-chat-client"
...
export async function connect(input: ConnectInput): Promise<ClawChatClient> {
  ...
  const chat = new AltruCoderChatClient({
    baseUrl: input.envelope.altruChatUrl,
    getToken: async () => input.envelope.token,
  })
  ...
  const sandboxCtx = `/altru-coder-claw/${input.sandboxId}`
  ...
  function setActive(id: string | null): void {
    ...
    clearTyping()
    if (id) {
      activeCtx = `/altru-coder-claw/${input.sandboxId}/${id}`
