...
import { ProvidersCommand } from "../cli/cmd/providers" // altrucoder_change — upstream renamed auth → providers
...
// from `altru-coder help --all` and the docs table. They're also guarded the same way
// at the yargs registration site in src/index.ts, so the commands-in-sync
// regex in test/altrucoder/help.test.ts sees DevSetup/DevAlias on neither side.
...
export const commands = [
  ...
  GenerateCommand,
  DebugCommand,
  ProvidersCommand, // altrucoder_change — upstream renamed AuthCommand → ProvidersCommand
  AgentCommand,
  UpgradeCommand,
  UninstallCommand,
  ServeCommand,
