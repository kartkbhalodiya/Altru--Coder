...
} from "@altru-coder/altru-coder-indexing/engine"
import { toIndexingConfigInput, type IndexingConfig } from "@altru-coder/altru-coder-indexing/config"
import { hasIndexingPlugin } from "@altru-coder/altru-coder-indexing/detect"
import {
  IndexingStatus,
  INDEXING_STATUS_STATES,
  disabledIndexingStatus,
  normalizeIndexingStatus,
} from "@altru-coder/altru-coder-indexing/status"
...
function isWorktreePath(dir: string): boolean {
  return /(?:\/|\\)\.altru-coder(?:code)?(?:\/|\\)worktrees(?:\/|\\)/.test(dir)
}
...
async function altruAuth(cfg: Awaited<ReturnType<typeof Config.get>>): Promise<AltruCoderIndexingAuth> {
  const auth = await Auth.get("altru-coder")
  return resolveAltruCoderIndexingAuth({ config: cfg, auth })
}

function enrichAltruCoder(input: ReturnType<typeof toIndexingConfigInput>, auth: AltruCoderIndexingAuth) {
  if (input.embedderProvider !== "altru-coder") return input

  return {
    ...input,
    altruApiKey: input.altruApiKey ?? auth.apiKey,
    altruBaseUrl: input.altruBaseUrl ?? auth.baseUrl,
    altruOrganizationId: input.altruOrganizationId ?? auth.organizationId,
  }
}

async function model(input: ReturnType<typeof toIndexingConfigInput>, auth: AltruCoderIndexingAuth) {
  if (input.embedderProvider !== "altru-coder") return input
  if (input.modelId && input.modelDimension) return input

  const catalog = await fetchAltruCoderEmbeddingModelCatalog({ baseURL: auth.baseUrl, token: auth.apiKey })
...
export namespace AltruCoderIndexing {
  ...
  const boot = async (hit: Cache): Promise<Entry> => {
    ...
    if (process.env["ALTRU_CODER_DISABLE_CODEBASE_INDEXING"] === "vscode-no-workspace") {
    ...
    const auth = await altruAuth(cfg)
    const globalConfig = await Config.getGlobal()
    const merged = indexingWithAltruCoderDefault(
      { ...cfg, indexing: { ...globalConfig.indexing, ...cfg.indexing } },
      auth,
    ) as Config.Indexing | undefined
    const cfgInput = await model(enrichAltruCoder(input(merged, globalConfig.indexing), auth), auth)
