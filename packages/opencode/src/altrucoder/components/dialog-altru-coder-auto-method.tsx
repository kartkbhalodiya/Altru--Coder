/**
 * Custom OAuth handler for Altru Coder Gateway
 *
...
import { DialogAltruCoderOrganization } from "./dialog-altru-coder-organization.js"
...
interface AltruCoderAutoMethodProps {
  index: number
  providerID: string
  title: string
...
export function AltruCoderAutoMethod(props: AltruCoderAutoMethodProps) {
  ...
  onMount(async () => {
    try {
      ...
      const profileResponse = await sdk.client.altruCoder.profile()
      ...
      console.warn("Failed to fetch Altru Coder profile, using personal account:", error)
      setStatus("error")
