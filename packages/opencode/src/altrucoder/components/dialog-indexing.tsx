...
function altruModel(model: string | undefined): string | undefined {
  return getAltruCoderEmbeddingModel(model)?.id
}

const PROVIDER_LABELS: Record<EmbeddingProvider, string> = {
  "altru-coder": "Altru Coder",
  openai: "OpenAI",
  ollama: "Ollama (local)",
...
function hasAltruCoderAuth(sync: ReturnType<typeof useSync>): boolean {
  const provider = sync.data.provider_next.all.find((item) => item.id === "altru-coder")
  return hasAltruCoderIndexingAuth({ config: sync.data.config, provider })
}
...
function defaultIndexing(sync: ReturnType<typeof useSync>, global?: IndexingConfig): IndexingConfig {
  ...
  const provider = sync.data.provider_next.all.find((item) => item.id === "altru-coder")
  const auth = resolveAltruCoderIndexingAuth({ config: sync.data.config, provider })
  if (!shouldDefaultIndexingToAltruCoder({ ...global, ...indexing }, auth)) return indexing
  return { ...indexing, provider: "altru-coder", model: altruModel(indexing.model) }
}
...
function providerSettingsDescription(
  ...
  if (provider === "altru-coder") return hasAltruCoderAuth(sync) ? "uses Altru Coder account" : "sign in to Altru Coder"
...
function ProviderSelect(props: SubDialogProps) {
  ...
  const options: DialogSelectOption<EmbeddingProvider>[] = (
    ...
    .filter(([value]) => value !== "altru-coder" || hasAltruCoderAuth(sync) || indexing.provider === "altru-coder")
    .map(([value, title]) => ({
      value,
      title,
        ...
        const updated: IndexingConfig =
          provider === "altru-coder"
            ? {
                ...current,
                provider,
                model: altruModel(current.model),
                dimension: undefined,
              }
            : {
                ...current,
      ...
      onSelect={async (option) => {
        switch (option.value) {
          case "globalToggle": {
            ...
            const updated =
              enabled && !globalCfg().provider && !getIndexing(sync).provider && hasAltruCoderAuth(sync)
          ...
          case "model": {
            const result = await DialogPrompt.show(dialog, "Embedding Model", {
              ...
              placeholder: indexing.provider === "altru-coder" ? "provider/model" : "e.g. text-embedding-3-small",
            })
            if (result !== null) {
              const trimmed = result.trim()
              const updated = {
                ...getIndexing(sync),
                model: trimmed
                  ? indexing.provider === "altru-coder"
                    ? (normalizeAltruCoderEmbeddingModelId(trimmed) ?? trimmed)
                    : trimmed
                  : undefined,
              }
              await saveIndexing(sdk, sync, updated, toast)
