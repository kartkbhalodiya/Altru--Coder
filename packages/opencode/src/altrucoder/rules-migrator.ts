...
export namespace RulesMigrator {
  // Only support .altrucoderrules (no migration for .roorules or .clinerules)
  const LEGACY_RULE_FILE = ".altrucoderrules"
  const home = () => process.env.ALTRU_CODER_TEST_HOME || process.env.HOME || process.env.USERPROFILE || os.homedir()

  // Directory-based rules (read from both .altru-coder and .altrucoder)
  const ALTRU_CODER_RULES_DIRS = [".altru-coder/rules", ".altrucoder/rules"]
  const globalRulesDirs = () => [path.join(home(), ".altru-coder", "rules"), path.join(home(), ".altrucoder", "rules")]
  ...
  export async function discoverRules(projectDir: string): Promise<RuleFile[]> {
    ...
    // 1. Global rules directories (~/.altru-coder/rules/*.md and ~/.altrucoder/rules/*.md)
    ...
    // 2. Project .altru-coder/rules/ and .altrucoder/rules/ directories
    const seen = new Set<string>()
    for (const rulesRel of ALTRU_CODER_RULES_DIRS) {
    ...
    // 3. Legacy .altrucoderrules file (only altrucoder, not roo/cline)
    ...
    for (const mode of KNOWN_MODES) {
      // Mode-specific directories (.altru-coder/rules-{mode}/*.md and .altrucoder/rules-{mode}/*.md)
      const modeSeen = new Set<string>()
      for (const prefix of [".altru-coder", ".altrucoder"]) {
      ...
      // Legacy mode-specific file (.altrucoderrules-{mode})
      const legacyModeFile = path.join(projectDir, `.altrucoderrules-${mode}`)
  ...
  export async function migrate(options: {
    ...
    for (const rule of rules) {
      ...
      if (rule.source === "legacy") {
        warnings.push(
          `Legacy rule file '${path.basename(rule.path)}' found. Consider migrating to .altru-coder/rules/ directory.`,
        )
      }
    }
