...
export namespace WorkflowsMigrator {
  ...
  // .altrucoder first (lower precedence), .altru-coder second (higher precedence / wins)
  const ALTRU_CODER_WORKFLOWS_DIRS = [".altrucoder/workflows", ".altru-coder/workflows"]
  const globalWorkflowsDirs = () => [
    path.join(home(), ".altrucoder", "workflows"),
    path.join(home(), ".altru-coder", "workflows"),
  ]

  export interface AltruCoderWorkflow {
    name: string
    path: string
    content: string
  ...
  async function loadWorkflowsFromDir(dir: string, source: "global" | "project"): Promise<AltruCoderWorkflow[]> {
  ...
  export async function discoverWorkflows(projectDir: string, skipGlobalPaths?: boolean): Promise<AltruCoderWorkflow[]> {
    ...
    if (!skipGlobalPaths) {
      ...
      const vscodeWorkflowsDir = path.join(AltruCoderPaths.vscodeGlobalStorage(), "workflows")
      workflows.push(...(await loadWorkflowsFromDir(vscodeWorkflowsDir, "global")))

      // 2. Home directories ~/.altrucoder/workflows and ~/.altru-coder/workflows
      for (const dir of globalWorkflowsDirs()) {
        workflows.push(...(await loadWorkflowsFromDir(dir, "global")))
      }
    }

    // 3. Project workflows (.altru-coder/workflows/ and .altrucoder/workflows/)
    for (const dir of ALTRU_CODER_WORKFLOWS_DIRS) {
  ...
  export function convertToCommand(workflow: AltruCoderWorkflow): ConfigCommand.Info {
    return {
  ...
  export async function migrate(options: {
    ...
    const workflowsByName = new Map<string, AltruCoderWorkflow>()
