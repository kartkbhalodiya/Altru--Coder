...
import { fetchOrganizationModes } from "@altru-coder/altru-coder-gateway"
...
export namespace AltruCoderConfig {
  const log = Log.create({ service: "altrucoder.config" })
  ...
  /** Altru Coder-specific config file names (highest-to-lowest precedence within altru-coder). */
  export const ALTRU_CODER_CONFIG_FILES = ["altru-coder.jsonc", "altru-coder.json"] as const

  /** All config file names in precedence order (altru-coder + opencode). */
  export const ALL_CONFIG_FILES = ["altru-coder.jsonc", "altru-coder.json", "opencode.jsonc", "opencode.json"] as const

  /** Directory suffixes that Altru Coder recognizes in addition to .opencode. */
  export const ALTRU_CODER_DIR_SUFFIXES = [".altru-coder", ".altrucoder"] as const

  /** All config directory suffixes Altru Coder can update, including upstream .opencode. */
   ...
   *
   * This mirrors the Altru Coder project-config load chain: prefer existing config files
   * in ancestor config directories, then existing root config files, and create
   * `.altru-coder/altru-coder.json` when no project config exists yet.
   */
  export const projectConfigUpdateTarget = Effect.fn("AltruCoderConfig.projectConfigUpdateTarget")(function* (input: {
    ...
    return files.find((file) => existsSync(file)) ?? path.join(input.directory, ".altru-coder", "altru-coder.json")
  })

  export const updateProjectConfig = Effect.fn("AltruCoderConfig.updateProjectConfig")(function* (input: {
    fs: AppFileSystem.Interface
    directory: string
    worktree?: string
    config: Config.Info
  ...
  /**
   * Load all AltruCoder legacy configs (modes, workflows, rules, MCP, ignore).
  ...
  export async function loadLegacyConfigs(input: {
    ...
    try {
      ...
      if (Object.keys(migration.agents).length > 0) {
        ...
        log.debug("loaded altrucoder custom modes", {
          count: Object.keys(migration.agents).length,
          modes: Object.keys(migration.agents),
        })
      }
      for (const skipped of migration.skipped) {
        log.debug("skipped altrucoder mode", { slug: skipped.slug, reason: skipped.reason })
      }
    } catch (err) {
      log.warn("failed to load altrucoder modes", { error: err })
    }

    // Load AltruCoder workflows as commands
    try {
      ...
      if (Object.keys(migration.commands).length > 0) {
        ...
        log.debug("loaded altrucoder workflows as commands", {
          count: Object.keys(migration.commands).length,
          commands: Object.keys(migration.commands),
        })
      }
    } catch (err) {
      log.warn("failed to load altrucoder workflows", { error: err })
    }

    // Load AltruCoder rules
    try {
      ...
      if (migration.instructions.length > 0) {
        ...
        log.debug("loaded altrucoder rules", {
          count: migration.instructions.length,
          files: migration.instructions,
        })
      }
      for (const warning of migration.warnings) {
        log.debug("altrucoder rules warning", { warning })
      }
    } catch (err) {
      log.warn("failed to load altrucoder rules", { error: err })
    }

    // Load AltruCoder MCP servers (skip global VSCode extension paths unless running in the extension)
    const skipGlobal = process.env["ALTRU_CODER_PLATFORM"] !== "vscode"
    const mcp = await McpMigrator.loadMcpConfig(input.projectDir, skipGlobal)
    if (Object.keys(mcp).length > 0) {
      result = input.merge(result, { mcp })
    }

    // Load .altrucoderignore patterns
    try {
      const permission = await IgnoreMigrator.loadIgnoreConfig(input.projectDir)
      if (Object.keys(permission).length > 0) {
        result = input.merge(result, { permission })
        log.debug("loaded altrucoder ignore patterns", {
          hasRead: !!(permission as Record<string, unknown>).read,
          hasEdit: !!(permission as Record<string, unknown>).edit,
        })
      }
    } catch (err) {
      log.warn("failed to load altrucoder ignore patterns", { error: err })
    }
  ...
  /**
   * Load organization custom modes from the Altru Coder Cloud API.
  ...
  export async function loadOrganizationModes(
    ...
    const warnings: Config.Warning[] = []
    try {
      const altru = auth["altru-coder"]
      if (altru?.type === "oauth" && altru.access && altru.accountId) {
        const modes = await fetchOrganizationModes(altru.access, altru.accountId)
        if (modes.length > 0) {
  ...
  export function isConfigDir(dir: string, flagDir?: string): boolean {
    return dir.endsWith(".altru-coder") || dir.endsWith(".altrucoder") || dir.endsWith(".opencode") || dir === flagDir
  }
}
