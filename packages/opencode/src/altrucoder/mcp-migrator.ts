...
import { AltruCoderPaths } from "./paths"

export namespace McpMigrator {
  const log = Log.create({ service: "altrucoder.mcp-migrator" })

  // Remote transport types used by the AltruCoder extension
  const REMOTE_TYPES = new Set(["streamable-http", "sse"])

  function isRemote(server: AltruCoderMcpServer): boolean {
    return !!server.type && REMOTE_TYPES.has(server.type)
  }

  // AltruCoder MCP server structure
  export interface AltruCoderMcpServer {
    command?: string
    args?: string[]
  ...
  export interface AltruCoderMcpSettings {
    mcpServers: Record<string, AltruCoderMcpServer>
  }
  ...
  export async function readMcpSettings(filepath: string): Promise<AltruCoderMcpSettings | null> {
    if (!(await Filesystem.exists(filepath))) return null

    try {
      const content = await fs.readFile(filepath, "utf-8")
      return JSON.parse(content) as AltruCoderMcpSettings
    } catch (err) {
  ...
  export function convertServer(name: string, server: AltruCoderMcpServer): ConfigMCP.Info | null {
    if (isRemote(server)) {
      if (!server.url) {
  ...
  export async function migrate(options?: {
    ...
    const allServers: Array<{ name: string; server: AltruCoderMcpServer }> = []

    if (!options?.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global MCP settings)
      const vscodeSettingsPath = path.join(AltruCoderPaths.vscodeGlobalStorage(), "settings", "mcp_settings.json")
    ...
    // Check .altru-coder/mcp.json and .altrucoder/mcp.json for project-level settings
    // (not "mcp_settings.json" which is only used for global settings)
    // .altrucoder is loaded first (lower precedence), .altru-coder second (higher precedence)
    if (options?.projectDir) {
      for (const dir of [".altrucoder", ".altru-coder"]) {
    ...
    const serversByName = new Map<string, AltruCoderMcpServer>()
  ...
  /**
   * Load AltruCoder MCP servers and return them as an opencode config partial.
  ...
  export async function loadMcpConfig(
    ...
    try {
      ...
      if (Object.keys(result.mcp).length > 0) {
        log.debug("loaded altrucoder MCP servers", {
      ...
      for (const skipped of result.skipped) {
        log.debug("skipped altrucoder MCP server", { name: skipped.name, reason: skipped.reason })
      }

      for (const warning of result.warnings) {
        log.warn("altrucoder MCP migration warning", { warning })
      }

      return result.mcp
    } catch (err) {
      log.warn("failed to load altrucoder MCP servers", { error: err })
      return {}
    }
  }
}
