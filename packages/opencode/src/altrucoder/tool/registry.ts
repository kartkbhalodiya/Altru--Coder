// altrucoder_change - new file
import { CodebaseSearchTool } from "../../tool/warpgrep"
import { RecallTool } from "../../tool/recall"
import { AgentManagerTool } from "./agent-manager"
import { MemoryTool } from "./memory"
import { TerminalTool } from "./terminal"
import * as Tool from "../../tool/tool"
import { Flag } from "@opencode-ai/core/flag/flag"
import { Effect } from "effect"
import * as Log from "@opencode-ai/core/util/log"
import { Agent } from "@/agent/agent"
import * as Truncate from "@/tool/truncate"

const log = Log.create({ service: "altrucoder-tool-registry" })
type Deps = { agent: Agent.Interface; truncate: Truncate.Interface }

export namespace AltruCoderToolRegistry {
  /** Resolve Altru Coder-specific tool Infos outside any InstanceState, so their Truncate/Agent deps are
   * satisfied at the outer registry scope instead of leaking into InstanceState's Effect. */
  export function infos() {
    return Effect.gen(function* () {
      const codebase = yield* CodebaseSearchTool
      const recall = yield* RecallTool
      const manager = yield* AgentManagerTool
      const memory = yield* MemoryTool
      const terminal = yield* TerminalTool
      return { codebase, recall, manager, memory, terminal }
    })
  }

  /** Finalize Altru Coder-specific tools into Tool.Defs. Call this inside the InstanceState state Effect —
   * it has no Service deps beyond what Tool.init itself needs. */
  export function build(
    tools: { codebase: Tool.Info; recall: Tool.Info; manager: Tool.Info; memory: Tool.Info; terminal: Tool.Info },
    deps: Deps,
  ) {
    return Effect.gen(function* () {
      const base = yield* Effect.all({
        codebase: Tool.init(tools.codebase),
        recall: Tool.init(tools.recall),
        manager: Tool.init(tools.manager),
        memory: Tool.init(tools.memory),
        terminal: Tool.init(tools.terminal),
      })
      const semantic = yield* semanticTool(deps)
      return { ...base, semantic }
    })
  }

  function semanticTool(deps: Deps) {
    return Effect.gen(function* () {
      const ready = yield* Effect.tryPromise(() =>
        import("@/altrucoder/indexing").then((mod) => mod.AltruCoderIndexing.ready()),
      ).pipe(
        Effect.catch((err) =>
          Effect.sync(() => {
            log.warn("semantic search unavailable", { err })
            return false
          }),
        ),
      )
      if (!ready) return undefined

      const mod = yield* Effect.tryPromise(() => import("@/altrucoder/tool/semantic-search")).pipe(
        Effect.catch((err) =>
          Effect.sync(() => {
            log.warn("semantic search tool unavailable", { err })
            return undefined
          }),
        ),
      )
      if (!mod) return undefined

      const info = yield* mod.SemanticSearchTool.pipe(
        Effect.provideService(Agent.Service, deps.agent),
        Effect.provideService(Truncate.Service, deps.truncate),
      )
      if (!info) return undefined
      return yield* Tool.init(info)
    })
  }

  /** Altru Coder-specific tools to append to the builtin list */
  export function extra(
    tools: {
      codebase: Tool.Def
      semantic?: Tool.Def
      recall: Tool.Def
      manager: Tool.Def
      memory?: Tool.Def
      terminal?: Tool.Def
    },
    cfg: { experimental?: { codebase_search?: boolean; agent_manager_tool?: boolean } },
  ): Tool.Def[] {
    return [
      ...(cfg.experimental?.codebase_search === true ? [tools.codebase] : []),
      ...(tools.semantic ? [tools.semantic] : []),
      tools.recall,
      ...(tools.memory ? [tools.memory] : []),
      ...(tools.terminal ? [tools.terminal] : []),
      // The extension is the only client that can consume the Agent Manager start event.
      ...(Flag.ALTRU_CODER_CLIENT === "vscode" && cfg.experimental?.agent_manager_tool === true ? [tools.manager] : []),
    ]
  }
}
