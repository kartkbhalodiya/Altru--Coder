...
function planEditRules(worktree: string) {
  return {
    "*": "deny" as const,
    [path.join(".altru-coder", "plans", "*.md")]: "allow" as const,
...
export interface AltruCoderData {
...
// Patch the base agents map in-place with all altru-coder-specific changes:
// - Rename build → code
// - Patch plan with readOnlyBash, mcpRules, .altru-coder paths
...
export function patchAgents(
  ...
  if (agents.plan) {
    agents.plan = {
      ...
      permission: Permission.merge(
        defaults,
        planGuard(worktree, altru.mcpRules),
        user,
  ...
  agents.ask = {
    ...
    prompt: PROMPT_ASK,
    options: {},
    permission: Permission.merge(defaults, askGuard(altru.mcpRules), user, askEditGuard(), denies(user)),
    mode: "primary",
    native: true,
  }
}
 ...
 * removing it from legacy .altrucodermodes YAML files.
 * Scans all config directories for agent/mode .md files matching the name,
 * then also checks the .altrucodermodes files the ModesMigrator reads.
 */
export async function remove(name: string) {
  ...
  // 2. Remove from legacy .altrucodermodes YAML files (read by ModesMigrator)
  const { ModesMigrator } = await import("@/altrucoder/modes-migrator")
  const { AltruCoderPaths } = await import("@/altrucoder/paths")
  const os = await import("os")
  const matter = (await import("gray-matter")).default
  const home = os.default.homedir()
  const modesFiles = [
    path.join(AltruCoderPaths.vscodeGlobalStorage(), "settings", "custom_modes.yaml"),
    path.join(home, ".altrucoder", "cli", "global", "settings", "custom_modes.yaml"),
    path.join(home, ".altrucodermodes"),
    path.join(Instance.directory, ".altrucodermodes"),
  ]
