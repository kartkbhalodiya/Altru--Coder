...
/**
 * Altru Coder-specific home footer plugin.
 *
 ...
 */
import type { TuiPlugin, TuiPluginApi, TuiPluginModule } from "@altru-coder/plugin/tui"
...
const id = "internal:altru-coder-home-footer"
...
// ---------------------------------------------------------------------------
// RemoteIndicator – adapted from @/altrucoder/remote-tui for plugin API usage
// ---------------------------------------------------------------------------

function RemoteIndicator(props: { api: TuiPluginApi; altru: boolean }) {
  ...
  onMount(() => {
    void props.api.client.remote
      ...
      .catch(() => undefined)
    const off = props.api.event.on("altru-coder-sessions.remote-status-changed", (evt) => setStatus(evt.properties))
    onCleanup(off)
  })

  return (
    <Show when={props.altru && status()?.enabled}>
...
// ---------------------------------------------------------------------------
// Sub-components (mirror upstream home/footer with altru-coder additions)
// ---------------------------------------------------------------------------
...
function View(props: { api: TuiPluginApi }) {
  const altru = createMemo(() => props.api.state.provider.some((p) => p.id === "altru-coder"))
  const theme = () => props.api.theme.current
  const sync = useSync()
  ...
  return (
    ...
    >
      ...
      <box gap={1} flexDirection="row" flexShrink={0}>
        <RemoteIndicator api={props.api} altru={altru()} />
        <Mcp api={props.api} />
