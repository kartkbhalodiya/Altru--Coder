...
/**
 * Read project ID from .altru-coder/config.json, falling back to .altrucoder/config.json
...
async function getProjectIdFromConfig(directory: string): Promise<string | undefined> {
  ...
  for (const dir of [".altru-coder", ".altrucoder"]) {
...
/**
 * Resolve project ID with priority: .altru-coder/config.json -> .altrucoder/config.json -> git origin URL
 * @returns Normalized project ID or undefined
 */
async function resolveProjectId(): Promise<string | undefined> {
  const dir = Instance.directory

  // Priority 1: .altru-coder/config.json (falls back to .altrucoder/config.json)
...
export namespace AltruCoderProjectID {
  export interface Interface {
    readonly get: () => Effect.Effect<string | undefined>
  }

  export class Service extends Context.Service<Service, Interface>()("@altru-coder/AltruCoderProjectID") {}

  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const state = yield* InstanceState.make(
        Effect.fn("AltruCoderProjectID.state")(function* () {
...
const { runPromise } = makeRuntime(AltruCoderProjectID.Service, AltruCoderProjectID.defaultLayer)

/**
 * Get the project ID for the current Instance context (cached per-project)
 * @returns Normalized project ID or undefined
 */
export async function getAltruCoderProjectId(): Promise<string | undefined> {
