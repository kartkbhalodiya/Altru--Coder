...
//          persist `"snapshot": false` to `.altru-coder/altru-coder.json`, and skip. All
...
//     Instead we write the file directly via `AltruCoderConfig.updateProjectConfig`
...
//
// All of this is Altru Coder-specific — the upstream snapshot module remains a thin
...
export namespace AltruCoderSnapshotTrack {
  const log = Log.create({ service: "snapshot.track" })

  export const TIMEOUT_MS = iife(() => {
    const raw = process.env["ALTRU_CODER_SNAPSHOT_TRACK_TIMEOUT_MS"]
    if (raw) {
