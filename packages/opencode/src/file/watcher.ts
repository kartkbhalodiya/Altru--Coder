...
declare const ALTRU_CODER_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const binding = require(
      `@parcel/watcher-${process.platform}-${process.arch}${process.platform === "linux" ? `-${ALTRU_CODER_LIBC || "glibc"}` : ""}`,
    )
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("FileWatcher.state")(
        function* () {
          if (yield* Flag.ALTRU_CODER_EXPERIMENTAL_DISABLE_FILEWATCHER) return
          ...
          if (yield* Flag.ALTRU_CODER_EXPERIMENTAL_FILEWATCHER) {
