...
import * as Encoding from "../altrucoder/encoding" // altrucoder_change
...
interface ApplyPatchFileUpdate {
  ...
  content: string
  bom: boolean
  encoding: string // altrucoder_change
}

export function deriveNewContentsFromChunks(filePath: string, chunks: UpdateFileChunk[]): ApplyPatchFileUpdate {
  // Read original file content
  let originalContent: ReturnType<typeof Bom.split>
  let encoding: string // altrucoder_change - track detected encoding for round-trip write
  try {
    // altrucoder_change start - encoding-aware read replaces readFileSync(filePath, "utf-8").
  ...
  return {
    ...
    encoding, // altrucoder_change - include detected encoding for round-trip write
  }
}
...
export async function applyHunksToFiles(hunks: Hunk[]): Promise<AffectedPaths> {
  ...
  for (const hunk of hunks) {
    switch (hunk.type) {
      case "add":
        await Encoding.write(hunk.path, hunk.contents) // altrucoder_change - encoding-aware write (mkdirs)
      ...
      case "update":
        ...
        if (hunk.move_path) {
          // Handle file move
          await Encoding.write(hunk.move_path, Bom.join(fileUpdate.content, fileUpdate.bom), fileUpdate.encoding) // altrucoder_change - encoding-aware write (mkdirs) replaces fs.mkdir + fs.writeFile
          await fs.unlink(hunk.path)
          modified.push(hunk.move_path)
          log.info(`Moved file: ${hunk.path} -> ${hunk.move_path}`)
        } else {
          // Regular update
          await Encoding.write(hunk.path, Bom.join(fileUpdate.content, fileUpdate.bom), fileUpdate.encoding) // altrucoder_change - encoding-aware write replaces fs.writeFile
      ...
      for (const hunk of args.hunks) {
        ...
        switch (hunk.type) {
          ...
          case "delete":
            ...
            try {
              const content = (await Encoding.read(deletePath)).text // altrucoder_change - encoding-aware read
              changes.set(resolvedPath, {
                type: "delete",
                content,
              })
