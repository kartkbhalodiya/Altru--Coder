...
// altrucoder_change start — support absolute glob patterns (e.g. ~/.config/altru-coder/command/*.md)
    ...
    return {
      ...
      execute: (params: { pattern: string; path?: string }, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const absolute = split(params.pattern) // altrucoder_change
          yield* ctx.ask({
            permission: "glob",
          ...
          const base = absolute?.dir ?? params.path ?? ins.directory // altrucoder_change
          ...
          const files = yield* rg
            .files({ cwd: search, glob: [absolute?.pattern ?? params.pattern], signal: ctx.abort }) // altrucoder_change
            .pipe(
