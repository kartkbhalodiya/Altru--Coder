...
async function search(params: { query?: string; limit?: number }, ctx: Tool.Context) {
  ...
  const dirs = await WorktreeFamily.list() // altrucoder_change
  const { Session } = await import("../session/session") // altrucoder_change
  ...
  for (const session of Session.listGlobal({
    projectID: Instance.project.id, // altrucoder_change
    directories: dirs, // altrucoder_change
    search: params.query,
    roots: true,
    limit,
  })) {
...
async function read(params: { sessionID?: string }, ctx: Tool.Context) {
  ...
  const { Session } = await import("../session/session") // altrucoder_change
  const { SessionID } = await import("../session/schema") // altrucoder_change
