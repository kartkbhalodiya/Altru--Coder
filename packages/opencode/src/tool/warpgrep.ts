...
import { WarpGrepClient } from "@morphllm/morphsdk/tools/warp-grep/client" // altrucoder_change
import { Instance } from "../project/instance"
import { Bus } from "../bus"
import { TuiEvent } from "../cli/cmd/tui/event"
import DESCRIPTION from "./warpgrep.txt"

// FREE_PERIOD_TODO: Remove ALTRU_CODER_WARPGREP_PROXY_URL constant and the proxy
// fallback below. After the free period ends, require MORPH_API_KEY and
// return an error when it is missing.
const ALTRU_CODER_WARPGREP_PROXY_URL = "https://api.altru-coder.ai/api/gateway"

const Parameters = Schema.Struct({
  query: Schema.String.annotate({
    description: "Search query describing what code you are looking for. Be specific and descriptive for best results.", // altrucoder_change
  }),
})
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const client = new WarpGrepClient({
            morphApiKey: apiKey ?? "altru-coder-free",
            ...(apiKey ? {} : { morphApiUrl: ALTRU_CODER_WARPGREP_PROXY_URL }),
            timeout: 60_000,
          })
          ...
          if (!result.success || !result.contexts?.length) {
            ...
            const isAuthOrRateLimit =
              result.error && /401|402|429|rate.limit|free.period|unauthorized/i.test(result.error) // altrucoder_change
            const apiKeyMsg =
