import { lstat } from "fs/promises" // altrucoder_change
import { Effect, Option, Schema, Scope } from "effect"
import { NonNegativeInt } from "@/util/schema"
import { createReadStream } from "fs"
import * as path from "path"
import { Readable } from "stream" // altrucoder_change
...
import * as Encoding from "../altrucoder/encoding"
...
const DIRECTORY_CONCURRENCY = 8 // altrucoder_change
...
export const ReadTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const isBinaryFile = (filepath: string, bytes: Uint8Array) => {
      ...
      // altrucoder_change start - UTF-16/32 BOM: NUL bytes are legitimate, skip the NUL/control-char heuristic
...
export async function lines(filepath: string, opts: { limit: number; offset: number }) {
  // altrucoder_change end
  // altrucoder_change start - decode with detected encoding; replaces createReadStream(filepath, { encoding: "utf8" })
