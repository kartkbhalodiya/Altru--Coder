...
import { SuggestTool } from "../altrucoder/suggestion/tool" // altrucoder_change
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const altruToolInfos = yield* AltruCoderToolRegistry.infos()
    // altrucoder_change end
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const cfg = yield* config.get() // altrucoder_change: capture for AltruCoderToolRegistry.extra
        const questionEnabled =
          ["app", "cli", "desktop", "vscode"].includes(Flag.ALTRU_CODER_CLIENT) || Flag.ALTRU_CODER_ENABLE_QUESTION_TOOL // altrucoder_change: add vscode client + ALTRU_CODER_* flag
        ...
        const tool = yield* Effect.all({
          ...
          plan: Tool.init(plan),
          suggest: Tool.init(suggesttool), // altrucoder_change
        })

        const altru = yield* AltruCoderToolRegistry.build(altruToolInfos, { agent: agents, truncate }) // altrucoder_change
        ...
        return {
          ...
          builtin: [
            ...
            // altrucoder_change start
            tool.plan,
            ...(["cli", "vscode"].includes(Flag.ALTRU_CODER_CLIENT) ? [tool.suggest] : []),
            ...AltruCoderToolRegistry.extra(altru, cfg),
            // altrucoder_change end
            ...(Flag.ALTRU_CODER_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
          ],
          task: tool.task,
          read: tool.read,
        }
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const filtered = (yield* all()).filter((tool) => {
        if (tool.id === WebSearchTool.id) {
          return input.providerID === ProviderID["altru-coder"] || Flag.ALTRU_CODER_ENABLE_EXA // altrucoder_change
        }
