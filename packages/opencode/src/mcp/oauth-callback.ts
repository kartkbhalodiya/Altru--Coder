...
import * as AltruCoderOAuthCallback from "../altrucoder/mcp-oauth-callback" // altrucoder_change
...
<head>
  ...
  <title>Altru Coder - Authorization Successful</title>
...
<body>
  <div class="container">
    ...
    <!-- altrucoder_change start -->
    <p>You can close this window and return to Altru Coder.</p>
    <!-- altrucoder_change end -->
  </div>
...
<head>
  <!-- altrucoder_change start -->
  <title>Altru Coder - Authorization Failed</title>
  <!-- altrucoder_change end -->
  <style>
    body { font-family: system-ui, -apple-system, sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #1a1a2e; color: #eee; }
    .container { text-align: center; padding: 2rem; }
...
export async function ensureRunning(redirectUri?: string): Promise<void> {
  // altrucoder_change start - delegate Altru Coder-specific callback binding from here because OAuth state lives in this module
  await AltruCoderOAuthCallback.ensureRunning({
    redirectUri,
  ...
  await new Promise<void>((resolve, reject) => {
    // altrucoder_change start - EADDRINUSE can still fire when another process
    ...
    server!.listen(currentPort, () => {
      server!.off("error", onError) // altrucoder_change
