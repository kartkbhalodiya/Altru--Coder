...
if (process.platform === "win32" && !("type" in process)) {
  Object.defineProperty(process, "type", { value: "altru-coder-bun", configurable: true })
}
...
// altrucoder_change start — inject --rm for Docker containers to prevent stopped container accumulation
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const connectTransport = (transport: Transport, timeout: number) =>
      Effect.acquireUseRelease(
        ...
        (t) =>
          Effect.tryPromise({
            try: () => {
              const client = new Client({ name: "altru-coder", version: InstallationVersion }) // altrucoder_change
            ...
            if (isAuthError) {
              ...
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return bus
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: altru-coder mcp auth ${key}`, // altrucoder_change
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const finalArgs = ensureDockerRm(cmd, args) // altrucoder_change
      const cwd = yield* InstanceState.directory
      const transport = new StdioClientTransport({
        stderr: "pipe",
        command: cmd,
        args: finalArgs, // altrucoder_change
        cwd,
        env: {
          ...process.env,
    ...
    // altrucoder_change start - `opts?: { callback?: boolean }` parameter is Altru Coder-specific
    const startAuth = Effect.fn("MCP.startAuth")(function* (mcpName: string, opts?: { callback?: boolean }) {
      ...
      // altrucoder_change start - authenticate() defers binding the callback port until a redirect is needed
    ...
    const authenticate = Effect.fn("MCP.authenticate")(function* (mcpName: string) {
      const result = yield* startAuth(mcpName, { callback: false }) // altrucoder_change
      ...
      // altrucoder_change start - bind only after redirect exists, and clean up if binding fails
      const mcpConfig = yield* getMcpConfig(mcpName)
