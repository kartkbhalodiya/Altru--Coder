...
import { Telemetry } from "@altru-coder/altru-coder-telemetry" // altrucoder_change
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const all = Effect.fn("Auth.all")(function* () {
      if (process.env.ALTRU_CODER_AUTH_CONTENT) {
        try {
          return JSON.parse(process.env.ALTRU_CODER_AUTH_CONTENT)
        } catch (err) {}
      }
    ...
    const remove = Effect.fn("Auth.remove")(function* (key: string) {
      ...
      // altrucoder_change start - Track logout and reset telemetry identity for Altru Coder
      if (key === "altru-coder") {
