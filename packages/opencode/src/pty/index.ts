  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Pty.create")(function* (input: CreateInput) {
      ...
      const env = {
        ...
        ...shell.env,
        TERM: "xterm-256color",
        ALTRU_CODER_TERMINAL: "1",
      } as Record<string, string>
      // altrucoder_change start
      // Don't leak the altru-coder server's auth credential into user shells.
      ...
      // with zero effort. Users who genuinely need `altru-coder run`/`altru-coder tui
      // attach` to auto-connect from inside a altru-coder-spawned terminal can
      // pass `--password` explicitly or re-export the env themselves.
      delete env.ALTRU_CODER_SERVER_PASSWORD
      delete env.ALTRU_CODER_SERVER_USERNAME
