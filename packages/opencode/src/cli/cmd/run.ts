...
import { createAltruCoderClient, type AltruCoderClient, type ToolPart } from "@altru-coder/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run altru-coder with a message", // altrucoder_change
  builder: (yargs: Argv) => {
    return (
      yargs
        .positional("message", {
        ...
        .option("password", {
          alias: ["p"],
          type: "string",
          describe: "basic auth password (defaults to ALTRU_CODER_SERVER_PASSWORD)",
        })
        .option("dir", {
          type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: AltruCoderClient) {
    ...
    async function share(sdk: AltruCoderClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.ALTRU_CODER_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: AltruCoderClient) {
      ...
      async function loop() {
        ...
        const MAX_RETRIES = 3 // altrucoder_change
        let retries = 0 // altrucoder_change
        ...
        for await (const event of events.stream) {
          ...
          if (event.type === "permission.asked") {
            ...
            if (args.auto) {
              // altrucoder_change - In auto mode, automatically approve all permissions without prompting
          ...
          // altrucoder_change start - network retry handling
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.ALTRU_CODER_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.ALTRU_CODER_SERVER_USERNAME ?? "altru-coder" // altrucoder_change
        const auth = `Basic ${Buffer.from(`${username}:${password}`).toString("base64")}`
        return { Authorization: auth }
      })()
      const sdk = createAltruCoderClient({ baseUrl: args.attach, directory, headers })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createAltruCoderClient({ baseUrl: "http://altru.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
