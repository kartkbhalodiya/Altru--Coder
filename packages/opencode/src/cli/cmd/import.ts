...
import type { Session as SDKSession, Message, Part } from "@altru-coder/sdk/v2"
...
const log = Log.create({ service: "import" }) // altrucoder_change
...
/** Extract share ID from a Altru Coder share URL like https://app.altru-coder.ai/s/abc123 */
export function parseShareUrl(url: string): string | null {
  const match = url.match(/^https?:\/\/app\.altru-coder\.ai\/s\/([a-zA-Z0-9_-]+)$/)
...
async function ingestBootstrap(sessionId: string) {
  ...
  return AltruCoderSessions.bootstrap(sessionId)
}
  ...
  handler: async (args) => {
    await bootstrap(process.cwd(), async () => {
      ...
      if (isUrl) {
        ...
        if (!slug) {
          process.stdout.write(`Invalid URL format. Expected: https://app.altru-coder.ai/s/<id>`)
