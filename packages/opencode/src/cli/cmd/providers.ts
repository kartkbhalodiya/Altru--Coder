...
import type { Hooks } from "@altru-coder/plugin"
...
export const ProvidersCommand = cmd({
  // altrucoder_change start - keep "auth" as primary command name
  command: "auth",
  aliases: ["providers"],
...
export const ProvidersListCommand = cmd({
  ...
  async handler() {
    // altrucoder_change start - wrap with Instance.provide for ModelsDev.get() -> Config.get() dependency
...
export const ProvidersLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "altru-coder auth provider", // altrucoder_change
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        const options = [
          ...pipe(
            ...
            map((x) => ({
              ...
              value: x.id,
              hint: {
                "altru-coder": "recommended", // altrucoder_change
                opencode: "recommended",
                openai: "ChatGPT login or API key", // altrucoder_change
              }[x.id],
            })),
          ),
        ...
        if (args.provider) {
          ...
          // altrucoder_change start - accept codex as an alias for OpenAI ChatGPT auth
