...
import type { GlobalEvent } from "@altru-coder/sdk/v2"
...
declare global {
  const ALTRU_CODER_WORKER_PATH: string
}
...
async function target() {
  if (typeof ALTRU_CODER_WORKER_PATH !== "undefined") return ALTRU_CODER_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start altru-coder tui", // altrucoder_change
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start altru-coder in", // altrucoder_change
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const env = sanitizedProcessEnv({
        [ALTRU_CODER_PROCESS_ROLE]: "worker",
        [ALTRU_CODER_RUN_ID]: ensureRunID(),
      })
      ...
      // altrucoder_change start - graceful shutdown on external signals
      ...
      try {
        // altrucoder_change start - import cloud session before TUI renders
        if (args.cloudFork && args.session) {
          UI.println("Importing session from cloud...")
          const sdk = createAltruCoderClient({
