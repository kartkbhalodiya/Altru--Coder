...
export const rpc = {
  ...
  async shutdown() {
    ...
    if (server) await server.stop(true)
    // altrucoder_change start - Clear the Rpc message channel so the worker's event loop can drain and
...
function getAuthorizationHeader(): string | undefined {
  const password = Flag.ALTRU_CODER_SERVER_PASSWORD
  if (!password) return undefined
  const username = Flag.ALTRU_CODER_SERVER_USERNAME ?? "altru-coder" // altrucoder_change
