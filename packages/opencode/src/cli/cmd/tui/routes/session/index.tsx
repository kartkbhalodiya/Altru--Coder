...
import type { AssistantMessage, Part, Provider, ToolPart, UserMessage, TextPart, ReasoningPart } from "@altru-coder/sdk/v2"
...
import type { SemanticSearchTool } from "@/altrucoder/tool/semantic-search" // altrucoder_change
...
import { AltruCoderErrorBlock } from "@/altrucoder/components/altru-coder-error-display" // altrucoder_change
...
import { Suggest } from "@/altrucoder/suggestion/tui/render" // altrucoder_change
import { SuggestPrompt } from "@/altrucoder/suggestion/tui/prompt" // altrucoder_change
import { NetworkPrompt } from "./network" // altrucoder_change
import { DialogExportOptions } from "../../ui/dialog-export-options"
import * as Model from "../../util/model"
import { formatTranscript } from "../../util/transcript"
import { UI } from "@/cli/ui.ts"
import { useTuiConfig } from "../../context/tui-config"
import { formatMarkdownTables } from "../../util/markdown" // altrucoder_change
import { bell } from "@/altrucoder/bell" // altrucoder_change
import { SessionIndexing } from "@/altrucoder/components/session-indexing" // altrucoder_change
import { submitFeedback } from "@/altrucoder/cli/cmd/tui/feedback" // altrucoder_change
...
export function Session() {
  ...
  // altrucoder_change start - footer overlay only hosts blocking suggestions now;
  ...
  // altrucoder_change start - ring terminal bell on task completion
  createEffect(
    on(
  ...
  // altrucoder_change start - ring terminal bell when input is needed
  createEffect(
    on(
  ...
  createEffect(
    on(
      () => [route.sessionID, suggestions().length + network().length] as const, // altrucoder_change
      ([id, len], prev) => {
        if (!prev || prev[0] !== id) return
        if (len > prev[1] && bellEnabled()) bell()
      },
  ...
  const [bellEnabled, _setBellEnabled] = kv.signal("bell_enabled", true) // altrucoder_change - terminal bell toggle (toggled via kv.set in app.tsx command)
  ...
  event.on("message.part.updated", (evt) => {
    ...
    // altrucoder_change - plan_exit no longer switches agent; PlanFollowup handles it
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ████   ██ ██     ██  ██  ${UI.Style.TEXT_DIM}altru-coder -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        `  ██ ▀█▄ ██ ██████ ▀████▀  `,
      ].join("\n"),
    )
    // altrucoder_change end
  ...
  // altrucoder_change start - double ctrl+c to exit for child sessions
  ...
  command.register(() => [
    ...
    // altrucoder_change start - message feedback
    {
...
function TextPart(props: { last: boolean; part: TextPart; message: AssistantMessage }) {
  ...
  // altrucoder_change start - format markdown tables with fixed-width columns
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.ALTRU_CODER_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
