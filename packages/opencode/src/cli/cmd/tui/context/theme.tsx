...
import altru from "./theme/altru-coder.json" with { type: "json" } // altrucoder_change
...
import colorblind from "./theme/colorblind.json" with { type: "json" } // altrucoder_change
...
import type { TuiThemeCurrent } from "@altru-coder/plugin/tui"
...
export const DEFAULT_THEMES: Record<string, ThemeJson> = {
  ...
  gruvbox,
  kanagawa,
  ["altru-coder"]: altru, // altrucoder_change
  material,
  matrix,
  mercury,
  monokai,
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "altru-coder") // altrucoder_change
        draft.active = typeof active === "string" ? active : "altru-coder" // altrucoder_change
        draft.ready = false
      }),
    )
    ...
    function init() {
      void Promise.allSettled([
        ...
        getCustomThemes()
          ...
          .catch(() => {
            setStore("active", "altru-coder") // altrucoder_change
          }),
      ]).finally(() => {
        setStore("ready", true)
      })
    ...
    // altrucoder_change start - safe fallback to altru-coder import if store lookup fails
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes["altru-coder"], store.mode) // altrucoder_change
    })
    // altrucoder_change end
    ...
    // altrucoder_change - use empty object as proxy target; all reads go through the getter
    return {
...
async function getCustomThemes() {
  const directories = [
    ...
    ...(await Array.fromAsync(
      Filesystem.up({
        targets: [".altru-coder", ".opencode"], // altrucoder_change
        start: process.cwd(),
      }),
    )),
  ]
  ...
  for (const dir of directories) {
    for (const item of await Glob.scan("themes/*.json", {
      ...
      // altrucoder_change start - validate custom theme JSON and protect built-in keys
