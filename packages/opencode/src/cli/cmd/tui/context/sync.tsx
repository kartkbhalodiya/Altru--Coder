import type {
  ...
  PermissionRequest,
  QuestionRequest,
  SuggestionRequest, // altrucoder_change
  SessionNetworkWait, // altrucoder_change
  LspStatus,
  McpStatus,
  McpResource,
  FormatterStatus,
...
import { batch, createEffect, on, onMount } from "solid-js" // altrucoder_change - add createEffect/on for workspace re-bootstrap
import { handleSuggestionEvent } from "@/altrucoder/suggestion/tui/sync" // altrucoder_change
import { useToast } from "@tui/ui/toast" // altrucoder_change
import * as Log from "@opencode-ai/core/util/log"
import { emptyConsoleState, type ConsoleState } from "@/config/console-state"
import type { IndexingStatus } from "@altru-coder/altru-coder-indexing/status" // altrucoder_change
import { AltruCoderIndexing } from "@/altrucoder/indexing" // altrucoder_change
...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
  ...
  init: () => {
    const [store, setStore] = createStore<{
      ...
      config: Config
      globalConfig: Config // altrucoder_change
      session: Session[]
      session_status: {
        [sessionID: string]: SessionStatus
      }
      session_diff: {
        [sessionID: string]: Omit<Snapshot.FileDiff, "before" | "after">[] // altrucoder_change
      }
      todo: {
      ...
      indexing: IndexingStatus // altrucoder_change
    }>({
      provider_next: {
        all: [],
        default: {},
      ...
      provider_auth: {},
      config: {},
      globalConfig: {}, // altrucoder_change
      status: "loading",
      agent: [],
      permission: {},
      question: {},
      ...
      formatter: [],
      vcs: undefined,
      indexing: { state: "Disabled", message: "Indexing disabled.", processedFiles: 0, totalFiles: 0, percent: 0 }, // altrucoder_change
    })

    const event = useEvent()
    const project = useProject()
    const sdk = useSDK()
    const toast = useToast() // altrucoder_change
    const kv = useKV()
    ...
    async function bootstrap(input: { fatal?: boolean } = {}) {
      ...
      const globalConfigPromise = sdk.client.global.config.get({ throwOnError: true }) // altrucoder_change
      const blockingRequests: Promise<unknown>[] = [
        providersPromise,
        providerListPromise,
        agentsPromise,
        configPromise,
        globalConfigPromise, // altrucoder_change
        projectPromise,
      ...
      await Promise.all(blockingRequests)
        .then(async () => {
          ...
          const globalConfigResponse = globalConfigPromise.then((x) => x.data!) // altrucoder_change
          ...
          return Promise.all([
            ...
            agentsResponse,
            configResponse,
            globalConfigResponse, // altrucoder_change
            ...
            const globalConfig = responses[5] // altrucoder_change
            const sessions = responses[6] // altrucoder_change
            ...
            batch(() => {
              ...
              setStore("globalConfig", reconcile(globalConfig)) // altrucoder_change
        ...
        .then(() => {
          ...
          void Promise.all([
            ...
            sdk.client.formatter.status({ workspace }).then((x) => setStore("formatter", reconcile(x.data!))), // altrucoder_change
            // altrucoder_change start
            ...
            // altrucoder_change start - show config warnings as persistent toast
            sdk.client.config
              .warnings()
              .then((x) => {
                const list = x.data ?? []
              ...
              .catch(() => {}),
            AltruCoderIndexing.current().then((x) => setStore("indexing", reconcile(x))),
            // altrucoder_change end
          ]).then(() => {
    ...
    // altrucoder_change start - re-bootstrap when workspace changes (Agent Manager)
    createEffect(
      on(
    ...
    const result = {
      ...
      get ready() {
        // return true // altrucoder_change - upstream #23037 left this debug path enabled; keep it commented so future merges do not restore eager ready state.
        if (process.env.ALTRU_CODER_FAST_BOOT) return true
        ...
        async sync(sessionID: string) {
          ...
          setStore(
            produce((draft) => {
              ...
              draft.message[sessionID] = messages.data!.map((x) => strip(x.info)) // altrucoder_change
