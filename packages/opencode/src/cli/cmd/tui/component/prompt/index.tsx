...
import type { AssistantMessage, FilePart, UserMessage } from "@altru-coder/sdk/v2"
...
import { AltruCoderSessionTuiSync } from "@/altrucoder/session/tui-sync" // altrucoder_change
...
export function Prompt(props: PromptProps) {
  ...
  const [store, setStore] = createStore<{
    ...
    interrupt: number
    exitPress: number // altrucoder_change - track double ctrl+c to exit
    placeholder: number
  }>({
  ...
  // altrucoder_change start - sync local agent/model whenever newest user message changes
  let syncedKey: string | undefined
  createEffect(() => {
    const sessionID = props.sessionID
    const msg = lastUserMessage()
    if (!sessionID || !msg) return
    // altrucoder_change start - skip compaction messages while syncing local agent/model
    const parts = sync.data.part[msg.id]
    if (!parts) return
    if (!AltruCoderSessionTuiSync.model({ role: msg.role, parts })) return
  ...
  // altrucoder_change start - close autocomplete while blocking overlays hide the prompt
  createEffect(() => {
  ...
  async function submit() {
    ...
    if (store.mode === "shell") {
      void sdk.client.session.shell({
        sessionID,
        agent: local.agent.current()?.name ?? "", // altrucoder_change
        model: {
    ...
    toast.dismiss() // altrucoder_change - dismiss persistent config warning on first submit
    history.append({
      ...store.prompt,
      mode: currentMode,
    })
  ...
  const highlight = createMemo(() => {
    ...
    return local.agent.color(agent.name ?? "") // altrucoder_change
  })
  ...
  const spinnerDef = createMemo(() => {
    const agent = local.agent.current()
    const color = agent ? local.agent.color(agent.name ?? "") : theme.border // altrucoder_change
    return {
      frames: createFrames({
        color,
        style: "blocks",
  ...
  return (
    <>
      ...
      <box ref={(r) => (anchor = r)} visible={props.visible !== false}>
            ...
            <textarea
              ...
              onKeyDown={async (e) => {
                ...
                if (keybind.match("app_exit", e)) {
                  if (store.prompt.input === "") {
                    // altrucoder_change start - double ctrl+c to exit, single ctrl+d exits immediately
                    if (e.ctrl && e.name === "c") {
              ...
              onPaste={async (event: PasteEvent) => {
                ...
                if (
                  (lineCount >= 5 || pastedContent.length > 800) && // altrucoder_change #7252 delay paste summary
        ...
        <box
          height={1}
          flexShrink={0} // altrucoder_change - prevent border box from shrinking in narrow terminals (#6309)
          border={["left"]}
          borderColor={borderHighlight()}
          customBorderChars={{
            ...EmptyBorder,
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          ...
          <Show when={status().type !== "retry"}>
            <box gap={2} flexDirection="row">
              {/* altrucoder_change start - show "ctrl+c again to exit" hint */}
