...
import { access, constants } from "fs/promises" // altrucoder_change
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  // altrucoder_change start: use altru-coder directory everywhere
  const project = Flag.ALTRU_CODER_DISABLE_PROJECT_CONFIG
    ? []
    : await Filesystem.findUp(["altru-coder.json", "altru-coder.jsonc"], input.cwd, undefined, { rootFirst: true })
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "altru-coder")]
  // altrucoder_change end
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "altru-coder"))
  }
  if (Flag.ALTRU_CODER_CONFIG) files.push(Flag.ALTRU_CODER_CONFIG)
