...
import { AltruCoderDefaultPlugins } from "@/altrucoder/config/default-plugins" // altrucoder_change
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and ALTRU_CODER_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ALTRU_CODER_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ALTRU_CODER_TUI_CONFIG override, if set.
  if (Flag.ALTRU_CODER_TUI_CONFIG) {
    const configFile = Flag.ALTRU_CODER_TUI_CONFIG
  ...
  // 4. `.opencode` directories (and ALTRU_CODER_CONFIG_DIR) discovered while
  ...
  for (const dir of dirs) {
    // if (!dir.endsWith(".opencode") && dir !== Flag.ALTRU_CODER_CONFIG_DIR) continue // altrucoder_change
  ...
  // altrucoder_change start — inject Altru Coder default plugins to keep TUI aligned with server config
  AltruCoderDefaultPlugins.apply(acc.result, { disabled: Flag.ALTRU_CODER_DISABLE_DEFAULT_PLUGINS, log })
  // altrucoder_change end
