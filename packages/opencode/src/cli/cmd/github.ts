...
const AGENT_USERNAME = "altruconnect[bot]" // altrucoder_change
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/altru.yml" // altrucoder_change
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/altru summarize` to see the agent in action", // altrucoder_change
              ].join("\n"),
            )
          }
          ...
          async function promptProvider() {
            const priority: Record<string, number> = {
              "altru-coder": 0, // altrucoder_change
              anthropic: 1,
              openai: 2,
              google: 3,
            }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/altruconnect" // altrucoder_change
            const command =
          ...
          async function addWorkflowFiles() {
            // altrucoder_change start - updated workflow template with Altru Coder branding and gateway secrets
            const providerEnvStr =
              provider === "amazon-bedrock"
                ? ""
                : providers[provider].env.map((e) => `\n          ${e}: \${{ secrets.${e} }}`).join("")

            const altruGatewayEnv =
              provider === "altru-coder"
                ? `\n          ALTRU_CODER_API_KEY: \${{ secrets.ALTRU_CODER_API_KEY }}\n          ALTRU_CODER_ORG_ID: \${{ secrets.ALTRU_CODER_ORG_ID }}`
                : ""

            const envStr = providerEnvStr || altruGatewayEnv ? `\n        env:${providerEnvStr}${altruGatewayEnv}` : ""
...
jobs:
  "altru-coder":
    if: |
      ...
      startsWith(github.event.comment.body, '/altru')
    ...
    steps:
      ...
      - name: Run Altru Coder
        uses: Altru-Coder/altrucoder/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.altru-coder.ai" : "https://altru-coder.ai" // altrucoder_change
      ...
      try {
        ...
        // Setup altru-coder session // altrucoder_change
        ...
        console.log("altru-coder session", session.id) // altrucoder_change
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.altru-coder.ai" // altrucoder_change
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/altru,/kc") // altrucoder_change
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to altru-coder...") // altrucoder_change
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("altru-coder-github-action") // altrucoder_change
        } catch (error) {
      ...
      async function exchangeForAppToken(token: string) {
        // altrucoder_change start - updated endpoint URLs per new API structure
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `altru-coder/${type}-${hex}-${timestamp}` // altrucoder_change
        }
        return `altru-coder/${type}${issueId}-${timestamp}` // altrucoder_change
      }
      ...
      function footer(opts?: { image?: boolean }) {
        // altrucoder_change start - simplified footer with text branding (no image backend yet)
        const share = shareId ? `[altru-coder session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
        return `\n\n---\n*Powered by [Altru Coder](https://altru-coder.ai)*&nbsp;&nbsp;|&nbsp;&nbsp;${share}[github run](${runUrl})`
        // altrucoder_change end
      }
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the altru-coder infrastructure after your response", // altrucoder_change
