...
import { existsSync } from "node:fs" // altrucoder_change

const subcommand = "pr" // altrucoder_change

// altrucoder_change start - resolve the currently running CLI instead of hardcoding opencode
export function cliCommand(
  ...
  if (script === subcommand) return [input.execPath] // altrucoder_change
...
export const PrCommand = cmd({
  command: `${subcommand} <number>`, // altrucoder_change
  describe: "fetch and checkout a GitHub PR branch, then run altru-coder", // altrucoder_change
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        const cli = cliCommand() // altrucoder_change
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/app\.altru-coder\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
        ...
        UI.println()
        UI.println("Starting altru-coder...") // altrucoder_change
        UI.println()

        const run = sessionId ? [...cli, "-s", sessionId] : cli // altrucoder_change
        // altrucoder_change start
        ...
        if (code !== 0) throw new Error(`altru-coder exited with code ${code}`) // altrucoder_change
      },
    })
  },
})
