...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall altru-coder and remove all related files", // altrucoder_change
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Altru Coder") // altrucoder_change
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @altru-coder/cli", // altrucoder_change
      pnpm: "pnpm uninstall -g @altru-coder/cli", // altrucoder_change
      bun: "bun remove -g @altru-coder/cli", // altrucoder_change
      yarn: "yarn global remove @altru-coder/cli", // altrucoder_change
      brew: "brew uninstall opencode",
      choco: "choco uninstall altru-coder", // altrucoder_change
      scoop: "scoop uninstall altru-coder", // altrucoder_change
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(cmd, { nothrow: true }) // altrucoder_change - removed choco special case
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".opencode") || binDir.includes(".altru-coder")) {
      // altrucoder_change
  ...
  UI.empty()
  prompts.log.success("Thank you for using Altru Coder!") // altrucoder_change
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    // altrucoder_change start - detect both opencode and altru-coder markers
    if (
      content.includes("# opencode") ||
      content.includes(".opencode/bin") ||
      content.includes("# altru-coder") ||
      content.includes(".altru-coder/bin")
    ) {
      return file
    }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    // altrucoder_change start - clean both opencode and altru-coder markers
    if (trimmed === "# opencode" || trimmed === "# altru-coder") {
      skip = true
      continue
    }
