    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: altru-coder mcp add") // altrucoder_change
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in altru-coder.json:") // altrucoder_change
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // altrucoder_change start - prefer altru-coder.json/.altru-coder over opencode.json/.opencode
  // Check for existing config files (prefer .jsonc over .json, check .altru-coder/ and .opencode/ subdirectory too)
  const candidates = [
    path.join(baseDir, "altru-coder.json"),
    path.join(baseDir, "altru-coder.jsonc"),
  ...
  if (!global) {
    candidates.push(
      path.join(baseDir, ".altru-coder", "altru-coder.json"),
      path.join(baseDir, ".altru-coder", "altru-coder.jsonc"),
      path.join(baseDir, ".altru-coder", "opencode.json"),
      path.join(baseDir, ".altru-coder", "opencode.jsonc"),
  ...
  // Default to altru-coder.json if none exist
  return path.join(baseDir, "altru-coder.json")
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., altru-coder x @modelcontextprotocol/server-filesystem", // altrucoder_change
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "altru-coder-debug", version: InstallationVersion }, // altrucoder_change
              },
              id: 1,
            }),
          })
          ...
          if (response.status === 401) {
            ...
            try {
              const client = new Client({
                name: "altru-coder-debug", // altrucoder_change
