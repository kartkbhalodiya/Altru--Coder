import { chmod, mkdir, readFile, rename, stat as statFile, writeFile } from "fs/promises" // altrucoder_change
import { createWriteStream, existsSync, statSync } from "fs"
import { realpathSync } from "fs"
// altrucoder_change start - harden containment checks
...
export async function write(p: string, content: string | Buffer | Uint8Array, mode?: number): Promise<void> {
  // altrucoder_change start - atomic write via temp-file + rename to avoid partial reads on concurrent saves
...
export function contains(parent: string, child: string) {
  // altrucoder_change start - reject cross-drive and escaped relative paths
