import { RemoteProtocol } from "@/altru-coder-sessions/remote-protocol"
import type { RemoteWS } from "@/altru-coder-sessions/remote-ws"
import { GlobalBus } from "@/bus/global"
// altrucoder_change - AppRuntime is imported lazily inside dispatch() below to break a
// module init cycle: Worktree → project/bootstrap → altru-coder-sessions → remote-sender →
...
// altrucoder_change start — lazy init to avoid circular dependency
...
function normalizeModel(model: string | undefined) {
  ...
  return {
    providerID: ProviderID.make("altru-coder"),
    modelID: ModelID.make(model.startsWith("altrucoder/") ? model.slice("altrucoder/".length) : model),
  }
}
