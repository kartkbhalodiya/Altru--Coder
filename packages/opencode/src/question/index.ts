...
import { AltruCoderQuestion } from "@/altrucoder/question" // altrucoder_change
...
export class Option extends Schema.Class<Option>("QuestionOption")({
  ...
  // altrucoder_change start - optional i18n keys so clients can translate while still
...
const base = {
  ...
  // altrucoder_change start - optional i18n keys for question text and header
  ...
  Effect.gen(function* () {
    ...
    const ask = Effect.fn("Question.ask")(function* (input: {
      ...
      const info = Schema.decodeUnknownSync(Request)({
        ...
        blocking: input.blocking, // altrucoder_change
        tool: input.tool,
      })

      // altrucoder_change start
      yield* AltruCoderQuestion.guardFollowup(input.sessionID, () => new RejectedError())
      // altrucoder_change end
    ...
    // altrucoder_change start - body lives in @/altrucoder/question/AltruCoderQuestion.makeDismissAll
    const dismissAll = AltruCoderQuestion.makeDismissAll({
      state,
    ...
    return Service.of({ ask, reply, reject, list, dismissAll }) // altrucoder_change
  }),
)
