...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.ALTRU_CODER_FAKE_VCS)
    ...
    const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
      // altrucoder change start
      return yield* fs.readFileString(pathSvc.join(dir, "altru-coder")).pipe(
        // altrucoder change end
        Effect.map((x) => x.trim()),
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        if (!id) {
          ...
          if (id) {
            yield* fs.writeFileString(pathSvc.join(common, "altru-coder"), id).pipe(Effect.ignore) // altrucoder_change
          }
        }
      ...
      if (Flag.ALTRU_CODER_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
