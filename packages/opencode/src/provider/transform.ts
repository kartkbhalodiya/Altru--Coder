...
import { altruProviderOptions } from "@/altrucoder/provider-options"
import { isLing } from "@/altrucoder/model-match" // altrucoder_change
...
export const OUTPUT_TOKEN_MAX = Flag.ALTRU_CODER_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
...
function sdkKey(npm: string): string | undefined {
  switch (npm) {
    ...
    case "@openrouter/ai-sdk-provider":
      return "openrouter"
    case "@altru-coder/altru-coder-gateway": // altrucoder_change
      return "openrouter"
  }
  return undefined
}
...
function normalizeMessages(
  ...
  // altrucoder_change start - only filter for Claude models on Bedrock, not all Bedrock models
...
export function temperature(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 0.3 // altrucoder_change
  return undefined
}
...
export function topP(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 0.95 // altrucoder_change
  return undefined
}
...
export function topK(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 20 // altrucoder_change
  return undefined
}
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  if (
    ["@altru-coder/altru-coder-gateway", "@ai-sdk/openai-compatible"].includes(model.api.npm) &&
    model.variants &&
  ...
  if (
    ...
    id.includes("minimax") ||
    // id.includes("glm") || // altrucoder_change
    // id.includes("kimi") || // altrucoder_change
  ...
  if (id.includes("grok") && id.includes("grok-3-mini")) {
    if (model.api.npm === "@openrouter/ai-sdk-provider" || model.api.npm === "@altru-coder/altru-coder-gateway") {
      // altrucoder_change - add Altru Coder Gateway support
      return {
  ...
  switch (model.api.npm) {
    ...
    case "@openrouter/ai-sdk-provider":
      ...
      if (
        ...
        !model.id.includes("claude") &&
        !model.id.includes("mercury") // altrucoder_change
      )
        return {}
    ...
    case "@ai-sdk/openai-compatible":
      const efforts = [...WIDELY_SUPPORTED_EFFORTS, "xhigh"] // altrucoder_change
...
export function options(input: {
  ...
  if (
    ...
    input.model.api.npm === "@altru-coder/altru-coder-gateway" // altrucoder_change
  ) {
    result["usage"] = {
      include: true,
    }
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    if (!input.model.api.id.includes("gpt-5-pro")) {
      ...
      if (
        ...
        input.model.api.npm === "@ai-sdk/github-copilot" || // altrucoder_change
        input.model.api.npm === "@openrouter/ai-sdk-provider" || // altrucoder_change
        input.model.api.npm === "@altru-coder/altru-coder-gateway" // altrucoder_change
      ) {
    ...
    if (
      // altrucoder_change start - gate textVerbosity to Responses-API providers
      (input.model.api.npm === "@ai-sdk/openai" ||
        input.model.api.npm === "@ai-sdk/azure" ||
        input.model.api.npm === "@ai-sdk/github-copilot" ||
        input.model.api.npm === "@openrouter/ai-sdk-provider" ||
        input.model.api.npm === "@altru-coder/altru-coder-gateway") &&
      // altrucoder_change end
...
export function smallOptions(model: Provider.Model) {
  ...
  if (
    ...
    model.api.npm === "@altru-coder/altru-coder-gateway" // altrucoder_change
  ) {
...
export function providerOptions(model: Provider.Model, options: { [x: string]: any }) {
  ...
  // altrucoder_change start
  if (model.api.npm === "@altru-coder/altru-coder-gateway") {
    return altruProviderOptions(options)
  }
  // altrucoder_change end
