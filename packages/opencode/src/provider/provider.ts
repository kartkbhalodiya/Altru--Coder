...
import {
  ALTRU_CODER_BUNDLED_PROVIDERS,
  altruCustomLoaders,
  ALTRU_CODER_MODEL_SCHEMA_EXTENSIONS,
  patchModelsDevModel as patchAltruCoderModel,
  patchConfigModel as patchAltruCoderConfigModel,
  patchCustomLoaderResult,
  altruSmallModelPriority,
  buildTimeoutSignal,
  REQUEST_TIMEOUT_MS,
} from "@/altrucoder/provider/provider"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  ...ALTRU_CODER_BUNDLED_PROVIDERS, // altrucoder_change
}
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    azure: Effect.fnUntraced(function* (provider: Info) {
      ...
      // altrucoder_change start - prefer explicit Azure endpoint over resource name to avoid conflicting SDK options
      const endpoint = iife(() => {
        return [
      ...
      if (!resource && !endpoint) { // altrucoder_change
        return {
          ...
          async getModel() {
            throw new Error(
              "Azure resource name or endpoint is missing. Set AZURE_RESOURCE_NAME, AZURE_OPENAI_RESOURCE_NAME, AZURE_OPENAI_ENDPOINT, or reconnect the azure provider.", // altrucoder_change
            )
          },
        }
      }
      ...
      return {
        ...
        options: {
          ...(endpoint ? { baseURL: endpoint } : { resourceName: resource }), // altrucoder_change
        },
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `altru-coder/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`, // altrucoder_change
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `altru-coder auth cloudflare-ai-gateway`.", // altrucoder_change
        )
      }
...
export const Model = Schema.Struct({
  ...
  ...ALTRU_CODER_MODEL_SCHEMA_EXTENSIONS, // altrucoder_change
})
...
function fromModelsDevModel(provider: ModelsDev.Provider, model: ModelsDev.Model): Model {
  ...
  Object.assign(base, patchAltruCoderModel(provider.id, model)) // altrucoder_change
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const [providerID, provider] of configProviders) {
          if (!provider) continue // altrucoder_change - null entries are transient delete sentinels
          ...
          for (const [modelID, model] of Object.entries(provider.models ?? {})) {
            if (!model) continue // altrucoder_change - null entries are transient delete sentinels
            ...
            const parsedModel: Model = {
              ...
              // variants: {}, // altrucoder_change, moved into patchAltruCoderConfigModel
              ...patchAltruCoderConfigModel(model, existingModel), // altrucoder_change
            }
            const merged = mergeDeep(ProviderTransform.variants(parsedModel), model.variants ?? {})
            parsedModel.variants = mapValues(
              pickBy(merged, (v): v is NonNullable<typeof v> => !!v && !v.disabled), // altrucoder_change - drop null delete sentinels
              (v) => omit(v, ["disabled"]),
            )
        ...
        // altrucoder_change start - load auths before env so OAuth plugins can override inherited credentials
        ...
        for (const [id, provider] of Object.entries(database)) {
          ...
          if (disabled.has(providerID)) continue
          // altrucoder_change start - prefer explicit OAuth auth over inherited env credentials
          if (
        ...
        for (const [id, fn] of Object.entries({ ...custom(dep), ...altruCustomLoaders(dep) })) { // altrucoder_change
          ...
          if (result) patchCustomLoaderResult(id, result, altruEnv) // altrucoder_change
        ...
        for (const [id, provider] of configProviders) {
          if (!provider) continue // altrucoder_change - null entries are transient delete sentinels
          const providerID = ProviderID.make(id)
          // altrucoder_change start - keep OAuth plugin source when config and Codex auth coexist
          const oauth =
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
          ...
          // altrucoder_change start - use cancellable timeout for connection phase
          ...
          // altrucoder_change start - clear connection-phase timeout once headers arrive
          try {
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
      ...
      // altrucoder_change start
      const altruPriority = altruSmallModelPriority(providerID)
      if (altruPriority) priority = altruPriority
      // altrucoder_change end
      ...
      // altrucoder_change start - fall back to altru-coder's auto small model
      const altruFallback = s.providers[ProviderID.make("altru-coder")]
      if (altruFallback?.models["altru-coder-auto/small"]) {
        return yield* getModel(ProviderID.make("altru-coder"), ModelID.make("altru-coder-auto/small"))
      }
      // altrucoder_change end
