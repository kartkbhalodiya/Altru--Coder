...
const INGEST_BASE = process.env.ALTRU_CODER_SESSION_INGEST_URL ?? "https://ingest.altru-coder-sessions.ai"
...
export async function fetchCloudSessionForImport(token: string, sessionId: string): Promise<FetchResult> {
  ...
  if (!response.ok) {
    ...
    console.error("[Altru Coder Gateway] cloud/session/import: export failed", {
