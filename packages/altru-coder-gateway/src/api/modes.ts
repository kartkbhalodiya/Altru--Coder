import { z } from "zod"
import { ALTRU_CODER_API_BASE, MODELS_FETCH_TIMEOUT_MS } from "./constants.js"
...
/**
 * Fetch custom modes for an organization from the Altru Coder Cloud API.
 *
...
export async function fetchOrganizationModes(token: string, organizationId: string): Promise<OrganizationMode[]> {
  ...
  try {
    const url = `${ALTRU_CODER_API_BASE}/api/organizations/${encodeURIComponent(organizationId)}/modes`
    ...
    if (!response.ok) {
      console.warn(`[Altru Coder Gateway] Failed to fetch organization modes: ${response.status}`)
      return []
    }
    ...
    if (!parsed.success) {
      console.warn("[Altru Coder Gateway] Organization modes response validation failed:", parsed.error.format())
      return []
    }
    ...
    console.warn("[Altru Coder Gateway] Error fetching organization modes:", err)
    return []
  }
}
