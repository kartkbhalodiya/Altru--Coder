...
/**
 * Fetch models from Altru Coder API (OpenRouter-compatible endpoint)
 *
 * @param options - Configuration options
 * @returns Record of models in ModelsDev.Model format
 */
export async function fetchAltruCoderModels(options?: {
  ...
  const finalBaseURL = token ? getAltruCoderUrlFromToken(baseURL, token) : baseURL
  ...
  try {
    ...
    if (!result.success) {
      console.error("Altru Coder models response validation failed:", result.error.format())
      return {}
    }
    ...
    for (const model of result.data.data) {
      ...
      // Skip models that don't support tools — Altru Coder requires tool calling
    ...
    console.error("Error fetching Altru Coder models:", error)
    return {}
  }
}
