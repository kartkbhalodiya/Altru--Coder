...
import { getAltruCoderProfile, getAltruCoderDefaultModel, promptOrganizationSelection } from "../api/profile.js"
import { ALTRU_CODER_API_BASE, POLL_INTERVAL_MS } from "../api/constants.js"
...
async function initiateDeviceAuth(): Promise<DeviceAuthInitiateResponse> {
  const response = await fetch(`${ALTRU_CODER_API_BASE}/api/device-auth/codes`, {
    method: "POST",
    headers: {
...
async function pollDeviceAuth(code: string): Promise<DeviceAuthPollResponse> {
  const response = await fetch(`${ALTRU_CODER_API_BASE}/api/device-auth/codes/${code}`)
...
export async function authenticateWithDeviceAuth(): Promise<DeviceAuthResult> {
  ...
  const profileData = await getAltruCoderProfile(token)
  ...
  const model = await getAltruCoderDefaultModel(token, organizationId)
