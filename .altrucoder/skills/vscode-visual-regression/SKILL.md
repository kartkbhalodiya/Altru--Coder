...
description: Write Storybook stories and visual regression tests for the Altru Coder VS Code extension webview UI
---

Use this skill when the user asks you to add visual regression tests, screenshot tests, or Storybook stories for components in `packages/altru-coder-vscode/`.
...
Stories live in `packages/altru-coder-vscode/webview-ui/src/stories/`. Existing files and their scope:
...
```bash
# From packages/altru-coder-vscode/
bun run storybook
...
Snapshots live at `packages/altru-coder-vscode/tests/visual-regression.spec.ts-snapshots/`:
...
Key settings in `packages/altru-coder-vscode/playwright.config.ts`:
...
- `packages/util/**`
- `packages/sdk/js/**`
- `packages/altru-coder-vscode/webview-ui/**`
- `packages/altru-coder-vscode/.storybook/**`
- `packages/altru-coder-vscode/tests/visual-regression*`
...
altru-coder-ui components are imported via deep subpaths:

```tsx
import { Part } from "@altru-coder/altru-coder-ui/message-part"
import { BasicTool } from "@altru-coder/altru-coder-ui/basic-tool"
...
```tsx
import type { AssistantMessage as SDKAssistantMessage, TextPart, ToolPart } from "@altru-coder/sdk/v2"
...
```
globals=colorScheme:dark;theme:altru-coder-vscode;vscodeTheme:dark-modern
```

The `.storybook/preview.tsx` applies these via a decorator that calls `applyVscodeTheme()` / `applyAltruCoderTheme()` from altru-coder-ui. Stories do NOT need to handle theming — it happens automatically.
