# Releasing Altru Coder

Altru Coder uses a fully automated CI pipeline triggered via GitHub Actions `workflow_dispatch`. A single workflow handles version bumping, building all artifacts, publishing to every distribution channel, and updating package registries.

## How to Trigger a Release

1. Go to the [`publish` workflow](https://github.com/Altru-Coder/altrucoder/actions/workflows/publish.yml) in GitHub Actions.
...
- Runs `packages/opencode/script/build.ts` to compile the Altru Coder CLI binary.
...
- Uploads the `dist/` directory as a workflow artifact (`altru-coder-cli`) for subsequent jobs.

### 3. Build VS Code Extension (`build-vscode`)

- Downloads the CLI artifacts from the previous job.
- Runs `packages/altru-coder-vscode/script/build.ts` to build VSIX packages for all target platforms:
...
- Uploads the VSIX files as a workflow artifact (`altru-coder-vscode`).
...
- Publishes platform-specific binary packages to **npm** (e.g. `@altru-coder/cli-linux-x64`, `@altru-coder/cli-darwin-arm64`, etc.).
- Publishes the main `@altru-coder/cli` package to **npm** with optional dependencies on the binary packages.
- Builds and pushes a multi-arch **Docker image** (`ghcr.io/altru-org/altrucoder`) to GitHub Container Registry (linux/amd64 + linux/arm64).

#### SDK (`@altru-coder/sdk`)

- Builds and publishes the TypeScript SDK to **npm**.

#### Plugin (`@altru-coder/plugin`)
...
- **AUR (Arch Linux)**: Clones `altru-coder-bin` from the AUR, updates the `PKGBUILD` with new version and SHA256 checksums, and pushes.
- **Homebrew**: Clones `Altru-Coder/homebrew-tap`, updates the `altru-coder.rb` formula with new version, download URLs, and SHA256 checksums, and pushes.

## Prerequisites and Permissions

### Repository Access

- The workflow only runs in the `Altru-Coder/altrucoder` repository (guarded by `if: github.repository == 'Altru-Coder/altrucoder'`).
...
| Secret | Purpose |
|---|---|
| `ALTRU_CODER_API_KEY` | Altru Coder API key used during version computation |
| `ALTRU_CODER_ORG_ID` | Altru Coder organization ID |
| `ALTRU_CODER_MAINTAINER_APP_ID` | GitHub App ID for the altru-coder-maintainer bot (used for git commits) |
| `ALTRU_CODER_MAINTAINER_APP_SECRET` | GitHub App secret for the altru-coder-maintainer bot |
